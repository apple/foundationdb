...
Future<grpc::Status> getCoordinators(Reference<IDatabase> db,
	...
	loop {
		Error err;
		try {
			ThreadFuture<Optional<Value>> descriptionF = tr->get(special_keys::clusterDescriptionSpecialKey);
...
Future<grpc::Status> changeCoordinators(Reference<IDatabase> db,
	...
	loop {
		...
		try {
			...
			if (req->disable_config_db()) {
				// All that matters is the key is set.
				tr->set(special_keys::configDBSpecialKey, ""_sr);
			}
...
Reference<ITransaction> getTransaction(Reference<IDatabase> db, Reference<ITransaction>& tr, FdbOptions* options) {
	if (!tr) {
...
Future<grpc::Status> getReadVersion(Reference<IDatabase> db,
	...
	Version v = co_await safeThreadFutureToFuture(getTransaction(db, tr, nullptr)->getReadVersion());
	rep->set_version(v);
	co_return grpc::Status::OK;
}
			...
			for (int i = 0; i < processClasses.get().size(); i++) {
				try {
					...
					co_return grpc::Status(grpc::StatusCode::ABORTED,
					                       "Client version is too old, please use a newer version");
				}
			}
...
Future<grpc::Status> kill(Reference<IDatabase> db, const KillRequest* req, KillReply* rep) {
	...
	Reference<ITransaction> tr = getTransaction(db, tr, nullptr);
	std::map<Key, std::pair<Value, ClientLeaderRegInterface>> address_interface;
