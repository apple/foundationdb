...
#pragma pack(pop)
	...
	ACTOR static Future<Void> recover(DWALPager* self) {
		...
		if (exists) {
			...
			CODE_PROBE(true, "DWALPager recovering using existing file");
			...
			if (fileSize < (self->smallestPhysicalBlock * 2)) {
				CODE_PROBE(true, "DWALPager file size too small for recovery", probe::decoration::rare);
			...
			try {
				...
				CODE_PROBE(true, "DWALPager failed to read primary header during recovery");
				...
				if (!tryBackupHeader) {
					CODE_PROBE(true, "DWALPager skip reading backup header due to fatal error");
					throw;
				}
			}
			...
			if (!self->headerPage.isValid()) {
				CODE_PROBE(true, "DWALPager fallback to recover from backup header", probe::decoration::rare);
			...
			CODE_PROBE(true, "DWALPager creating new pager");
	...
	ACTOR static Future<LogicalPageID> newPageID_impl(DWALPager* self) {
		...
		if (freePageID.present()) {
			...
			CODE_PROBE(true, "DWALPager new page id from free list");
		...
		if (delayedFreePageID.present()) {
			...
			CODE_PROBE(true, "DWALPager new page id from delayed free list");
		...
		CODE_PROBE(true, "DWALPager new page id at end of file");
		return id;
	};
	...
	ACTOR static Future<LogicalPageID> newExtentPageID_impl(DWALPager* self, QueueID queueID) {
		...
		if (freeExtentID.present()) {
			...
			CODE_PROBE(true, "DWALPager new extent page id from free list");
		...
		debug_printf("DWALPager(%s) remapQueue newExtentPageID() returning %s at end of file\n",
		             self->filename.c_str(),
		             toString(id).c_str());
		CODE_PROBE(true, "DWALPager new extent page id at end of file");
	...
	Future<Void> writePhysicalPage(PagerEventReasons reason,
		...
		if (pageIDs.size() == 1) {
			...
			CODE_PROBE(true, "DWALPager write physical page with multiple blocks");
	...
	void updatePage(PagerEventReasons reason,
		...
		if (!cacheEntry.initialized()) {
			CODE_PROBE(true, "DWALPager update uncached page");
			...
			CODE_PROBE(true, "DWALPager update page while it is being read", probe::decoration::rare);
	...
	void freePage(LogicalPageID pageID, Version v) override {
		...
		if (i != remappedPages.end()) {
			debug_printf("DWALPager(%s) op=freeRemapped %s @%" PRId64 " oldestVersion=%" PRId64 "\n",
			             ...
			             v,
			             lastCommittedHeader.oldestVersion);
			CODE_PROBE(true, "DWALPager delay remapped page deletion");
			remapQueue.pushBack(RemappedPage{ v, pageID, invalidLogicalPageID });
		...
		try {
			...
			if (page->isEncrypted()) {
				if (!self->keyProvider.isValid()) {
					CODE_PROBE(true, "DWALPager readPhysicalPage wait for encryption key provider");
	...
	ACTOR static Future<Reference<ArenaPage>> readPhysicalMultiPage(DWALPager* self,
		...
		try {
			...
			if (page->isEncrypted()) {
				if (!self->keyProvider.isValid()) {
					CODE_PROBE(true,
					           "DWALPager readPhysicalMultiPage wait for encryption key provider",
	...
	ACTOR static Future<Void> removeRemapEntry(DWALPager* self, RemappedPage p, Version oldestRetainedVersion) {
		...
		if (p.newPageID == iVersionPagePair->second) {
			...
			CODE_PROBE(true, "DWALPager removeRemapEntry remapped page deleted at same version");
		...
		CODE_PROBE(freeNewID, "DWALPager removeRemapEntry free new id");
		CODE_PROBE(firstType == RemappedPage::REMAP && secondType == RemappedPage::DETACH,
		           "DWALPager removeRemapEntry keeping new id since original page is detached");
		CODE_PROBE(firstType == RemappedPage::REMAP && deleteAtSameVersion,
		           "DWALPager removeRemapEntry keeping new id since page is deleted at same version");
		CODE_PROBE(copyNewToOriginal, "DWALPager removeRemapEntry copy new page content to original page");
		CODE_PROBE(firstType == RemappedPage::REMAP && !copyNewToOriginal,
		           "DWALPager removeRemapEntry not copying new page content since the new content is not visible");
		CODE_PROBE(freeOriginalID, "DWALPager removeRemapEntry free original page id");
		CODE_PROBE(freeOriginalID && firstType == RemappedPage::FREE,
		           "DWALPager removeRemapEntry free original page id since the page is deleted");
		CODE_PROBE(freeOriginalID && firstType == RemappedPage::DETACH,
		           "DWALPager removeRemapEntry free original page id since the page is detached");
...
public:
	...
	ACTOR static Future<int> incrementalLazyClear(VersionedBTree* self) {
		...
		loop {
			...
			CODE_PROBE(entries.size() > 0, "Redwood processing lazy clear entries");
	...
	void initEncryptionKeyProvider() {
		if (!m_keyProvider.isValid()) {
			...
			case EncodingType::XXHash64:
				CODE_PROBE(true, "Redwood init with null encryption key provider");
				m_keyProvider = makeReference<NullEncryptionKeyProvider>();
				break;
			case EncodingType::XOREncryption_TestOnly:
				CODE_PROBE(true, "Redwood init with XOR encryption key provider");
			...
			case EncodingType::AESEncryption:
				...
				ASSERT(m_db.isValid());
				CODE_PROBE(true, "Redwood init with AES encryption key provider");
				m_keyProvider =
			...
			case EncodingType::AESEncryptionWithAuth:
				ASSERT(m_expectedEncryptionMode.present());
				ASSERT(m_db.isValid());
				CODE_PROBE(true, "Redwood init with AES with auth encryption key provider");
	...
	ACTOR static Future<Void> init_impl(VersionedBTree* self) {
		...
		if (btreeHeader.size() == 0) {
			CODE_PROBE(true, "Redwood recovering from empty BTree header");
			// Create new BTree
			...
			CODE_PROBE(true, "Redwood recovering from existing BTree header");
			...
			if (self->m_expectedEncryptionMode.present()) {
				...
				CODE_PROBE(true, "Redwood upgrade from a version without encryption support");
				self->m_expectedEncryptionMode = self->m_header.encryptionMode;
			}
			...
			if (self->m_encodingType == EncodingType::MAX_ENCODING_TYPE) {
				...
				CODE_PROBE(true, "Redwood encoding type mismatch on recovery");
...
private:
	...
	struct PageToBuild {
		...
		static bool shiftItem(PageToBuild& a, PageToBuild& b, int deltaSize, int kvBytes) {
			...
			CODE_PROBE(true, "Redwood balance pages");
			return true;
		}
		...
		bool addRecord(const RedwoodRecordRef& rec, const RedwoodRecordRef& nextRecord, int deltaSize, bool force) {
			...
			if (enableEncryptionDomain) {
				...
				if (count == 0) {
					...
					if (domainId == currentDomainId) {
						...
						CODE_PROBE(domainId != defaultDomainId,
						           "Redwood PageToBuild switching to use default encryption domain");
			...
			while (bytesLeft < 0) {
				CODE_PROBE(true, "Redwood PageToBuild expand page to fit record");
				...
				if (!largeDeltaTree && pageSize > BTreePage::BinaryTree::SmallSizeLimit) {
					CODE_PROBE(true, "Redwood PageToBuild page become large after expand");
					largeDeltaTree = true;
		...
		void finish() {
			if (enableEncryptionDomain && canUseDefaultDomain) {
				CODE_PROBE(true, "Redwood PageToBuild using default encryption domain");
	...
	std::vector<PageToBuild> splitPages(const RedwoodRecordRef* lowerBound,
		...
		if (enableEncryptionDomain && records.size() > 1) {
			...
			if (firstDomain != lastDomain || firstDomain == m_keyProvider->getDefaultEncryptionDomainId()) {
				CODE_PROBE(true, "Redwood need split page by encryption domain");
				splitByDomain = true;
			}
		}
	...
	ACTOR static Future<Standalone<VectorRef<RedwoodRecordRef>>> writePages(VersionedBTree* self,
		...
		if (enableEncryptionDomain) {
			...
			if (!self->m_keyProvider->keyFitsInDomain(domainId, pageLowerBound.key, true)) {
				CODE_PROBE(true,
				           "Redwood writePages adjust first page lower bound to fit in the page encryption domain");
				if (domainId == self->m_keyProvider->getDefaultEncryptionDomainId()) {
					CODE_PROBE(true,
					           "Redwood writePages adjust first page lower bound to be the key of the first entry");
					pageLowerBound = RedwoodRecordRef(entries[0].key);
				} else {
					CODE_PROBE(true,
					           "Redwood writePages adjust first page lower bound to be the encryption domain prefix");
		...
		for (pageIndex = 0; pageIndex < pagesToBuild.size(); ++pageIndex) {
			...
			if (!lastPage && height == 1) {
				CODE_PROBE(true, "Redwood writePages truncate leaf page upper bound");
			...
			if (enableEncryptionDomain && pageUpperBound.key != dbEnd.key) {
				...
				if (ubDomainId != self->m_keyProvider->getDefaultEncryptionDomainId() &&
					...
					CODE_PROBE(true, "Redwood writePages adjust page upper bound by encryption domain");
			...
			if (height != 1 && !entries[p->startIndex].value.present()) {
				...
				CODE_PROBE(true, "Redwood writePages purge redundant null link at the beginning of a page");
				...
				if (p->count == 0) {
					...
					CODE_PROBE(true, "Redwood writePages page become empty after null link purged");
			...
			if (pagesToBuild.size() == 1 && p->blockCount == 1 && previousID.size() == 1) {
				CODE_PROBE(true, "Redwood writePages atomic update original page");
	...
	ACTOR static Future<Standalone<VectorRef<RedwoodRecordRef>>> buildNewRootsIfNeeded(
		...
		// dbBegin as key.
		while (true) {
			int maxRootPointerSize = BUGGIFY ? 1 : BTreeCommitHeader::maxRootPointerSize;
			if (records.size() == 1 && records.front().getChildPage().size() <= maxRootPointerSize &&
			    records[0].key == dbBegin.key) {
				break;
			}
			CODE_PROBE(records.size() > 1, "Redwood writing a new root because there are multiple child records");
			CODE_PROBE(records.front().getChildPage().size() > maxRootPointerSize,
			           "Redwood writing a new root because the current root pointer would be too large");
			if (records[0].key != dbBegin.key) {
				...
				CODE_PROBE(
				    true,
				    "Redwood writing a new root because the current root is encrypted with non-default encryption "
				    "domain cipher key");
			}
		...
		CODE_PROBE(height == 2, "Redwood new root at level 2");
		CODE_PROBE(height == 3, "Redwood new root at level 3");
		CODE_PROBE(height == 4, "Redwood new root at level 4");
		CODE_PROBE(height == 5, "Redwood new root at level 5");
		CODE_PROBE(height == 6, "Redwood new root at level 6");
		CODE_PROBE(height == 7, "Redwood new root at level 7");
		CODE_PROBE(height == 8, "Redwood new root at level 8", probe::decoration::rare);
		CODE_PROBE(height == 9, "Redwood new root at level 9", probe::decoration::rare);
	...
	ACTOR static Future<BTreeNodeLinkRef> updateBTreePage(VersionedBTree* self,
		...
		if (oldID.size() == 1) {
			CODE_PROBE(true, "Redwood updateBTreePage atomic update page");
		...
		CODE_PROBE(true, "Redwood updateBTreePage update with new page id");
		state int i = 0;
	...
	struct InternalPageSliceUpdate : public FastAllocated<InternalPageSliceUpdate> {
		...
		void updatedInPlace(BTreeNodeLinkRef maybeNewID, BTreePage* btPage, int capacity) {
			CODE_PROBE(true, "Redwood internal page updated in-place");
			inPlaceUpdate = true;
		...
		void rebuilt(Standalone<VectorRef<RedwoodRecordRef>> newRecords) {
			CODE_PROBE(true, "Redwood internal page rebuilt");
			inPlaceUpdate = false;
			newLinks = newRecords;
			childrenChanged = true;
			...
			if (updating) {
				...
				while (i != recs.size()) {
					...
					if (page->isEncrypted() && keyProvider->enableEncryptionDomain()) {
						...
						CODE_PROBE(!canInsert,
						           "Redwood InternalPageModifier reject inserting entry not belonging to the current "
						           "encryption domain");
					}
					if (canInsert) {
						canInsert = end.insert(rec, 0, maxHeightAllowed);
						CODE_PROBE(!canInsert,
						           "Redwood InternalPageModifier reject inserting entry since node is full");
					}

					if (!canInsert) {
						debug_printf("internal page: failed to insert %s, switching to rebuild\n",
						             rec.toString(false).c_str());
						CODE_PROBE(true, "Redwood InternalPageModifier switch from in-place update to rebuild");
		...
		void applyUpdate(InternalPageSliceUpdate& u, const RedwoodRecordRef* nextBoundary) {
			...
			if (u.expectedUpperBound.present()) {
				...
				if (nextBoundary == nullptr || (nextBoundary != &u.expectedUpperBound.get() &&
					...
					CODE_PROBE(true, "Redwood InternalPageModifier adding null link");
		...
		if (btPage->isLeaf()) {
			...
			while (mBegin != mEnd) {
				...
				if (applyBoundaryChange) {
					...
					if (boundaryExists && updatingDeltaTree && shouldInsertBoundary &&
						...
						CODE_PROBE(true, "Redwood commitSubtree in-place update of new same-sized value");
					...
					if (shouldInsertBoundary) {
						...
						if (updatingDeltaTree) {
							...
							if (enableEncryptionDomain) {
								...
								CODE_PROBE(!canInsert,
								           "Redwood commitSubtree item inserting to leaf node does not fit in "
								           "encryption domain");
							}
							if (canInsert) {
								copyForUpdate();
								...
								CODE_PROBE(
								    !canInsert,
								    "Redwood commitSubtree item inserting to leaf node does not fit in the node");
							}
							if (canInsert) {
								btPage->kvBytes += rec.kvBytes();
								...
								debug_printf("%s Insert failed for %s [mutation, boundary start]\n",
								             context.c_str(),
								             rec.toString().c_str());
								CODE_PROBE(true, "Redwood commitSubtree switch to merge mode for leaf node update");
			...
			if (updatingDeltaTree) {
				...
				if (cursor.tree->numItems == 0) {
					...
					debug_print(addPrefix(context, update->toString()));
					CODE_PROBE(true, "Redwood commitSubtree leaf node become empty in update mode");
				} else {
					// Otherwise update it.
					...
					debug_print(addPrefix(context, update->toString()));
					CODE_PROBE(true, "Redwood commitSubtree in-place updated leaf node");
				}
				return Void();
			}
			...
			if (merged.empty()) {
				...
				debug_print(addPrefix(context, update->toString()));
				CODE_PROBE(true, "Redwood commitSubtree leaf node become empty in merge mode");
				return Void();
			}
			...
			debug_print(addPrefix(context, update->toString()));
			CODE_PROBE(true, "Redwood commitSubtree rebuilt leaf node");
			return Void();
		} else {
			// Internal Page
			...
			while (cursor.valid()) {
				...
				if (cursor.moveNext()) {
					...
					if (!cursor.get().value.present()) {
						...
						CODE_PROBE(true,
						           "Redwood commitSubtree skip over null link when deciding expected upper bound for "
						           "child of internal page");
					} else {
				...
				++next;
				if (next == mEnd) {
					CODE_PROBE(true, "Redwood commitSubtree single mutation range cover whole child page");
					...
					if (uniform) {
						CODE_PROBE(true, "Redwood commitSubtree whole child page cleared or unchanged");
						...
						if (cursor.valid() && mEnd.key() != u.subtreeUpperBound.key) {
							...
							if (cursor != u.cEnd) {
								...
								CODE_PROBE(true,
								           "Redwood commitSubtree multiple child page fully cleared or unchanged");
								ASSERT(cursor.valid());
								if (!cursor.get().value.present()) {
									cursor.movePrev();
								}
						...
						if (range.clearAfterBoundary) {
							CODE_PROBE(true, "Redwood commitSubtree child page(s) fully cleared");
							// Cleared
							u.cleared();
							auto c = u.cBegin;
							while (c != u.cEnd) {
								...
								if (rec.value.present()) {
									if (height == 2) {
										...
										CODE_PROBE(true, "Redwood commitSubtree clear child page");
									} else {
										...
										CODE_PROBE(true, "Redwood commitSubtree queuing lazy clear entry");
									}
								}
								c.moveNext();
							}
							...
							CODE_PROBE(true, "Redwood commitSubtree child page(s) unchanged");
						}
			...
			debug_print(addPrefix(context, update->toString()));
			if (modifier.changesMade || slices.back()->childrenChanged) {
				CODE_PROBE(true, "Redwood commitSubtree using subtree uppper bound as expected upper bound");
				modifier.applyUpdate(*slices.back(), &update->subtreeUpperBound);
			} else {
				CODE_PROBE(true, "Redwood commitSubtree using decode upper bound as expected upper bound");
				modifier.applyUpdate(*slices.back(), &update->decodeUpperBound);
			}
			...
			if (!modifier.changesMade && detachChildren) {
				debug_printf(
				    ...
				    context.c_str(),
				    parentInfo->count);
				CODE_PROBE(true, "Redwood commitSubtree force update page to detache remapped children");
			...
			if (modifier.changesMade || forceUpdate) {
				if (modifier.empty()) {
					...
					debug_printf(
					    ...
					    context.c_str());
					debug_print(addPrefix(context, update->toString()));
					CODE_PROBE(true, "Redwood commitSubtree all internal page children deleted");
					...
					if (modifier.updating) {
						...
						debug_printf(
						    ...
						    forceUpdate,
						    detachChildren);
						CODE_PROBE(true, "Redwood commitSubtree internal page in-place update");
						...
						if (detachChildren) {
							...
							parentInfo->clear();
							CODE_PROBE(detached > 0, "Redwood commitSubtree detached children during in-place update");
							if (forceUpdate && detached == 0) {
								debug_printf("%s No children detached during forced update, returning slice:\n",
								             context.c_str());
								debug_print(addPrefix(context, update->toString()));
								CODE_PROBE(true,
								           "Redwood commitSubtree no children detached during forced update",
								           probe::decoration::rare);
						...
						debug_printf("%s Internal page could not be modified, rebuilding replacement(s).\n",
						             context.c_str());
						CODE_PROBE(true, "Redwood commitSubtree internal page rebuild");
						...
						if (detachChildren) {
							...
							parentInfo->clear();
							CODE_PROBE(detached > 0, "Redwood commitSubtree detached children during rebuild");
						}
	...
	ACTOR static Future<Void> commit_impl(VersionedBTree* self, Version writeVersion, Future<Void> previousCommit) {
		...
		if (all.childrenChanged) {
			if (all.newLinks.empty()) {
				...
				CODE_PROBE(true, "Redwood commit_impl writing new empty root");
...
public:
	...
	public:
		...
		void prefetch(KeyRef rangeEnd, bool directionForward, int recordLimit, int byteLimit) {
			...
			CODE_PROBE(true, "Redwood prefetch pages for read");
