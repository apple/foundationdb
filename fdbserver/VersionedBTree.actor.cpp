...
#pragma pack(pop)
	...
	struct Cursor {
		...
		void init(FIFOQueue* q = nullptr,
			...
			debug_printf_always("FIFOQueue::Cursor(%s) initialized\n", toString().c_str());

			if (mode == WRITE && initialPageID != invalidLogicalPageID) {
				debug_printf_always("FIFOQueue::Cursor(%s) init. Adding new page %u\n", toString().c_str(), initialPageID);
		...
		Future<Void> loadExtent() {
			ASSERT(mode == POP | mode == READONLY);
			debug_printf_always("FIFOQueue::Cursor(%s) loadExtent\n", toString().c_str());
			return map(queue->pager->readExtent(pageID), [=](Reference<ArenaPage> p) {
				page = p;
				debug_printf_always("FIFOQueue::Cursor(%s) loadExtent done. Page: %p\n", toString().c_str(), page->begin());
				return Void();
			});
		}
		...
		void addNewPage(LogicalPageID newPageID,
			...
			if (initializeNewPage) {
				...
				if (queue->usesExtents) {
					...
					if (initializeExtentInfo) {
						...
						if (newExtentPage) {
							...
							debug_printf_always("FIFOQueue::Cursor(%s) newExtentPage. newPageID %u, pagesPerExtent %d, "
							             "ExtentEndPageID: %s\n",
							             toString().c_str(),
							             newPageID,
							             pagesPerExtent,
				...
				debug_printf_always("FIFOQueue::Cursor(%s) Clearing new page\n", toString().c_str());
				page.clear();
			}
		}
		...
		ACTOR static Future<Optional<T>> waitThenReadNext(Cursor* self,
			...
			if (!locked) {
				debug_printf_always("FIFOQueue::Cursor(%s) waitThenReadNext locking mutex\n", self->toString().c_str());
				wait(self->mutex.take());
			}
			...
			if (!locked) {
				debug_printf_always("FIFOQueue::Cursor(%s) waitThenReadNext unlocking mutex\n", self->toString().c_str());
				self->mutex.release();
			}
		...
		Future<Optional<T>> readNext(const Optional<T>& upperBound = {}, bool locked = false) {
			...
			if (!page) {
				debug_printf_always("FIFOQueue::Cursor(%s) loading\n", toString().c_str());
				...
				if (pageID != nextPageID) {
					debug_printf_always("FIFOQueue::Cursor(%s) reloading\n", toString().c_str());
					startNextPageLoad(pageID);
				}
			...
			auto p = raw();
			debug_printf_always("FIFOQueue::Cursor(%s) readNext reading at current position\n", toString().c_str());
			...
			debug_printf_always("FIFOQueue::Cursor(%s) after read of %s\n", toString().c_str(), ::toString(result).c_str());
			ASSERT(offset <= p->endOffset);
			...
			if (offset == p->endOffset) {
				...
				page.clear();
				debug_printf_always("FIFOQueue::Cursor(%s) readNext page exhausted, moved to new page\n", toString().c_str());
...
public:
	...
	void recover(IPager2* p, const QueueState& qs, std::string queueName, bool loadExtents = true) {
		debug_printf_always("FIFOQueue(%s) recover from queue state %s\n", queueName.c_str(), qs.toString().c_str());
		pager = p;
		pagerError = pager->getError();
		name = queueName;
		queueID = qs.queueID;
	...
	void resetHeadReader() {
		...
		debug_printf_always("FIFOQueue(%s) read cursor reset\n", name.c_str());
	}
	...
	ACTOR static Future<Void> peekAll_ext(FIFOQueue* self, PromiseStream<Standalone<VectorRef<T>>> res) {
		...
		debug_printf_always("FIFOQueue::Cursor(%s) peekAllExt begin\n", c.toString().c_str());
		if (c.pageID == invalidLogicalPageID || c.pageID == c.endPageID) {
			debug_printf_always("FIFOQueue::Cursor(%s) peekAllExt returning nothing\n", c.toString().c_str());
		...
		loop {
			...
			loop {
				...
				if (c.pageID == invalidLogicalPageID || c.pageID == c.endPageID) {
					debug_printf_always("FIFOQueue::Cursor(%s) peekAllExt Queue exhausted\n", c.toString().c_str());
					res.send(results);
	...
	QueueState getState() const {
		...
		debug_printf_always("FIFOQueue(%s) getState(): %s\n", name.c_str(), s.toString().c_str());
		return s;
	}
	...
	ACTOR static Future<bool> preFlush_impl(FIFOQueue* self) {
		debug_printf_always("FIFOQueue(%s) preFlush begin\n", self->name.c_str());
		wait(self->notBusy());
		...
		if (!workPending) {
			...
			if (self->newTailPage.isReady() && self->newTailPage.get() == invalidLogicalPageID) {
				if (self->tailWriter.pendingTailWrites()) {
					debug_printf_always("FIFOQueue(%s) preFlush starting to get new page ID\n", self->name.c_str());
					if (self->usesExtents) {
						...
						debug_printf_always("FIFOQueue(%s) newTailPage tailPageNewExtent:%d prevExtentEndPageID: %u "
						             "tailWriterPage %u\n",
						             self->name.c_str(),
...
#pragma pack(pop)
	...
	ACTOR static Future<Void> recover(DWALPager* self) {
		...
		if (exists && fileSize >= (self->smallestPhysicalBlock * 2)) {
			...
			debug_printf_always("DWALPager(%s) Queue recovery complete.\n", self->filename.c_str());
			...
			debug_printf_always("DWALPager(%s) ExtentUsedList size: %d.\n", self->filename.c_str(), extents.size());
			if (extents.size() > 1) {
				...
				for (int i = 1; i < extents.size() - 1; i++) {
					if (extents[i].queueID == remapQueueID) {
						...
						debug_printf_always("DWALPager Extents: ID: %s ", toString(extID).c_str());
			...
			try {
				loop choose {
					when(Standalone<VectorRef<RemappedPage>> remaps = waitNext(remapStream.getFuture())) {
						debug_printf_always(
						    "DWALPager(%s) recovery. remaps size: %d, queueEntries: %d\n",
						    self->filename.c_str(),
						    remaps.size(),
	...
	Future<Void> writePhysicalPage(PhysicalPageID pageID, Reference<ArenaPage> page, bool header = false) {
		...
		debug_printf_always("DWALPager(%s) writePhysicalPage %s CalculatedChecksum=%d ChecksumInPage=%d\n",
					 filename.c_str(),
					 toString(pageID).c_str(),
	...
	Future<LogicalPageID> atomicUpdatePage(LogicalPageID pageID, Reference<ArenaPage> data, Version v) override {
		debug_printf_always("DWALPager(%s) op=writeAtomic %s @%" PRId64 "\n", filename.c_str(), toString(pageID).c_str(), v);
	...
	LogicalPageID detachRemappedPage(LogicalPageID pageID, Version v) override {
		...
		if (iLast->first == v) {
			debug_printf_always("DWALPager(%s) op=detachDelete originalID=%s newID=%s @%" PRId64 " oldestVersion=%" PRId64
			             "\n",
			             filename.c_str(),
			             toString(pageID).c_str(),
	...
	Future<Reference<ArenaPage>> readPage(LogicalPageID pageID,
		...
		if (!cacheable) {
			...
			if (pCacheEntry != nullptr) {
				debug_printf_always("DWALPager(%s) op=readUncachedHit %s\n", filename.c_str(), toString(pageID).c_str());
		...
		if (!cacheEntry.initialized()) {
			debug_printf_always("DWALPager(%s) issuing actual read of %s\n", filename.c_str(), toString(pageID).c_str());
	...
	ACTOR static Future<Reference<ArenaPage>> readPhysicalExtent(DWALPager* self,
		...
		debug_printf_always("DWALPager(%s) op=readPhysicalExtentStart %s readSize %d offset %d physicalReadSize %d parallelReads %d\n",
					 self->filename.c_str(),
					 toString(pageID).c_str(),
					 readSize,
		...
		for (i = 0; i < parallelReads; i++)
		{
			...
			debug_printf_always("DWALPager(%s) current offset %d\n",
								self->filename.c_str(),
								currentOffset);
			reads.push_back(self->pageFile->read(extent->mutate() + currentOffset,
												 physicalReadSize,
		...
		if (lastReadSize) {
			currentOffset = i * lastReadSize;
			debug_printf_always("DWALPager(%s) iter %d current offset %d lastReadSize %d\n",
								self->filename.c_str(),
								i, currentOffset, lastReadSize);
			reads.push_back(self->pageFile->read(extent->mutate() + currentOffset,
												 lastReadSize,
												 startOffset + currentOffset));
		}
		...
		debug_printf_always("DWALPager(%s) op=readPhysicalExtentComplete %s ptr=%p bytes=%d file offset=%d\n",
		             self->filename.c_str(),
		             toString(pageID).c_str(),
		             extent->begin(),
		             readSize,
	...
	Future<Reference<ArenaPage>> readExtent(LogicalPageID pageID) override {
		debug_printf_always("DWALPager(%s) op=readExtent %s\n", filename.c_str(), toString(pageID).c_str());
		PageCacheEntry* pCacheEntry = extentCache.getIfExists(pageID);
		if (pCacheEntry != nullptr) {
			debug_printf_always("DWALPager(%s) Cache Entry exists for %s\n", filename.c_str(), toString(pageID).c_str());
			return pCacheEntry->readFuture;
		}
	...
	ACTOR static Future<Void> removeRemapEntry(DWALPager* self, RemappedPage p, Version oldestRetainedVersion) {
		...
		if (freeNewID) {
			debug_printf_always("DWALPager(%s) remapCleanup freeNew %s\n", self->filename.c_str(), p.toString().c_str());
			self->freeUnmappedPage(p.newPageID, 0);
			++g_redwoodMetrics.pagerRemapFree;
		}

		if (freeOriginalID) {
			debug_printf_always("DWALPager(%s) remapCleanup freeOriginal %s\n", self->filename.c_str(), p.toString().c_str());
	...
	ACTOR static Future<Void> remapCleanup(DWALPager* self) {
		...
		debug_printf_always("DWALPager(%s) remapCleanup stopped (stop=%d)\n", self->filename.c_str(), self->remapCleanupStop);
		signal.send(Void());
...
TEST_CASE(":/redwood/performance/set") {
	...
	DWALPager* pager =
	    new DWALPager(pageSize, extentSize, fileName, pageCacheBytes, remapCleanupWindow, concurrentExtentReads);
	state VersionedBTree* btree = new VersionedBTree(pager, fileName);
	wait(btree->init());
