...
static void applyMetadataMutations(
    UID const& dbgid, Arena& arena, VectorRef<MutationRef> const& mutations, IKeyValueStore* txnStateStore,
    LogPushData* toCommit, bool* confChange, Reference<ILogSystem> logSystem = Reference<ILogSystem>(),
    Version popVersion = 0, KeyRangeMap<std::set<Key>>* vecBackupKeys = NULL,
    KeyRangeMap<ServerCacheInfo>* keyInfo = NULL, std::map<Key, applyMutationsData>* uid_applyMutationsData = NULL,
    ...
    std::map<UID, Reference<StorageInfo>>* storageCache = NULL, std::map<Tag, Version>* tag_popped = NULL,
	...
	for (auto const& m : mutations) {
		...
		if (m.param1.size() && m.param1[0] == systemKeys.begin[0] && m.type == MutationRef::SetValue) {
			if (m.param1.startsWith(keyServersPrefix)) {
					...
					if (k != allKeys.end) {
						...
						for (auto id : src) {
							...
							if (cacheItr == storageCache->end()) {
								...
								storageInfo->tag =
								    decodeServerTagValue(txnStateStore->readValue(serverTagKeyFor(id)).get().get());
								storageInfo->interf =
								    decodeServerListValue(txnStateStore->readValue(serverListKeyFor(id)).get().get());
				...
				if (toCommit) {
					...
					TraceEvent(SevDebug, "SendingPrivateMutation", dbgid)
					    ...
					    .detail("TagKey", printable(serverTagKeyFor(serverKeysDecodeServer(m.param1))))
					    .detail(
					        "Tag",
				...
				if (Optional<StringRef>(m.param2) !=
				    txnStateStore->readValue(m.param1)
				        .get()
				        .cast_to<StringRef>()) { // FIXME: Make this check more specific, here or by reading
					...
					if (!m.param1.startsWith(excludedServersPrefix) && m.param1 != excludedServersVersionKey) {
						...
						TraceEvent("MutationRequiresRestart", dbgid)
						    .detail("M", m.toString())
						    .detail("PrevValue", t.present() ? printable(t.get()) : "(none)")
			...
			} else if (m.param1 == databaseLockedKey || m.param1 == mustContainSystemMutationsKey ||
				...
				if (vecBackupKeys) {
					...
					TraceEvent("LogRangeAdd")
					    ...
					    .detail("MutationKey", printable(m.param1))
					    .detail("LogRangeBegin", printable(logRangeBegin))
					    .detail("LogRangeEnd", printable(logRangeEnd));
				}
				...
				if (toCommit) {
					...
					std::set<Tag> allTags;
					for (auto& kv : allServers) allTags.insert(decodeServerTagValue(kv.value));

					if (m.param1 == lastEpochEndKey) {
						for (auto t : allTags) toCommit->addTag(t);
						toCommit->addTypedMessage(LogProtocolMessage());
					}
			...
			if (range.intersects(logRangesRange)) {
				...
				TraceEvent("LogRangeClear")
				    ...
				    .detail("IntersectBegin", printable(commonLogRange.begin))
				    .detail("IntersectEnd", printable(commonLogRange.end));
				...
				if (vecBackupKeys) {
					...
					for (auto logRangeAffected : logRangesAffected) {
						...
						TraceEvent("LogRangeErase")
						    .detail("AffectedKey", printable(logRangeAffected.key))
						    .detail("AffectedValue", printable(logRangeAffected.value))
						    .detail("LogKeyBegin", printable(logKeyBegin))
						    .detail("LogKeyEnd", printable(logKeyEnd))
