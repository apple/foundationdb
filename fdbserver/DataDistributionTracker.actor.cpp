...
ACTOR Future<Void> updateMaxShardSize(Standalone<StringRef> dbName, Reference<AsyncVar<int64_t>> dbSizeEstimate,
...
struct DataDistributionTracker {
	...
	DataDistributionTracker(Database cx, UID masterId, Promise<Void> const& readyToStart,
...
ACTOR Future<Void> trackShardBytes(DataDistributionTracker* self, KeyRange keys,
                                   Reference<AsyncVar<Optional<StorageMetrics>>> shardSize, UID trackerID,
	...
	try {
		loop {
			try {
				...
				if (shardSize->get().present()) {
					...
					if (bandwidthStatus == BandwidthStatusNormal) { // Not high or low
						...
						bounds.permittedError.bytesPerKSecond = bounds.min.bytesPerKSecond / 4;
					           ...
					           BandwidthStatusHigh) { // > 10MB/sec for 100MB shard, proportionally lower for smaller
						                              // shard, > 200KB/sec no matter what
						bounds.max.bytesPerKSecond = bounds.max.infinity;
				...
				StorageMetrics metrics = wait(tr.waitStorageMetrics(keys, bounds.min, bounds.max, bounds.permittedError,
...
ACTOR Future<int64_t> getFirstSize(Reference<AsyncVar<Optional<StorageMetrics>>> stats) {
	loop {
		if (stats->get().present()) return stats->get().get().bytes;
		wait(stats->onChange());
	}
}
...
ACTOR Future<Void> shardSplitter(DataDistributionTracker* self, UID trackerId, KeyRange keys,
                                 Reference<AsyncVar<Optional<StorageMetrics>>> shardSize, ShardSizeBounds shardBounds) {
	...
	if (numShards > 1) {
		...
		wait(delay(1.0, TaskDataDistribution)); // In case the reason the split point was off was due to a discrepancy
		                                        // between storage servers
	}
	return Void();
}

Future<Void> shardMerger(DataDistributionTracker* self, UID trackerId, KeyRange const& keys,
...
ACTOR Future<Void> shardEvaluator(DataDistributionTracker* self, KeyRange keys,
                                  Reference<AsyncVar<Optional<StorageMetrics>>> shardSize, HasBeenTrueFor* wantsToMerge,
                                  UID trackerID) {
	...
	bool shouldMerge = stats.bytes < shardBounds.min.bytes && getBandwidthStatus(stats) == BandwidthStatusLow;
...
ACTOR Future<Void> dataDistributionTracker(Reference<InitialDataDistribution> initData, Database cx,
                                           ...
                                           UID masterId) {
	state DataDistributionTracker self(cx, masterId, readyToStart, output, shardsAffectedByTeamFailure,
	                                   anyZeroHealthyTeams);
...
void ShardsAffectedByTeamFailure::check() {
	if (EXPENSIVE_VALIDATION) {
		for (auto t = team_shards.begin(); t != team_shards.end(); ++t) {
			...
			if (i->range() != t->second || !std::count(i->value().begin(), i->value().end(), t->first)) {
				ASSERT(false);
			}
		}
