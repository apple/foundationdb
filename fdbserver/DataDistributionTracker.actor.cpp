...
ACTOR Future<Void> trackShardBytes(DataDistributionTracker::SafeAccessor self,
	...
	state double lastLowBandwidthStartTime =
	    shardSize->get().present() ? shardSize->get().get().lastLowBandwidthStartTime : now();
...
struct HasBeenTrueFor : ReferenceCounted<HasBeenTrueFor> {
	...
	Future<Void> set() {
		if (!trigger.isValid()) {
			...
			trigger = delayJittered(SERVER_KNOBS->DD_MERGE_COALESCE_DELAY,
			                        decrementPriority(TaskPriority::DataDistribution)) ||
			          cleared.getFuture();
		}
		return trigger;
...
ACTOR Future<Void> shardEvaluator(DataDistributionTracker* self,
	...
	bool shouldMerge =
	    stats.metrics.bytes < shardBounds.min.bytes && getBandwidthStatus(stats.metrics) == BandwidthStatusLow;
