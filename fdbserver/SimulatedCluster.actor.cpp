			...
			loop {
				try {
					...
					state Future<std::string> futureStatusDoc =
					    getLayerStatus(tr, localIP, name, id, taskDest, Snapshot::True);
...
ACTOR Future<Void> runBackup(Reference<IClusterConnectionRecord> connRecord) {
	...
	if (g_simulator->backupAgents == ISimulator::BackupAgentType::BackupToFile) {
		...
		state std::shared_ptr<double> pollDelay =
		    std::make_shared<double>(1.0 / CLIENT_KNOBS->BACKUP_AGGREGATE_POLL_RATE);
		state Future<Void> status = statusUpdateActor(cx, "backup", pollDelay);
