...
ACTOR void simulationSetupAndRun(std::string dataFolder,
	...
	if (!SERVER_KNOBS->SHARD_ENCODE_LOCATION_METADATA &&
	    // NOTE: PhysicalShardMove and BulkLoadTask and BulkDumpAndLoad are required to have SHARDED_ROCKSDB storage
	    // engine working. Inside the TOML file, the SHARD_ENCODE_LOCATION_METADATA is overridden, however, the override
	    // will not take effect until the test starts. Here, we do an additional check for this special simulation test.
	    (std::string_view(testFile).find("PhysicalShardMove") == std::string_view::npos &&
	     std::string_view(testFile).find("BulkLoadTask") == std::string_view::npos &&
	     std::string_view(testFile).find("BulkDumpAndLoad") == std::string_view::npos)) {
