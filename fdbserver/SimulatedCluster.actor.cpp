...
static const char* certBytes = "-----BEGIN CERTIFICATE-----\n"
...
ACTOR Future<ISimulator::KillType> simulatedFDBDRebooter(Reference<ClusterConnectionFile> connFile, uint32_t ip,
	...
	loop {
		auto waitTime = SERVER_KNOBS->MIN_REBOOT_TIME +
		                (SERVER_KNOBS->MAX_REBOOT_TIME - SERVER_KNOBS->MIN_REBOOT_TIME) * g_random->random01();
		cycles++;
...
ACTOR Future<Void> simulatedMachine(ClusterConnectionString connStr, std::vector<uint32_t> ips, bool sslEnabled,
	...
	try {
		...
		for (int i = 0; i < ips.size(); i++) {
			if (restarting) {
				myFolders.push_back(ini.GetValue(printable(localities.zoneId()).c_str(), format("%d", i).c_str(),
				                                 joinPath(baseFolder, g_random->randomUniqueID().toString()).c_str()));

				if (i == 0) {
					std::string coordinationFolder =
					    ini.GetValue(printable(localities.zoneId()).c_str(), "coordinationFolder", "");
					if (!coordinationFolder.size())
						coordinationFolder =
						    ini.GetValue(printable(localities.zoneId()).c_str(), format("c%d", i).c_str(),
						                 joinPath(baseFolder, g_random->randomUniqueID().toString()).c_str());
		...
		*pTesterCount = testerCount;
		...
		for (int i = 0; i < machineCount; i++) {
			...
			ProcessClass processClass =
			    ProcessClass((ProcessClass::ClassType)atoi(ini.GetValue(zoneIdString.c_str(), "mClass")),
			...
			if (ip == NULL) {
				for (int i = 0; i < processes; i++) {
					ipAddrs.push_back(
					    strtoul(ini.GetValue(zoneIdString.c_str(), format("ipAddr%d", i).c_str()), NULL, 10));
				}
			} else {
				// old way
...
void SimulationConfig::generateNormalConfig(int minimumReplication) {
	...
	datacenters =
	    generateFearless ? (minimumReplication > 0 || g_random->random01() < 0.5 ? 4 : 6) : g_random->randomInt(1, 4);
	...
	int replication_type = std::max(minimumReplication, datacenters > 4 ? g_random->randomInt(1, 3)
	                                                                    : std::min(g_random->randomInt(0, 6), 3));
	...
	if (generateFearless && minimumReplication > 1) {
		...
		machine_count = std::max(datacenters + 2, ((db.minDatacentersRequired() > 0) ? datacenters : 1) *
		                                              std::max(3, db.minMachinesRequiredPerDatacenter()));
...
void setupSimulatedSystem(vector<Future<Void>>* systemActors, std::string baseFolder, int* pTesterCount,
	...
	*pConnString = conn;
		...
		for (int machine = 0; machine < machines; machine++) {
			...
			if (assignClasses) {
				if (assignedMachines < 4)
					processClass = ProcessClass((ProcessClass::ClassType)g_random->randomInt(0, 2),
					                            ProcessClass::CommandLineSource); // Unset or Storage
				else if (assignedMachines == 4 && !simconfig.db.regions.size())
					processClass = ProcessClass(
					    (ProcessClass::ClassType)(g_random->randomInt(0, 2) * ProcessClass::ResolutionClass),
					    ProcessClass::CommandLineSource); // Unset or Resolution
				else
					processClass = ProcessClass((ProcessClass::ClassType)g_random->randomInt(0, 3),
	...
	for(int i=0; i<testerCount; i++)
	    g_simulator.asNewProcess("TestWorker", 0x03040301 + i, LocalityData(g_random->randomUniqueID().toString(),
	Optional<Standalone<StringRef>>()), [&] { vector<Future<Void>> v;
...
ACTOR void setupAndRun(std::string dataFolder, const char* testFile, bool rebooting, Reference<TLSOptions> tlsOptions) {
	...
	try {
		...
		wait(timeoutError(runTests(Reference<ClusterConnectionFile>(
		                  ...
		                  buggifyActivated ? 36000.0 : 5400.0));
	} catch (Error& e) {
