...
public:
	...
	void checkSatelliteTagLocations() {
		...
		TraceEvent(((maxUsed - minUsed > 1) || (maxUsedBest - minUsedBest > 1))
		               ? (g_network->isSimulated() && !foundDuplicate ? SevError : SevWarnAlways)
		               : SevInfo,
		           "CheckSatelliteTagLocations")
...
struct ILogSystem {
	...
	struct MergedPeekCursor : IPeekCursor, ReferenceCounted<MergedPeekCursor> {
		...
		MergedPeekCursor(std::vector<Reference<AsyncVar<OptionalInterface<TLogInterface>>>> const& logServers,
		                 ...
		                 std::vector<LocalityData> const& tLogLocalities, IRepPolicyRef const tLogPolicy,
		                 int tLogReplicationFactor);
		MergedPeekCursor(vector<Reference<IPeekCursor>> const& serverCursors, LogMessageVersion const& messageVersion,
		                 int bestServer, int readQuorum, Optional<LogMessageVersion> nextVersion,
		                 Reference<LogSet> logSet, int tLogReplicationFactor);
	...
	struct BufferedCursor : IPeekCursor, ReferenceCounted<BufferedCursor> {
		struct BufferedMessage {
			...
			BufferedMessage() {}
			BufferedMessage(Arena arena, StringRef message, const std::vector<Tag>& tags,
	...
	virtual Reference<IPeekCursor> peekSingle(UID dbgid, Version begin, Tag tag,
	                                          vector<pair<Version, Tag>> history = vector<pair<Version, Tag>>()) = 0;
