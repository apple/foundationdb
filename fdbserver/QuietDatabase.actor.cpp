...
ACTOR Future<vector<std::pair<WorkerInterface, ProcessClass>>> getWorkers(Reference<AsyncVar<ServerDBInfo>> dbInfo,
	...
	loop {
		choose {
			when(vector<std::pair<WorkerInterface, ProcessClass>> w = wait(brokenPromiseToNever(
...
ACTOR Future<int64_t> getDataInFlight(Database cx, WorkerInterface masterWorker) {
	try {
		...
		TraceEventFields md = wait(timeoutError(
		    masterWorker.eventLogRequest.getReply(EventLogRequest(LiteralStringRef("TotalDataInFlight"))), 1.0));
...
ACTOR Future<int64_t> getMaxTLogQueueSize(Database cx, Reference<AsyncVar<ServerDBInfo>> dbInfo,
	...
	TraceEvent("MaxTLogQueueSize")
	    .detail("Database", printable(cx->dbName))
	    .detail("Stage", "ComputingMax")
...
ACTOR Future<int64_t> getMaxStorageServerQueueSize(Database cx, Reference<AsyncVar<ServerDBInfo>> dbInfo,
                                                   WorkerInterface masterWorker) {
	TraceEvent("MaxStorageServerQueueSize")
	    .detail("Database", printable(cx->dbName))
...
ACTOR Future<int64_t> getDataDistributionQueueSize(Database cx, WorkerInterface masterWorker, bool reportInFlight) {
	try {
		TraceEvent("DataDistributionQueueSize")
		    .detail("Database", printable(cx->dbName))
		...
		TraceEvent("DataDistributionQueueSize")
		    .detail("Database", printable(cx->dbName))
		    .detail("Stage", "GotString") /*.detail("Result", printable(movingDataMessage))*/.detail(
		        "TrackLatest", printable(StringRef(cx->dbName.toString() + "/MovingData")));
...
ACTOR Future<bool> getDataDistributionActive(Database cx, WorkerInterface masterWorker) {
	try {
		TraceEvent("DataDistributionActive")
		    .detail("Database", printable(cx->dbName))
		    .detail("Stage", "ContactingMaster");

		TraceEventFields activeMessage =
		    wait(timeoutError(masterWorker.eventLogRequest.getReply(
		                          EventLogRequest(StringRef(cx->dbName.toString() + "/DDTrackerStarting"))),
		                      1.0));
...
ACTOR Future<bool> getStorageServersRecruiting(Database cx, Reference<AsyncVar<ServerDBInfo>> dbInfo,
                                               WorkerInterface masterWorker) {
	try {
		TraceEvent("StorageServersRecruiting")
		    .detail("Database", printable(cx->dbName))
		    .detail("Stage", "ContactingMaster");

		TraceEventFields recruitingMessage = wait(timeoutError(
		    masterWorker.eventLogRequest.getReply(EventLogRequest(StringRef(
		        cx->dbName.toString() + "/StorageServerRecruitment_" + dbInfo->get().master.id().toString()))),
		    1.0));
...
ACTOR Future<Void> repairDeadDatacenter(Database cx, Reference<AsyncVar<ServerDBInfo>> dbInfo, std::string context) {
	if (g_network->isSimulated()) {
		...
		if (primaryDead || remoteDead) {
			...
			ConfigurationResult::Type _ =
			    wait(changeConfig(cx, (primaryDead ? g_simulator.disablePrimary : g_simulator.disableRemote) +
	...
	loop {
		try {
			...
			state Future<int64_t> dataDistributionQueueSize =
			    getDataDistributionQueueSize(cx, masterWorker, dataInFlightGate == 0);
			...
			if (dataInFlight.get() > dataInFlightGate || tLogQueueSize.get() > maxTLogQueueGate ||
			    ...
			    storageServersRecruiting.get() == true) {
