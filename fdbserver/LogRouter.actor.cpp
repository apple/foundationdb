...
struct LogRouterData {
	...
	LogRouterData(UID dbgid, InitializeLogRouterRequest req)
	  : dbgid(dbgid), routerTag(req.routerTag), logSystem(new AsyncVar<Reference<ILogSystem>>()),
	    version(req.startVersion - 1), minPopped(req.startVersion - 1), startVersion(req.startVersion),
	    allowPops(false), minKnownCommittedVersion(0) {
	...
	for (auto& msg : taggedMessages) {
		...
		for (auto& tag : msg.tags) {
			...
			if (version >= tagData->popped) {
				tagData->version_messages.push_back(
...
ACTOR Future<Void> logRouterPop(LogRouterData* self, TLogPopRequest req) {
	...
	if (self->logSystem->get() && self->allowPops) {
		self->logSystem->get()->pop(std::min(minKnownCommittedVersion, self->minKnownCommittedVersion),
		                            self->routerTag);
	}
	req.reply.send(Void());
