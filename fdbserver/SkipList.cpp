...
void sortPoints(vector<KeyInfo>& points) {
	...
	while (tasks.size()) {
		...
		for (int i = 0; i < counts.size(); i++) {
			int temp = counts[i];
			if (temp > 1) tasks.push_back(SortTask(st.begin + total, temp, st.character + 1));
			counts[i] = total;
			total += temp;
		}
...
public:
	...
	~SkipList() { destroy(); }
	SkipList(SkipList&& other) noexcept(true) : header(other.header) { other.header = NULL; }
...
class MiniConflictSet : NonCopyable {
	...
	void setNthBit(vector<wordType>& v, const unsigned int bit) { v[bit >> bucketShift] |= bitMask(bit); }
	void clearNthBit(vector<wordType>& v, const unsigned int bit) { v[bit >> bucketShift] &= ~(bitMask(bit)); }
...
void ConflictBatch::detectConflicts(Version now, Version newOldestVersion, vector<int>& nonConflicting,
                                    vector<int>* tooOldTransactions) {
	double t = timer();
	sortPoints(points);
...
void ConflictBatch::addConflictRanges(Version now, vector<pair<StringRef, StringRef>>::iterator begin,
                                      vector<pair<StringRef, StringRef>>::iterator end, SkipList* part) {
	int count = end - begin;
#if 0
...
void ConflictBatch::mergeWriteConflictRanges(Version now) {
	...
	if (PARALLEL_THREAD_COUNT) {
		...
		for (int i = 0; i < PARALLEL_THREAD_COUNT; i++) parts.push_back(SkipList());
...
void ConflictBatch::combineWriteConflictRanges() {
	...
	for (int i = 0; i < points.size(); i++) {
		...
		if (point.write && !transactionConflictStatus[point.transaction]) {
			if (point.begin) {
				activeWriteCount++;
				if (activeWriteCount == 1) combinedWriteConflictRanges.push_back(make_pair(point.key, KeyRef()));
			} else /*if (point.end)*/ {
				activeWriteCount--;
