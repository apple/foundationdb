...
struct BlobManagerStats {
	...
	ActiveCounter<int> activePurges;
	...
	explicit BlobManagerStats(UID id,
	  ...
	  : cc("BlobManagerStats", id.toString()), granuleSplits("GranuleSplits", cc),
	    ...
	    filesPurged("FilesPurged", cc), granulesHitMedianLimit("GranulesHitMedianLimit", cc),
	    purgeLatestReads("PurgeLatestReads", cc), purgeDAGReads("PurgeDAGReads", cc), activeMerges(0),
	    blockedAssignments(0), lastFlushVersion(0), lastMLogTruncationVersion(0), lastManifestSeqNo(0),
	    lastManifestDumpTs(0), manifestSizeInBytes(0), activeGranuleSplitChecks(0), activePurges(0) {
		...
		specialCounter(cc, "ActivePurges", [this]() -> int { return this->activePurges.getValue(); });
