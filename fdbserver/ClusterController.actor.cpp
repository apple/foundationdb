...
ACTOR Future<Void> clusterWatchDatabase(ClusterControllerData* cluster,
	...
	loop {
		...
		try {
			...
			wait(ClusterControllerRecovery::recruitNewMaster(cluster, db, std::addressof(newMaster)));
			...
			recoveryCore.cancel();
			wait(ClusterControllerRecovery::cleanupActorCollection(recoveryData, /*exThrown=*/false));
