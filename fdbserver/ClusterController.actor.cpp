...
public:
	...
	std::vector<WorkerDetails> getWorkersForSeedServers(
		...
		for (auto& it : id_worker) {
			...
			if (workerAvailable(it.second, false) && !conf.isExcludedServer(it.second.details.interf.address()) &&
		...
		for (auto& it : id_worker) {
			if (std::find(exclusionWorkerIds.begin(), exclusionWorkerIds.end(), it.second.details.interf.id()) ==
				...
				if (workerAvailable(it.second, checkStable) &&
				    !conf.isExcludedServer(it.second.details.interf.address()) &&
				    fitness != ProcessClass::NeverAssign &&
	...
	WorkerFitnessInfo getWorkerForRoleInDatacenter(Optional<Standalone<StringRef>> const& dcId,
		...
		for (auto& it : id_worker) {
			...
			if (workerAvailable(it.second, checkStable) && fitness < unacceptableFitness &&
				...
				if ((db.serverInfo->get().read().distributor.present() &&
				     db.serverInfo->get().read().distributor.get().locality.processId() == it.first) ||
				    (db.serverInfo->get().read().ratekeeper.present() &&
				     db.serverInfo->get().read().ratekeeper.get().locality.processId() == it.first)) {
	...
	void checkRecoveryStalled() {
		if ((db.serverInfo->get().read().recoveryState == RecoveryState::RECRUITING ||
		     db.serverInfo->get().read().recoveryState == RecoveryState::ACCEPTING_COMMITS ||
		     db.serverInfo->get().read().recoveryState == RecoveryState::ALL_LOGS_RECRUITED) &&
		    db.recoveryStalled) {
...
ACTOR Future<Void> clusterGetServerInfo(ClusterControllerData::DBInfo* db,
	...
	loop {
		useMasterOnly = db->serverInfo->get().read().recoveryState < RecoveryState::ACCEPTING_COMMITS &&
		                !db->requiredAddresses.count(reply.getEndpoint().getPrimaryAddress());
		if ((useMasterOnly ? db->serverInfoMasterOnly.read().id : db->serverInfo->get().read().id) !=
		    knownServerInfoID) {
			break;
		}
		choose {
...
void checkBetterDDOrRK(ClusterControllerData* self) {
	if (!self->masterProcessId.present() ||
	    self->db.serverInfo->get().read().recoveryState < RecoveryState::ACCEPTING_COMMITS) {
		return;
	}
	...
	try {
		...
		if (self->betterMasterExists()) {
			...
			TraceEvent("MasterRegistrationKill", self->id)
			    .detail("MasterId", self->db.serverInfo->get().read().master.id());
		}
	} catch (Error& e) {
		if (e.code() != error_code_no_more_servers) {
...
ACTOR Future<Void> failureDetectionServer(UID uniqueID,
	...
	loop choose {
			...
			if (req.senderStatus.present()) {
				...
				if (req.senderStatus != stat.status) {
					...
					TraceEvent("FailureDetectionStatus", uniqueID)
					    .detail("System", req.addresses.toString())
		...
		when(wait(periodically)) {
			...
			bool useEmergencyDelay = (std::max(self->db.unfinishedRecoveries, self->db.logGenerations) >
			                          CLIENT_KNOBS->FAILURE_MAX_GENERATIONS) ||
			...
			for (auto it = currentStatus.begin(); it != currentStatus.end();) {
				...
				if (it->first != g_network->getLocalAddresses() &&
				    (useEmergencyDelay ? (delay > CLIENT_KNOBS->FAILURE_EMERGENCY_DELAY)
				                       : (delay > pivotDelay * 2 + FLOW_KNOBS->SERVER_REQUEST_INTERVAL +
				                                      CLIENT_KNOBS->FAILURE_MIN_DELAY ||
					...
					TraceEvent("FailureDetectionStatus", uniqueID)
					    .detail("System", describe(it->first))
					    .detail("Status", "Failed")
					    .detail("Why", "Timeout")
					    .detail("LastRequestAge", delay)
					    .detail("PivotDelay", pivotDelay)
					    .detail("UnfinishedRecoveries", self->db.unfinishedRecoveries)
					    .detail("LogGenerations", self->db.logGenerations);
					++self->serversFailed;
...
void clusterRegisterMaster(ClusterControllerData* self, RegisterMasterRequest const& req) {
	...
	if (db->serverInfo->get().read().master.id() != req.id || req.registrationCount <= db->masterRegistrationCount) {
		TraceEvent("MasterRegistrationNotFound", self->id)
		    .detail("MasterId", req.id)
		    .detail("ExistingId", db->serverInfo->get().read().master.id())
		...
		if (req.recoveryState >= RecoveryState::ACCEPTING_COMMITS) {
			...
			for (auto& it : self->id_worker) {
				bool isExcludedFromConfig =
				    db->fullyRecoveredConfig.isExcludedServer(it.second.details.interf.address());
...
ACTOR Future<Void> updateDatacenterVersionDifference(ClusterControllerData* self) {
	...
	loop {
		...
		if (self->db.serverInfo->get().read().recoveryState >= RecoveryState::ACCEPTING_COMMITS &&
	...
	loop {
		try {
			...
			while (!self->masterProcessId.present() ||
			       self->masterProcessId != self->db.serverInfo->get().read().master.locality.processId() ||
			       self->db.serverInfo->get().read().recoveryState < RecoveryState::ACCEPTING_COMMITS) {
...
ACTOR Future<Void> monitorDataDistributor(ClusterControllerData* self) {
	...
	loop {
		if (self->db.serverInfo->get().read().distributor.present()) {
			wait(waitFailureClient(self->db.serverInfo->get().read().distributor.get().waitFailure,
...
ACTOR Future<Void> monitorRatekeeper(ClusterControllerData* self) {
	...
	loop {
		if (self->db.serverInfo->get().read().ratekeeper.present() && !self->recruitRatekeeper.get()) {
			choose {
				when(wait(waitFailureClient(self->db.serverInfo->get().read().ratekeeper.get().waitFailure,
		...
		when(GetWorkersRequest req = waitNext(interf.getWorkers.getFuture())) {
			...
			for (auto& it : self.id_worker) {
				if ((req.flags & GetWorkersRequest::NON_EXCLUDED_PROCESSES_ONLY) &&
				    self.db.config.isExcludedServer(it.second.details.interf.address())) {
					continue;
				}
