...
public:
	...
	std::vector<WorkerDetails> getWorkersForTlogsComplex(DatabaseConfiguration const& conf,
		...
		for (const auto& [worker_process_id, worker_info] : id_worker) {
			...
			if (std::find(exclusionWorkerIds.begin(), exclusionWorkerIds.end(), worker_details.interf.id()) !=
			    ...
			    conf.isExcludedServer(worker_details.interf.addresses()) ||
			    isExcludedDegradedServer(worker_details.interf.addresses()) || fitness == ProcessClass::NeverAssign ||
	...
	void updateWorkerHealth(const NetworkAddress& workerAddress, const std::vector<NetworkAddress>& degradedPeers) {
		...
		// `degradedPeers` contains the latest peer performance view from the worker. Clear the worker if the
