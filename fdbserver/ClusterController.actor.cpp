...
struct WorkerInfo : NonCopyable {
	...
	Future<Void> haltConsistencyChecker;
	...
	WorkerInfo(WorkerInfo&& r) noexcept
	  : watcher(std::move(r.watcher)), reply(std::move(r.reply)), gen(r.gen), reboots(r.reboots),
	    ...
	    haltConsistencyChecker(r.haltConsistencyChecker), issues(r.issues) {}
	void operator=(WorkerInfo&& r) noexcept {
		...
		haltConsistencyChecker = r.haltConsistencyChecker;
		issues = r.issues;
	}
};
...
public:
	struct DBInfo {
		...
		void setConsistencyChecker(const ConsistencyCheckerInterface& interf) {
			auto newInfo = serverInfo->get();
			newInfo.id = deterministicRandom()->randomUniqueID();
			newInfo.infoGeneration = ++dbInfoCount;
		...
		void clearInterf(ProcessClass::ClassType t) {
			...
			} else if (t == ProcessClass::ConsistencyCheckerClass) {
				newInfo.consistencyChecker = Optional<ConsistencyCheckerInterface>();
			}
			serverInfo->set(newInfo);
		}
	};
	...
	bool isLongLivedStateless(Optional<Key> const& processId) {
		return (db.serverInfo->get().distributor.present() &&
		       ...
		       (db.serverInfo->get().consistencyChecker.present() &&
		        db.serverInfo->get().consistencyChecker.get().locality.processId() == processId);
	}
	...
	AsyncVar<bool> recruitConsistencyChecker;
	Optional<UID> recruitingConsistencyCheckerID;
	...
	ClusterControllerData(ClusterControllerFullInterface const& ccInterface,
	  ...
	  : gotProcessClasses(false), gotFullyRecoveredConfig(false), clusterControllerProcessId(locality.processId()),
	    ...
	    recruitDistributor(false), recruitRatekeeper(false), recruitConsistencyChecker(false),
...
void checkBetterSingletons(ClusterControllerData* self) {
	...
	WorkerDetails newCKWorker = findNewProcessForSingleton(self, ProcessClass::ConsistencyChecker, id_used);
	...
	auto bestFitnessForCK = findBestFitnessForSingleton(self, newCKWorker, ProcessClass::ConsistencyChecker);
	...
	auto ckSingleton = ConsistencyCheckerSingleton(db.consistencyChecker);
	...
	if (newColocMap[newRKProcessId] <= currColocMap[currRKProcessId] &&
		...
		} else if (newColocMap[newCKProcessId] < currColocMap[currCKProcessId]) {
...
void registerWorker(RegisterWorkerRequest req, ClusterControllerData* self, ConfigBroadcaster* configBroadcaster) {
	...
	if (req.consistencyCheckerInterf.present()) {
