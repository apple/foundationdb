...
struct WorkerInfo : NonCopyable {
	...
	WorkerInfo(Future<Void> watcher, ReplyPromise<RegisterWorkerReply> reply, Generation gen, WorkerInterface interf,
	  ...
	  : watcher(watcher), reply(reply), gen(gen), reboots(0), interf(interf), initialClass(initialClass),
	    processClass(processClass), priorityInfo(priorityInfo) {}

	WorkerInfo(WorkerInfo&& r) noexcept(true)
	  : watcher(std::move(r.watcher)), reply(std::move(r.reply)), gen(r.gen), reboots(r.reboots),
	    interf(std::move(r.interf)), initialClass(r.initialClass), processClass(r.processClass),
...
struct WorkerFitnessInfo {
	...
	WorkerFitnessInfo(std::pair<WorkerInterface, ProcessClass> worker, ProcessClass::Fitness fitness, int used)
...
public:
	struct DBInfo {
		...
		DBInfo()
		  : masterRegistrationCount(0), recoveryStalled(false), forceRecovery(false),
		    clientInfo(new AsyncVar<ClientDBInfo>(ClientDBInfo())),
		    serverInfo(new AsyncVar<ServerDBInfo>(ServerDBInfo(LiteralStringRef("DB")))),
		    db(DatabaseContext::create(clientInfo, Future<Void>(), LocalityData(), true, TaskDefaultEndpoint,
	...
	bool workerAvailable(WorkerInfo const& worker, bool checkStable) {
		return (now() - startTime < 2 * FLOW_KNOBS->SERVER_REQUEST_INTERVAL) ||
		       (IFailureMonitor::failureMonitor().getState(worker.interf.storage.getEndpoint()).isAvailable() &&
	...
	std::pair<WorkerInterface, ProcessClass> getStorageWorker(RecruitStorageRequest const& req) {
		...
		for (auto& it : id_worker)
			if (workerAvailable(it.second, false) && !excludedMachines.count(it.second.interf.locality.zoneId()) &&
		...
		if (req.criticalRecruitment) {
			...
			for (auto& it : id_worker) {
				...
				if (workerAvailable(it.second, false) && !excludedMachines.count(it.second.interf.locality.zoneId()) &&
				    (includeDCs.size() == 0 || includeDCs.count(it.second.interf.locality.dcId())) &&
				    !addressExcluded(excludedAddresses, it.second.interf.address()) && fit < bestFit) {
					bestFit = fit;
	...
	std::vector<std::pair<WorkerInterface, ProcessClass>> getWorkersForSeedServers(
	    DatabaseConfiguration const& conf, IRepPolicyRef const& policy,
		...
		LocalitySetRef logServerSet =
		    Reference<LocalitySet>(new LocalityMap<std::pair<WorkerInterface, ProcessClass>>());
		LocalityMap<std::pair<WorkerInterface, ProcessClass>>* logServerMap =
		    (LocalityMap<std::pair<WorkerInterface, ProcessClass>>*)logServerSet.getPtr();
		bool bCompleted = false;

		for (auto& it : id_worker) {
			auto fitness = it.second.processClass.machineClassFitness(ProcessClass::Storage);
			if (workerAvailable(it.second, false) && !conf.isExcludedServer(it.second.interf.address()) &&
			    fitness != ProcessClass::NeverAssign &&
			    (!dcId.present() || it.second.interf.locality.dcId() == dcId.get())) {
	...
	std::vector<std::pair<WorkerInterface, ProcessClass>> getWorkersForTlogs(
	    DatabaseConfiguration const& conf, int32_t required, int32_t desired, IRepPolicyRef const& policy,
		...
		for (auto& it : id_worker) {
			...
			if (workerAvailable(it.second, checkStable) && !conf.isExcludedServer(it.second.interf.address()) &&
			    fitness != ProcessClass::NeverAssign &&
			    (!dcIds.size() || dcIds.count(it.second.interf.locality.dcId()))) {
				fitness_workers[fitness].push_back(std::make_pair(it.second.interf, it.second.processClass));
			} else {
		...
		for (int fitness = ProcessClass::BestFit; fitness != ProcessClass::NeverAssign; fitness++) {
			...
			if (fitness_workers.find(fitnessEnum) == fitness_workers.end()) continue;
	...
	std::vector<std::pair<WorkerInterface, ProcessClass>> getWorkersForSatelliteLogs(
	    const DatabaseConfiguration& conf, const RegionInfo& region,
	    std::map<Optional<Standalone<StringRef>>, int>& id_used, bool& satelliteFallback, bool checkStable = false) {
		int startDC = 0;
		loop {
				...
				if (satelliteFallback) {
					return getWorkersForTlogs(conf, region.satelliteTLogReplicationFactorFallback,
					                          conf.getDesiredSatelliteLogs(region.dcId) *
					                              region.satelliteTLogUsableDcsFallback / region.satelliteTLogUsableDcs,
					                          region.satelliteTLogPolicyFallback, id_used, checkStable, satelliteDCs);
				} else {
					return getWorkersForTlogs(conf, region.satelliteTLogReplicationFactor,
					                          conf.getDesiredSatelliteLogs(region.dcId), region.satelliteTLogPolicy,
					                          id_used, checkStable, satelliteDCs);
				}
			} catch (Error& e) {
	...
	WorkerFitnessInfo getWorkerForRoleInDatacenter(Optional<Standalone<StringRef>> const& dcId,
		...
		std::map<std::pair<ProcessClass::Fitness, int>, vector<std::pair<WorkerInterface, ProcessClass>>>
		    fitness_workers;
		...
		for (auto& it : id_worker) {
			...
			if (workerAvailable(it.second, checkStable) && fitness < unacceptableFitness &&
			    it.second.interf.locality.dcId() == dcId) {
				fitness_workers[std::make_pair(fitness, id_used[it.first])].push_back(
				    std::make_pair(it.second.interf, it.second.processClass));
			}
		}
	...
	vector<std::pair<WorkerInterface, ProcessClass>> getWorkersForRoleInDatacenter(
		...
		for (auto& it : id_worker) {
			...
			if (workerAvailable(it.second, checkStable) && !conf.isExcludedServer(it.second.interf.address()) &&
			    it.second.interf.locality.dcId() == dcId &&
			    (!minWorker.present() ||
			     (it.second.interf.id() != minWorker.get().worker.first.id() &&
	...
	struct RoleFitness {
		...
		RoleFitness(RoleFitness first, RoleFitness second)
		  : bestFit(std::min(first.worstFit, second.worstFit)), worstFit(std::max(first.worstFit, second.worstFit)),
	...
	RecruitRemoteFromConfigurationReply findRemoteWorkersForConfiguration(
		...
		auto remoteLogs = getWorkersForTlogs(req.configuration, req.configuration.getRemoteTLogReplicationFactor(),
		                                     req.configuration.getDesiredRemoteLogs(),
		                                     req.configuration.getRemoteTLogPolicy(), id_used, false, remoteDC);
		...
		if (now() - remoteStartTime.get() < SERVER_KNOBS->WAIT_FOR_GOOD_REMOTE_RECRUITMENT_DELAY &&
		    ((RoleFitness(SERVER_KNOBS->EXPECTED_TLOG_FITNESS, req.configuration.getDesiredRemoteLogs())
		     ...
		     (RoleFitness(SERVER_KNOBS->EXPECTED_LOG_ROUTER_FITNESS, req.logRouterCount)
	...
	ErrorOr<RecruitFromConfigurationReply> findWorkersForConfiguration(RecruitFromConfigurationRequest const& req,
		...
		auto proxies =
		    getWorkersForRoleInDatacenter(dcId, ProcessClass::Proxy, req.configuration.getDesiredProxies() - 1,
		                                  req.configuration, id_used, first_proxy);
		auto resolvers =
		    getWorkersForRoleInDatacenter(dcId, ProcessClass::Resolver, req.configuration.getDesiredResolvers() - 1,
		                                  req.configuration, id_used, first_resolver);

		proxies.push_back(first_proxy.worker);
		resolvers.push_back(first_resolver.worker);

		for (int i = 0; i < resolvers.size(); i++) result.resolvers.push_back(resolvers[i].first);
		for (int i = 0; i < proxies.size(); i++) result.proxies.push_back(proxies[i].first);

		auto oldLogRouters = getWorkersForRoleInDatacenter(dcId, ProcessClass::LogRouter, req.maxOldLogRouters,
		                                                   req.configuration, id_used);
	...
	RecruitFromConfigurationReply findWorkersForConfiguration(RecruitFromConfigurationRequest const& req) {
		if (req.configuration.regions.size() > 1) {
			...
			if (regions[0].priority == regions[1].priority && clusterControllerDcId.present() &&
			    regions[1].dcId == clusterControllerDcId.get()) {
				std::swap(regions[0], regions[1]);
			}

			if (clusterControllerDcId.present() && regions[1].dcId == clusterControllerDcId.get() &&
			...
			try {
				...
				if (now() - startTime < SERVER_KNOBS->WAIT_FOR_GOOD_REMOTE_RECRUITMENT_DELAY &&
				    (!clusterControllerDcId.present() || regions[1].dcId != clusterControllerDcId.get())) {
					throw operation_failed();
				}
			...
			if (reply.isError()) {
				throw reply.getError();
			} else if (clusterControllerDcId.present() &&
			           req.configuration.regions[0].dcId == clusterControllerDcId.get()) {
				return reply.get();
			}
			throw no_more_servers();
		} else {
			...
			for (auto dcId : datacenters) {
				try {
					...
					auto proxies = getWorkersForRoleInDatacenter(dcId, ProcessClass::Proxy,
					                                             req.configuration.getDesiredProxies() - 1,
					                                             req.configuration, used, first_proxy);
					auto resolvers = getWorkersForRoleInDatacenter(dcId, ProcessClass::Resolver,
					                                               req.configuration.getDesiredResolvers() - 1,
					                                               req.configuration, used, first_resolver);

					proxies.push_back(first_proxy.worker);
					resolvers.push_back(first_resolver.worker);

					auto fitness = RoleFitness(RoleFitness(proxies, ProcessClass::Proxy),
					...
					if (dcId == clusterControllerDcId) {
						...
						auto oldLogRouters = getWorkersForRoleInDatacenter(
						    dcId, ProcessClass::LogRouter, req.maxOldLogRouters, req.configuration, used);
						for (int i = 0; i < oldLogRouters.size(); i++) {
							result.oldLogRouters.push_back(oldLogRouters[i].first);
						}
						break;
			...
			if (now() - startTime < SERVER_KNOBS->WAIT_FOR_GOOD_RECRUITMENT_DELAY &&
			    (RoleFitness(SERVER_KNOBS->EXPECTED_TLOG_FITNESS, req.configuration.getDesiredLogs())
			         .betterCount(RoleFitness(tlogs, ProcessClass::TLog)) ||
			     RoleFitness(std::min(SERVER_KNOBS->EXPECTED_PROXY_FITNESS, SERVER_KNOBS->EXPECTED_RESOLVER_FITNESS),
			                 std::max(SERVER_KNOBS->EXPECTED_PROXY_FITNESS, SERVER_KNOBS->EXPECTED_RESOLVER_FITNESS),
			                 req.configuration.getDesiredProxies() + req.configuration.getDesiredResolvers())
	...
	bool betterMasterExists() {
		...
		if (db.config.regions.size() > 1 && clusterControllerDcId.present() &&
		    db.config.regions[0].priority > db.config.regions[1].priority &&
		...
		for (auto& logSet : dbi.logSystemConfig.tLogs) {
			for (auto& it : logSet.tLogs) {
				...
				if (logSet.isLocal && logSet.locality == tagLocalitySatellite) {
					satellite_tlogs.push_back(
					    std::make_pair(tlogWorker->second.interf, tlogWorker->second.processClass));
		...
		ProcessClass::Fitness oldMasterFit =
		    masterWorker->second.processClass.machineClassFitness(ProcessClass::Master);
		...
		if (oldMasterFit < mworker.fitness) return false;
		if (oldMasterFit > mworker.fitness || (dbi.master.locality.processId() == clusterControllerProcessId &&
		                                       mworker.worker.first.locality.processId() != clusterControllerProcessId))
			return true;
		...
		RoleFitness newTLogFit(getWorkersForTlogs(db.config, db.config.tLogReplicationFactor,
		                                          db.config.getDesiredLogs(), db.config.tLogPolicy, id_used, true,
		                                          primaryDC),
		                       ProcessClass::TLog);
		...
		RoleFitness newSatelliteTLogFit(
		    region.satelliteTLogReplicationFactor > 0
		        ? getWorkersForSatelliteLogs(db.config, region, id_used, newSatelliteFallback, true)
		        : satellite_tlogs,
		    ProcessClass::TLog);
		...
		RoleFitness oldInFit(RoleFitness(proxyClasses, ProcessClass::Proxy),
		...
		auto proxies =
		    getWorkersForRoleInDatacenter(clusterControllerDcId, ProcessClass::Proxy, db.config.getDesiredProxies() - 1,
		                                  db.config, id_used, first_proxy, true);
		auto resolvers = getWorkersForRoleInDatacenter(clusterControllerDcId, ProcessClass::Resolver,
		                                               db.config.getDesiredResolvers() - 1, db.config, id_used,
		                                               first_resolver, true);
		proxies.push_back(first_proxy.worker);
		resolvers.push_back(first_resolver.worker);
		...
		if (oldTLogFit > newTLogFit || oldInFit > newInFit || (oldSatelliteFallback && !newSatelliteFallback) ||
		    oldSatelliteTLogFit > newSatelliteTLogFit || oldRemoteTLogFit > newRemoteTLogFit ||
		    oldLogRoutersFit > newLogRoutersFit) {
			TraceEvent("BetterMasterExists", id)
			    .detail("OldMasterFit", oldMasterFit)
			    .detail("NewMasterFit", mworker.fitness)
			    .detail("OldTLogFit", oldTLogFit.toString())
			    .detail("NewTLogFit", newTLogFit.toString())
			    .detail("OldInFit", oldInFit.toString())
			    .detail("NewInFit", newInFit.toString())
	...
	explicit ClusterControllerData(ClusterControllerFullInterface ccInterface)
	  : id(ccInterface.id()), ac(false), outstandingRequestChecker(Void()), gotProcessClasses(false),
	    ...
	    versionDifferenceUpdated(false) {
...
vector<T> values(std::map<K, T> const& map) {
	vector<T> t;
	for (auto i = map.begin(); i != map.end(); ++i) t.push_back(i->second);
	return t;
}
...
ACTOR Future<Void> clusterWatchDatabase(ClusterControllerData* cluster, ClusterControllerData::DBInfo* db) {
	...
	loop {
		...
		try {
			...
			if ((masterWorker.worker.second.machineClassFitness(ProcessClass::Master) >
			         SERVER_KNOBS->EXPECTED_MASTER_FITNESS ||
			     masterWorker.worker.first.locality.processId() == cluster->clusterControllerProcessId) &&
			    now() - cluster->startTime < SERVER_KNOBS->WAIT_FOR_GOOD_RECRUITMENT_DELAY) {
				TraceEvent("CCWDB", cluster->id)
				    .detail("Fitness", masterWorker.worker.second.machineClassFitness(ProcessClass::Master));
				wait(delay(SERVER_KNOBS->ATTEMPT_RECRUITMENT_DELAY));
				continue;
			}
			...
			if (newMaster.present()) {
				...
				wait(delay(SERVER_KNOBS->MASTER_SPIN_DELAY)); // Don't retry master recovery more than once per second,
				                                              // but don't delay the "first" recovery after more than a
				                                              // second of normal operation
				...
				loop choose {
					when(wait(waitFailureClient(
					          ...
					          db->forceMasterFailure.getFuture())) {
						break;
					}
...
void removeIssue(ProcessIssuesMap& issueMap, NetworkAddress const& addr, std::string const& issue, UID& issueID) {
	...
	if (issueMap.count(addr) && issueMap[addr].second == issueID) issueMap.erase(addr);
}

ACTOR Future<Void> clusterGetServerInfo(ClusterControllerData::DBInfo* db, UID knownServerInfoID, std::string issues,
...
ACTOR Future<Void> clusterOpenDatabase(ClusterControllerData::DBInfo* db, Standalone<StringRef> dbName,
                                       UID knownClientInfoID, std::string issues,
                                       Standalone<VectorRef<ClientVersionRef>> supportedVersions,
                                       Standalone<StringRef> traceLogGroup, ReplyPromise<ClientDBInfo> reply) {
...
void checkOutstandingRequests(ClusterControllerData* self) {
	if (!self->outstandingRequestChecker.isReady()) return;
...
ACTOR Future<Void> workerAvailabilityWatch(WorkerInterface worker, ProcessClass startingClass,
	...
	cluster->updateWorkerList.set(worker.locality.processId(),
	                              ProcessData(worker.locality, startingClass, worker.address()));
	loop {
		choose {
			...
			when(wait(failed)) { // remove workers that have failed
				...
				if (!failedWorkerInfo.reply.isSet()) {
					failedWorkerInfo.reply.send(
					    RegisterWorkerReply(failedWorkerInfo.processClass, failedWorkerInfo.priorityInfo));
				}
...
ACTOR Future<Void> failureDetectionServer(UID uniqueID, FutureStream<FailureMonitoringRequest> requests) {
	...
	state std::deque<SystemFailureStatus>
	    statusHistory; // The last change in statusHistory is from currentVersion-1 to currentVersion
		...
		when(FailureMonitoringRequest req = waitNext(requests)) {
			if (req.senderStatus.present()) {
				...
				if (req.senderStatus != stat.status) {
					TraceEvent("FailureDetectionStatus", uniqueID)
					    .detail("System", address)
					    .detail("Status", newStat.failed ? "Failed" : "OK")
					    .detail("Why", "Request");
		...
		when(wait(periodically)) {
			...
			for (auto it = currentStatus.begin(); it != currentStatus.end();) {
				...
				if (it->first != g_network->getLocalAddress() &&
				    (delay > pivotDelay * 2 + FLOW_KNOBS->SERVER_REQUEST_INTERVAL + CLIENT_KNOBS->FAILURE_MIN_DELAY ||
				     delay > CLIENT_KNOBS->FAILURE_MAX_DELAY)) {
					...
					TraceEvent("FailureDetectionStatus", uniqueID)
					    .detail("System", it->first)
					    .detail("Status", "Failed")
					    .detail("Why", "Timeout")
					    .detail("LastRequestAge", delay)
...
ACTOR Future<Void> clusterRecruitFromConfiguration(ClusterControllerData* self, RecruitFromConfigurationRequest req) {
	...
	loop {
		try {
			...
			if (e.code() == error_code_no_more_servers &&
			    now() - self->startTime >= SERVER_KNOBS->WAIT_FOR_GOOD_RECRUITMENT_DELAY) {
...
ACTOR Future<Void> clusterRecruitRemoteFromConfiguration(ClusterControllerData* self,
	...
	loop {
		try {
			...
			if (e.code() == error_code_no_more_servers && self->remoteStartTime.present() &&
			    now() - self->remoteStartTime.get() >= SERVER_KNOBS->WAIT_FOR_GOOD_REMOTE_RECRUITMENT_DELAY) {
...
void clusterRegisterMaster(ClusterControllerData* self, RegisterMasterRequest const& req) {
	...
	TraceEvent("MasterRegistrationReceived", self->id)
	    .detail("DbName", printable(req.dbName))
...
void registerWorker(RegisterWorkerRequest req, ClusterControllerData* self) {
	...
	TraceEvent("ClusterControllerActualWorkers", self->id)
	    ...
	    .detailext("ProcessId", w.locality.processId())
	    .detailext("ZoneId", w.locality.zoneId())
	    .detailext("DataHall", w.locality.dataHallId())
	    .detail("PClass", req.processClass.toString())
	    .detail("Workers", self->id_worker.size())
	    .detail("Registered", (info == self->id_worker.end() ? "False" : "True"))
	    .backtrace();
	...
	if (info == self->id_worker.end()) {
		self->id_worker[w.locality.processId()] =
		    WorkerInfo(workerAvailabilityWatch(w, newProcessClass, self), req.reply, req.generation, w,
		               req.initialClass, newProcessClass, newPriorityInfo);
		checkOutstandingRequests(self);
		return;
	}
...
ACTOR Future<Void> statusServer(FutureStream<StatusRequest> requests, ClusterControllerData* self,
	...
	loop {
		try {
			...
			while (requests.isReady()) requests_batch.push_back(requests.pop());
			...
			state ErrorOr<StatusReply> result = wait(errorOr(
			    clusterGetStatus(self->db.serverInfo, self->cx, workers, self->db.workersWithIssues,
			                     self->db.clientsWithIssues, self->db.clientVersionMap, self->db.traceLogGroupMap,
...
ACTOR Future<Void> updatedChangingDatacenters(ClusterControllerData* self) {
	...
	loop {
		...
		if (!self->desiredDcIds.get().present()) {
			...
			uint8_t newFitness = ClusterControllerPriorityInfo::calculateDCFitness(worker.interf.locality.dcId(),
			                                                                       self->desiredDcIds.get().get());
					...
					for (auto& it : self->id_worker) {
						if ((!it.second.priorityInfo.isExcluded &&
							...
							uint8_t fitness = ClusterControllerPriorityInfo::calculateDCFitness(
							    it.second.interf.locality.dcId(), self->changingDcIds.get().second.get());
							if (it.first != self->clusterControllerProcessId &&
								...
								if (!it.second.reply.isSet()) {
									it.second.reply.send(
									    RegisterWorkerReply(it.second.processClass, it.second.priorityInfo));
								}
							}
						}
					}
				...
				if (self->changedDcIds.get().second.present()) {
					...
					if (!self->changedDcIds.get().first) {
						...
						uint8_t newFitness = ClusterControllerPriorityInfo::calculateDCFitness(
						    worker.interf.locality.dcId(), self->changedDcIds.get().second.get());
						if (worker.priorityInfo.dcFitness != newFitness) {
							worker.priorityInfo.dcFitness = newFitness;
							if (!worker.reply.isSet()) {
								worker.reply.send(RegisterWorkerReply(worker.processClass, worker.priorityInfo));
						...
						while (currentFit <= ProcessClass::NeverAssign) {
							...
							for (auto& it : self->id_worker) {
								if ((!it.second.priorityInfo.isExcluded &&
								    ...
								    currentFit == ProcessClass::NeverAssign) {
									uint8_t fitness = ClusterControllerPriorityInfo::calculateDCFitness(
									    it.second.interf.locality.dcId(), self->changedDcIds.get().second.get());
									if (it.first != self->clusterControllerProcessId &&
									    it.second.priorityInfo.dcFitness != fitness) {
										updated = true;
										it.second.priorityInfo.dcFitness = fitness;
...
ACTOR Future<Void> clusterControllerCore(ClusterControllerFullInterface interf, Future<Void> leaderFail,
	...
	loop choose {
		...
		when(OpenDatabaseRequest req = waitNext(interf.clientInterface.openDatabase.getFuture())) {
			addActor.send(clusterOpenDatabase(&self.db, req.dbName, req.knownClientInfoID, req.issues.toString(),
			                                  req.supportedVersions, req.traceLogGroup, req.reply));
		}
		...
		when(RegisterWorkerRequest req = waitNext(interf.registerWorker.getFuture())) { registerWorker(req, &self); }
		when(GetWorkersRequest req = waitNext(interf.getWorkers.getFuture())) {
			...
			for (auto& it : self.id_worker) {
				if ((req.flags & GetWorkersRequest::NON_EXCLUDED_PROCESSES_ONLY) &&
				    self.db.config.isExcludedServer(it.second.interf.address())) {
					continue;
				}

				if ((req.flags & GetWorkersRequest::TESTER_CLASS_ONLY) &&
				    it.second.processClass.classType() != ProcessClass::TesterClass) {
					continue;
				}
		...
		when(wait(coordinationPingDelay)) {
			...
			for (auto& it : self.id_worker) it.second.interf.coordinationPing.send(message);
		...
		when(GetServerDBInfoRequest req = waitNext(interf.getServerDBInfo.getFuture())) {
			addActor.send(clusterGetServerInfo(&self.db, req.knownServerInfoID, req.issues.toString(),
			                                   req.incompatiblePeers, req.reply));
		}
...
ACTOR Future<Void> clusterController(ServerCoordinators coordinators,
	...
	loop {
		...
		try {
			...
			if (inRole)
				endRole(cci.id(), "ClusterController", "Error",
