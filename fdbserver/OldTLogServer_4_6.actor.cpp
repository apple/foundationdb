...
namespace oldTLog_4_6 {
	...
	struct LogData : NonCopyable, public ReferenceCounted<LogData> {
		struct TagData {
			...
			TagData(TagData&& r) noexcept : version_messages(std::move(r.version_messages)), nothing_persistent(r.nothing_persistent), popped_recently(r.popped_recently), popped(r.popped), update_version_sizes(r.update_version_sizes) {}
