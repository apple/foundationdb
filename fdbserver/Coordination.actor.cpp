	...
	loop choose {
		when(GenerationRegReadRequest _req = waitNext(interf.read.getFuture())) {
			TraceEvent("GenerationRegReadRequest")
			    .detail("From", _req.reply.getEndpoint().address)
			    .detail("K", printable(_req.key));
		...
		when(GenerationRegWriteRequest _wrq = waitNext(interf.write.getFuture())) {
			...
			if (v.readGen <= wrq.gen && v.writeGen < wrq.gen) {
				...
				TraceEvent("GenerationRegWrote")
				    .detail("From", wrq.reply.getEndpoint().address)
				    .detail("Key", printable(wrq.kv.key))
...
TEST_CASE("fdbserver/Coordination/localGenerationReg/simple") {
	...
	UniqueGeneration g =
	    wait(reg.write.getReply(GenerationRegWriteRequest(KeyValueRef(the_key, LiteralStringRef("Value1")), firstGen)));
...
ACTOR Future<Void> leaderRegister(LeaderElectionRegInterface interf, Key key) {
	...
	loop choose {
		when(GetLeaderRequest req = waitNext(interf.getLeader.getFuture())) {
			if (currentNominee.present() && currentNominee.get().changeID != req.knownLeader) {
				...
				if (notify.size() > SERVER_KNOBS->MAX_NOTIFICATIONS) {
					...
					for (int i = 0; i < notify.size(); i++) notify[i].send(currentNominee.get());
					notify.clear();
				}
			}
		}
		...
		when(ForwardRequest req = waitNext(interf.forward.getFuture())) {
			...
			for (int i = 0; i < notify.size(); i++) notify[i].send(newInfo);
			notify.clear();
			req.reply.send(Void());
			return Void();
		}
		when(wait(nextInterval)) {
			if (!availableLeaders.size() && !availableCandidates.size() && !notify.size() &&
				...
				if (availableLeaders.size() && availableCandidates.size()) {
					nextNominee = (*availableLeaders.begin() < *availableCandidates.begin())
					                  ? *availableLeaders.begin()
					                  : *availableCandidates.begin();
				...
				if (!nextNominee.present() || !foundCurrentNominee ||
				    currentNominee.get().leaderChangeRequired(nextNominee.get())) {
					TraceEvent("NominatingLeader")
					    .detail("Nominee", nextNominee.present() ? nextNominee.get().changeID : UID())
					    .detail("Changed", nextNominee != currentNominee)
					    .detail("Key", printable(key));
					for (int i = 0; i < notify.size(); i++) notify[i].send(nextNominee);
					notify.clear();
...
ACTOR Future<Void> coordinationServer(std::string dataFolder) {
	...
	TraceEvent("CoordinationServer", myID)
	    .detail("MyInterfaceAddr", myInterface.read.getEndpoint().address)
