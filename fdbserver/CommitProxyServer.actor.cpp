...
struct ResolutionRequestBuilder {
	...
	void addTransaction(CommitTransactionRequest& trRequest, Version ver, int transactionNumberInBatch) {
		...
		for (auto& m : trIn.mutations) {
			TraceEvent("GuruProxyServerAddTransaction").detail("Version", ver).detail("Len1", m.param1.size()).detail("Param1", m.param1).detail("Len2", m.param2.size()).detail("Param2", m.param2).log();
