...
void commitMessages(Reference<TLogGroupData> self,
	...
	std::cout << __FUNCTION__ << "  " << storedMessage.toHexString() << std::endl;
...
ACTOR Future<Void> commitPeekAndCheck(std::shared_ptr<test::TestDriverContext> pContext) {
	state test::print::PrintTiming printTiming("tlog/commitPeekAndCheck");
	...
	// Generate mutations
	ProxySubsequencedMessageSerializer serializer(beginVersion);
	state std::vector<MutationRef> mutations;
	const int numMutations = 1;
	for (int i = 0; i < numMutations; i++) {
		...
		serializer.write(m, storageTeamID);
	...
	// Peek
	TLogPeekRequest peekRequest(debugID, beginVersion, endVersion, storageTeamID);
	test::print::print(peekRequest);

	state TLogPeekReply peekReply = wait(tli->peek.getReply(peekRequest));
	test::print::print(peekReply);

	// Verify
	SubsequencedMessageDeserializer deserializer(peekReply.data);
	...
	for (auto iter = deserializer.begin(); iter != deserializer.end(); ++iter, ++i) {
		const VersionSubsequenceMessage& m = *iter;
