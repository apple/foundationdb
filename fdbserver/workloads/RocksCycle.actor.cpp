...
// This workload is very similar to general CycleWorkload except that we only run it for RocksDB storage engine
// and turn on knobs such as ROCKSDB_METRICS_IN_SIMULATION. These knobs are known to cause non-determinism, which is
