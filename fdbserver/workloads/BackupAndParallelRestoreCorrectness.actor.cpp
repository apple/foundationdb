...
struct BackupAndParallelRestoreCorrectnessWorkload : TestWorkload {
	...
	BackupAndParallelRestoreCorrectnessWorkload(WorkloadContext const& wcx)
		...
		if (shareLogRange) {
			...
			// Add backup ranges
			// MX:Q: why the range endpoints (the range interval) are randomly generated? Won't this cause unbalanced range interval in backup?
...
*/
	...
	ACTOR static Future<Void> doBackup(BackupAndParallelRestoreCorrectnessWorkload* self, double startDelay, FileBackupAgent* backupAgent, Database cx,
	 ...
	 */
	ACTOR static Future<Void> attemptDirtyRestore(BackupAndParallelRestoreCorrectnessWorkload* self, Database cx, FileBackupAgent* backupAgent, Standalone<StringRef> lastBackupContainer, UID randomID) {
		state Transaction tr(cx);
...
//						}
			...
			if ((g_simulator.backupAgents == ISimulator::BackupToFile) && (!BackupAndParallelRestoreCorrectnessWorkload::backupAgentRequests)) {
