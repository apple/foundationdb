...
struct StorageQuotaWorkload : TestWorkload {
	...
	ACTOR Future<Void> _start(Database cx) {
		...
		state Optional<size_t> quota1 = wait(getStorageQuotaHelper(cx, "name1"_sr));
		ASSERT(quota1.present() && quota1.get() == 300);
		state Optional<size_t> quota2 = wait(getStorageQuotaHelper(cx, "name2"_sr));
		ASSERT(quota2.present() && quota2.get() == 200);
		state Optional<size_t> quota3 = wait(getStorageQuotaHelper(cx, "name3"_sr));
		ASSERT(!quota3.present());

		return Void();
	}

	ACTOR static Future<Void> setStorageQuotaHelper(Database cx, StringRef tenantName, size_t quota) {
	...
	ACTOR static Future<Optional<size_t>> getStorageQuotaHelper(Database cx, StringRef tenantName) {
		state Transaction tr(cx);
		loop {
			try {
				state Optional<size_t> quota = wait(getStorageQuota(&tr, tenantName));
				wait(tr.commit());
				return quota;
			} catch (Error& e) {
				wait(tr.onError(e));
