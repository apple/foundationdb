	...
	ACTOR Future<Void> incrementClient(Database cx, Increment* self, double delay) {
		...
		try {
			loop {
				...
				while (true) {
					try {
						tr.atomicOp(intToTestKey(g_random->randomInt(0, self->nodeCount / 2)), LiteralStringRef("\x01"),
						            MutationRef::AddValue);
						tr.atomicOp(intToTestKey(g_random->randomInt(self->nodeCount / 2, self->nodeCount)),
