...
struct ReadWriteWorkload : KVWorkload {
				...
				if (workerList.present()) {
					...
					for (int i = 0; i < workerList.get().size(); i++)
						dumpRequests.push_back(
						    workerList.get()[i].first.traceBatchDumpRequest.tryGetReply(TraceBatchDumpRequest()));
	...
	Value randomValue() {
		return StringRef((uint8_t*)valueString.c_str(), g_random->randomInt(minValueBytes, maxValueBytes + 1));
	}
	...
	ACTOR static Future<Void> tracePeriodically(ReadWriteWorkload* self) {
		...
		loop {
			...
			if (recordBegin && recordEnd) {
				...
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "5% Latency (ms, averaged)", 1000 * self->latencies.percentile(.05), true));
				self->periodicMetrics.push_back(
				...
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "5% GRV Latency (ms, averaged)", 1000 * self->GRVLatencies.percentile(.05), true));
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "95% GRV Latency (ms, averaged)", 1000 * self->GRVLatencies.percentile(.95), true));
				...
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "Max Latency (ms, averaged)", 1000 * self->latencies.max(), true));
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "Max Row Read Latency (ms, averaged)", 1000 * self->readLatencies.max(), true));
				self->periodicMetrics.push_back(PerfMetric(ts + "Max Total Read Latency (ms, averaged)",
				                                           1000 * self->fullReadLatencies.max(), true));
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "Max GRV Latency (ms, averaged)", 1000 * self->GRVLatencies.max(), true));
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "Max Commit Latency (ms, averaged)", 1000 * self->commitLatencies.max(), true));
			}
			last_ops = ops;
	...
	int64_t getRandomKey(uint64_t nodeCount) {
		if (forceHotProbability && g_random->random01() < forceHotProbability)
			return g_random->randomInt64(0, nodeCount * hotKeyFraction) /
	...
	Future<Void> randomReadWriteClient(Database cx, ReadWriteWorkload* self, double delay, int clientIndex) {
		...
		loop {
			...
			if (!self->rampUpLoad || g_random->random01() < self->sweepAlpha(startTime)) {
				...
				state bool aTransaction =
				    g_random->random01() > (self->rampTransactionType ? self->sweepAlpha(startTime) : self->alpha);
