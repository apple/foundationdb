...
struct DataDistributionMetricsWorkload : KVWorkload {
	...
	int64_t avgBytes;
	int64_t avgBytesPerKSeconds;
	int64_t transactionTimeLimit;
	double testDuration;
	...
	DataDistributionMetricsWorkload(WorkloadContext const& wcx)
	  : KVWorkload(wcx), numShards(0), avgBytes(0), avgBytesPerKSeconds(0), commits("Commits"), errors("Errors") {
	...
	ACTOR static Future<bool> _check(Database cx, DataDistributionMetricsWorkload* self) {
		...
		loop {
			...
			try {
				...
				state int64_t totalBytesShardPerKSecond = 0;
				...
				for (i = 0; i < result.size(); ++i) {
					...
					totalBytesShardPerKSecond += valueObj["shard_bytes_per_ksecond"].get_int64();
				}
				self->avgBytes = totalBytes / self->numShards;
				self->avgBytesPerKSeconds = totalBytesShardPerKSecond / self->numShards;
				break;
			} catch (Error& e) {
				if (e.code() == error_code_timed_out || e.code() == error_code_transaction_timed_out) {
					tr->reset();
	...
	void getMetrics(std::vector<PerfMetric>& m) override {
		...
		m.emplace_back("AvgBytesPerKSeconds", avgBytesPerKSeconds, Averaged::True);
