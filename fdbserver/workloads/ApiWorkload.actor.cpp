...
Key ApiWorkload::generateKey(VectorRef<KeyValueRef> const& data, int minKeyLength, int maxKeyLength,
	...
	if (onlyLowerCase) {
		for (int i = 0; i < keyLength; i++) keyBuffer[i] = g_random->randomInt('a', 'z' + 1);
	} else {
		...
		if (keyBuffer[0] == '\xff') keyBuffer[0] = g_random->randomInt(0, 255);
	}
...
KeySelector ApiWorkload::generateKeySelector(VectorRef<KeyValueRef> const& data, int maxOffset) {
	...
	return KeySelector(
	    KeySelectorRef(key, g_random->randomInt(0, 2) == 1, g_random->randomInt(-maxOffset, maxOffset + 1)));
}
