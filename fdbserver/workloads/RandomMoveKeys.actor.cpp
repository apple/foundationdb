...
struct MoveKeysWorkload : TestWorkload {
	...
	ACTOR Future<Void> doMoveKeys(Database cx, MoveKeysWorkload* self, KeyRange keys,
		...
		try {
			...
			wait(moveKeys(cx, keys, destinationTeamIDs, destinationTeamIDs, lock, signal, &fl1, &fl2, invalidVersion,
			              false, relocateShardInterval.pairID));
			...
			try {
				...
				loop {
					...
					for (int r = 0; r < ranges.size(); r++) {
						...
						inFlightActors.insert(
						    ranges[r], self->doMoveKeys(cx, self, ranges[r], rTeam, lock, self->dbName.toString()));
					}
				}
			} catch (Error& e) {
