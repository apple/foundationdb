...
ACTOR template <class T>
Future<uint64_t> setupRangeWorker(Database cx, T* workload, vector<std::pair<uint64_t, uint64_t>>* jobs,
...
ACTOR static Future<vector<pair<uint64_t, double>>> trackInsertionCount(Database cx, vector<uint64_t> countsOfInterest,
	...
	while (currentCountIndex < countsOfInterest.size()) {
		try {
			...
			while (currentCountIndex < countsOfInterest.size() &&
				...
				countInsertionRates.push_back(
				    pair<uint64_t, double>(countsOfInterest[currentCountIndex++], bytesInserted / (now() - startTime)));
...
ACTOR template <class T>
Future<Void> bulkSetup(
    Database cx, T* workload, uint64_t nodeCount, Promise<double> setupTime, bool valuesInconsequential = false,
    double postSetupWarming = 0.0, double maxKeyInsertRate = 1e12,
    vector<uint64_t> insertionCountsToMeasure = vector<uint64_t>(),
    Promise<vector<pair<uint64_t, double>>> ratesAtKeyCounts = Promise<vector<pair<uint64_t, double>>>(),
	...
	for (int i = 0; i < 100; i++) {
		Standalone<KeyValueRef> sampleKV =
		    (*workload)(startNode + (uint64_t)(g_random->random01() * (endNode - startNode)));
