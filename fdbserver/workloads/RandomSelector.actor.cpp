			...
			try {
				for (i = 0; i < deterministicRandom()->randomInt(self->minOperationsPerTransaction,
					...
					if (j < 3) {
						...
						ValueResult getTest = wait(trRYOW.get(StringRef(clientID + "b/" + myKeyA)));
						getTest1 = getTest;

						loop {
							try {
								ValueResult getTest2 = wait(tr.get(StringRef(clientID + "d/" + myKeyA)));
						...
						loop {
							try {
								...
								if (error.code() == error_code_commit_unknown_result) {
									ValueResult thing = wait(tr.get(StringRef(clientID + "z/" + myRandomIDKey)));
									if (thing.present())
										break;
								}
							}
