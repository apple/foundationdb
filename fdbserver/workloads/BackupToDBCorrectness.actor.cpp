...
struct BackupToDBCorrectnessWorkload : TestWorkload {
	...
	BackupToDBCorrectnessWorkload(WorkloadContext const& wcx) : TestWorkload(wcx) {
		...
		abortAndRestartAfter = getOption(options, LiteralStringRef("abortAndRestartAfter"),
		                                 (!locked && g_random->random01() < 0.5)
		                                     ? g_random->random01() * (restoreAfter - backupAfter) + backupAfter
		                                     : 0.0);
		differentialBackup =
