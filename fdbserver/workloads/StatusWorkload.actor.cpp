...
struct StatusWorkload : TestWorkload {
	...
	static bool schemaMatch(StatusObject const schema, StatusObject const result, Severity sev = SevError,
	                        std::string path = std::string(), std::string schema_path = std::string()) {
				...
				if (sv.type() == json_spirit::obj_type && sv.get_obj().count("$enum")) {
					...
					for (auto& value_pair : value_obj) {
						...
						if (value_pair.second.type() != json_spirit::obj_type) {
							TraceEvent(sev, "SchemaMismatch")
							    .detail("Path", vpath)
							    .detail("ValueType", value_pair.second.type());
							ok = false;
							continue;
						}
						if (!schemaMatch(schema_obj, value_pair.second.get_obj(), sev, vpath, upath)) ok = false;
					}
				} else {
					// The schema entry isn't an operator, so it asserts a type and (depending on the type) recursive
					// schema definition
					...
					if (rv.type() == json_spirit::array_type) {
						...
						if (!schema_array.size()) {
							...
							if (value_array.size()) {
								TraceEvent(sev, "SchemaMismatch")
								    .detail("Path", kpath)
								    .detail("SchemaSize", schema_array.size())
								    .detail("ValueSize", value_array.size());
								ok = false;
								continue;
							}
						} else if (schema_array.size() == 1 && schema_array[0].type() == json_spirit::obj_type) {
							// A one item schema array means that all items in the value must match the first item in
							// the schema
							...
							for (auto& value_item : value_array) {
								if (value_item.type() != json_spirit::obj_type) {
									TraceEvent(sev, "SchemaMismatch")
									    .detail("Path", kpath + format("[%d]", index))
									    .detail("ValueType", value_item.type());
									ok = false;
									continue;
								}
								if (!schemaMatch(schema_obj, value_item.get_obj(), sev, kpath + format("[%d]", index),
								                 spath + "[0]"))
									ok = false;
								index++;
							}
						...
						auto& value_obj = rv.get_obj();
						if (!schemaMatch(schema_obj, value_obj, sev, kpath, spath)) ok = false;
					}
				}
			}
			return ok;
...
TEST_CASE("fdbserver/status/schema/basic") {
	json_spirit::mValue schema;
	json_spirit::read_string(
	    std::string("{\"apple\":3,\"banana\":\"foo\",\"sub\":{\"thing\":true},\"arr\":[{\"a\":1,\"b\":2}],\"en\":{\"$"
	...
	auto check = [&schema](bool expect_ok, std::string t) {
		...
		ASSERT(expect_ok ==
		       StatusWorkload::schemaMatch(schema.get_obj(), test.get_obj(), expect_ok ? SevError : SevInfo));
	};
	check(true, "{}");
	check(true, "{\"apple\":4}");
