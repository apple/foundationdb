...
struct StalePeerTestWorkload : TestWorkload {
	...
	explicit(false) StalePeerTestWorkload(WorkloadContext const& wcx) : TestWorkload(wcx), testPassed(true) {
	...
	void addIfKillable(std::vector<NetworkAddress>& out, NetworkAddress addr) {
		if (addr.isValid() && !g_simulator->isProtectedAddress(addr)) {
			out.push_back(addr);
		}
	}
	...
	static Future<Void> _start(StalePeerTestWorkload* self, Database cx) {
		...
		std::string trackerRole = self->computeTrackerRole();

		// Resolve kill targets.
		std::vector<NetworkAddress> targetAddresses;
		...
		NetworkAddress oldAddr;
		if (self->dstKillRole == "ss") {
			double warmDeadline = now() + 30.0;
			for (;;) {
				NetworkAddress best;
				int bestN = -1;
		...
		TraceEvent("StalePeerTestKilling")
		    ...
		    .detail("ProcessClass", getSimulatorProcessClass(proc).toString())
		...
		if (!proc->failed) {
			TraceEvent(SevError, "StalePeerTestKillIneffective")
			    .detail("Address", oldAddr)
			    .detail("Protected", g_simulator->isProtectedAddress(oldAddr))
		...
		double recoveryTriggerDeadline = now() + 30.0;
		...
		if (self->dstKillRole == "coordinator") {
			...
			int autoChangeAttempts = 0;
			for (;;) {
				++autoChangeAttempts;
				CoordinatorsResult res = co_await changeQuorum(cx, autoQuorumChange());
		...
		co_await delay(self->waitAfterKill);
		...
		int clientSourcesChecked = 0; // source processes inspected (post-filter)
		int clientSourcesWithLeak = 0; // ...of which had a per-role Delta > 0 (a leak)
			...
			for (const auto& oldKillAddr : self->oldKillAddresses) {
				...
				if (delta > 0) {
					...
					TraceEvent(SevError, "StalePeerTestFailed")
					    ...
					    .detail("CheckedProcessClass", getSimulatorProcessClass(proc).toString())
