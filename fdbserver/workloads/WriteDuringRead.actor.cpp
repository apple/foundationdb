...
struct WriteDuringReadWorkload : TestWorkload {
	...
	WriteDuringReadWorkload(WorkloadContext const& wcx)
	  : TestWorkload(wcx), transactions("Transactions"), retries("Retries"), success(true) {
		...
		valueSizeRange = std::make_pair(0, std::min<int>(g_random->randomInt(0, 4 << g_random->randomInt(0, 16)),
...
ACTOR Future<Void> commitAndUpdateMemory(ReadYourWritesTransaction* tr, WriteDuringReadWorkload* self, bool* cancelled,
                                         ...
                                         bool* doingCommit, double* startTime, Key timebombStr) {
	state UID randomID = g_nondeterministic_random->randomUniqueID();
	...
	loop {
		...
		loop {
			wait(delay(now() - startTime > self->slowModeStart ||
			               ...
			               : 0.1));
			try {
				wait(self->randomTransaction((self->useExtraDB && g_random->random01() < 0.5) ? self->extraDB : cx,
				                             self, startTime));
			} catch (Error& e) {
...
KeyRange getRandomRange(int sizeLimit) {
	...
	int endLocation =
	    startLocation + g_random->randomInt(0, 1 + std::min(sizeLimit, std::min(nodes - startLocation, 1 << scale)));
	...
	loop {
		...
		try {
					...
					try {
						...
						if (operationType == 0 && !disableGetKey) {
							operations.push_back(self->getKeyAndCompare(
							    &tr, self->getRandomKeySelector(), g_random->random01() < 0.5, readYourWritesDisabled,
							    snapshotRYWDisabled, self, &doingCommit, &memLimit));
							...
							operations.push_back(self->getRangeAndCompare(
							    ...
							    self->getRandomLimits(), g_random->random01() < 0.5, g_random->random01() < 0.5,
							...
							operations.push_back(self->getAndCompare(
							    &tr, self->getRandomKey(), g_random->random01() > 0.5, readYourWritesDisabled,
							    snapshotRYWDisabled, self, &doingCommit, &memLimit));
							...
							if (!self->rarelyCommit || g_random->random01() < 1.0 / self->numOps) {
								Future<Void> commit = self->commitAndUpdateMemory(
								    ...
								    readAheadDisabled, &doingCommit, &startTime, timebombStr);
							...
							operations.push_back(
							    delay(g_random->random01() * g_random->random01() * g_random->random01() * maxTime));
							...
							if (!self->useSystemKeys && g_random->random01() < 0.01) {
								...
								Value value = self->getRandomValue();
								KeyRangeRef range =
								    getVersionstampKeyRange(versionStampKey.arena(), versionStampKey, normalKeys.end);
								self->changeCount.insert(range, changeNum++);
			...
			if (e.code() == error_code_not_committed || e.code() == error_code_commit_unknown_result ||
			    ...
			    e.code() == error_code_value_too_large || cancelled)
				throw not_committed();
			try {
				wait(tr.onError(e));
			} catch (Error& e) {
