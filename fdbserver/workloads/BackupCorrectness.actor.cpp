...
struct BackupAndRestoreCorrectnessWorkload : TestWorkload {
	...
	BackupAndRestoreCorrectnessWorkload(WorkloadContext const& wcx) : TestWorkload(wcx) {
		...
		abortAndRestartAfter = getOption(
		    options, LiteralStringRef("abortAndRestartAfter"),
		    g_random->random01() < 0.5 ? g_random->random01() * (restoreAfter - backupAfter) + backupAfter : 0.0);
		differentialBackup =
		    getOption(options, LiteralStringRef("differentialBackup"), g_random->random01() < 0.5 ? true : false);
		stopDifferentialAfter = getOption(
		    options, LiteralStringRef("stopDifferentialAfter"),
		    differentialBackup ? g_random->random01() * (restoreAfter - std::max(abortAndRestartAfter, backupAfter)) +
	...
	ACTOR static Future<Void> doBackup(BackupAndRestoreCorrectnessWorkload* self, double startDelay,
		...
		try {
			wait(backupAgent->submitBackup(cx, StringRef(backupContainer), g_random->randomInt(0, 100), tag.toString(),
			                               backupRanges, stopDifferentialDelay ? false : true));
		} catch (Error& e) {
	...
	ACTOR static Future<Void> attemptDirtyRestore(BackupAndRestoreCorrectnessWorkload* self, Database cx,
		...
		if (rowCount > 0) {
			try {
				Version _ = wait(backupAgent->restore(cx, self->backupTag, KeyRef(lastBackupContainer), true, -1, true,
		...
		try {
			...
			if (!self->locked && BUGGIFY) {
				...
				try {
					extraBackup = backupAgent.submitBackup(cx, LiteralStringRef("file://simfdb/backups/"),
					                                       g_random->randomInt(0, 100), self->backupTag.toString(),
					                                       self->backupRanges, true);
				} catch (Error& e) {
			...
			if (lastBackupContainer && self->performRestore) {
				...
				for (restoreIndex = 0; restoreIndex < self->backupRanges.size(); restoreIndex++) {
					...
					restores.push_back(backupAgent.restore(cx, restoreTag, KeyRef(lastBackupContainer->getURL()), true,
					                                       -1, true, range, Key(), Key(), self->locked));
				}
					...
					for (restoreIndex = 0; restoreIndex < restores.size(); restoreIndex++) {
						...
						if (rs == FileBackupAgent::ERestoreState::ABORTED) {
							...
							restores[restoreIndex] = backupAgent.restore(
							    cx, restoreTags[restoreIndex], KeyRef(lastBackupContainer->getURL()), true, -1, true,
							    self->backupRanges[restoreIndex], Key(), Key(), self->locked);
						}
					}
				}
