...
struct BlobGranuleRangesWorkload : TestWorkload {
	...
	ACTOR Future<Void> verifyRangeUnit(Database cx, BlobGranuleRangesWorkload* self, KeyRange range) {
		...
		bool setSuccess = wait(cx->blobbifyRange(activeRange));
		ASSERT(setSuccess);
	...
	ACTOR Future<Void> verifyRangeGapUnit(Database cx, BlobGranuleRangesWorkload* self, KeyRange range) {
		...
		for (i = 0; i < rangeCount; i++) {
			...
			if (i != rangeToNotBlobbify) {
				bool setSuccess = wait(cx->blobbifyRange(subRange));
				ASSERT(setSuccess);
	...
	ACTOR Future<Void> rangesMisalignedUnit(Database cx, BlobGranuleRangesWorkload* self, KeyRange range) {
		bool setSuccess = wait(cx->blobbifyRange(range));
		ASSERT(setSuccess);
	...
	ACTOR Future<Void> blobbifyIdempotentUnit(Database cx, BlobGranuleRangesWorkload* self, KeyRange range) {
		...
		if (deterministicRandom()->coinflip()) {
			bool unblobbifyStartSuccess = wait(cx->blobbifyRange(activeRange));
			ASSERT(unblobbifyStartSuccess);
		}

		bool success = wait(cx->blobbifyRange(activeRange));
		ASSERT(success);
		wait(self->checkRange(cx, self, activeRange, true));

		// check that re-blobbifying same range is successful
		bool retrySuccess = wait(cx->blobbifyRange(activeRange));
		ASSERT(retrySuccess);
		wait(self->checkRange(cx, self, activeRange, true));

		// check that blobbifying range that overlaps but does not match existing blob range fails
		bool fail1 = wait(cx->blobbifyRange(range));
		ASSERT(!fail1);

		bool fail2 = wait(cx->blobbifyRange(KeyRangeRef(range.begin, activeRange.end)));
		ASSERT(!fail2);

		bool fail3 = wait(cx->blobbifyRange(KeyRangeRef(activeRange.begin, range.end)));
		ASSERT(!fail3);

		bool fail4 = wait(cx->blobbifyRange(KeyRangeRef(range.begin, middleKey)));
		ASSERT(!fail4);

		bool fail5 = wait(cx->blobbifyRange(KeyRangeRef(middleKey, range.end)));
		ASSERT(!fail5);

		bool fail6 = wait(cx->blobbifyRange(KeyRangeRef(activeRange.begin, middleKey)));
		ASSERT(!fail6);

		bool fail7 = wait(cx->blobbifyRange(KeyRangeRef(middleKey, activeRange.end)));
		ASSERT(!fail7);

		bool fail8 = wait(cx->blobbifyRange(KeyRangeRef(middleKey, middleKey2)));
		ASSERT(!fail8);
...

		{
			...
			bool unblobbifyFail1 = wait(cx->unblobbifyRange(range));
			ASSERT(!unblobbifyFail1);

			bool unblobbifyFail2 = wait(cx->unblobbifyRange(KeyRangeRef(range.begin, activeRange.end)));
			ASSERT(!unblobbifyFail2);

			bool unblobbifyFail3 = wait(cx->unblobbifyRange(KeyRangeRef(activeRange.begin, range.end)));
			ASSERT(!unblobbifyFail3);

			bool unblobbifyFail4 = wait(cx->unblobbifyRange(KeyRangeRef(activeRange.begin, middleKey)));
			ASSERT(!unblobbifyFail4);

			bool unblobbifyFail5 = wait(cx->unblobbifyRange(KeyRangeRef(middleKey, activeRange.end)));
			ASSERT(!unblobbifyFail5);

			bool unblobbifyFail6 = wait(cx->unblobbifyRange(KeyRangeRef(activeRange.begin, middleKey)));
			ASSERT(!unblobbifyFail6);

			bool unblobbifyFail7 = wait(cx->unblobbifyRange(KeyRangeRef(middleKey, activeRange.end)));
			ASSERT(!unblobbifyFail7);

			bool unblobbifyFail8 = wait(cx->unblobbifyRange(KeyRangeRef(middleKey, middleKey2)));
			ASSERT(!unblobbifyFail8);

			bool unblobbifySuccess = wait(cx->unblobbifyRange(activeRange));
			ASSERT(unblobbifySuccess);

			bool unblobbifySuccessAgain = wait(cx->unblobbifyRange(activeRange));
	...
	ACTOR Future<Void> reBlobbifyUnit(Database cx, BlobGranuleRangesWorkload* self, KeyRange range) {
		...
		bool unsetSuccess = wait(cx->unblobbifyRange(range));
		ASSERT(unsetSuccess);
		wait(self->checkRange(cx, self, range, false));

		bool reSetSuccess = wait(cx->blobbifyRange(range));
		ASSERT(reSetSuccess);
		wait(self->checkRange(cx, self, range, true));
