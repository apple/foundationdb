			...
			if (self->performRestore) {
				...
				if (lastBackupContainer) {
					...
					if (self->performValidation) {
						...
						// Step 5: Negative test - corrupt data and verify audit catches mismatch
						// This validates that the audit actually detects errors, not just passes blindly
						TraceEvent("BS3BCW_ValidationNegativeTestStart")
						    .detail("Purpose", "Verify audit detects data corruption");

						// 5a. Read a key from normalKeys within backup ranges
						state Key corruptedKey;
						state Value originalValue;
						state bool foundKeyToCorrupt = false;

						if (!self->backupRanges.empty()) {
							state Transaction readTr(cx);
							loop {
								try {
									readTr.setOption(FDBTransactionOptions::READ_LOCK_AWARE);
									KeyRange rangeToRead = self->backupRanges[0];
									RangeResult result = wait(readTr.getRange(rangeToRead, 1));
									if (!result.empty()) {
										corruptedKey = result[0].key;
										originalValue = result[0].value;
										foundKeyToCorrupt = true;
									}
									break;
								} catch (Error& e) {
									wait(readTr.onError(e));
								}
							}
						}

						if (foundKeyToCorrupt) {
							// 5b. Corrupt the key by writing a different value
							state Value corruptedValue = "CORRUPTED_VALUE_FOR_NEGATIVE_TEST"_sr;
							...
							TraceEvent("BS3BCW_ValidationNegativeTestCorrupted")
							    .detail("Key", corruptedKey)
							    .detail("OriginalValue", originalValue)
							    .detail("CorruptedValue", corruptedValue);

							// 5c. Re-run audit - should detect mismatch
							state UID negativeAuditId;
							state bool auditDetectedMismatch = false;

							try {
								UID negResult = wait(timeoutError(auditStorage(clusterFile,
								                                               normalKeys,
								                                               AuditType::ValidateRestore,
								                                               KeyValueStoreType::END,
								                                               300.0),
								                                  60.0));
								negativeAuditId = negResult;
							} catch (Error& e) {
								// Audit scheduling might fail, which is also acceptable
								auditDetectedMismatch = true;
								TraceEvent("BS3BCW_ValidationNegativeTestAuditScheduleFailed").error(e);
							}

							if (!auditDetectedMismatch) {
								// Monitor audit - it should fail
								state double negAuditStartTime = now();
								loop {
									wait(delay(2.0));

									std::vector<AuditStorageState> negAuditStates =
									    wait(getAuditStates(cx, AuditType::ValidateRestore, true));

									for (const auto& auditState : negAuditStates) {
										if (auditState.id == negativeAuditId) {
											if (auditState.getPhase() == AuditPhase::Error ||
											    auditState.getPhase() == AuditPhase::Failed) {
												auditDetectedMismatch = true;
												TraceEvent("BS3BCW_ValidationNegativeTestPassed")
												    .detail("AuditID", negativeAuditId)
												    .detail("Phase", (int)auditState.getPhase())
												    .detail("Error", auditState.error)
												    .detail("Result", "Audit correctly detected corruption!");
												break;
											} else if (auditState.getPhase() == AuditPhase::Complete) {
												// Audit completed without detecting error - this is a bug!
									...
									if (now() - negAuditStartTime > 120.0) {
										TraceEvent(SevWarn, "BS3BCW_ValidationNegativeTestTimeout")
										    .detail("AuditID", negativeAuditId);
										// Timeout is acceptable - the important thing is it didn't pass
										break;
									}
								}
							}

							// 5d. Restore original value so subsequent tests aren't affected
							// Copy state vars to locals for lambda capture
							...
							TraceEvent("BS3BCW_ValidationNegativeTestComplete")
							    ...
							    .detail("AuditDetectedMismatch", auditDetectedMismatch);
						} else {
							TraceEvent(SevWarn, "BS3BCW_ValidationNegativeTestSkipped")
							    .detail("Reason", "No keys found in backup ranges to corrupt");
						}

						// Step 6: Clean up validation data from system keyspace
						wait(runRYWTransaction(cx, [=](Reference<ReadYourWritesTransaction> tr) -> Future<Void> {
							tr->setOption(FDBTransactionOptions::ACCESS_SYSTEM_KEYS);
							tr->setOption(FDBTransactionOptions::LOCK_AWARE);
