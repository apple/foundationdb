...
struct TPCC : TestWorkload {
	...
	TPCC(WorkloadContext const& ctx) : TestWorkload(ctx) {
		...
		warehousesPerClient = getOption(options, LiteralStringRef("warehousesPerClient"), 4);
		expectedTransactionsPerMinute = getOption(options, LiteralStringRef("expectedTransactionsPerMinute"), 1000);
		testDuration = getOption(options, LiteralStringRef("testDuration"), 3600);
		warmupTime = getOption(options, LiteralStringRef("warmupTime"), 300);
		clientsUsed = getOption(options, LiteralStringRef("clientsUsed"), 40);
	}

	int NURand(int C, int A, int x, int y) {
		return (((g_random->randomInt(0, A + 1) | g_random->randomInt(x, y + 1)) + C) % (y - x + 1)) + x;
	}
		...
		try {
			...
			for (; ol_id < order.o_ol_cnt; ++ol_id) {
				...
				if (g_random->randomInt(0, 100) == 0) {
					orderLine.ol_supply_w_id = g_random->randomInt(0, self->clientsUsed * self->warehousesPerClient);
				}
				state Item item;
	...
	ACTOR static Future<Customer> getRandomCustomer(TPCC* self, ReadYourWritesTransaction* tr, int w_id, int d_id) {
		...
		if (g_random->randomInt(0, 100) >= 85) {
			...
			result.c_w_id = g_random->randomInt(0, self->clientsUsed * self->warehousesPerClient);
		}
	...
	ACTOR static Future<bool> payment(TPCC* self, Database cx, int w_id) {
		...
		try {
			...
			history.h_w_id = w_id;
			{
				...
				kW << k;
				auto key = kW.toStringRef().withPrefix(LiteralStringRef("History/"));
	...
	ACTOR static Future<Void> emulatedUser(TPCC* self, Database cx, int w_id, int d_id) {
		// stagger users
		wait(delay(20.0 * g_random->random01()));
		...
		loop {
			...
			if (type < 4) {
				...
				wait(delay(g_random->random01() * 5));
			} else if (type < 8) {
				...
				wait(delay(g_random->random01() * 12));
			} else {
	...
	virtual void getMetrics(vector<PerfMetric>& m) override {
		double multiplier = static_cast<double>(clientCount) /
		                    static_cast<double>(clientsUsed); // Don't let unused clients affect averages
