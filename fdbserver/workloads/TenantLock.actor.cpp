...
struct TenantLock : TestWorkload {
	...
	ACTOR static Future<Optional<UID>> probeTx(Database db,
		...
		state ReadYourWritesTransaction tr(db, name);
	...
	ACTOR static Future<Void> _start(TenantLock* self, Database db) {
		...
		wait(TenantAPI::lockTenant(db.getReference(), TenantLockState::LOCKED, self->tenant1, lockID1));
		wait(TenantAPI::lockTenant(db.getReference(), TenantLockState::LOCKED, self->tenant2, lockID2));
		...
		wait(TenantAPI::unlockTenant(db.getReference(), self->tenant1, lockID1));
		...
		wait(testExpectedError(TenantAPI::unlockTenant(db.getReference(), self->tenant2, lockID1),
		                       "UnlockWithWrongID",
		                       tenant_locked(),
		                       Optional<bool*>(),
		                       {},
		...
		wait(TenantAPI::unlockTenant(db.getReference(), self->tenant2, lockID2));
