...
struct GetMappedRangeWorkload : ApiWorkload {
	...
	ACTOR Future<Void> scanMappedRange(Database cx,
		...
		state int limit = 100;
		// each record size: 3 splites, each has key and value, plus index (49 + 40) + (50 + 40) + (50 + 40) + 3 * 24 =
		// 341 each index size: 77 + 24 = 101 (each KV data structure has 24 bytes) in total, each record + index is 442
		// bytes as a result, despite matchIndex mode, < 34100 bytes would make it less than 100 records
		state int byteLimit = deterministicRandom()->randomInt(1, 9) * 10000;
		std::cout << "hfu5 byte limit is " << byteLimit << "  knob is " << SERVER_KNOBS->STRICTLY_ENFORCE_BYTE_LIMIT
		          << std::endl;
