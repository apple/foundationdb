...
struct ConsistencyCheckWorkload : TestWorkload {
			...
			try {
				...
				if (self->firstClient && self->performQuiescentChecks) {
					...
					try {
						...
						if (maxStorageServerQueueSize > 0) {
							TraceEvent("ConsistencyCheck_NonZeroStorageServerQueue")
				...
				if (keyServerResult) {
					...
					if (keyLocationResult) {
						...
						bool dataConsistencyResult =
						    wait(self->checkDataConsistency(cx, keyLocations, configuration, self));
					}
				}
			} catch (Error& e) {
				if (e.code() == error_code_transaction_too_old || e.code() == error_code_future_version ||
				    e.code() == error_code_wrong_shard_server || e.code() == error_code_all_alternatives_failed ||
				    e.code() == error_code_server_request_queue_full)
	...
	ACTOR Future<bool> getKeyServers(
	    ...
	    Promise<vector<pair<KeyRange, vector<StorageServerInterface>>>> keyServersPromise) {
		state vector<pair<KeyRange, vector<StorageServerInterface>>> keyServers;
	...
	ACTOR Future<bool> getKeyLocations(Database cx, vector<pair<KeyRange, vector<StorageServerInterface>>> shards,
	...
	ACTOR Future<bool> checkDataConsistency(Database cx, VectorRef<KeyValueRef> keyLocations,
		...
		state Reference<IRateControl> rateLimiter =
		    Reference<IRateControl>(new SpeedLimit(self->rateLimit, CLIENT_KNOBS->CONSISTENCY_CHECK_RATE_WINDOW));
		...
		for (; i < ranges.size(); i += increment) {
			...
			if (self->firstClient && self->performQuiescentChecks &&
				...
				TraceEvent("ConsistencyCheck_InvalidTeamSize")
				    ...
				    .detail("TeamSize", sourceStorageServers.size())
	...
	ACTOR Future<bool> checkForExtraDataStores(Database cx, ConsistencyCheckWorkload* self) {
		...
		for (itr = workers.begin(); itr != workers.end(); ++itr) {
			ErrorOr<Standalone<VectorRef<UID>>> stores =
			    wait(itr->first.diskStoreRequest.getReplyUnlessFailedFor(DiskStoreRequest(false), 2, 0));
			if (stores.isError()) {
				TraceEvent("ConsistencyCheck_GetDataStoreFailure")
				    .error(stores.getError())
				    .detail("Address", itr->first.address());
				...
				if (!statefulProcesses[itr->first.address()].count(id)) {
					...
					if (g_network->isSimulated()) {
						TraceEvent("ConsistencyCheck_RebootProcess")
						    .detail("Address", itr->first.address())
						    .detail("DataStoreID", id);
						g_simulator.rebootProcess(g_simulator.getProcessByAddress(itr->first.address()),
	...
	// The interfaceType paramater is used in a TraceEvent, should be something like (Master, MasterProxy,
	// StorageServer, ...)
	bool workerHasClass(vector<std::pair<WorkerInterface, ProcessClass>> workers, NetworkAddress address,
	                    ProcessClass::ClassType machineClass, std::string interfaceType) {
		// Search all workers until the correct one is found
		for (int i = 0; i < workers.size(); i++) {
			if (workers[i].first.address() == address) {
				if (workers[i].second == machineClass || workers[i].second == ProcessClass::UnsetClass) return true;

				TraceEvent("ConsistencyCheck_InvalidClassType")
				    .detail("RequestedClass", workers[i].second.toString())
				    .detail("ActualClass", ProcessClass(machineClass, ProcessClass::CommandLineSource).toString())
		...
		TraceEvent("ConsistencyCheck_WorkerNotFound")
	...
	static ProcessClass::Fitness getBestAvailableFitness(std::set<ProcessClass::ClassType>& availableClassTypes,
	...
	ACTOR Future<bool> checkUsingDesiredClasses(Database cx, ConsistencyCheckWorkload* self) {
		...
		state vector<std::pair<WorkerInterface, ProcessClass>> nonExcludedWorkers =
		...
		ProcessClass::Fitness bestClusterControllerFitness =
		    getBestAvailableFitness(nonExcludedClassTypes, ProcessClass::ClusterController);
		if (!nonExcludedWorkerProcessMap.count(db.clusterInterface.clientInterface.address()) ||
		    nonExcludedWorkerProcessMap[db.clusterInterface.clientInterface.address()].machineClassFitness(
		...
		if (!allWorkerProcessMap.count(db.master.address()) ||
		    (!nonExcludedWorkerProcessMap.count(db.master.address()) &&
		     bestMasterFitness != ProcessClass::ExcludeFit) ||
		    nonExcludedWorkerProcessMap[db.master.address()].machineClassFitness(ProcessClass::Master) !=
		        bestMasterFitness) {
		...
		ProcessClass::Fitness bestMasterProxyFitness =
		    getBestAvailableFitness(nonExcludedClassTypes, ProcessClass::Proxy);
		for (auto masterProxy : db.client.proxies) {
			if (!nonExcludedWorkerProcessMap.count(masterProxy.address()) ||
			    nonExcludedWorkerProcessMap[masterProxy.address()].machineClassFitness(ProcessClass::Proxy) !=
			        bestMasterProxyFitness) {
				TraceEvent("ConsistencyCheck_ProxyNotBest")
				    ...
				    .detail("ExistingMasterProxyFitness",
				            nonExcludedWorkerProcessMap.count(masterProxy.address())
				                ? nonExcludedWorkerProcessMap[masterProxy.address()].machineClassFitness(
				                      ProcessClass::Proxy)
				                : -1);
				return false;
			}
		...
		ProcessClass::Fitness bestResolverFitness =
		    getBestAvailableFitness(nonExcludedClassTypes, ProcessClass::Resolver);
		for (auto resolver : db.resolvers) {
			if (!nonExcludedWorkerProcessMap.count(resolver.address()) ||
			    nonExcludedWorkerProcessMap[resolver.address()].machineClassFitness(ProcessClass::Resolver) !=
			        bestResolverFitness) {
				TraceEvent("ConsistencyCheck_ResolverNotBest")
				    ...
				    .detail("ExistingResolverFitness",
				            nonExcludedWorkerProcessMap.count(resolver.address())
				                ? nonExcludedWorkerProcessMap[resolver.address()].machineClassFitness(
