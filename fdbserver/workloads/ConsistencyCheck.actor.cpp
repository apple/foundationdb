...
struct ConsistencyCheckWorkload : TestWorkload {
	...
	ConsistencyCheckWorkload(WorkloadContext const& wcx)
	  : TestWorkload(wcx), onTimeout(*this), onTimeoutEvent({ "Timeout"_sr, "TracedTooManyLines"_sr }, onTimeout) {
		...
		// Urgent mode only uses sharedRandomNumber option
	...
	Future<Void> start(Database const& cx) override {
		TraceEvent("ConsistencyCheckStart").detail("ConsistencyCheckerId", consistencyCheckerId);
	...
	ACTOR Future<Void> _startUrgent(Database cx, ConsistencyCheckWorkload* self) {
		TraceEvent("ConsistencyCheckUrgent_TesterStart")
	...
	ACTOR Future<Void> runUrgentCheck(Database cx, ConsistencyCheckWorkload* self) {
		try {
			...
			if (e.code() == error_code_actor_cancelled) {
				...
				TraceEvent(SevWarnAlways, "ConsistencyCheckUrgent_TesterExit")
				    .error(e)
	...
	ACTOR Future<std::vector<std::pair<KeyRange, Value>>> getKeyLocationsForRangeList(Database cx,
		...
		TraceEvent("ConsistencyCheckUrgent_GetKeyLocationsForRangeList")
		    .detail("Ops", "Input")
		...
		loop {
			try {
				...
				TraceEvent("ConsistencyCheckUrgent_RetryGetKeyLocationsForRangeList")
				    .error(e)
				    .detail("ClientCount", self->clientCount)
				    .detail("ClientId", self->clientId);
				wait(tr.onError(e));
