...
struct ConsistencyCheckWorkload : TestWorkload {
	...
	ACTOR Future<bool> checkForStorage(Database cx,
		...
		for (int i = 0; i < workers.size(); i++) {
			...
			if (!configuration.isExcludedServer(addr) && (workers[i].processClass == ProcessClass::StorageClass ||
					...
					for (auto& logRouter : tlogSet.logRouters) {
						...
						if (logRouter.interf().locality.dcId() != expectedRemoteDcId) {
							TraceEvent("ConsistencyCheck_LogRouterNotBestDC")
							    ...
							    .detail("ActualDC", getOptionalString(logRouter.interf().locality.dcId()));
							return false;
						}
					}
				}
