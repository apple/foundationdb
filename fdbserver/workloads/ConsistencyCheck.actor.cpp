...
struct ConsistencyCheckWorkload : TestWorkload {
	...
	static ProcessClass::Fitness getBestAvailableFitness(
		...
		for (auto classType : availableClassTypes) {
			...
			//TraceEvent("ConsistencyCheck_AvailableFitness")
			//	.detail("Role", to_string(role))
			//	.detail("ProcessClass", ProcessClass(classType, ProcessClass::InvalidSource).toString())
			//	.detail("Fitness", ProcessClass(classType, ProcessClass::InvalidSource).machineClassFitness(role));
		}
