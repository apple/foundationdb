...
struct RWTransactor : ITransactor {
	...
	Value randomValue() {
		return StringRef((const uint8_t*)valueString.c_str(), g_random->randomInt(minValueBytes, maxValueBytes + 1));
	};
...
struct ThroughputWorkload : TestWorkload {
	...
	ACTOR static Future<Void> throughputActor(Database db, ThroughputWorkload* self, PromiseStream<Future<Void>> add) {
		...
		// if (g_random->random01() < .001) TraceEvent("ThroughputControl").detail("Error", error).detail("IError",
		// ierror).detail("DesiredSuccessors", desiredSuccessors).detail("ActiveActors", self->activeActors);
