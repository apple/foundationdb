...
struct FileSystemWorkload : TestWorkload {
	...
	ACTOR Future<Void> setupRange(Database cx, FileSystemWorkload* self, int begin, int end) {
		...
		while (true) {
			try {
				ValueResult f = wait(tr.get(self->keyForFileID(begin)));
				if (f.present())
		...
		loop {
			...
			loop {
				try {
					...
					if (isDeleting) {
						state ValueResult deleted = wait(tr.get(StringRef(keyStr + "/deleted")));
						ASSERT(deleted.present());
						ValueResult serverStr = wait(tr.get(StringRef(keyStr + "/server")));
