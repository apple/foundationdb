...
struct AtomicRestoreWorkload : TestWorkload {
	...
	ACTOR static Future<Void> _start(Database cx, AtomicRestoreWorkload* self) {
		...
		try {
			wait(backupAgent.submitBackup(cx, StringRef(backupContainer), g_random->randomInt(0, 100),
		...
		loop {
			...
			for (auto& range : self->backupRanges) {
				restores.push_back(
				    backupAgent.atomicRestore(cx, BackupAgentBase::getDefaultTag(), range, StringRef(), StringRef()));
			}
			try {
