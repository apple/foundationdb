...
class RocksDBWriteStallListener : public rocksdb::EventListener {
public:
	RocksDBWriteStallListener(UID id) : logId(id) {}
...
// Returns string representation of RocksDB flush reason.
// Flush reason code:
...
std::string getFlushReason(const FlushReason& reason) {
	switch (reason) {
	case FlushReason::kOthers:
		return format("%d Others", reason);
	case FlushReason::kGetLiveFiles:
		return format("%d GetLiveFiles", reason);
	case FlushReason::kShutDown:
		return format("%d ShutDown", reason);
	case FlushReason::kExternalFileIngestion:
		return format("%d ExternalFileIngestion", reason);
	case FlushReason::kManualCompaction:
		return format("%d ManualCompaction", reason);
	case FlushReason::kWriteBufferManager:
		return format("%d WriteBufferManager", reason);
	case FlushReason::kWriteBufferFull:
		return format("%d WriteBufferFull", reason);
	case FlushReason::kTest:
		return format("%d Test", reason);
	case FlushReason::kDeleteFiles:
		return format("%d DeleteFiles", reason);
	case FlushReason::kAutoCompaction:
		return format("%d AutoCompaction", reason);
	case FlushReason::kManualFlush:
		return format("%d ManualFlush", reason);
	case FlushReason::kErrorRecovery:
		return format("%d ErrorRecovery", reason);
	case FlushReason::kErrorRecoveryRetryFlush:
		return format("%d ErrorRecoveryRetryFlush", reason);
	case FlushReason::kWalFull:
		return format("%d WalFull", reason);
	default:
		return format("%d Unknown", reason);
	}
}
...
public:
	...
	void logRecentRocksDBFlushStats() {
		...
		for (const auto& flushStats : recentFlushStats) {
			...
			cfNames.insert(flushStats.cfName);
		}
		TraceEvent e(SevInfo, "RecentRocksDBFlushStats", logId);
		e.detail("CFCount", cfNames.size());
		int64_t totalCount = 0;
		for (const auto& [reason, count] : flushReasonStats) {
			e.detail("Reason-" + getFlushReason(reason), count);
			totalCount += count;
		}
		e.detail("FlushCountTotal", totalCount);
	}

private:
	UID logId;
	std::deque<FlushStats> recentFlushStats;
	std::mutex mutex;
};

// Returns string representation of RocksDB compaction reason.
...
std::string getCompactionReason(const CompactionReason& reason) {
	switch (reason) {
	case CompactionReason::kUnknown:
		return format("%d Unknown", reason);
	case CompactionReason::kLevelL0FilesNum:
		return format("%d LevelL0FilesNum", reason);
	case CompactionReason::kLevelMaxLevelSize:
		return format("%d LevelMaxLevelSize", reason);
	case CompactionReason::kUniversalSizeAmplification:
		return format("%d UniversalSizeAmplification", reason);
	case CompactionReason::kUniversalSizeRatio:
		return format("%d UniversalSizeRatio", reason);
	case CompactionReason::kUniversalSortedRunNum:
		return format("%d UniversalSortedRunNum", reason);
	case CompactionReason::kFIFOMaxSize:
		return format("%d FIFOMaxSize", reason);
	case CompactionReason::kFIFOReduceNumFiles:
		return format("%d FIFOReduceNumFiles", reason);
	case CompactionReason::kFIFOTtl:
		return format("%d FIFOTtl", reason);
	case CompactionReason::kManualCompaction:
		return format("%d ManualCompaction", reason);
	case CompactionReason::kFilesMarkedForCompaction:
		return format("%d FilesMarkedForCompaction", reason);
	case CompactionReason::kBottommostFiles:
		return format("%d BottommostFiles", reason);
	case CompactionReason::kTtl:
		return format("%d Ttl", reason);
	case CompactionReason::kFlush:
		return format("%d Flush", reason);
	case CompactionReason::kExternalSstIngestion:
		return format("%d ExternalSstIngestion", reason);
	case CompactionReason::kPeriodicCompaction:
		return format("%d PeriodicCompaction", reason);
	case CompactionReason::kChangeTemperature:
		return format("%d ChangeTemperature", reason);
	case CompactionReason::kForcedBlobGC:
		return format("%d ForcedBlobGC", reason);
	case CompactionReason::kRoundRobinTtl:
		return format("%d RoundRobinTtl", reason);
	case CompactionReason::kRefitLevel:
		return format("%d RefitLevel", reason);
	case CompactionReason::kNumOfReasons:
		return format("%d NumOfReasons", reason);
	default:
		return format("%d Unknown", reason);
	}
}
...
public:
	...
	void OnCompactionBegin(rocksdb::DB* db, const rocksdb::CompactionJobInfo& info) override {
		...
		while (recentCompactionStats.size() > SERVER_KNOBS->LOGGING_ROCKSDB_BG_WORK_COUNT_MAX) {
			recentCompactionStats.pop_front();
		}
		recentCompactionStats.push_back(CompactionStats(info.cf_name, info.thread_id, info.compaction_reason, now()));
	}

	void logRecentRocksDBCompactionStats() {
		std::lock_guard<std::mutex> lock(mutex);
		...
		for (const auto& compactionStats : recentCompactionStats) {
			...
			cfNames.insert(compactionStats.cfName);
		}
		TraceEvent e(SevInfo, "RecentRocksDBCompactionStats", logId);
		e.detail("CFCount", cfNames.size());
		int64_t totalCount = 0;
		for (const auto& [reason, count] : compactionReasonStats) {
			e.detail("Reason-" + getCompactionReason(reason), count);
			totalCount += count;
		}
		e.detail("CompactionCountTotal", totalCount);
	}

private:
	UID logId;
	std::deque<CompactionStats> recentCompactionStats;
	std::mutex mutex;
};
...
public:
	ShardManager(std::string path,
	             ...
	             std::shared_ptr<RocksDBWriteStallListener> writeStallListener,
	             std::shared_ptr<RocksDBFlushListener> flushListener,
	             std::shared_ptr<RocksDBCompactionListener> compactionListener,
	             Counters* cc)
	  : path(path), logId(logId), dbOptions(options), cfOptions(getCFOptions()), dataShardMap(nullptr, specialKeys.end),
		...
		if (!g_network->isSimulated()) {
			...
			// cluster.
			dbOptions.listeners.push_back(writeStallListener);
			if (SERVER_KNOBS->LOGGING_ROCKSDB_BG_WORK_WHEN_IO_TIMEOUT ||
			    SERVER_KNOBS->LOGGING_ROCKSDB_BG_WORK_PROBABILITY > 0) {
				dbOptions.listeners.push_back(flushListener);
				dbOptions.listeners.push_back(compactionListener);
			}
		}
	}
...
struct ShardedRocksDBKeyValueStore : IKeyValueStore {
	...
	explicit ShardedRocksDBKeyValueStore(const std::string& path, UID id)
	  : rState(std::make_shared<ShardedRocksDBState>()), path(path), id(id),
	    ...
	    writeStallListener(std::make_shared<RocksDBWriteStallListener>(id)),
	    flushListener(std::make_shared<RocksDBFlushListener>(id)),
	    compactionListener(std::make_shared<RocksDBCompactionListener>(id)),
	    errorFuture(forwardError(errorListener->getFuture())), dbOptions(getOptions()), shardManager(path,
	                                                                                                 id,
	                                                                                                 dbOptions,
	                                                                                                 errorListener,
	                                                                                                 writeStallListener,
	...
	void logRecentRocksDBFlushStats() override { return flushListener->logRecentRocksDBFlushStats(); }
	void logRecentRocksDBCompactionStats() override { return compactionListener->logRecentRocksDBCompactionStats(); }
