	...
	virtual Future<Void> commit(bool sequential) {
		if (getAvailableSize() <= 0) {
			if (g_network->isSimulated()) { // FIXME: known bug in simulation we are supressing
				...
				TraceEvent("ElapsedTime").detail("SimTime", now()).detail("RealTime", 0).detail("RandomUnseed", unseed);
				flushAndExit(0);
			}
		...
		if (transactionIsLarge) {
			...
			if (!disableSnapshot) {
				committedWriteBytes +=
				    bytesWritten + OP_DISK_OVERHEAD; // OP_DISK_OVERHEAD is for the following log_op(OpCommit)
			}

			// If there have been no mutations since the last commit, do nothing
			if (notifiedCommittedWriteBytes.get() == committedWriteBytes) return Void();
...
private:
	...
	ACTOR static Future<Void> recover(KeyValueStoreMemory* self) {
		...
		// permanent variables (in self) in OpCommit.  OpRollback does the reverse (copying the permanent versions over
		// the uncommitted versions) the uncommitted and committed variables should be equal initially (to whatever
		// makes sense if there are no committed transactions recovered)
		...
		try {
					...
					if (h.op == OpSnapshotItem) { // snapshot data item
						...
						if (p1 >= uncommittedNextKey)
							recoveryQueue.clear(
							    ...
							    &uncommittedNextKey.arena()); // FIXME: Not sure what this line is for, is it necessary?
			...
			// make sure that before any new operations are added to the log that all uncommitted operations are "rolled
			// back"
	...
	ACTOR static Future<Standalone<VectorRef<KeyValueRef>>> waitAndReadRange(KeyValueStoreMemory* self, KeyRange keys,
...
KeyValueStoreMemory::KeyValueStoreMemory(IDiskQueue* log, UID id, int64_t memoryLimit, bool disableSnapshot,
  ...
  : log(log), id(id), previousSnapshotEnd(-1), currentSnapshotEnd(-1), resetSnapshot(false), memoryLimit(memoryLimit),
    committedWriteBytes(0), committedDataSize(0), transactionSize(0), transactionIsLarge(false),
