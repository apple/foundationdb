						...
						try {
							wait(store(newBulkLoadTaskState,
							           getBulkLoadTask(&tr,
							                           ...
							                           { BulkLoadTaskPhase::Triggered, BulkLoadTaskPhase::Running })));
							...
							newBulkLoadTaskState.phase = BulkLoadTaskPhase::Running;
						} catch (Error& e) {
							...
							try {
								wait(store(
								    ...
								    getBulkLoadTask(&tr,
								                    bulkLoadTaskState.get().getRange(),
								                    bulkLoadTaskState.get().getTaskId(),
								                    { BulkLoadTaskPhase::Running, BulkLoadTaskPhase::Complete })));
								newBulkLoadTaskState.phase = BulkLoadTaskPhase::Complete;
							} catch (Error& e) {
								if (e.code() == error_code_bulkload_task_outdated) {
									cancelDataMove = true;
									throw retry();
