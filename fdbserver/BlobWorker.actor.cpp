...
ACTOR Future<std::pair<BlobGranuleSplitState, Version>> getGranuleSplitState(Transaction* tr,
	...
	ValueResult st = wait(tr->get(myStateKey));
	if (st.present()) {
			...
			while (!queue.empty()) {
				...
				loop {
					...
					try {
						...
						ValueResult v = wait(tr.get(blobGranuleHistoryKeyFor(parentRange, parentVersion)));
						if (v.present()) {
...
ACTOR Future<GranuleFiles> loadParentGranuleForMergeSnapshot(Transaction* tr, KeyRange range, Version historyVersion) {
	...
	ValueResult historyParentValue = wait(tr->get(blobGranuleHistoryKeyFor(range, historyVersion)));
...
ACTOR Future<GranuleStartState> openGranule(Reference<BlobWorkerData> bwData, AssignBlobRangeRequest req) {
	...
	loop {
		try {
			...
			state Future<ValueResult> fLockValue = tr.get(lockKey);
			...
			ValueResult prevLockValue = wait(fLockValue);
			...
			if (info.history.present() && info.history.get().value.parentVersions.size() > 0) {
				...
				if (info.history.get().value.parentVersions.size() == 1) { // split
					...
					ValueResult historyParentValue = wait(tr.get(parentHistoryKey));
	...
	loop {
		loop {
			...
			try {
				...
				ValueResult val = wait(tr.get(blobWorkerListKey));
				if (!val.present()) {
