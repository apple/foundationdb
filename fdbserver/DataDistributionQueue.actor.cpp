...
struct RelocateData {
	...
	RelocateData(RelocateShard const& rs)
	  : keys(rs.keys), priority(rs.priority), startTime(now()), randomId(g_random->randomUniqueID()), workFactor(0),
	    wantsNewServers(rs.priority == PRIORITY_REBALANCE_SHARD ||
	                    rs.priority == PRIORITY_REBALANCE_OVERUTILIZED_TEAM ||
	                    rs.priority == PRIORITY_REBALANCE_UNDERUTILIZED_TEAM || rs.priority == PRIORITY_SPLIT_SHARD),
	...
	bool operator==(const RelocateData& rhs) const {
		return priority == rhs.priority && keys == rhs.keys && startTime == rhs.startTime &&
	...
	bool changesBoundaries() {
		return priority == PRIORITY_MERGE_SHARD || priority == PRIORITY_SPLIT_SHARD ||
...
public:
	...
	bool all(std::function<bool(Reference<IDataDistributionTeam>)> func) {
		return !any([func](Reference<IDataDistributionTeam> team) { return !func(team); });
	}

	virtual vector<StorageServerInterface> getLastKnownServerInterfaces() {
		return collect<StorageServerInterface>(
		    [](Reference<IDataDistributionTeam> team) { return team->getLastKnownServerInterfaces(); });
	}
	...
	virtual int64_t getDataInFlightToTeam() {
		return sum([](Reference<IDataDistributionTeam> team) { return team->getDataInFlightToTeam(); });
	}
	...
	virtual bool hasHealthyFreeSpace() {
		return all([](Reference<IDataDistributionTeam> team) { return team->hasHealthyFreeSpace(); });
	}
	...
	virtual bool isOptimal() {
		return all([](Reference<IDataDistributionTeam> team) { return team->isOptimal(); });
	}

	virtual bool isWrongConfiguration() {
		return any([](Reference<IDataDistributionTeam> team) { return team->isWrongConfiguration(); });
	}
	...
	virtual bool isHealthy() {
		return all([](Reference<IDataDistributionTeam> team) { return team->isHealthy(); });
	}
...
struct Busyness {
	...
	void addWork(int prio, int work) {
		ASSERT(prio > 0 && prio < 1000);
		for (int i = 0; i <= (prio / 100); i++) ledger[i] += work;
	}
	...
	std::string toString() {
		...
		for (int i = 1; i < ledger.size();) {
			int j = i + 1;
			while (j < ledger.size() && ledger[i] == ledger[j]) j++;
			if (i != 1) result += ", ";
...
struct DDQueueData {
	...
	DDQueueData(MasterInterface mi, MoveKeysLock lock, Database cx,
	            std::vector<TeamCollectionInterface> teamCollections, Reference<ShardsAffectedByTeamFailure> sABTF,
	            PromiseStream<Promise<int64_t>> getAverageShardBytes, int teamSize, PromiseStream<RelocateShard> output,
	            FutureStream<RelocateShard> input, PromiseStream<GetMetricsRequest> getShardMetrics,
	  ...
	  : activeRelocations(0), queuedRelocations(0), bytesWritten(0), teamCollections(teamCollections),
	    shardsAffectedByTeamFailure(sABTF), getAverageShardBytes(getAverageShardBytes), mi(mi), lock(lock), cx(cx),
	    teamSize(teamSize), output(output), input(input), getShardMetrics(getShardMetrics),
	    ...
	    recoveryVersion(recoveryVersion), suppressIntervals(0), lastInterval(0), unhealthyRelocations(0),
	...
	ACTOR Future<Void> getSourceServersForRange(Database cx, MasterInterface mi, RelocateData input,
		...
		loop {
			...
			try {
				Standalone<RangeResultRef> keyServersEntries = wait(tr.getRange(
				    lastLessOrEqual(keyServersKey(input.keys.begin)),
		...
		for (auto r = ranges.begin(); r != ranges.end(); ++r) {
			...
			if (rd.keys.contains(rrs.keys)) {
				if (foundActiveFetching)
					...
					for (int i = 1; i < rrs.src.size(); i++) queue[rrs.src[i]].erase(rrs);
				}
			}
					...
					if (serverQueue.erase(rrs) > 0) {
						if (!foundActiveRelocation) {
							...
							/*TraceEvent(newData.interval.begin(), mi.id());
							    .detail("KeyBegin", printable(newData.keys.begin)).detail("KeyEnd",
							   printable(newData.keys.end)) .detail("Priority",
							   newData.priority).detail("WantsNewServers", newData.wantsNewServers);*/
							queuedRelocations++;
	...
	void launchQueuedWork(KeyRange keys) {
		...
		for (auto it = f.begin(); it != f.end(); ++it) {
			if (it->value().src.size() && queue[it->value().src[0]].count(it->value())) combined.insert(it->value());
		}
...
ACTOR Future<Void> dataDistributionRelocator(DDQueueData* self, RelocateData rd) {
	...
	try {
		...
		loop {
			...
			loop {
				...
				while (tciIndex < self->teamCollections.size()) {
					...
					if (rd.priority >= PRIORITY_TEAM_UNHEALTHY)
					...
					auto req = GetTeamRequest(rd.wantsNewServers, rd.priority == PRIORITY_REBALANCE_UNDERUTILIZED_TEAM,
					                          true, inflightPenalty);
					req.sources = rd.src;
					req.completeSources = rd.completeSources;
					Optional<Reference<IDataDistributionTeam>> bestTeam =
			...
			state Future<Void> pollHealth =
			    signalledTransferComplete ? Never() : delay(SERVER_KNOBS->HEALTH_POLL_TIME, TaskDataDistributionLaunch);
			try {
				loop {
					choose {
						when(wait(doMoveKeys)) {
							if (extraIds.size()) {
								...
								extraIds.clear();
								doMoveKeys = moveKeys(self->cx, rd.keys, destIds, healthyIds, self->lock,
								                      Promise<Void>(), &self->startMoveKeysParallelismLock,
								                      &self->finishMoveKeysParallelismLock, self->recoveryVersion,
								                      self->teamCollections.size() > 1, relocateShardInterval.pairID);
							} else {
						...
						when(wait(pollHealth)) {
							...
							pollHealth = signalledTransferComplete
							                 ? Never()
							                 : delay(SERVER_KNOBS->HEALTH_POLL_TIME, TaskDataDistributionLaunch);
						}
				...
				if (!error.code()) {
					try {
						wait(healthyDestinations
						         .updatePhysicalMetrics()); // prevent a gap between the polling for an increase in
						                                    // physical metrics and decrementing data in flight
					} catch (Error& e) {
						error = e;
					}
				}
		...
		if (!signalledTransferComplete) dataTransferComplete.send(rd);
...
ACTOR Future<bool> rebalanceTeams(DDQueueData* self, int priority, Reference<IDataDistributionTeam> sourceTeam,
                                  Reference<IDataDistributionTeam> destTeam, bool primary) {
	...
	if (sourceBytes - destBytes <= 3 * std::max<int64_t>(SERVER_KNOBS->MIN_SHARD_BYTES, metrics.bytes) ||
	    metrics.bytes == 0)
		return false;
	...
	for (int i = 0; i < shards.size(); i++) {
		if (moveShard == shards[i]) {
			TraceEvent(priority == PRIORITY_REBALANCE_OVERUTILIZED_TEAM ? "BgDDMountainChopper" : "BgDDValleyFiller",
			           self->mi.id())
			...
			if (randomTeam.present()) {
				if (randomTeam.get()->getMinFreeSpaceRatio() > SERVER_KNOBS->FREE_SPACE_RATIO_DD_CUTOFF) {
					...
					if (loadedTeam.present()) {
						bool moved = wait(rebalanceTeams(self, PRIORITY_REBALANCE_OVERUTILIZED_TEAM, loadedTeam.get(),
			...
			if (randomTeam.present()) {
				...
				if (unloadedTeam.present()) {
					if (unloadedTeam.get()->getMinFreeSpaceRatio() > SERVER_KNOBS->FREE_SPACE_RATIO_DD_CUTOFF) {
						bool moved = wait(rebalanceTeams(self, PRIORITY_REBALANCE_UNDERUTILIZED_TEAM, randomTeam.get(),
...
ACTOR Future<Void> dataDistributionQueue(Database cx, PromiseStream<RelocateShard> output,
                                         ...
                                         MasterInterface mi, int teamSize, double* lastLimited,
                                         Version recoveryVersion) {
	state DDQueueData self(mi, lock, cx, teamCollections, shardsAffectedByTeamFailure, getAverageShardBytes, teamSize,
	                       output, input, getShardMetrics, lastLimited, recoveryVersion);
			...
			choose {
				...
				when(wait(recordMetrics)) {
					...
					for (auto it = self.priority_relocations.begin(); it != self.priority_relocations.end(); ++it) {
						if (it->second) highestPriorityRelocation = std::max(highestPriorityRelocation, it->first);
						if (it->first < 200)
