	...
	struct Writer : IThreadPoolReceiver {
		...
		void action(CommitAction& a) {
			...
			if (!s.ok()) {
				...
				deleteCompactRangeHistogram->sampleSeconds((useTimerMonotonic ? timer_monotonic() : now()) -
	...
	struct Reader : IThreadPoolReceiver {
		...
		void action(ReadRangeAction& a) {
			...
			if (a.rowLimit >= 0) {
				...
				readRangeNewIteratorHistogram->sampleSeconds((useTimerMonotonic ? timer_monotonic() : now()) -
				                                             iterCreationBeginTime);
