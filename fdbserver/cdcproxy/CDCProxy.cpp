...
class CDCProxy {
	friend class CDCProxyPrefixTests;
	...
	// Synchronous unit fixtures do not open a database or start background actors.
	CDCProxy()
...
class CDCReplayTestCursor final : public IReplayPeekCursor, public ReferenceCounted<CDCReplayTestCursor> {
public:
	struct Message {
		Version version;
...
private:
	struct Data : ReferenceCounted<Data> {
		std::vector<Message> messages;
		Version endVersion;
		int clones = 0;
		int reads = 0;
		Data(std::vector<Message> messages, Version endVersion)
		  : messages(std::move(messages)), endVersion(endVersion) {}
	};
	Reference<Data> data;
	size_t position;
	LogMessageVersion messageVersion;
	ProtocolVersion protocol;
	std::unique_ptr<ArenaReader> messageReader;

	void resetReader() {
		messageReader.reset();
		messageVersion = LogMessageVersion(hasMessage() ? data->messages[position].version : data->endVersion,
		                                   static_cast<uint32_t>(position));
	}

public:
	CDCReplayTestCursor(std::vector<Message> messages, Version endVersion)
	  : data(makeReference<Data>(std::move(messages), endVersion)), position(0),
	    protocol(g_network->protocolVersion()) {
		resetReader();
	}
	CDCReplayTestCursor(Reference<Data> data, size_t position, ProtocolVersion protocol)
	  : data(data), position(position), protocol(protocol) {
		resetReader();
	}
	template <class T>
	static Message message(Version version, T const& value) {
		return { version, BinaryWriter::toValue(value, AssumeVersion(g_network->protocolVersion())) };
	}
	int cloneCount() const { return data->clones; }
	int readCount() const { return data->reads; }
	void addref() override { ReferenceCounted<CDCReplayTestCursor>::addref(); }
	void delref() override { ReferenceCounted<CDCReplayTestCursor>::delref(); }
	void setProtocolVersion(ProtocolVersion version) override {
		protocol = version;
		if (messageReader) {
			messageReader->setProtocolVersion(version);
		}
	}
	bool hasMessage() const override { return position < data->messages.size(); }
	VectorRef<Tag> getTags() const override { return {}; }
	Arena& arena() override {
		ASSERT(hasMessage());
		return data->messages[position].bytes.arena();
	}
	ArenaReader* reader() override {
		...
		if (!messageReader) {
			messageReader =
			    std::make_unique<ArenaReader>(arena(), data->messages[position].bytes, AssumeVersion(protocol));
		}
	...
	bool isExhausted() const override { return !hasMessage(); }
	LogMessageVersion const& version() const override { return messageVersion; }
	Version popped() const override { return invalidVersion; }
	Version getMinKnownCommittedVersion() const override { return data->endVersion - 1; }
	int64_t getMaxRetainedReplyCount() const override { return 1; }
	void setReplyByteLimit(int limitBytes) override { ASSERT(false); }
	Optional<UID> getPrimaryPeekLocation() const override { return {}; }
	Optional<UID> getCurrentPeekLocation() const override { return {}; }
	Version getMaxKnownVersion() const override { return data->endVersion - 1; }
	Reference<IReplayPeekCursor> cloneNoMore() override {
		++data->clones;
		return makeReference<CDCReplayTestCursor>(data, position, protocol);
	}
	void advanceTo(LogMessageVersion target) override {
		while (hasMessage() && version() < target) {
			nextMessage();
		}
	}
};

class CDCProxyPrefixTests {
	using Bytes = std::unordered_map<CDCStreamId, int64_t>;
	using Cursor = CDCReplayTestCursor;
	struct Result {
		CDCCommittedPrefix prefix;
		CDCBufferSelection selection;
	};

	static Reference<CDCBufferedStream> addStream(CDCProxy& proxy,
	                                              Reference<CDCBufferedTag> tag,
	                                              CDCStreamId id,
	                                              KeyRangeRef keys = KeyRangeRef("a"_sr, "z"_sr)) {
		auto stream = makeReference<CDCBufferedStream>(id);
		stream->keys = KeyRange(keys);
		stream->initialized = true;
		stream->readDemand = 1;
		stream->minVersion = 100;
		stream->tagIntervals.emplace_back(tag->tag, 100, 1000);
		proxy.streams.emplace(id, stream);
		proxy.tags.emplace(tag->tag, tag);
		tag->streamIds.insert(id);
		return stream;
	}

	// Independent accounting from the removed second traversal, using the same production decoder/filter.
	static Bytes originalEstimate(CDCProxy& proxy,
	                              Reference<CDCBufferedTag> tag,
	                              Reference<IReplayPeekCursor> cursor,
	                              Version through) {
		std::unordered_map<CDCStreamId, Version> lastMutationVersion;
		Bytes estimatedBytes;
		proxy.visitBufferedMutations(
		    tag,
		    cursor,
		    through,
		    nullptr,
	...
	static Result compare(CDCProxy& proxy,
	                      Reference<CDCBufferedTag> tag,
	                      Reference<Cursor> cursor,
	                      Version committedThrough,
	                      int64_t preferred,
	                      int64_t hard,
	                      Version expectedThrough,
	                      int discoveryReads,
	                      int estimateReads) {
		const auto originalVersion = cursor->version();
		auto prefix = proxy.committedBufferedPrefix(tag, cursor, 100, committedThrough, preferred);
		ASSERT_EQ(prefix.bufferedThrough(), expectedThrough);
		ASSERT_EQ(cursor->cloneCount(), 1);
		ASSERT_EQ(cursor->readCount(), discoveryReads);
		ASSERT(cursor->version() == originalVersion);
		auto estimateCursor = cursor->cloneNoMore();
		estimateCursor->setProtocolVersion(g_network->protocolVersion());
		const auto oldEstimate = originalEstimate(proxy, tag, estimateCursor, prefix.bufferedThrough());
		ASSERT(prefix.estimatedBytes() == oldEstimate);
		ASSERT_EQ(cursor->cloneCount(), 2);
		ASSERT_EQ(cursor->readCount(), discoveryReads + estimateReads);
		const auto expected = selectBufferCandidates(
		    proxy.getBufferCandidates(tag, prefix.bufferedThrough(), oldEstimate), preferred, hard);
		const auto selected = proxy.selectBufferCandidatesForTag(tag, prefix, preferred, hard);
		ASSERT(selected.selectedStreamIds == expected.selectedStreamIds);
		ASSERT(selected.oversizedStreamIds == expected.oversizedStreamIds);
		ASSERT_EQ(selected.selectedBytes, expected.selectedBytes);
		ASSERT_EQ(cursor->cloneCount(), 2);
		ASSERT_EQ(cursor->readCount(), discoveryReads + estimateReads);
		ASSERT(cursor->version() == originalVersion);
		return { std::move(prefix), selected };
	}

public:
	static void filtering() {
		CDCProxy proxy;
		auto tag = makeReference<CDCBufferedTag>(Tag(tagLocalityCDC, 1));
		auto first = addStream(proxy, tag, 1, KeyRangeRef("a"_sr, "m"_sr));
		// A partial buffered version already owns its version header, but not these raw mutations.
		first->mutations.emplace_back();
		first->mutations.back().version = 100;
		auto second = addStream(proxy, tag, 2, KeyRangeRef("d"_sr, "z"_sr));
		second->tagIntervals.front().bufferedThrough = 100;
		addStream(proxy, tag, 3)->active = false;
		addStream(proxy, tag, 4)->readDemand = 0;
		addStream(proxy, tag, 5)->keys.reset();
		tag->streamIds.insert(6);
		addStream(proxy, tag, 7)->tagIntervals.front().tag = Tag(tagLocalityCDC, 2);
		addStream(proxy, tag, 8)->tagIntervals.front().end = 100;
		addStream(proxy, tag, 9, KeyRangeRef("za"_sr, "zz"_sr));
		const MutationRef set(MutationRef::SetValue, "d"_sr, "one"_sr);
		const MutationRef clear(MutationRef::ClearRange, "0"_sr, "q"_sr);
		const MutationRef later(MutationRef::SetValue, "f"_sr, "two"_sr);
		auto cursor = makeReference<Cursor>(
		    std::vector<Cursor::Message>{
		        Cursor::message(100, LogProtocolMessage()),
		        Cursor::message(100, set),
		        Cursor::message(100, clear),
		        Cursor::message(100, MutationRef(MutationRef::SetValue, "z"_sr, "excluded"_sr)),
		        Cursor::message(102, later),
		        Cursor::message(200, set) },
		    201);
		const auto result = compare(proxy, tag, cursor, 105, 10000, 10000, 105, 5, 5);
		const int64_t setBytes = set.expectedSize() + sizeof(MutationRef);
		const int64_t clearBytes =
		    MutationRef(MutationRef::ClearRange, "a"_sr, "m"_sr).expectedSize() + sizeof(MutationRef);
		const int64_t laterBytes = later.expectedSize() + sizeof(MutationRef);
		ASSERT((result.prefix.estimatedBytes() ==
		        Bytes{ { 1, setBytes + clearBytes + laterBytes + sizeof(VersionedMutationsRef) },
		               { 2, laterBytes + sizeof(VersionedMutationsRef) } }));
		ASSERT((result.selection.selectedStreamIds == std::unordered_set<CDCStreamId>{ 1, 2, 5, 9 }));
		ASSERT(result.selection.oversizedStreamIds.empty());
		const auto batches = proxy.bufferMessages(tag, cursor, 105, result.selection.selectedStreamIds);
		ASSERT_EQ(batches.size(), 2);
		ASSERT_EQ(batches.at(1).bufferedBytes, result.prefix.estimatedBytes().at(1));
		ASSERT_EQ(batches.at(2).bufferedBytes, result.prefix.estimatedBytes().at(2));
		ASSERT_EQ(batches.at(1).mutations[0].mutations[1].param1, "a"_sr);
		ASSERT_EQ(batches.at(1).mutations[0].mutations[1].param2, "m"_sr);
		ASSERT_EQ(cursor->version().version, 200);
	}

	static void limits() {
		const MutationRef mutation(MutationRef::SetValue, "d"_sr, "value"_sr);
		const int64_t bytes = sizeof(VersionedMutationsRef) + mutation.expectedSize() + sizeof(MutationRef);
		for (const int mode : { 0, 1, 2 }) {
			CDCProxy proxy;
			auto tag = makeReference<CDCBufferedTag>(Tag(tagLocalityCDC, 1));
			auto stream = addStream(proxy, tag, 1);
			auto cursor = makeReference<Cursor>(
			    std::vector<Cursor::Message>{ Cursor::message(100, mutation), Cursor::message(102, mutation) }, 103);
			const int64_t preferred = mode == 0 ? bytes : bytes - 1;
			const int64_t hard = mode == 2 ? bytes - 1 : bytes * 2;
			const auto result = compare(proxy, tag, cursor, 102, preferred, hard, 101, 2, 1);
			ASSERT((result.prefix.estimatedBytes() == Bytes{ { 1, bytes } }));
			ASSERT_EQ(result.selection.selectedBytes, mode == 2 ? 0 : bytes);
			ASSERT_EQ(result.selection.selectedStreamIds.contains(1), mode != 2);
			ASSERT_EQ(result.selection.oversizedStreamIds.size(), mode == 2 ? 1 : 0);
			ASSERT_EQ(stream->bufferLimitExceeded, mode == 2);
		}
		for (const bool matching : { false, true }) {
			CDCProxy proxy;
			auto tag = makeReference<CDCBufferedTag>(Tag(tagLocalityCDC, 1));
			addStream(proxy, tag, 1, matching ? KeyRangeRef("a"_sr, "z"_sr) : KeyRangeRef("x"_sr, "z"_sr));
			std::vector<Cursor::Message> messages;
			for (int i = 0; i <= CDC_PROXY_BUFFERED_PREFIX_VERSIONS; ++i) {
				messages.push_back(Cursor::message(100 + i, mutation));
			}
			auto cursor = makeReference<Cursor>(std::move(messages), 166);
			const auto result = compare(proxy, tag, cursor, 165, 100000, 100000, 163, 65, 64);
			ASSERT_EQ(result.selection.selectedBytes, matching ? 64 * bytes : 0);
			ASSERT_EQ(result.prefix.estimatedBytes().size(), matching ? 1 : 0);
		}
		CDCProxy proxy;
		auto tag = makeReference<CDCBufferedTag>(Tag(tagLocalityCDC, 1));
		addStream(proxy, tag, 1);
		auto empty = makeReference<Cursor>(std::vector<Cursor::Message>{}, 120);
		const auto result = compare(proxy, tag, empty, 110, 1000, 1000, 110, 0, 0);
		ASSERT(result.prefix.estimatedBytes().empty());
		ASSERT(result.selection.selectedStreamIds.contains(1));
	}

	static void refreshedMaterialization() {
		CDCProxy proxy;
		auto tag = makeReference<CDCBufferedTag>(Tag(tagLocalityCDC, 1));
		auto acknowledged = addStream(proxy, tag, 1);
		auto removed = addStream(proxy, tag, 2);
		addStream(proxy, tag, 3);
		const MutationRef mutation(MutationRef::SetValue, "d"_sr, "value"_sr);
		const int64_t bytes = sizeof(VersionedMutationsRef) + mutation.expectedSize() + sizeof(MutationRef);
		auto cursor = makeReference<Cursor>(std::vector<Cursor::Message>{ Cursor::message(100, mutation),
		                                                                  Cursor::message(101, mutation),
		                                                                  Cursor::message(102, mutation) },
		                                    103);
		CDCBufferSelection selection;
		Version through;
		{
			const auto result = compare(proxy, tag, cursor, 102, 10000, 10000, 102, 3, 3);
			selection = result.selection;
			through = result.prefix.bufferedThrough();
			ASSERT_EQ(selection.selectedBytes, 9 * bytes);
		}
		// These are the state changes an ACK/removal may make while the pass awaits capacity.
		proxy.reconcileStreamMinVersion(acknowledged, 102);
		proxy.deactivateStream(removed);
		proxy.streams.erase(3);
		const auto batches = proxy.bufferMessages(tag, cursor, through, selection.selectedStreamIds);
		ASSERT_EQ(batches.size(), 1);
		ASSERT_EQ(batches.at(1).bufferedBytes, bytes);
		ASSERT_LT(batches.at(1).bufferedBytes, selection.selectedBytes);
		ASSERT_EQ(batches.at(1).mutations.size(), 1);
		ASSERT_EQ(batches.at(1).mutations.front().version, 102);
		cursor.clear();
		ASSERT_EQ(batches.at(1).mutations.front().mutations.front().param1, "d"_sr);
		ASSERT_EQ(batches.at(1).mutations.front().mutations.front().param2, "value"_sr);
	}
};

} // namespace

TEST_CASE("/NativeCDC/ProxyPrefixEstimateReuse/Filtering") {
	CDCProxyPrefixTests::filtering();
	return Void();
}

TEST_CASE("/NativeCDC/ProxyPrefixEstimateReuse/Limits") {
	CDCProxyPrefixTests::limits();
	return Void();
}

TEST_CASE("/NativeCDC/ProxyPrefixEstimateReuse/RefreshedMaterialization") {
	CDCProxyPrefixTests::refreshedMaterialization();
	return Void();
}
