...
class DDTxnProcessorImpl {
		...
		loop {
			...
			try {
				...
				for (i = 0; i < shards.size() - 1; ++i) {
					...
					std::vector<ValueResult> serverListValues = wait(getAll(serverListEntries));
	...
	ACTOR static Future<int> tryUpdateReplicasKeyForDc(Database cx, Optional<Key> dcId, int storageTeamSize) {
		...
		loop {
			...
			try {
				ValueResult val = wait(tr.get(datacenterReplicasKeyFor(dcId)));
	...
	ACTOR static Future<Reference<InitialDataDistribution>> getInitialDataDistribution(
		...
		loop {
			...
			try {
				...
				result->mode = 1;
				ValueResult mode = wait(tr.get(dataDistributionModeKey));
				if (mode.present()) {
	...
	ACTOR static Future<bool> isDataDistributionEnabled(Database cx, const DDEnabledState* ddEnabledState) {
		...
		loop {
			...
			try {
				...
				ValueResult readVal = wait(tr.get(moveKeysLockOwnerKey));
				UID currentOwner =
	...
	ACTOR static Future<ValueResult> readRebalanceDDIgnoreKey(Database cx) {
		state Transaction tr(cx);
		loop {
			try {
				tr.setOption(FDBTransactionOptions::READ_LOCK_AWARE);
				...
				ValueResult res = wait(tr.get(rebalanceDDIgnoreKey));
				return res;
			} catch (Error& e) {
				wait(tr.onError(e));
			}
...
Future<ValueResult> DDTxnProcessor::readRebalanceDDIgnoreKey() const {
