...
class DDTxnProcessorImpl {
	...
	static Future<bool> rewriteShardEncodedMetadata(Transaction& tr,
		...
		// pattern natural DD moves use in MoveKeys.actor.cpp. Correctness
	...
	ACTOR static Future<Reference<InitialDataDistribution>> getInitialDataDistribution(
		...
		// the head each time.
		state Key phase2Cursor = keyServersPrefix;
		...
		loop {
			...
			try {
				...
				if (!targetIsNewFormat) {
					...
					if (migrationEnabled) {
						bool rewrote = wait(
						    rewriteShardEncodedMetadata(tr, distributorId, moveKeysLock, ddEnabledState, phase2Cursor));
						if (rewrote) {
					...
					bool cleared = wait(clearStaleShardEncodedRewriteSentinel(tr, distributorId));
					if (cleared) {
