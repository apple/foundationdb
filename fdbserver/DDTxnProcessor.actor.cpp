	...
	ACTOR static Future<Reference<InitialDataDistribution>> getInitialDataDistribution(
		...
		while (beginKey < allKeys.end) {
			...
			loop {
				...
				try {
					tr.setOption(FDBTransactionOptions::PRIORITY_SYSTEM_IMMEDIATE);
					wait(checkMoveKeysLockReadOnly(&tr, moveKeysLock, ddEnabledState.get()));
					state RangeResult UIDtoTagMap = wait(tr.getRange(serverTagKeys, CLIENT_KNOBS->TOO_MANY));
					ASSERT(!UIDtoTagMap.more && UIDtoTagMap.size() < CLIENT_KNOBS->TOO_MANY);
...
Future<Void> DDTxnProcessor::updateReplicaKeys(std::vector<Optional<Key>>* primaryIds,
                                               std::vector<Optional<Key>>* remoteIds,
                                               DatabaseConfiguration* configuration) const {
	ASSERT(primaryIds && remoteIds && configuration);
