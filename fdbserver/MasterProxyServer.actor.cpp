...
struct ProxyStats {
	...
	explicit ProxyStats(UID id, Version* pVersion, NotifiedVersion* pCommittedVersion,
	  ...
	  : cc("ProxyStats", id.toString()), txnStartIn("TxnStartIn", cc), txnStartOut("TxnStartOut", cc),
	    ...
	    mutations("Mutations", cc), conflictRanges("ConflictRanges", cc), lastCommitVersionAssigned(0) {
...
int getBytes(Promise<Version> const& r) {
	return 0;
}
...
ACTOR Future<Void> queueTransactionStartRequests(
    ...
    double* GRVBatchTime, FutureStream<double> replyTimes, ProxyStats* stats) {
	...
	loop choose {
		when(GetReadVersionRequest req = waitNext(readVersionRequests)) {
			...
			if (transactionQueue->empty()) {
				if (now() - *lastGRVTime > *GRVBatchTime) *lastGRVTime = now() - *GRVBatchTime;
...
struct ProxyCommitData {
	...
	ProxyCommitData(UID dbgid, MasterInterface master, RequestStream<GetReadVersionRequest> getConsistentReadVersion,
	  ...
	  : dbgid(dbgid), stats(dbgid, &version, &committedVersion, &commitBatchesMemBytesCount), master(master),
	    logAdapter(NULL), txnStateStore(NULL), committedVersion(recoveryTransactionVersion), version(0),
	    minKnownCommittedVersion(0), lastVersionTime(0), commitVersionRequestNumber(1),
	    mostRecentProcessedRequestNumber(0), getConsistentReadVersion(getConsistentReadVersion), commit(commit),
	    lastCoalesceTime(0), localCommitBatchesStarted(0), locked(false), firstProxy(firstProxy),
...
struct ResolutionRequestBuilder {
	...
	void addTransaction(CommitTransactionRef& trIn, int transactionNumberInBatch) {
		...
		for (auto& r : trIn.read_conflict_ranges) {
			...
			for (auto& ir : ranges) {
				...
				for (int i = version_resolver.size() - 1; i >= 0; i--) {
					...
					if (version_resolver[i].first < trIn.read_snapshot) break;
				}
			}
		...
		for (auto& r : trIn.write_conflict_ranges) {
			auto ranges = self->keyResolvers.intersectingRanges(r);
			std::set<int> resolvers;
			for (auto& ir : ranges) resolvers.insert(ir.value().back().second);
			ASSERT(resolvers.size());
			for (int resolver : resolvers)
				getOutTransaction(resolver, trIn.read_snapshot)
		...
		for (int r = 0; r < outTr.size(); r++)
			if (outTr[r]) resolversUsed.push_back(r);
		transactionResolverMap.push_back(std::move(resolversUsed));
	}
};

ACTOR Future<Void> commitBatch(ProxyCommitData* self, vector<CommitTransactionRequest> trs, double* commitBatchTime,
	...
	for (int r = 0; r < self->resolvers.size(); r++) {
		...
		replies.push_back(
		    brokenPromiseToNever(self->resolvers[r].resolve.getReply(requests.requests[r], TaskProxyResolverReply)));
	}
	...
	for (int r = 0; r < resolution.size(); r++) ASSERT(nextTr[r] == resolution[r].committed.size());
	...
	for (t = 0; t < trs.size() && !forceRecovery; t++) {
		if (committed[t] == ConflictBatch::TransactionCommitted && (!locked || trs[t].isLockAware())) {
			...
			applyMetadataMutations(self->dbgid, arena, trs[t].transaction.mutations, self->txnStateStore, &toCommit,
			                       ...
			                       &self->keyInfo, self->firstProxy ? &self->uid_applyMutationsData : NULL,
	...
	if (forceRecovery) {
		for (; t < trs.size(); t++) committed[t] = ConflictBatch::TransactionConflict;
		...
		if (committed[t] == ConflictBatch::TransactionCommitted && (!locked || trs[t].isLockAware())) {
			...
			for (auto m : trs[t].transaction.mutations) {
				...
				if (isSingleKeyMutation((MutationRef::Type)m.type)) {
					...
					for (auto& tag : tags) toCommit.addTag(tag);
					...
					if (firstRange == ranges.end()) {
						...
						if (debugMutation("ProxyCommit", commitVersion, m))
							TraceEvent("ProxyCommitTo", self->dbgid)
							    ...
							    .detail("Mutation", m.toString())
							    .detail("Version", commitVersion);
						for (auto& tag : allSources) toCommit.addTag(tag);
					}
					toCommit.addTypedMessage(m);
				} else
					UNREACHABLE();
					...
					if (isAtomicOp((MutationRef::Type)m.type)) {
						...
						case MutationRef::Type::ClearRange: {
							...
							for (auto backupRange : self->vecBackupKeys.intersectingRanges(mutationRange)) {
								...
								MutationRef backupMutation(MutationRef::Type::ClearRange, intersectionRange.begin,
								                           intersectionRange.end);

								// Add the mutation to the relevant backup tag
								for (auto backupName : backupRange.value()) {
									logRangeMutations[backupName].push_back_deep(logRangeMutationsArena,
									                                             backupMutation);
								}
							}
						} break;
	...
	while (self->committedVersion.get() < commitVersion - SERVER_KNOBS->MAX_READ_TRANSACTION_LIFE_VERSIONS) {
		...
		choose {
			when(wait(
			    self->committedVersion.whenAtLeast(commitVersion - SERVER_KNOBS->MAX_READ_TRANSACTION_LIFE_VERSIONS))) {
				wait(yield());
				break;
			}
			when(GetReadVersionReply v = wait(self->getConsistentReadVersion.getReply(
			         GetReadVersionRequest(0, GetReadVersionRequest::PRIORITY_SYSTEM_IMMEDIATE |
	...
	Future<Version> loggingComplete = self->logSystem->push(prevVersion, commitVersion, self->committedVersion.get(),
	                                                        self->minKnownCommittedVersion, toCommit, debugID);
	...
	if (now() - self->lastCoalesceTime > SERVER_KNOBS->RESOLVER_COALESCE_TIME) {
		...
		for (auto r = rs.begin(); r != rs.end(); ++r) {
			while (r->value().size() > 1 && r->value()[1].first < oldestVersion) r->value().pop_front();
			if (r->value().size() && r->value().front().first < oldestVersion) r->value().front().first = 0;
		}
	    ...
	    std::max(SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_INTERVAL_MIN,
	             std::min(SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_INTERVAL_MAX,
	                      target_latency * SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_INTERVAL_SMOOTHER_ALPHA +
	                          *commitBatchTime * (1 - SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_INTERVAL_SMOOTHER_ALPHA)));
...
ACTOR Future<GetReadVersionReply> getLiveCommittedVersion(ProxyCommitData* commitData, uint32_t flags,
                                                          vector<MasterProxyInterface>* otherProxies,
                                                          Optional<UID> debugID, int transactionCount,
                                                          int systemTransactionCount, int defaultPriTransactionCount,
	...
	for (auto const& p : *otherProxies)
		proxyVersions.push_back(brokenPromiseToNever(
		    p.getRawCommittedVersion.getReply(GetRawCommittedVersionRequest(debugID), TaskTLogConfirmRunningReply)));
	...
	if (debugID.present())
		g_traceBatch.addEvent("TransactionDebug", debugID.get().first(),
		                      "MasterProxyServer.getLiveCommittedVersion.confirmEpochLive");
	...
	if (debugID.present())
		g_traceBatch.addEvent("TransactionDebug", debugID.get().first(),
		                      "MasterProxyServer.getLiveCommittedVersion.After");
...
ACTOR Future<Void> fetchVersions(ProxyCommitData* commitData) {
	loop {
		...
		GetCommitVersionRequest req(commitData->commitVersionRequestNumber++,
		                            commitData->mostRecentProcessedRequestNumber, commitData->dbgid);
		commitData->commitBatchVersions.send(brokenPromiseToNever(commitData->master.getCommitVersion.getReply(req)));
	}
}

ACTOR static Future<Void> transactionStarter(MasterProxyInterface proxy, MasterInterface master,
                                             Reference<AsyncVar<ServerDBInfo>> db, PromiseStream<Future<Void>> addActor,
	...
	state std::priority_queue<std::pair<GetReadVersionRequest, int64_t>,
	                          std::vector<std::pair<GetReadVersionRequest, int64_t>>>
	    transactionQueue;
	...
	addActor.send(queueTransactionStartRequests(&transactionQueue, proxy.getConsistentReadVersion.getFuture(), GRVTimer,
	                                            &lastGRVTime, &GRVBatchTime, replyTimes.getFuture(),
	                                            &commitData->stats));
	...
	for (MasterProxyInterface mp : db->get().client.proxies) {
		if (mp != proxy) otherProxies.push_back(mp);
	}
	...
	loop {
		...
		double nTransactionsToStart =
		    std::min(transactionRate * elapsed, SERVER_KNOBS->START_TRANSACTION_MAX_TRANSACTIONS_TO_START) +
		    transactionBudget;
		...
		vector<vector<ReplyPromise<GetReadVersionReply>>> start(
		...
		while (!transactionQueue.empty()) {
			...
			bool startNext = tc < leftToStart || req.priority() >= GetReadVersionRequest::PRIORITY_SYSTEM_IMMEDIATE ||
			                 tc * g_random->random01() < leftToStart - std::max(0.0, transactionBudget);
			if (!startNext) break;
			...
			start[req.flags & 1].push_back(std::move(req.reply));
		...
		if (!transactionQueue.empty())
			forwardPromise(
			    GRVTimer, delayJittered(SERVER_KNOBS->START_TRANSACTION_BATCH_QUEUE_CHECK_INTERVAL, TaskProxyGRVTimer));
		...
		if (start[0].size()) {
			start[0].push_back(
			    GRVReply); // for now, base dynamic batching on the time for normal requests (not read_risky)
			addActor.send(timeReply(GRVReply.getFuture(), replyTimes));
		}

		transactionCount += transactionsStarted[0] + transactionsStarted[1];
		transactionBudget = std::max(std::min(nTransactionsToStart - transactionsStarted[0] - transactionsStarted[1],
		                                      SERVER_KNOBS->START_TRANSACTION_MAX_BUDGET_SIZE),
		                             -SERVER_KNOBS->START_TRANSACTION_MAX_BUDGET_SIZE);
		if (debugID.present())
			g_traceBatch.addEvent("TransactionDebug", debugID.get().first(),
			                      "MasterProxyServer.masterProxyServerCore.Broadcast");
		for (int i = 0; i < start.size(); i++) {
			if (start[i].size()) {
				addActor.send(
				    broadcast(getLiveCommittedVersion(commitData, i, &otherProxies, debugID, transactionsStarted[i],
				                                      systemTransactionsStarted[i], defaultPriTransactionsStarted[i],
		...
		choose {
			...
			when(GetStorageServerRejoinInfoRequest req = waitNext(proxy.getStorageServerRejoinInfo.getFuture())) {
				if (commitData->txnStateStore->readValue(serverListKeyFor(req.id)).get().present()) {
					...
					rep.tag =
					    decodeServerTagValue(commitData->txnStateStore->readValue(serverTagKeyFor(req.id)).get().get());
					Standalone<VectorRef<KeyValueRef>> history =
					    commitData->txnStateStore->readRange(serverTagHistoryRangeFor(req.id)).get();
					for (int i = history.size() - 1; i >= 0; i--) {
						rep.history.push_back(std::make_pair(decodeServerTagHistoryKey(history[i].key),
						                                     decodeServerTagValue(history[i].value)));
					}
...
ACTOR Future<Void> masterProxyServerCore(MasterProxyInterface proxy, MasterInterface master,
	...
	for (auto r = rs.begin(); r != rs.end(); ++r) r->value().push_back(std::make_pair<Version, int>(0, 0));
	    ...
	    (int)std::min<double>(SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_BYTES_MAX,
	                          std::max<double>(SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_BYTES_MIN,
	                                           SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_BYTES_SCALE_BASE *
	                                               pow(db->get().client.proxies.size(),
	                                                   SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_BYTES_SCALE_POWER)));
	commitBatcher = batcher(batchedCommits, proxy.commit.getFuture(),
	                        SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_INTERVAL_FROM_IDLE, &commitBatchInterval,
	                        SERVER_KNOBS->MAX_COMMIT_BATCH_INTERVAL, SERVER_KNOBS->COMMIT_TRANSACTION_BATCH_COUNT_MAX,
	                        commitBatchByteLimit, CLIENT_KNOBS->TRANSACTION_SIZE_LIMIT,
	                        commitData.commitBatchStartNotifications, &commitData.commitBatchesMemBytesCount,
	                        commitBatchesMemoryLimit, TaskProxyCommitBatcher, &commitData.stats.txnCommitIn);
	loop choose {
		when(wait(dbInfoChange)) {
			dbInfoChange = db->onChange();
			if (db->get().master.id() == master.id() &&
		...
		when(std::pair<vector<CommitTransactionRequest>, int> batchedRequests = waitNext(batchedCommits.getFuture())) {
			...
			if (trs.size() || (db->get().recoveryState >= RecoveryState::ACCEPTING_COMMITS &&
		...
		when(TxnStateRequest req = waitNext(proxy.txnState.getFuture())) {
			...
			if (!txnSequences.count(req.sequence)) {
				...
				for (auto& kv : req.data) commitData.txnStateStore->set(kv, &req.arena);
					...
					loop {
						...
						Standalone<VectorRef<KeyValueRef>> data =
						    commitData.txnStateStore
						        ->readRange(txnKeys, SERVER_KNOBS->BUGGIFIED_ROW_LIMIT,
								...
								if (k != allKeys.end) {
									...
									for (auto& id : src) {
										...
										if (cacheItr == commitData.storageCache.end()) {
											...
											storageInfo->tag = decodeServerTagValue(
											    commitData.txnStateStore->readValue(serverTagKeyFor(id)).get().get());
											storageInfo->interf = decodeServerListValue(
											    commitData.txnStateStore->readValue(serverListKeyFor(id)).get().get());
						...
						Arena arena;
						bool confChanges;
						applyMetadataMutations(commitData.dbgid, arena, mutations, commitData.txnStateStore, NULL,
						                       &confChanges, Reference<ILogSystem>(), 0, &commitData.vecBackupKeys,
						                       &commitData.keyInfo,
						                       commitData.firstProxy ? &commitData.uid_applyMutationsData : NULL,
...
ACTOR Future<Void> masterProxyServer(MasterProxyInterface proxy, InitializeMasterProxyRequest req,
	...
	try {
		state Future<Void> core = masterProxyServerCore(proxy, req.master, db, req.recoveryCount,
		                                                req.recoveryTransactionVersion, req.firstProxy);
		loop choose {
		...
		if (e.code() == error_code_actor_cancelled || e.code() == error_code_worker_removed ||
		    e.code() == error_code_tlog_stopped || e.code() == error_code_master_tlog_failed ||
