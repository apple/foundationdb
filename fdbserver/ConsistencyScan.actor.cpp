	...
	for (; i < ranges.size(); i++) {
		...
		loop {
			try {
				...
				state std::vector<ValueResult> serverListValues = wait(getAll(serverListEntries));
						...
						if (data.size()) {
							...
							loop {
								try {
									...
									state ValueResult val = wait(ConsistencyScanInfo::getInfo(csInfoTr));
...
ACTOR Future<Void> watchConsistencyScanInfoKey(ConsistencyScanData* self) {
	...
	loop {
		try {
			...
			state ValueResult val = wait(ConsistencyScanInfo::getInfo(tr));
			if (val.present()) {
				ConsistencyScanInfo consistencyScanInfo =
