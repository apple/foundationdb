...
public:
	...
	//     4. respects preferred sharing order, just a map from worker -> rank.
	std::vector<WorkerDetails> getWorkersForRoleInDatacenter(
		...
		for (const auto& [worker_process_id, worker_info] : id_worker) {
			...
			//TraceEvent("Recruit_CandidateWorker")
			//	.detail("Role", to_string(role))
			//	.detail("WorkerClass", worker_details.processClass.toString())
			//	.detail("ProcessID", worker_process_id)
			//	.detail("Fitness", fitness)
			//	.detail("Used", id_used[worker_process_id])
			//	.detail("Sharing", sharing != preferredSharing.end() ? sharing->second : 1e6);
		...
		for (auto& it : fitness_workers) {
			...
			for (int i = 0; i < it.second.size(); i++) {
				//TraceEvent("Pick_CandidateWorker")
				//	.detail("Role", to_string(role))
				//	.detail("WorkerClass", it.second[i].processClass.toString())
				//	.detail("ProcessID", it.second[i].interf.locality.processId())
				//	.detail("Fitness", std::get<0>(it.first))
				//	.detail("Used", std::get<1>(it.first));
	...
	ErrorOr<RecruitFromConfigurationReply> findWorkersForConfigurationFromDC(RecruitFromConfigurationRequest const& req,
		...
		// Version indexer is optional, but if we do use it, we must ensure that all version indexers
		// have the same fitness, and the way this code accomplishes that is by recruiting the first
		// one separately, then excluding lower fitness processes for the others.
		Optional<WorkerFitnessInfo> first_version_indexer;
		if (req.configuration.getDesiredVersionIndexers() > 0) {
			first_version_indexer = getWorkerForRoleInDatacenter(dcId,
			                                                     ProcessClass::VersionIndexer,
			                                                     ProcessClass::ExcludeFit,
			                                                     req.configuration,
			                                                     id_used,
			                                                     preferredSharing);
			preferredSharing[first_version_indexer.get().worker.interf.locality.processId()] = 3;
		}
		...
		if (first_version_indexer.present()) {
			maxUsed = std::max(maxUsed, first_version_indexer.get().used);
			first_version_indexer.get().used = maxUsed;
		}
	...
	void checkRegions(const std::vector<RegionInfo>& regions) {
		...
		try {
			...
			// XXX(dadkins): version indexer is an optional part of the transaction system.
			// getWorkerForRoleInDatacenter(
	...
	bool betterMasterExists() {
		...
		// one separately, then excluding lower fitness processes for the others.
		Optional<WorkerFitnessInfo> first_version_indexer;
		if (db.config.getDesiredVersionIndexers() > 0) {
			first_version_indexer = getWorkerForRoleInDatacenter(clusterControllerDcId,
			                                                     ProcessClass::VersionIndexer,
			                                                     ProcessClass::ExcludeFit,
			                                                     db.config,
			                                                     id_used,
