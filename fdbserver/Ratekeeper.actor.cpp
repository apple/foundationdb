...
struct Ratekeeper {
	...
	Ratekeeper()
	  : smoothReleasedTransactions(SERVER_KNOBS->SMOOTHING_AMOUNT),
	    smoothTotalDurableBytes(SERVER_KNOBS->SLOW_SMOOTHING_AMOUNT), TPSLimit(std::numeric_limits<double>::infinity()),
	    tpsLimitMetric(LiteralStringRef("Ratekeeper.TPSLimit")),
	    actualTpsMetric(LiteralStringRef("Ratekeeper.ActualTPS")), reasonMetric(LiteralStringRef("Ratekeeper.Reason")),
	    lastWarning(0) {}
};
...
ACTOR Future<Void> trackEachStorageServer(
    Ratekeeper* self, FutureStream<std::pair<UID, Optional<StorageServerInterface>>> serverChanges) {
...
void updateRate(Ratekeeper* self) {
	...
	for (auto i = self->storageQueueInfo.begin(); i != self->storageQueueInfo.end(); ++i) {
		...
		int64_t minFreeSpace = std::max(SERVER_KNOBS->MIN_FREE_SPACE, (int64_t)(SERVER_KNOBS->MIN_FREE_SPACE_RATIO *
		                                                                        ss.smoothTotalSpace.smoothTotal()));

		worstFreeSpaceStorageServer =
		    std::min(worstFreeSpaceStorageServer, (int64_t)ss.smoothFreeSpace.smoothTotal() - minFreeSpace);

		int64_t springBytes = std::max<int64_t>(1, std::min(SERVER_KNOBS->SPRING_BYTES_STORAGE_SERVER,
		                                                    (ss.smoothFreeSpace.smoothTotal() - minFreeSpace) * 0.2));
		int64_t targetBytes = std::max<int64_t>(1, std::min(SERVER_KNOBS->TARGET_BYTES_PER_STORAGE_SERVER,
		                                                    (int64_t)ss.smoothFreeSpace.smoothTotal() - minFreeSpace));
		...
		if (limitTPS < self->TPSLimit && (ss.limitReason == limitReason_t::storage_server_min_free_space ||
		                                  ss.limitReason == limitReason_t::storage_server_min_free_space_ratio)) {
			reasonID = ss.id;
	...
	for (auto ss = storageTPSLimitReverseIndex.begin();
	     ss != storageTPSLimitReverseIndex.end() && ss->first < self->TPSLimit; ++ss) {
...

	{
		...
		writeToReadLatencyLimit = ((maxTLVer - minLimitingSSVer) - SERVER_KNOBS->MAX_TL_SS_VERSION_DIFFERENCE / 2) /
		                          (SERVER_KNOBS->MAX_TL_SS_VERSION_DIFFERENCE / 4);
	...
	for (auto i = self->tlogQueueInfo.begin(); i != self->tlogQueueInfo.end(); ++i) {
		...
		int64_t minFreeSpace = std::max(SERVER_KNOBS->MIN_FREE_SPACE, (int64_t)(SERVER_KNOBS->MIN_FREE_SPACE_RATIO *
		                                                                        tl.smoothTotalSpace.smoothTotal()));
		...
		int64_t springBytes = std::max<int64_t>(
		    1, std::min(SERVER_KNOBS->SPRING_BYTES_TLOG, (tl.smoothFreeSpace.smoothTotal() - minFreeSpace) * 0.2));
		int64_t targetBytes = std::max<int64_t>(
		    1, std::min(SERVER_KNOBS->TARGET_BYTES_PER_TLOG, (int64_t)tl.smoothFreeSpace.smoothTotal() - minFreeSpace));
...
ACTOR Future<Void> rateKeeper(Reference<AsyncVar<ServerDBInfo>> dbInfo,
                              PromiseStream<std::pair<UID, Optional<StorageServerInterface>>> serverChanges,
                              FutureStream<struct GetRateInfoRequest> getRateInfo, Standalone<StringRef> dbName,
                              DatabaseConfiguration configuration, double* lastLimited) {
	...
	TraceEvent("RkStorageServerQueueSizeParameters")
	...
	loop {
		choose {
			...
			when(wait(timeout)) {
				...
				for (auto p = self.proxy_transactionCountAndTime.begin();
				     p != self.proxy_transactionCountAndTime.end();) {
			...
			when(GetRateInfoRequest req = waitNext(getRateInfo)) {
				...
				if (p.first > 0) self.smoothReleasedTransactions.addDelta(req.totalReleasedTransactions - p.first);
