...
ACTOR static Future<ERestoreState> waitFastRestore(Database cx, Key tagName, bool verbose) {
	...
	return ERestoreState::COMPLETED;
...
ACTOR Future<Version> fastRestore(Database cx, Key tagName, Key url, bool waitForComplete, Version targetVersion, bool verbose, KeyRange range, Key addPrefix, Key removePrefix) {
	...
	if(waitForComplete) {
		ERestoreState finalState = wait(waitFastRestore(cx, tagName, verbose));
