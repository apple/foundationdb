...
struct TCServerInfo : public ReferenceCounted<TCServerInfo> {
	...
	TCServerInfo(StorageServerInterface ssi, ProcessClass processClass)
...
ACTOR Future<Void> updateServerMetrics(TCServerInfo* server) {
	...
	state Future<ErrorOr<GetPhysicalMetricsReply>> metricsRequest =
	    ssi.getPhysicalMetrics.tryGetReply(GetPhysicalMetricsRequest(), TaskDataDistributionLaunch);
		...
		choose {
			...
			when(wait(resetRequest)) { // To prevent a tight spin loop
				if (IFailureMonitor::failureMonitor().getState(ssi.getPhysicalMetrics.getEndpoint()).isFailed()) {
					resetRequest = IFailureMonitor::failureMonitor().onStateEqual(ssi.getPhysicalMetrics.getEndpoint(),
					                                                              FailureStatus(false));
				} else {
					resetRequest = Never();
					metricsRequest =
...
public:
	...
	TCTeamInfo(vector<Reference<TCServerInfo>> const& servers)
	  : servers(servers), healthy(true), priority(PRIORITY_TEAM_HEALTHY), wrongConfiguration(false) {
		...
		for (int i = 0; i < servers.size(); i++) serverIDs.push_back(servers[i]->id);
	}
	virtual vector<StorageServerInterface> getLastKnownServerInterfaces() {
		vector<StorageServerInterface> v;
		v.reserve(servers.size());
		for (int i = 0; i < servers.size(); i++) v.push_back(servers[i]->lastKnownInterface);
		return v;
	}
	...
	virtual int64_t getLoadBytes(bool includeInFlight = true, double inflightPenalty = 1.0) {
		...
		double freeSpaceMultiplier =
		    SERVER_KNOBS->FREE_SPACE_RATIO_CUTOFF /
		    (std::max(std::min(SERVER_KNOBS->FREE_SPACE_RATIO_CUTOFF, minFreeSpaceRatio), 0.000001));
	...
	virtual bool hasHealthyFreeSpace() {
		return getMinFreeSpaceRatio() > SERVER_KNOBS->MIN_FREE_SPACE_RATIO &&
		       getMinFreeSpace() > SERVER_KNOBS->MIN_FREE_SPACE;
	}

	virtual Future<Void> updatePhysicalMetrics() { return doUpdatePhysicalMetrics(this); }
...
struct ServerStatus {
	...
	bool operator==(ServerStatus const& r) const {
		return isFailed == r.isFailed && isUndesired == r.isUndesired &&
		       isWrongConfiguration == r.isWrongConfiguration && locality.zoneId() == r.locality.zoneId();
	}
...
ACTOR Future<Void> storageServerFailureTracker(Database cx, StorageServerInterface server, ServerStatusMap* statusMap,
                                               ServerStatus* status, Debouncer* restartRecruiting,
                                               int64_t* unhealthyServers, UID masterId, Version addedVersion) {
	loop {
		...
		if (status->isFailed) restartRecruiting->trigger();

		state double startTime = now();
		choose {
			when(wait(status->isFailed
			              ? IFailureMonitor::failureMonitor().onStateEqual(server.waitFailure.getEndpoint(),
			                                                               FailureStatus(false))
			              : waitFailureClient(server.waitFailure, SERVER_KNOBS->DATA_DISTRIBUTION_FAILURE_REACTION_TIME,
			                                  0, TaskDataDistribution))) {
				...
				TraceEvent("StatusMapChange", masterId)
				    .detail("ServerID", server.id())
				    .detail("Status", status->toString())
				    .detail("Available",
				            IFailureMonitor::failureMonitor().getState(server.waitFailure.getEndpoint()).isAvailable());
			}
			when(wait(status->isUnhealthy() ? waitForAllDataRemoved(cx, server.id(), addedVersion) : Never())) {
				break;
			}
		}
	}
...
ACTOR Future<Reference<InitialDataDistribution>> getInitialDataDistribution(Database cx, UID masterId,
	...
	while (beginKey < allKeys.end) {
		...
		loop {
			...
			try {
				...
				Standalone<RangeResultRef> keyServers =
				    wait(krmGetRanges(&tr, keyServersPrefix, KeyRangeRef(beginKey, allKeys.end),
...
Future<Void> storageServerTracker(
    struct DDTeamCollection* const& self, Database const& cx, TCServerInfo* const& server,
    ServerStatusMap* const& statusMap, MoveKeysLock const& lock, UID const& masterId,
    ...
    Promise<Void> const& errorOut, Version const& addedVersion);
...
struct DDTeamCollection {
	...
	AsyncMap<AddressExclusion, bool> excludedServers; // true if an address is in the excluded list in the database.
	                                                  // Updated asynchronously (eventually)
	...
	ACTOR Future<Void> getTeam(DDTeamCollection* self, GetTeamRequest req) {
		try {
			...
			if (!req.wantsNewServers) {
				...
				for (int i = 0; i < req.sources.size(); i++) sources.insert(req.sources[i]);
				...
				for (int i = 0; i < req.sources.size(); i++) {
					if (!self->server_info.count(req.sources[i])) {
						...
						for (int j = 0; j < teamList.size(); j++) {
							if (teamList[j]->isHealthy() &&
							    (!req.preferLowerUtilization || teamList[j]->hasHealthyFreeSpace())) {
								int sharedMembers = 0;
								for (int k = 0; k < teamList[j]->serverIDs.size(); k++)
									if (sources.count(teamList[j]->serverIDs[k])) sharedMembers++;
								...
								if ((sharedMembers == teamList[j]->serverIDs.size()) ||
								    (!foundExact && req.wantsTrueBest)) {
									int64_t loadBytes =
									    SOME_SHARED * teamList[j]->getLoadBytes(true, req.inflightPenalty);
			...
			if (req.wantsTrueBest) {
				...
				while (randomTeams.size() < SERVER_KNOBS->BEST_TEAM_OPTION_COUNT &&
					...
					for (int i = 0; ok && i < randomTeams.size(); i++)
						if (randomTeams[i].second->getServerIDs() == dest->getServerIDs()) ok = false;
				...
				for (int i = 0; i < randomTeams.size(); i++) {
					int64_t loadBytes =
					    randomTeams[i].first * randomTeams[i].second->getLoadBytes(true, req.inflightPenalty);
					if (!bestOption.present() || (req.preferLowerUtilization && loadBytes < bestLoadBytes) ||
					    (!req.preferLowerUtilization && loadBytes > bestLoadBytes)) {
						bestLoadBytes = loadBytes;
	...
	void addSubsetOfEmergencyTeams() {
		for (int i = 0; i < badTeams.size(); i++) {
			...
			for (int j = 0; j < std::max(1, (int)(serverIds.size() - configuration.storageTeamSize + 1)) && !foundTeam;
			     j++) {
	...
	void evaluateTeamQuality() {
		...
		TraceEvent(minTeams > 0 ? SevInfo : SevWarn, "DataDistributionTeamQuality", masterId)
	...
	void addTeam(std::set<UID> const& team) { addTeam(team.begin(), team.end()); }
	...
	ACTOR Future<Void> addAllTeams(DDTeamCollection* self, int location, vector<LocalityEntry>* history,
		...
		for (; location < processes->size(); location++) {
			...
			history->pop_back();
			if (*addedTeams > teamLimit) break;
		}

		return Void();
	}

	ACTOR Future<int> addAllTeams(DDTeamCollection* self, vector<UID> input, vector<std::vector<UID>>* output,
	                              int teamLimit) {
	...
	int addTeamsBestOf(int teamsToBuild) {
		...
		if (totalServers.size() < configuration.storageTeamSize) {
			TraceEvent(SevWarn, "DataDistributionBuildTeams", masterId)
			    .detail("Reason", "Not enough servers for a team")
			    .detail("Servers", totalServers.size())
	...
	ACTOR Future<Void> buildTeams(DDTeamCollection* self) {
		...
		if (uniqueMachines >= self->configuration.storageTeamSize) {
			...
			// Count only properly sized teams against the desired number of teams. This is to prevent "emergency"
			// merged teams (see MoveKeys)
			//  from overwhelming the team count (since we really did not want that team in the first place). These
			//  larger teams will not be returned from getRandomTeam() (as used by bestTeam to find a new home for a
			//  shard).
			// Also exclude teams who have members in the wrong configuration, since we don't want these teams either
			int teamCount = 0;
			int totalTeamCount = 0;
			for (int i = 0; i < self->teams.size(); i++) {
				if (self->teams[i]->getServerIDs().size() == self->configuration.storageTeamSize &&
				    !self->teams[i]->isWrongConfiguration()) {
					if (self->teams[i]->isHealthy()) {
						teamCount++;
					}
					totalTeamCount++;
			...
			TraceEvent("BuildTeamsBegin", self->masterId)
			...
			if (desiredTeams > teamCount) {
				...
				for (auto i = self->server_info.begin(); i != self->server_info.end(); ++i)
					if (!self->server_status.get(i->first).isUnhealthy()) desiredServerSet.insert(i->second->id);
				...
				if (self->configuration.storageTeamSize > 3) {
					...
					TraceEvent("AddTeamsBestOf", self->masterId)
					...
					if (addedTeams < teamsToBuild) {
						...
						TraceEvent("AddAllTeams", self->masterId)
						    .detail("CurrentTeams", self->teams.size())
						    .detail("AddedTeams", builtTeams.size());
					} else {
						int addedTeams = self->addTeamsBestOf(teamsToBuild);
						TraceEvent("AddTeamsBestOf", self->masterId)
						    .detail("CurrentTeams", self->teams.size())
	...
	void addServer(StorageServerInterface newServer, ProcessClass processClass, Promise<Void> errorOut,
		...
		TraceEvent("AddedStorageServer", masterId)
		    .detail("ServerID", newServer.id())
		    .detail("ProcessClass", processClass.toString())
		    .detail("WaitFailureToken", newServer.waitFailure.getEndpoint().token)
		    .detail("Address", newServer.waitFailure.getEndpoint().address);
		auto& r = server_info[newServer.id()] = Reference<TCServerInfo>(new TCServerInfo(newServer, processClass));
		r->tracker = storageServerTracker(this, cx, r.getPtr(), &server_status, lock, masterId, &server_info,
		                                  serverChanges, errorOut, addedVersion);
...
ACTOR Future<Void> teamTracker(DDTeamCollection* self, Reference<IDataDistributionTeam> team) {
	...
	wait(yield());
	TraceEvent("TeamTrackerStarting", self->masterId)
	...
	try {
		loop {
			...
			for (auto s = servers.begin(); s != servers.end(); ++s) {
				...
				if (!status.isFailed) teamLocality->add(status.locality);
				if (status.isUndesired) anyUndesired = true;
				if (status.isWrongConfiguration) anyWrongConfiguration = true;
			}
			...
			bool recheck =
			    ...
			    (!matchesPolicy || anyUndesired || team->getServerIDs().size() != self->configuration.storageTeamSize);
			lastReady = self->initialFailureReactionDelay.isReady();
			lastZeroHealthy = self->zeroHealthyTeams->get();

			if (serversLeft != lastServersLeft || anyUndesired != lastAnyUndesired ||
			    anyWrongConfiguration != lastWrongConfiguration || wrongSize || recheck) {
				...
				bool healthy = matchesPolicy && !anyUndesired &&
				               team->getServerIDs().size() == self->configuration.storageTeamSize &&
				               team->getServerIDs().size() == serversLeft;
				...
				if (self->initialFailureReactionDelay.isReady() && !self->zeroHealthyTeams->get()) {
					...
					for (int i = 0; i < shards.size(); i++) {
						...
						if (maxPriority < PRIORITY_TEAM_0_LEFT) {
							...
							for (int t = 0; t < teams.size(); t++) {
								if (teams[t].servers.size() && self->server_info.count(teams[t].servers[0])) {
									...
									TEST(teams[t].servers.size()); // A removed server is still associated with a team
									                               // in SABTF
								}
							}
						}
						...
						if (maxPriority == team->getPriority() || lastPriority > maxPriority) {
							...
							TraceEvent("RelocationNotSentToDDQ", self->masterId).detail("Team", team->getDesc());
						}
					}
				} else {
...
ACTOR Future<Void> trackExcludedServers(DDTeamCollection* self, Database cx) {
	loop {
		...
		loop {
			try {
				state Future<Standalone<RangeResultRef>> fresults =
				...
				for (auto r = results.begin(); r != results.end(); ++r) {
					...
					if (addr.isValid()) excluded.insert(addr);
				}

				TraceEvent("DDExcludedServersChanged", self->masterId)
				    .detail("Rows", results.size())
				    .detail("Exclusions", excluded.size());

				// Reset and reassign self->excludedServers based on excluded, but weonly
				// want to trigger entries that are different
				auto old = self->excludedServers.getKeys();
				for (auto& o : old)
					if (!excluded.count(o)) self->excludedServers.set(o, false);
				for (auto& n : excluded) self->excludedServers.set(n, true);
		...
		loop {
			try {
				...
				if (nchid != lastChangeID) break;

				wait(delay(SERVER_KNOBS->SERVER_LIST_DELAY,
				           TaskDataDistribution)); // FIXME: make this tr.watch( excludedServersVersionKey ) instead
				tr = Transaction(cx);
			} catch (Error& e) {
				wait(tr.onError(e));
			}
...
ACTOR Future<vector<std::pair<StorageServerInterface, ProcessClass>>> getServerListAndProcessClasses(Transaction* tr) {
	...
	std::map<Optional<Standalone<StringRef>>, ProcessData> id_data;
	for (int i = 0; i < workers.get().size(); i++) id_data[workers.get()[i].locality.processId()] = workers.get()[i];
...
ACTOR Future<Void> serverMetricsPolling(TCServerInfo* server) {
	...
	loop {
		...
		wait(delayUntil(lastUpdate + SERVER_KNOBS->STORAGE_METRICS_POLLING_DELAY +
		                    SERVER_KNOBS->STORAGE_METRICS_RANDOM_DELAY * g_random->random01(),
		                TaskDataDistributionLaunch));
		lastUpdate = now();
	}
}
...
ACTOR Future<KeyValueStoreType> keyValueStoreTypeTracker(DDTeamCollection* self, TCServerInfo* server) {
	state KeyValueStoreType type = wait(brokenPromiseToNever(
	    server->lastKnownInterface.getKeyValueStoreType.getReplyWithTaskID<KeyValueStoreType>(TaskDataDistribution)));
	if (type == self->configuration.storageServerStoreType &&
	    (self->includedDCs.empty() || std::find(self->includedDCs.begin(), self->includedDCs.end(),
...
ACTOR Future<Void> storageServerTracker(
    ...
    ServerStatusMap* statusMap, MoveKeysLock lock, UID masterId, std::map<UID, Reference<TCServerInfo>>* other_servers,
    Optional<PromiseStream<std::pair<UID, Optional<StorageServerInterface>>>> changes, Promise<Void> errorOut,
    Version addedVersion) {
	...
	try {
		loop {
			...
			failureTracker =
			    storageServerFailureTracker(cx, server->lastKnownInterface, statusMap, &status,
			                                &self->restartRecruiting, &self->unhealthyServers, masterId, addedVersion);

			// We need to recruit new storage servers if the key value store type has changed
			if (hasWrongStoreTypeOrDC) self->restartRecruiting.trigger();

			if (lastIsUnhealthy && !status.isUnhealthy() && !server->teams.size()) self->doBuildTeams = true;
			...
			choose {
				...
				when(std::pair<StorageServerInterface, ProcessClass> newInterface = wait(interfaceChanged)) {
					bool restartRecruiting = newInterface.first.waitFailure.getEndpoint().address !=
					                         server->lastKnownInterface.waitFailure.getEndpoint().address;
					TraceEvent("StorageServerInterfaceChanged", masterId)
					    .detail("ServerID", server->id)
		...
		if (e.code() != error_code_actor_cancelled) errorOut.sendError(e);
		throw;
	}
}
	...
	loop {
		if (!recruiting) {
			...
			TraceEvent("StorageServerRecruitment", self->masterId)
			    ...
			    .trackLatest(
			        (self->cx->dbName.toString() + "/StorageServerRecruitment_" + self->masterId.toString()).c_str());
			recruiting = true;
		} else {
			loop {
				choose {
					...
					when(wait(self->recruitingStream.get() == 0
					              ? delay(SERVER_KNOBS->RECRUITMENT_IDLE_DELAY, TaskDataDistribution)
					              : Future<Void>(Never()))) {
						break;
					}
				}
...
ACTOR Future<Void> initializeStorage(DDTeamCollection* self, RecruitStorageReply candidateWorker) {
	...
	ErrorOr<InitializeStorageReply> newServer =
	    wait(candidateWorker.worker.storage.tryGetReply(isr, TaskDataDistribution));
	...
	loop {
		try {
			...
			if (!fCandidateWorker.isValid() || fCandidateWorker.isReady() ||
				...
				lastRequest = rsr;
				fCandidateWorker =
				    brokenPromiseToNever(db->get().clusterInterface.recruitStorage.getReply(rsr, TaskDataDistribution));
			}
...
ACTOR Future<Void> updateReplicasKey(DDTeamCollection* self, Optional<Key> dcId) {
	...
	loop {
		while (self->zeroHealthyTeams->get() || self->processingUnhealthy->get()) {
			TraceEvent("DDUpdatingStalled", self->masterId)
			    .detail("DcId", printable(dcId))
			    .detail("ZeroHealthy", self->zeroHealthyTeams->get())
			    .detail("ProcessingUnhealthy", self->processingUnhealthy->get());
			wait(self->zeroHealthyTeams->onChange() || self->processingUnhealthy->onChange());
		}
		wait(delay(FLOW_KNOBS->PREVENT_FAST_SPIN_DELAY,
		           TaskLowPriority)); // After the team trackers wait on the initial failure reaction delay, they yield.
		                              // We want to make sure every tracker has had the opportunity to send their
		                              // relocations to the queue.
		if (!self->zeroHealthyTeams->get() && !self->processingUnhealthy->get()) {
			break;
		}
	}
	TraceEvent("DDUpdatingReplicas", self->masterId)
	    .detail("DcId", printable(dcId))
	...
	loop {
		try {
			...
			wait(tr.commit());
			TraceEvent("DDUpdatedReplicas", self->masterId)
			    .detail("DcId", printable(dcId))
			    .detail("Replicas", self->configuration.storageTeamSize);
			return Void();
		} catch (Error& e) {
...
ACTOR Future<Void> dataDistributionTeamCollection(
    Reference<InitialDataDistribution> initData, TeamCollectionInterface tci, Database cx,
    Reference<AsyncVar<struct ServerDBInfo>> db, Reference<ShardsAffectedByTeamFailure> shardsAffectedByTeamFailure,
    MoveKeysLock lock, PromiseStream<RelocateShard> output, UID masterId, DatabaseConfiguration configuration,
    std::vector<Optional<Key>> includedDCs, Optional<std::vector<Optional<Key>>> otherTrackedDCs,
    Optional<PromiseStream<std::pair<UID, Optional<StorageServerInterface>>>> serverChanges, Future<Void> readyToStart,
    Reference<AsyncVar<bool>> zeroHealthyTeams, bool primary, Reference<AsyncVar<bool>> processingUnhealthy) {
	state DDTeamCollection self(cx, masterId, lock, output, shardsAffectedByTeamFailure, configuration, includedDCs,
	                            otherTrackedDCs, serverChanges, readyToStart, zeroHealthyTeams, primary,
	                            processingUnhealthy);
	...
	try {
		...
		loop choose {
			...
			when(wait(loggingTrigger)) {
				...
				TraceEvent("TotalDataInFlight", masterId)
...
ACTOR Future<bool> isDataDistributionEnabled(Database cx) {
	...
	loop {
		try {
			...
			if (currentOwner != dataDistributionModeLock) return true;
			return false;
		} catch (Error& e) {
			wait(tr.onError(e));
		}
				...
				for (int j = 0; j < ranges.size() - 2; j++)
					if (ranges[j].value == ranges[j + 1].value)
						TraceEvent(SevError, "UncoalescedValues", id)
						    .detail("Key1", printable(ranges[j].key))
						    .detail("Key2", printable(ranges[j + 1].key))
						    .detail("Value", printable(ranges[j].value));
			}
...
ACTOR Future<Void> dataDistribution(Reference<AsyncVar<struct ServerDBInfo>> db, MasterInterface mi,
                                    DatabaseConfiguration configuration,
                                    PromiseStream<std::pair<UID, Optional<StorageServerInterface>>> serverChanges,
                                    Reference<ILogSystem> logSystem, Version recoveryCommitVersion,
                                    std::vector<Optional<Key>> primaryDcId, std::vector<Optional<Key>> remoteDcIds,
                                    double* lastLimited, Future<Void> remoteRecovered) {
	...
	loop {
		try {
			loop {
				...
				state Reference<InitialDataDistribution> initData = wait(getInitialDataDistribution(
				    cx, mi.id(), lock, configuration.usableRegions > 1 ? remoteDcIds : std::vector<Optional<Key>>()));
				...
				TraceEvent("TotalDataInFlight", mi.id())
				    .detail("Primary", true)
				    .detail("TotalBytes", 0)
			...
			actors.push_back(reportErrorsExcept(
			    dataDistributionTracker(initData, cx, output, shardsAffectedByTeamFailure, getShardMetrics,
			                            getAverageShardBytes.getFuture(), readyToStart, anyZeroHealthyTeams, mi.id()),
			    "DDTracker", mi.id(), &normalDDQueueErrors()));
			actors.push_back(reportErrorsExcept(
			    dataDistributionQueue(cx, output, input.getFuture(), getShardMetrics, processingUnhealthy, tcis,
			                          shardsAffectedByTeamFailure, lock, getAverageShardBytes, mi, storageTeamSize,
			                          lastLimited, recoveryCommitVersion),
			    "DDQueue", mi.id(), &normalDDQueueErrors()));
			actors.push_back(reportErrorsExcept(
			    dataDistributionTeamCollection(
			        initData, tcis[0], cx, db, shardsAffectedByTeamFailure, lock, output, mi.id(), configuration,
			        primaryDcId, configuration.usableRegions > 1 ? remoteDcIds : std::vector<Optional<Key>>(),
			        serverChanges, readyToStart.getFuture(), zeroHealthyTeams[0], true, processingUnhealthy),
			    "DDTeamCollectionPrimary", mi.id(), &normalDDQueueErrors()));
			if (configuration.usableRegions > 1) {
				actors.push_back(reportErrorsExcept(
				    dataDistributionTeamCollection(
				        initData, tcis[1], cx, db, shardsAffectedByTeamFailure, lock, output, mi.id(), configuration,
				        remoteDcIds, Optional<std::vector<Optional<Key>>>(),
				        Optional<PromiseStream<std::pair<UID, Optional<StorageServerInterface>>>>(),
				        readyToStart.getFuture() && remoteRecovered, zeroHealthyTeams[1], false, processingUnhealthy),
				    "DDTeamCollectionSecondary", mi.id(), &normalDDQueueErrors()));
			}
			...
			state Error err = e;
			if (e.code() != error_code_movekeys_conflict) throw err;
