...
struct TagPartitionedLogSystem : ILogSystem, ReferenceCounted<TagPartitionedLogSystem> {
	...
	std::map<std::pair<UID, Tag>, std::pair<Version, Version>>
	    outstandingPops; // For each currently running popFromLog actor, (log server #, tag)->popped version
	...
	TagPartitionedLogSystem(UID dbgid, LocalityData locality,
	  ...
	  : dbgid(dbgid), locality(locality), addActor(addActor), popActors(false),
	    recoveryCompleteWrittenToCoreState(false), remoteLogsWrittenToCoreState(false), logSystemType(0),
	    logRouterTags(0), expectedLogSets(0), hasRemoteServers(false), stopped(false), repopulateRegionAntiQuorum(0) {}

	virtual void stopRejoins() { rejoins = Future<Void>(); }

	virtual void addref() { ReferenceCounted<TagPartitionedLogSystem>::addref(); }

	virtual void delref() { ReferenceCounted<TagPartitionedLogSystem>::delref(); }
	...
	static Reference<ILogSystem> fromLogSystemConfig(UID const& dbgid, LocalityData const& locality,
	                                                 LogSystemConfig const& lsConf, bool excludeRemote,
	                                                 bool useRecoveredAt,
	                                                 Optional<PromiseStream<Future<Void>>> addActor) {
		...
		for (int i = 0; i < lsConf.tLogs.size(); i++) {
			...
			if (!excludeRemote || tLogSet.isLocal) {
				...
				for (auto& log : tLogSet.tLogs) {
					logSet->logServers.push_back(Reference<AsyncVar<OptionalInterface<TLogInterface>>>(
					    new AsyncVar<OptionalInterface<TLogInterface>>(log)));
				}
				for (auto& log : tLogSet.logRouters) {
					logSet->logRouters.push_back(Reference<AsyncVar<OptionalInterface<TLogInterface>>>(
					    new AsyncVar<OptionalInterface<TLogInterface>>(log)));
				}
	...
	static Reference<ILogSystem> fromOldLogSystemConfig(UID const& dbgid, LocalityData const& locality,
	...
	virtual void toCoreState(DBCoreState& newState) {
		if (recoveryComplete.isValid() && recoveryComplete.isError()) throw recoveryComplete.getError();
	...
	virtual Future<Void> onError() { return onError_internal(this); }
		...
		for (auto& it : tLogs) {
			if (it->isLocal && it->logServers.size()) {
				...
				for (int loc = 0; loc < it->logServers.size(); loc++) {
					allReplies.push_back(it->logServers[loc]->get().interf().commit.getReply(
					    TLogCommitRequest(data.getArena(), prevVersion, version, knownCommittedVersion,
					                      minKnownCommittedVersion, data.getMessages(location), debugID),
					    TaskTLogCommitReply));
		...
		for (auto& log : tLogs) {
			if (log->isLocal && log->logServers.size() &&
			    (log->locality == tagLocalitySpecial || log->locality == tagLocalityUpgraded ||
			     log->locality == tag.locality || tag == txsTag || tag.locality == tagLocalityLogRouter ||
			     (tag.locality == tagLocalityUpgraded && log->locality != tagLocalitySatellite))) {
		...
		if (begin >= lastBegin) {
			...
			if (lastBegin < end) {
				TraceEvent("TLogPeekAllAddingCurrent", dbgid)
				    ...
				    .detail("End", end)
				    .detail("BestLogs", localSets[bestSet]->logServerString());
				cursors.push_back(Reference<ILogSystem::SetPeekCursor>(
				    new ILogSystem::SetPeekCursor(localSets, bestSet, localSets[bestSet]->bestLocationFor(tag), tag,
				                                  lastBegin, end, parallelGetMore)));
			}
			int i = 0;
			while (begin < lastBegin) {
				...
				if (!localOldSets.size()) {
					TraceEvent("TLogPeekNoLocalSets", dbgid)
					    .detail("Tag", tag.toString())
					    .detail("Begin", begin)
					    .detail("End", end)
					    .detail("LastBegin", lastBegin);
				...
				if (thisBegin < lastBegin) {
					if (thisBegin < end) {
						...
						cursors.push_back(Reference<ILogSystem::SetPeekCursor>(new ILogSystem::SetPeekCursor(
	...
	Reference<IPeekCursor> peekRemote(UID dbgid, Version begin, Tag tag, bool parallelGetMore) {
		...
		if (begin >= lastBegin) {
			...
			return Reference<ILogSystem::MergedPeekCursor>(new ILogSystem::MergedPeekCursor(
			    tLogs[bestSet]->logRouters, -1, (int)tLogs[bestSet]->logRouters.size(), tag, begin, getPeekEnd(), false,
			    std::vector<LocalityData>(), IRepPolicyRef(), 0));
		} else {
			std::vector<Reference<ILogSystem::IPeekCursor>> cursors;
			std::vector<LogMessageVersion> epochEnds;
			TraceEvent("TLogPeekRemoteAddingBest", dbgid)
			    ...
			    .detail("BestSetStart", lastBegin)
			    .detail("LogRouterIds", tLogs[bestSet]->logRouterString());
			cursors.push_back(Reference<ILogSystem::MergedPeekCursor>(new ILogSystem::MergedPeekCursor(
			    tLogs[bestSet]->logRouters, -1, (int)tLogs[bestSet]->logRouters.size(), tag, lastBegin, getPeekEnd(),
			    false, std::vector<LocalityData>(), IRepPolicyRef(), 0)));
			int i = 0;
			while (begin < lastBegin) {
				if (i == oldLogData.size()) {
					TraceEvent("TLogPeekRemoteDead", dbgid)
					    ...
					    .detail("OldLogDataSize", oldLogData.size());
					return Reference<ILogSystem::ServerPeekCursor>(
					    new ILogSystem::ServerPeekCursor(Reference<AsyncVar<OptionalInterface<TLogInterface>>>(), tag,
				...
				if (thisBegin < lastBegin) {
					...
					cursors.push_back(Reference<ILogSystem::MergedPeekCursor>(new ILogSystem::MergedPeekCursor(
					    oldLogData[i].tLogs[bestOldSet]->logRouters, -1,
					    (int)oldLogData[i].tLogs[bestOldSet]->logRouters.size(), tag, thisBegin, lastBegin, false,
					    std::vector<LocalityData>(), IRepPolicyRef(), 0)));
	...
	virtual Reference<IPeekCursor> peek(UID dbgid, Version begin, Optional<Version> end, std::vector<Tag> tags,
		...
		if (tags.empty()) {
			...
			return Reference<ILogSystem::ServerPeekCursor>(
			    new ILogSystem::ServerPeekCursor(Reference<AsyncVar<OptionalInterface<TLogInterface>>>(), invalidTag,
			                                     begin, getPeekEnd(), false, false));
		}
		...
		return Reference<ILogSystem::BufferedCursor>(new ILogSystem::BufferedCursor(
		    cursors, begin, end.present() ? end.get() + 1 : getPeekEnd(), tLogs[0]->locality == tagLocalityUpgraded));
	}
		...
		for (int t = 0; t < tLogs.size(); t++) {
			if (tLogs[t]->logServers.size() &&
			    (tLogs[t]->locality == tagLocalitySpecial || tLogs[t]->locality == tagLocalityUpgraded ||
			     tLogs[t]->locality == tag.locality || tag.locality == tagLocalityUpgraded)) {
		...
		if (begin >= tLogs[bestSet]->startVersion) {
			...
			if (tLogs[bestSet]->startVersion < end) {
				TraceEvent("TLogPeekLocalAddingBest", dbgid)
				    ...
				    .detail("BestSetStart", tLogs[bestSet]->startVersion)
				    .detail("LogId", tLogs[bestSet]->logServers[tLogs[bestSet]->bestLocationFor(tag)]->get().id());
				cursors.push_back(Reference<ILogSystem::ServerPeekCursor>(
				    new ILogSystem::ServerPeekCursor(tLogs[bestSet]->logServers[tLogs[bestSet]->bestLocationFor(tag)],
				                                     tag, tLogs[bestSet]->startVersion, end, false, false)));
			}
			...
			while (begin < lastBegin) {
				...
				for (int t = 0; t < oldLogData[i].tLogs.size(); t++) {
					if (oldLogData[i].tLogs[t]->logServers.size() &&
					    (oldLogData[i].tLogs[t]->locality == tagLocalitySpecial ||
					     ...
					     oldLogData[i].tLogs[t]->locality == tag.locality || tag.locality == tagLocalityUpgraded)) {
				...
				if (foundSpecial) {
					TraceEvent("TLogPeekLocalFoundSpecial", dbgid)
					    .detail("Tag", tag.toString())
					    .detail("Begin", begin)
					    .detail("End", end);
				...
				if (bestOldSet == -1) {
					if (oldLogData[i].logRouterTags == 0) {
						TraceEvent("TLogPeekLocalNoLogRouterTags", dbgid)
						    .detail("Tag", tag.toString())
						    .detail("Begin", begin)
						    .detail("End", end)
						    .detail("LastBegin", lastBegin)
				...
				if (thisBegin < lastBegin) {
					if (thisBegin < end) {
						TraceEvent("TLogPeekLocalAddingOldBest", dbgid)
						    ...
						    .detail("ThisBegin", thisBegin)
						    .detail("LastBegin", lastBegin);
						cursors.push_back(Reference<ILogSystem::MergedPeekCursor>(
						    new ILogSystem::MergedPeekCursor(oldLogData[i].tLogs[bestOldSet]->logServers,
						                                     ...
						                                     tag, thisBegin, std::min(lastBegin, end), false,
	...
	virtual Reference<IPeekCursor> peekSingle(UID dbgid, Version begin, Tag tag, vector<pair<Version, Tag>> history) {
		...
		if (history.size() == 0) {
			...
			for (int i = 0; i < history.size(); i++) {
				cursors.push_back(peekLocal(dbgid, history[i].second,
	...
	void popLogRouter(Version upTo, Tag tag, Version durableKnownCommittedVersion,
				...
				if (t->locality == popLocality) {
					for (auto& log : t->logRouters) {
						...
						if (prev < upTo)
							outstandingPops[std::make_pair(log->get().id(), tag)] =
							    std::make_pair(upTo, durableKnownCommittedVersion);
						if (prev == 0) popActors.add(popFromLog(this, log, tag, 0.0));
					}
				}
			}
		}
			...
			if (t->locality == tagLocalitySpecial || t->locality == tag.locality || tag.locality < 0) {
				for (auto& log : t->logServers) {
					...
					if (prev < upTo)
						outstandingPops[std::make_pair(log->get().id(), tag)] =
						    std::make_pair(upTo, durableKnownCommittedVersion);
					if (prev == 0) popActors.add(popFromLog(this, log, tag, 1.0)); //< FIXME: knob
				}
			}
		}
	}
	...
	ACTOR static Future<Void> popFromLog(TagPartitionedLogSystem* self,
		...
		loop {
			...
			try {
				if (!log->get().present()) return Void();
		...
		for (auto& t : logSet->logServers) {
			if (t->get().present()) {
				alive.push_back(brokenPromiseToNever(t->get().interf().confirmRunning.getReply(
				    TLogConfirmRunningRequest(debugID), TaskTLogConfirmRunningReply)));
				numPresent++;
			} else {
	...
	virtual Future<Reference<ILogSystem>> newEpoch(RecruitFromConfigurationReply const& recr,
		...
		return newEpoch(Reference<TagPartitionedLogSystem>::addRef(this), recr, fRemoteWorkers, config, recoveryCount,
		                primaryLocality, remoteLocality, allTags, recruitmentStalled);
	}
		...
		for (auto& t : tLogs) {
			if (t->isLocal || remoteLogsWrittenToCoreState) {
				for (int i = 0; i < t->logServers.size(); i++) {
					logs.push_back(
					    std::make_pair(t->logServers[i]->get().id(), t->logServers[i]->get().present()
					                                                     ? t->logServers[i]->get().interf().address()
					                                                     : NetworkAddress()));
				}
			}
		}
			...
			for (int i = 0; i < oldLogData.size(); i++) {
				for (auto& t : oldLogData[i].tLogs) {
					for (int j = 0; j < t->logServers.size(); j++) {
						oldLogs.push_back(std::make_pair(t->logServers[j]->get().id(),
	...
	virtual bool hasRemoteLogs() { return logRouterTags > 0; }

	virtual Tag getRandomRouterTag() { return Tag(tagLocalityLogRouter, g_random->randomInt(0, logRouterTags)); }
	...
	Optional<std::pair<Version, Version>> static getDurableVersion(
	    UID dbgid, LogLockInfo lockInfo,
		...
		if (!bTooManyFailures && (logSet->tLogWriteAntiQuorum) &&
		    (!validateAllCombinations(badCombo, unResponsiveSet, logSet->tLogPolicy, availableItems,
		                              logSet->tLogWriteAntiQuorum, false))) {
			...
			if (!lastEnd.present() || ((safe_range_end > 0) && (safe_range_end - 1 < results.size()) &&
				...
				Version knownCommittedVersion =
				    ...
				    (g_network->isSimulated()
				         ? 10 * SERVER_KNOBS->VERSIONS_PER_SECOND
				         : SERVER_KNOBS->MAX_READ_TRANSACTION_LIFE_VERSIONS); // In simulation this must be the maximum
				...
				TraceEvent("GetDurableResult", dbgid)
				    ...
				    .detail("Present", results.size())
				    .detail("ServerState", sServerState)
				    .detail("RecoveryVersion", ((safe_range_end > 0) && (safe_range_end - 1 < results.size()))
	...
	ACTOR static Future<Void> epochEnd(Reference<AsyncVar<Reference<ILogSystem>>> outLogSystem, UID dbgid,
	                                   DBCoreState prevState, FutureStream<TLogRejoinRequest> rejoinRequests,
	                                   LocalityData locality, bool forceRecovery) {
		...
		for (int i = 0; i < prevState.tLogs.size(); i++) {
			...
			for (int j = 0; j < coreSet.tLogs.size(); j++) {
				Reference<AsyncVar<OptionalInterface<TLogInterface>>> logVar =
				    Reference<AsyncVar<OptionalInterface<TLogInterface>>>(
				        new AsyncVar<OptionalInterface<TLogInterface>>(
				            OptionalInterface<TLogInterface>(coreSet.tLogs[j])));
			...
			for (int j = 0; j < old.tLogs.size(); j++) {
				...
				for (int k = 0; k < log.tLogs.size(); k++) {
					Reference<AsyncVar<OptionalInterface<TLogInterface>>> logVar =
					    Reference<AsyncVar<OptionalInterface<TLogInterface>>>(
					        new AsyncVar<OptionalInterface<TLogInterface>>(
					            OptionalInterface<TLogInterface>(log.tLogs[k])));
					logSet->logServers.push_back(logVar);
					allLogServers.push_back(logVar);
				}
	...
	ACTOR static Future<Void> recruitOldLogRouters(TagPartitionedLogSystem* self, vector<WorkerInterface> workers,
	                                               LogEpoch recoveryCount, int8_t locality, Version startVersion,
	                                               std::vector<LocalityData> tLogLocalities, IRepPolicyRef tLogPolicy,
	                                               bool forRemote) {
			...
			for (auto& tLogs : self->tLogs) {
				if (tLogs->locality == locality) {
					for (int i = 0; i < logRouterInitializationReplies[nextReplies].size(); i++) {
						tLogs->logRouters.push_back(Reference<AsyncVar<OptionalInterface<TLogInterface>>>(
						    new AsyncVar<OptionalInterface<TLogInterface>>(OptionalInterface<TLogInterface>(
						        logRouterInitializationReplies[nextReplies][i].get()))));
						failed.push_back(waitFailureClient(
	...
	ACTOR static Future<Void> newRemoteEpoch(TagPartitionedLogSystem* self,
	                                         ...
	                                         DatabaseConfiguration configuration, LogEpoch recoveryCount,
	                                         int8_t remoteLocality, std::vector<Tag> allTags) {
		...
		if (logSet->startVersion < oldLogSystem->knownCommittedVersion + 1) {
			oldRouterRecruitment = TagPartitionedLogSystem::recruitOldLogRouters(
			    self, remoteWorkers.logRouters, recoveryCount, remoteLocality, logSet->startVersion, localities,
			    logSet->tLogPolicy, true);
		}
		...
		for (int i = 0; i < logSet->logServers.size(); i++)
			recoveryComplete.push_back(transformErrors(
			    throwErrorOr(logSet->logServers[i]->get().interf().recoveryFinished.getReplyUnlessFailedFor(
			        TLogRecoveryFinishedRequest(), SERVER_KNOBS->TLOG_TIMEOUT,
	...
	ACTOR static Future<Reference<ILogSystem>> newEpoch(Reference<TagPartitionedLogSystem> oldLogSystem,
		...
		state Reference<TagPartitionedLogSystem> logSystem(
		    new TagPartitionedLogSystem(oldLogSystem->getDebugID(), oldLogSystem->locality));
		...
		if (configuration.usableRegions > 1) {
			logSystem->logRouterTags = recr.tLogs.size() * std::max<int>(1, configuration.desiredLogRouterCount /
		...
		if (oldLogSystem->logRouterTags > 0 ||
		    logSystem->tLogs[0]->startVersion < oldLogSystem->knownCommittedVersion + 1) {
			oldRouterRecruitment = TagPartitionedLogSystem::recruitOldLogRouters(
			    oldLogSystem.getPtr(), recr.oldLogRouters, recoveryCount, primaryLocality,
			    logSystem->tLogs[0]->startVersion, localities, logSystem->tLogs[0]->tLogPolicy, false);
		...
		for (Tag tag : localTags) {
			...
			for (int loc : locations) reqs[loc].recoverTags.push_back(tag);
		}
		...
		if (region.satelliteTLogReplicationFactor > 0) {
			...
			for (int i = -1; i < oldLogSystem->logRouterTags; i++) {
				...
				for (int loc : locations) sreqs[loc].recoverTags.push_back(tag);
			}
		...
		if (configuration.usableRegions > 1) {
			...
			logSystem->remoteRecovery =
			    TagPartitionedLogSystem::newRemoteEpoch(logSystem.getPtr(), oldLogSystem, fRemoteWorkers, configuration,
			                                            recoveryCount, remoteLocality, allTags);
		} else {
	...
	ACTOR static Future<Void> trackRejoins(
	    UID dbgid, std::vector<Reference<AsyncVar<OptionalInterface<TLogInterface>>>> logServers,
		...
		try {
			loop {
				...
				if (pos != -1) {
					TraceEvent("TLogJoinedMe", dbgid)
					    ...
					    .detail("Address", req.myInterface.commit.getEndpoint().address.toString());
					if (!logServers[pos]->get().present() ||
					    req.myInterface.commit.getEndpoint() != logServers[pos]->get().interf().commit.getEndpoint())
			...
			for (auto it = lastReply.begin(); it != lastReply.end(); ++it) it->second.send(true);
			throw;
		}
	}
	...
	g_simulator.connectionFailuresDisableDuration && BUGGIFY && g_random->random01() < 0.25;
	...
	static vector<T> getReadyNonError(vector<Future<T>> const& futures) {
		...
		std::vector<T> result;
		for (auto& f : futures)
			if (f.isReady() && !f.isError()) result.push_back(f.get());
		return result;
	}
...
Future<Void> ILogSystem::recoverAndEndEpoch(Reference<AsyncVar<Reference<ILogSystem>>> const& outLogSystem,
                                            ...
                                            LocalityData const& locality, bool forceRecovery) {
	return TagPartitionedLogSystem::recoverAndEndEpoch(outLogSystem, dbgid, oldState, rejoins, locality, forceRecovery);
}

Reference<ILogSystem> ILogSystem::fromLogSystemConfig(UID const& dbgid, struct LocalityData const& locality,
                                                      struct LogSystemConfig const& conf, bool excludeRemote,
                                                      bool useRecoveredAt,
                                                      Optional<PromiseStream<Future<Void>>> addActor) {
	if (conf.logSystemType == 0)
		return Reference<ILogSystem>();
	else if (conf.logSystemType == 2)
		return TagPartitionedLogSystem::fromLogSystemConfig(dbgid, locality, conf, excludeRemote, useRecoveredAt,
		                                                    addActor);
	else
		throw internal_error();
}

Reference<ILogSystem> ILogSystem::fromOldLogSystemConfig(UID const& dbgid, struct LocalityData const& locality,
...
Reference<ILogSystem> ILogSystem::fromServerDBInfo(UID const& dbgid, ServerDBInfo const& dbInfo, bool useRecoveredAt,
                                                   Optional<PromiseStream<Future<Void>>> addActor) {
