...
ACTOR Future<Void> tryBecomeLeaderInternal(ServerCoordinators coordinators, Value proposedSerializedInterface,
	...
	loop {
		...
		for (int i = 0; i < coordinators.leaderElectionServers.size(); i++) {
			Future<bool> hb = retryBrokenPromise(coordinators.leaderElectionServers[i].leaderHeartbeat,
		...
		state Future<Void> rate = delay(SERVER_KNOBS->HEARTBEAT_FREQUENCY, TaskCoordinationReply) ||
				...
				for (int i = 0; i < coordinators.leaderElectionServers.size(); ++i) {
					if (true_heartbeats[i].isReady())
						TraceEvent("LeaderTrueHeartbeat", myInfo.changeID)
						    .detail("Coordinator",
						            coordinators.leaderElectionServers[i].candidacy.getEndpoint().address);
