...
// is different from what tag throttling use for external customer, which is prefer a model that's easier to understand
// for customers.
...
TEST_CASE("/fdbserver/StorageMetricSample/readHotDetect/simple") {
	...
	std::vector<ReadHotRangeWithMetrics> t =
	    ssm._getReadHotRanges(KeyRangeRef(LiteralStringRef("A"), LiteralStringRef("C")), 2.0, 200 * sampleUnit, 0);
...
TEST_CASE("/fdbserver/StorageMetricSample/readHotDetect/moreThanOneRange") {
	...
	std::vector<ReadHotRangeWithMetrics> t =
	    ssm._getReadHotRanges(KeyRangeRef(LiteralStringRef("A"), LiteralStringRef("D")), 2.0, 200 * sampleUnit, 0);
