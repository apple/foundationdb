...
ACTOR static Future<JsonBuilderObject> recoveryStateStatusFetcher(Database cx,
	...
	try {
		state Future<TraceEventFields> mdActiveGensF =
		    timeoutError(mWorker.interf.eventLogRequest.getReply(EventLogRequest("MasterRecoveryGenerations"_sr)), 1.0);
		state Future<TraceEventFields> mdF =
		    timeoutError(mWorker.interf.eventLogRequest.getReply(EventLogRequest("MasterRecoveryState"_sr)), 1.0);
		state Future<TraceEventFields> mDBAvailableF =
		    timeoutError(mWorker.interf.eventLogRequest.getReply(EventLogRequest("MasterRecoveryAvailable"_sr)), 1.0);
...
ACTOR static Future<JsonBuilderObject> workloadStatusFetcher(
	...
	// Transactions
	try {
		state TraceEventFields ratekeeper =
		    wait(timeoutError(rkWorker.interf.eventLogRequest.getReply(EventLogRequest("RkUpdate"_sr)), 1.0));
		TraceEventFields batchRatekeeper =
		    wait(timeoutError(rkWorker.interf.eventLogRequest.getReply(EventLogRequest("RkUpdateBatch"_sr)), 1.0));
