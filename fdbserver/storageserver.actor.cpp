...
public:
	...
	struct Counters {
		...
		Counter kvReadRangeBytes;
		Counter kvSingleReadBytes;
		...
		Counters(StorageServer* self)
		  : cc("StorageServer", self->thisServerID.toString()), getKeyQueries("GetKeyQueries", cc),
		    ...
		    logicalInputBytes("LogicalInputBytes", cc), logicalMoveOverheadBytes("LogicalMoveOverheadBytes", cc),
		    sampledBytesCleared("SampledBytesCleared", cc), kvThroughput("KeyValueStoreThroughput", cc),
		    kvCommitBytes("KeyValueStoreCommitBytes", cc), kvClearRange("KVClearRange", cc),
		    pTreeUpdateThroughput("PTreeUpdateThroughput", cc), pTreeUpdateBytes("PTreeUpdateBytes", cc),
		    storageUpdateThroughput("StorageUpdateThroughput", cc), mutations("Mutations", cc),
		    ...
		    quickGetKeyValuesMiss("QuickGetKeyValuesMiss", cc), kvReadRangeBytes("KVReadRangeBytes", cc),
		    kvSingleReadBytes("KVSingleReadBytes", cc),
	...
	StorageServer(IKeyValueStore* storage,
	  ...
	  : fetchKeysHistograms(), instanceID(deterministicRandom()->randomUniqueID().first()), storage(this, storage),
		...
		this->storage.kvCommitBytes = &counters.kvCommitBytes;
...
ACTOR Future<Void> getValueQ(StorageServer* data, GetValueRequest req) {
	...
	try {
		...
		if (i && i->isValue() && i.key() == req.key) {
			...
			data->counters.kvSingleReadBytes += (vv.expectedSize());
			// data->counters.kvReadThroughput.addInterval(now() - startTime);
...
ACTOR Future<GetKeyValuesReply> readRange(StorageServer* data,
	...
	if (limit >= 0) {
		...
		while (limit > 0 && *pLimitBytes > 0 && readBegin < range.end) {
			...
			data->counters.kvReadRangeBytes += (atStorageVersion.logicalSize());
			// data->counters.kvReadRangeThroughput.addInterval(now() - startTime);
...
ACTOR Future<Void> doEagerReads(StorageServer* data, UpdateEagerReadInfo* eager) {
	...
	for (const auto& key : keyEndVal) {
		data->counters.kvReadRangeBytes += key.expectedSize();
	}
	vector<Optional<Value>> optionalValues = wait(futureValues);
	for (const auto& value : optionalValues) {
		if (value.present()) {
			data->counters.kvSingleReadBytes += value.expectedSize();
		}
	}
...
void StorageServer::addMutation(Version version,
	...
	counters.pTreeUpdateBytes += expanded.expectedSize();
		...
		*pReceivedUpdate = true;
		...
		for (; changeNum < fii.changes.size(); changeNum++) {
			...
			for (; mutationNum < pUpdate->mutations.size(); mutationNum++) {
				data->counters.pTreeUpdateThroughput += (pUpdate->mutations[mutationNum].expectedSize());
		...
		for (; cloneCursor2->hasMessage(); cloneCursor2->nextMessage()) {
			...
			if (LogProtocolMessage::isNextIn(rd)) {
				...
				if (g_network->isSimulated() && data->isTss() && !g_simulator.speedUpSimulation &&
					...
					data->counters.pTreeUpdateThroughput += (msg.expectedSize());
		...
		validate(data);
		data->counters.pTreeUpdateThroughput.addInterval(now() - updateBegin);
...
ACTOR Future<Void> updateStorage(StorageServer* data) {
	...
	loop {
		...
		data->counters.storageUpdateThroughput += data->counters.kvThroughput.getIntervalDelta();
		data->counters.kvThroughput.addInterval(commitDuration);
		...
		data->counters.storageUpdateThroughput.addInterval(now() - updateBegin);
	}
}
...
void StorageServerDisk::writeMutations(const VectorRef<MutationRef>& mutations,
	...
	for (const auto& m : mutations) {
		...
		if (m.type == MutationRef::SetValue) {
			*kvThroughput += m.expectedSize();
			*kvCommitBytes += m.expectedSize();
...
ACTOR Future<Void> applyByteSampleResult(StorageServer* data,
	...
	loop {
		...
		data->counters.kvReadRangeBytes += (bs.logicalSize());
		// data->counters.kvReadRangeThroughput.addInterval(now() - startTime);
		if (results)
