	...
	try {
		...
		if (begin >= end) {
			...
			GetKeyValuesReply _r = wait(readRange(data,
			                                      ...
			                                      type,
			                                      tenantPrefix));
			double duration = g_network->timer() - kvReadRange;
...
ACTOR Future<GetMappedKeyValuesReply> mapKeyValues(StorageServer* data,
	...
	result.more = input.more || result.data.size() < sz;
	return result;
}
	...
	try {
		...
		if (begin >= end) {
			...
			GetKeyValuesReply getKeyValuesReply = wait(readRange(
			    data, version, KeyRangeRef(begin, end), req.limit, &bytesForIndex, span.context, type, tenantPrefix));
...
ACTOR Future<Void> update(StorageServer* data, bool* pReceivedUpdate) {
	...
	state double start;
	state bool enableClearRangeEagerReads = data->storage.getKeyValueStoreType() == KeyValueStoreType::SSD_ROCKSDB_V1
	...
	try {
		...
		*pReceivedUpdate = true;
		...
		double duration = g_network->timer() - updateStart;
