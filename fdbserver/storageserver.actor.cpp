...
ACTOR Future<Void> fetchKeys(StorageServer* data, AddingShard* shard) {
	...
	try {
		...
		holdingFKPL.release();
		wait(data->version.whenAtLeast(fetchVersion) && data->durableVersion.whenAtLeast(data->storageVersion() + 1));
...
AddingShard::AddingShard(StorageServer* server, KeyRangeRef const& keys)
  : server(server), keys(keys), transferredVersion(invalidVersion), fetchVersion(invalidVersion), phase(WaitPrevious) {
