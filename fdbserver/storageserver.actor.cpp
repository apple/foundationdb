...
ACTOR Future<Void> installEmptyRange(StorageServer* data, AddingShard* shard) {
	state const UID fetchKeysID = deterministicRandom()->randomUniqueID();
	state TraceInterval interval("InstallEmptyRange");
	state KeyRange keys = shard->keys;
	state const double startTime = now();
	...
	try {
		DEBUG_KEY_RANGE("InstallEmptyRangeBegin", data->version.get(), shard->keys, data->thisServerID);
		...
		if (lastAvailable != invalidVersion && lastAvailable >= data->durableVersion.get()) {
			TEST(true); // InstallEmptyRange waits for previous available version to be durable
			wait(data->durableVersion.whenAtLeast(lastAvailable + 1));
		}

		TraceEvent(SevDebug, "InstallEmptyRangeVersionSatisfied", data->thisServerID).detail("ID", interval.pairID);
		...
		// can put mutation in.
		FetchInjectionInfo* batch = wait(p.getFuture());
		TraceEvent(SevDebug, "InstallEmptyRangeUpdateBatch", data->thisServerID).detail("ID", interval.pairID);
		...
		TraceEvent(SevDebug, "InstallEmptyRangeVersion", data->thisServerID)
		    .detail("FKID", interval.pairID)
		    .detail("Version", shard->transferredVersion)
		    .detail("StorageVersion", data->storageVersion());
		validate(data);
		...
		if (e.code() == error_code_actor_cancelled && !data->shuttingDown) {
			if (shard->phase < AddingShard::Waiting) {
				data->storage.clearRange(keys);
				data->byteSampleApplyClear(keys, invalidVersion);
			} else {
...
AddingShard::AddingShard(StorageServer* server, KeyRangeRef const& keys, bool emptyRange)
  : keys(keys), server(server), transferredVersion(invalidVersion), phase(), emptyRange(emptyRange) {
	fetchClient = installEmptyRange(server, this);
}
...
void changeServerKeys(StorageServer* data,
	...
	for (auto r = clearRanges.begin(); r != clearRanges.end(); ++r) {
		...
		// removeDataRange(data, data->addVersionToMutationLog(data->data().getLatestVersion()), data->shards, *r);
		// data->mutableData().insert(r->begin, ValueOrClearToRef::clearTo(r->end));
...
ACTOR Future<Void> reportStorageServerState(StorageServer* self) {
	...
	loop {
		...
		TraceEvent(level, "FetchKeyCurrentStatus", self->thisServerID)
