...
public:
	...
	/*ACTOR static Future<Void> getTeamForBulkLoad(DDTeamCollection* self, GetTeamRequest req) {
	    ...
	    if (!self->primary && !self->readyToStart.isReady()) {
	        ...
	        TraceEvent(SevDebug, "DDBulkLoadTaskGetTeamRemoteDCNotReady", self->distributorId)
	            .suppressFor(1.0)
	    ...
	    while (randomTeams.size() < SERVER_KNOBS->BEST_TEAM_OPTION_COUNT &&
	        ...
	        if (!ok) {
	            TraceEvent e(SevDebug, "DDBulkLoadTaskGetTeamUnheathy", self->distributorId);
	            e.detail("SrcIds", describe(req.src));
	            e.detail("DestIds", describe(dest->getServerIDs()));
	        }
	        ...
	        for (const auto& srcId : req.src) {
	            ...
	            for (const auto& serverId : serverIds) {
	                if (serverId == srcId) {
	                    TraceEvent(SevDebug, "DDBulkLoadTaskGetTeamDuplicatedSrc", self->distributorId)
	                        .detail("SrcIds", describe(req.src))
	                        .detail("DestIds", describe(dest->getServerIDs()))
	                        .detail("DuplicatedServerID", serverId);
	    ...
	    e.detail("SrcIds", describe(req.src));
	    e.detail("TeamSize", randomTeams.size());
	    if (res.present()) {
