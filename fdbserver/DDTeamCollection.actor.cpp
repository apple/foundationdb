		...
		loop {
			...
			if (totalMTCount > desiredMachineTeams && mt.isValid()) {
				...
				TraceEvent("MachineTeamRemover", self->distributorId)
				    .detail("MachineTeamIDToRemove", mt->getID().shortString())
			...
			loop {
				...
				if (worstStatus != DDTeamCollection::Status::NONE) {
					...
					if (worstStatus == DDTeamCollection::Status::FAILED) {
						TraceEvent(SevWarn, "FailedServerRemoveKeys", self->distributorId)
						    .detail("Server", server->getID())
						    .detail("Excluded", worstAddr.toString());
						wait(removeKeysFromFailedServer(cx, server->getID(), self->lock, ddEnabledState));
						if (BUGGIFY) wait(delay(5.0));
						self->shardsAffectedByTeamFailure->eraseServer(server->getID());
					}
				}
				...
				choose {
					when(wait(failureTracker)) {
						...
						wait(removeStorageServer(cx, server->getID(), self->lock, ddEnabledState));
		...
		for (; idx < self->badTeams.size(); idx++) {
			...
			if (servers.size() >= self->configuration.storageTeamSize) {
				...
				for (int j = 0; j < servers.size() - self->configuration.storageTeamSize + 1 && !foundTeam; j++) {
					const auto& serverTeams = servers[j]->getTeams();
				...
				if (!foundTeam) {
					if (self->satisfiesPolicy(servers)) {
						if (servers.size() == self->configuration.storageTeamSize ||
							...
							tempSet->clear();
							for (const auto& server : servers) {
								tempMap->add(server->lastKnownInterface.locality, &server->getID());
							}
...
void DDTeamCollection::addTeam(const vector<Reference<TCServerInfo>>& newTeamServers, bool isInitialTeam,
	...
	for (auto& newTeamServer : newTeamServers) {
		newTeamServer->addTeam(teamInfo);
	}
...
void DDTeamCollection::traceServerInfo() const {
	...
	for (const auto& [id, server] : server_info) {
		TraceEvent("ServerInfo", distributorId)
		    .detail("ServerInfoIndex", i++)
		    .detail("ServerID", id.toString())
		    .detail("ServerTeamOwned", server->getTeams().size())
		    .detail("MachineID", server->machine->getID().contents().toString())
		    .detail("StoreType", server->storeType.toString())
		    .detail("InDesiredDC", server->inDesiredDC);
	}
...
void DDTeamCollection::removeServer(UID removedServer) {
	...
	const auto& sharedTeams = removedServerInfo->getTeams();
	...
	for (const auto& id : serversWithAjoiningTeams) {
		auto& server = server_info[id];
		for (const auto& team : server->getTeams()) {
			const auto& serverIds = team->getServerIDs();
...
bool DDTeamCollection::notEnoughTeamsForAServer() const {
	...
	for (auto& [id, server] : server_info) {
		if (server->getTeams().size() < targetTeamNumPerServer && !server_status.get(id).isUnhealthy()) {
			return true;
		}
	}
