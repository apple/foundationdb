...
struct DeltaTree {
	...
	static int MaximumTreeSize() { return std::numeric_limits<OffsetT>::max(); };

	struct Node {
		OffsetT leftChildOffset;
		OffsetT rightChildOffset;

		inline DeltaT& delta() { return *(DeltaT*)(this + 1); };

		inline const DeltaT& delta() const { return *(const DeltaT*)(this + 1); };

		Node* rightChild() const {
			// printf("Node(%p): leftOffset=%d  rightOffset=%d  deltaSize=%d\n", this, (int)leftChildOffset,
			// (int)rightChildOffset, (int)delta().size());
		...
		int size() const { return sizeof(Node) + delta().size(); }
	};
...
#pragma pack(pop)
	...
	int size() const { return sizeof(DeltaTree) + nodeBytes; }

public:
	// Get count of total overhead bytes (everything but the user-formatted Delta) for a tree given size n
	static inline int GetTreeOverhead(int n = 0) { return sizeof(DeltaTree) + (n * sizeof(Node)); }
	...
	struct DecodedNode {
		...
		DecodedNode(Node* raw, DecodedNode* parent, bool left, Arena& arena)
		  : parent(parent), raw(raw), left(nullptr), right(nullptr), prev(left ? parent->prev : &parent->item),
	...
	struct Reader : FastAllocated<Reader> {
		Reader(const void* treePtr = nullptr, const T* lowerBound = nullptr, const T* upperBound = nullptr)
		  : tree((DeltaTree*)treePtr), lower(lowerBound), upper(upperBound) {
			...
			// TODO: Remove these copies into arena and require users of Reader to keep prev and next alive during its
			// lifetime
	...
	struct Cursor {
		...
		const T& getOrUpperBound() const { return valid() ? node->item : *reader->upperBound(); }
