...
TEST_CASE("/fdbserver/metrics/OTEL/VerifyMsgpack") {
	...
	state std::vector<uint8_t> counterBytes = {
		0xc9, 0x00, 0x00, 0x00, 0x2F, 0x01, 0x91, 0xA4, 0x74, 0x65, 0x73, 0x74, 203,  65,   216,  167,  126,  179,
		33,   156,  246,  203,  65,   216,  167,  126,  179,  33,   156,  246,  0x91, 0xA3, 0x6B, 0x65, 0x79, 0xA3,
		0x76, 0x61, 0x6C, 0xD3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xCC, 0x00, 0xCC, 0x01, 0xC3,
	};
	...
	*/
	...
	state CounterCollection collection{ "CounterCollection", "msgpack" };
	state Counter c{ "test", collection };
	c += 1;
	c.flush(batch);
	...
	state std::vector<uint8_t> histBytes = {
		0xc9, 0x00, 0x00, 0x00, 0x67, 0x02, 0x91, 0xA5, 0x74, 0x65, 0x73, 0x74, 0x68, 0x92, 0xA3, 0x6B, 0x65, 0x79,
	...
	*/
	...
	state LatencySample s{ "testh", UID(), 5, 2 };
