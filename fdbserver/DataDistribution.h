...
enum { SOME_SHARED = 2, NONE_SHARED = 3 };
...
struct GetTeamRequest {
	...
	GetTeamRequest() {}
	GetTeamRequest(bool wantsNewServers, bool wantsTrueBest, bool preferLowerUtilization, double inflightPenalty = 1.0)
...
private:
	...
	KeyRangeMap<vector<Team>>
	    shard_teams; // A shard can be affected by the failure of multiple teams if it is a queued merge
...
Future<Void> dataDistribution(Reference<AsyncVar<struct ServerDBInfo>> const& db, MasterInterface const& mi,
                              DatabaseConfiguration const& configuration,
                              PromiseStream<std::pair<UID, Optional<StorageServerInterface>>> const& serverChanges,
                              Reference<ILogSystem> const& logSystem, Version const& recoveryCommitVersion,
                              std::vector<Optional<Key>> const& primaryDcId,
                              std::vector<Optional<Key>> const& remoteDcIds, double* const& lastLimited,
                              Future<Void> const& remoteRecovered);

Future<Void> dataDistributionTracker(Reference<InitialDataDistribution> const& initData, Database const& cx,
                                     ...
                                     Reference<AsyncVar<bool>> const& zeroHealthyTeams, UID const& masterId);

Future<Void> dataDistributionQueue(
    Database const& cx, PromiseStream<RelocateShard> const& output, FutureStream<RelocateShard> const& input,
    PromiseStream<GetMetricsRequest> const& getShardMetrics, Reference<AsyncVar<bool>> const& processingUnhealthy,
    vector<TeamCollectionInterface> const& teamCollection,
    Reference<ShardsAffectedByTeamFailure> const& shardsAffectedByTeamFailure, MoveKeysLock const& lock,
    PromiseStream<Promise<int64_t>> const& getAverageShardBytes, MasterInterface const& mi, int const& teamSize,
    double* const& lastLimited, Version const& recoveryVersion);
