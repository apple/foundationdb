...
struct SyncQueue : ReferenceCounted<SyncQueue> {
	SyncQueue(int outstandingLimit, Reference<IAsyncFile> file) : outstandingLimit(outstandingLimit), file(file) {
		for (int i = 0; i < outstandingLimit; i++) outstanding.push_back(Void());
	}

	Future<Void> onSync() { // Future is set when all writes completed before the call to onSync are complete
		if (outstanding.size() <= outstandingLimit) outstanding.push_back(waitAndSync(this));
...
public:
	RawDiskQueue_TwoFiles(std::string basename, UID dbgid, int64_t fileSizeWarningLimit)
	  : basename(basename), onError(delayed(error.getFuture())), onStopped(stopped.getFuture()), readingFile(-1),
	    readingPage(-1), writingPos(-1), dbgid(dbgid), dbg_file0BeginSeq(0), fileExtensionBytes(10 << 20),
	    readingBuffer(dbgid), readyToPush(Void()), fileSizeWarningLimit(fileSizeWarningLimit), lastCommit(Void()),
	    isFirstCommit(true) {
		if (BUGGIFY) fileExtensionBytes = 8 << 10;
	...
	StorageBytes getStorageBytes() {
		...
		return StorageBytes(free, total, files[0].size + files[1].size,
	...
	struct TrackMe : NonCopyable {
		...
		~TrackMe() { self->recoveryActorCount.set(false, self->recoveryActorCount.get(false) - 1); }
	};
	...
	Future<Void> push(StringRef pageData, vector<Reference<SyncQueue>>& toSync) {
		...
		if (pageData.size() + writingPos > files[1].size) {
			if (files[0].popped == files[0].size) {
				...
				files[1].size +=
				    std::min(std::max(fileExtensionBytes, minExtension), files[0].size + files[1].size + minExtension);
				waitfor.push_back(files[1].f->truncate(files[1].size));

				if (fileSizeWarningLimit > 0 && files[1].size > fileSizeWarningLimit) {
					TraceEvent(SevWarnAlways, "DiskQueueFileTooLarge", dbgid)
					    .suppressFor(1.0)
					    .detail("Filename", filename(1))
					    .detail("Size", files[1].size);
				}
			}
		}
	...
	ACTOR static UNCANCELLABLE Future<Void> pushAndCommit(RawDiskQueue_TwoFiles* self, StringRef pageData,
	                                                      StringBuffer* pageMem, uint64_t poppedPages) {
	...
	ACTOR static Future<Void> setPoppedPage(RawDiskQueue_TwoFiles* self, int file, int64_t page, int64_t debugSeq) {
		...
		if (file == 1) wait(self->truncateFile(self, 0, 0));
	...
	ACTOR static Future<Void> openFiles(RawDiskQueue_TwoFiles* self) {
		...
		for (int i = 0; i < 2; i++) self->files[i].setFile(fs[i].get());
	...
	ACTOR static void shutdown(RawDiskQueue_TwoFiles* self, bool deleteFiles) {
		...
		try {
			...
			while (self->recoveryActorCount.get(false)) wait(self->recoveryActorCount.onChange(false));

			for (int i = 0; i < 2; i++) self->files[i].f.clear();
	...
	ACTOR static UNCANCELLABLE Future<Standalone<StringRef>> readFirstAndLastPages(RawDiskQueue_TwoFiles* self,
		...
		try {
			...
			for (int i = 0; i < 2; i++)
				if (self->files[i].size > 0) reads.push_back(self->files[i].f->read(&firstPage[i], sizeof(Page), 0));
			...
			if (!compare(&firstPage[1], &firstPage[1])) {
				...
				for (int i = 0; i < 2; ++i)
					if (self->files[i].size > 0) truncates.push_back(self->truncateFile(self, i, 0));
	...
	Future<int> fillReadingBuffer() {
		...
		int len = std::min<int64_t>((files[readingFile].size / sizeof(Page) - readingPage) * sizeof(Page),
		                            BUGGIFY_WITH_PROB(1.0) ? sizeof(Page) * g_random->randomInt(1, 4) : (1 << 20));
		readingBuffer.clear();
...
public:
	DiskQueue(std::string basename, UID dbgid, int64_t fileSizeWarningLimit)
	  : rawQueue(new RawDiskQueue_TwoFiles(basename, dbgid, fileSizeWarningLimit)), dbgid(dbgid), anyPopped(false),
	    nextPageSeq(0), poppedSeq(0), lastPoppedSeq(0), nextReadLocation(-1), readBufPage(NULL), readBufPos(0),
	    pushed_page_buffer(NULL), recovered(false), lastCommittedSeq(0), warnAlwaysForMemory(true) {}
	...
	int getMaxPayload() { return Page::maxPayload; }

	virtual int getCommitOverhead() {
		if (!pushedPageCount()) {
			if (!anyPopped) return 0;
	...
	virtual Future<Void> commit() {
		...
		if (pushedPageCount() >= 8000) {
			TraceEvent(warnAlwaysForMemory ? SevWarnAlways : SevWarn, "DiskQueueMemoryWarning", dbgid)
			    ...
			    .detail("File0Name", rawQueue->files[0].dbgFilename);
			if (g_network->isSimulated()) warnAlwaysForMemory = false;
		}
		...
		auto f = rawQueue->pushAndCommit(pushed_page_buffer->ref(), pushed_page_buffer,
		                                 poppedSeq / sizeof(Page) - lastPoppedSeq / sizeof(Page));
	...
	virtual StorageBytes getStorageBytes() { return rawQueue->getStorageBytes(); }
	...
	ACTOR static Future<Standalone<StringRef>> readNext(DiskQueue* self, int bytes) {
		...
		if (self->nextReadLocation < 0) {
			...
			if (!nonempty) {
				...
				// The next read location isn't necessarily the end of the last commit, but this is sufficient for
				// helping us check an ASSERTion
		...
		loop {
			...
			if (!self->readBufPage->checkHash() ||
			    self->readBufPage->seq < self->nextReadLocation / sizeof(Page) * sizeof(Page)) {
				TraceEvent("DQRecInvalidPage", self->dbgid)
				    .detail("NextReadLocation", self->nextReadLocation)
				    .detail("HashCheck", self->readBufPage->checkHash())
				    .detail("Seq", self->readBufPage->seq)
				    .detail("Expect", self->nextReadLocation / sizeof(Page) * sizeof(Page))
				    .detail("File0Name", self->rawQueue->files[0].dbgFilename);
...
public:
	DiskQueue_PopUncommitted(std::string basename, UID dbgid, int64_t fileSizeWarningLimit)
	  : queue(new DiskQueue(basename, dbgid, fileSizeWarningLimit)), pushed(0), popped(0), committed(0){};
...
private:
	...
	ACTOR static Future<Standalone<StringRef>> readNext(DiskQueue_PopUncommitted* self, int bytes) {
		...
		if (str.size() < bytes) self->pushed = self->getNextReadLocation();
