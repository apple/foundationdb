...
Database openDBOnServer(Reference<AsyncVar<ServerDBInfo>> const& db, int taskID, bool enableLocalityLoadBalance,
                        bool lockAware) {
	Reference<AsyncVar<ClientDBInfo>> info(new AsyncVar<ClientDBInfo>);
	return DatabaseContext::create(info, extractClientInfo(db, info),
	                               enableLocalityLoadBalance ? db->get().myLocality : LocalityData(),
	                               enableLocalityLoadBalance, taskID, lockAware);
}
...
Error checkIOTimeout(Error const& e) {
	...
	if (timeoutOccurred) {
		...
		if (e.isInjectedFault()) timeout = timeout.asInjectedFault();
		return timeout;
	}
	return e;
}

ACTOR Future<Void> forwardError(PromiseStream<ErrorInfo> errors, const char* context, UID id, Future<Void> process) {
	try {
		wait(process);
...
ACTOR Future<Void> workerHandleErrors(FutureStream<ErrorInfo> errors) {
	loop choose {
		when(ErrorInfo _err = waitNext(errors)) {
			...
			if (!ok) err.error = checkIOTimeout(err.error); // Possibly convert error to io_timeout

			endRole(err.id, err.context, "Error", ok, err.error);

			if (err.error.code() == error_code_please_reboot || err.error.code() == error_code_io_timeout)
				throw err.error;
		}
	}
}
	...
	loop {
		Future<RegisterWorkerReply> registrationReply =
		    ccInterface->get().present()
		        ? brokenPromiseToNever(ccInterface->get().get().registerWorker.getReply(RegisterWorkerRequest(
		              interf, initialClass, processClass, asyncPriorityInfo->get(), requestGeneration++)))
		        : Never();
		choose {
...
ACTOR Future<Void> storageServerRollbackRebooter(Future<Void> prevStorageServer, KeyValueStoreType storeType,
                                                 std::string filename, UID id, LocalityData locality,
                                                 Reference<AsyncVar<ServerDBInfo>> db, std::string folder,
                                                 ActorCollection* filesClosed, int64_t memoryLimit) {
	loop {
...
void startRole(UID roleId, UID workerId, std::string as, std::map<std::string, std::string> details,
               std::string origination) {
	TraceEvent ev("Role", roleId);
	ev.detail("As", as).detail("Transition", "Begin").detail("Origination", origination).detail("OnWorker", workerId);
	for (auto it = details.begin(); it != details.end(); it++) ev.detail(it->first.c_str(), it->second);
...
void endRole(UID id, std::string as, std::string reason, bool ok, Error e) {
	{
		...
		if (e.code() != invalid_error_code) ev.error(e, true);
		ev.detail("Transition", "End").detail("As", as).detail("Reason", reason);
			...
			when(wait(ccInterface->onChange())) {
				if (ccInterface->get().present())
					TraceEvent("GotCCInterfaceChange")
					    ...
					    .detail("CCMachine", ccInterface->get().get().getWorkers.getEndpoint().address);
			}
		}
	}
}
...
ACTOR Future<Void> workerServer(Reference<ClusterConnectionFile> connFile,
                                ...
                                Promise<Void> recoveredDiskFiles, UID processIDUid) {
	...
	if (metricsPrefix.size() > 0) {
		if (metricsConnFile.size() > 0) {
			try {
				...
				metricsLogger =
				    runMetrics(cluster->createDatabase(LiteralStringRef("DB"), locality), KeyRef(metricsPrefix));
			} catch (Error& e) {
			...
			metricsLogger =
			    runMetrics(openDBOnServer(dbInfo, TaskDefaultEndpoint, true, lockAware), KeyRef(metricsPrefix));
		}
	}
	...
	initializeSystemMonitorMachineState(
	    SystemMonitorMachineState(folder, locality.zoneId(), locality.machineId(), g_network->getLocalAddress().ip));
	...
	try {
		...
		for (int f = 0; f < stores.size(); f++) {
			...
			if (s.storedComponent == DiskStore::Storage) {
				...
				f = storageServerRollbackRebooter(f, s.storeType, s.filename, recruited.id(), recruited.locality,
				                                  dbInfo, folder, &filesClosed, memoryLimit);
				errorForwarders.add(forwardError(errors, "StorageServer", recruited.id(), f));
			} else if (s.storedComponent == DiskStore::TLogData) {
				IKeyValueStore* kv = openKVStore(s.storeType, s.filename, s.storeID, memoryLimit, validateDataFiles);
				IDiskQueue* queue =
				    openDiskQueue(joinPath(folder, fileLogQueuePrefix.toString() + s.storeID.toString() + "-"),
				                  s.storeID, 10 * SERVER_KNOBS->TARGET_BYTES_PER_TLOG);
				...
				Promise<Void> oldLog;
				Promise<Void> recovery;
				Future<Void> tl = tLog(kv, queue, dbInfo, locality,
				                       tlog.isReady() ? tlogRequests : PromiseStream<InitializeTLogRequest>(),
				                       s.storeID, true, oldLog, recovery);
				recoveries.push_back(recovery.getFuture());
		...
		loop choose {
			...
			when(RebootRequest req = waitNext(interf.clientInterface.reboot.getFuture())) {
				...
				if (rebootReq.checkData) {
					Reference<IAsyncFile> checkFile = wait(IAsyncFileSystem::filesystem()->open(
					    joinPath(folder, validationFilename), IAsyncFile::OPEN_CREATE | IAsyncFile::OPEN_READWRITE,
					    0600));
					wait(checkFile->sync());
				}
			...
			when(RecruitMasterRequest req = waitNext(interf.master.getFuture())) {
				...
				Future<Void> masterProcess =
				    masterServer(recruited, dbInfo, ServerCoordinators(connFile), req.lifetime, req.forceRecovery);
				errorForwarders.add(
				    zombie(recruited, forwardError(errors, "MasterServer", recruited.id(), masterProcess)));
			...
			when(InitializeTLogRequest req = waitNext(interf.tLog.getFuture())) {
				...
				if (tlog.isReady()) {
					...
					IDiskQueue* queue =
					    openDiskQueue(joinPath(folder, fileLogQueuePrefix.toString() + logId.toString() + "-"), logId);
			...
			when(InitializeStorageRequest req = waitNext(interf.storage.getFuture())) {
				if (!storageCache.exists(req.reqId)) {
					...
					s = storageServerRollbackRebooter(s, req.storeType, filename, recruited.id(), recruited.locality,
					                                  dbInfo, folder, &filesClosed, memoryLimit);
			...
			when(InitializeResolverRequest req = waitNext(interf.resolver.getFuture())) {
				...
				errorForwarders.add(zombie(
				    recruited, forwardError(errors, "Resolver", recruited.id(), resolver(recruited, req, dbInfo))));
				req.reply.send(recruited);
			}
			when(InitializeLogRouterRequest req = waitNext(interf.logRouter.getFuture())) {
				...
				errorForwarders.add(zombie(
				    recruited, forwardError(errors, "logRouter", recruited.id(), logRouter(recruited, req, dbInfo))));
				req.reply.send(recruited);
			}
				...
				for (DiskStore d : getDiskStores(folder)) {
					...
					if (!req.includePartialStores) {
						...
						if (d.storedComponent == DiskStore::COMPONENT::TLogData && included) {
							std::string basename =
							    fileLogQueuePrefix.toString() + d.filename.substr(fileLogDataPrefix.size());
...
ClusterControllerPriorityInfo getCCPriorityInfo(std::string filePath, ProcessClass processClass) {
	if (!fileExists(filePath))
		return ClusterControllerPriorityInfo(ProcessClass(processClass.classType(), ProcessClass::CommandLineSource)
		                                     ...
		                                     false, ClusterControllerPriorityInfo::FitnessUnknown);
...
ACTOR Future<Void> fdbd(Reference<ClusterConnectionFile> connFile, LocalityData localities, ProcessClass processClass,
	...
	try {
		...
		TraceEvent("StartingFDBD")
		    ...
		    .detailext("MachineId", localities.machineId())
		    .detail("DiskPath", dataFolder)
		    .detail("CoordPath", coordFolder);

		// SOMEDAY: start the services on the machine in a staggered fashion in simulation?
		state vector<Future<Void>> v;
		// Endpoints should be registered first before any process trying to connect to it. So coordinationServer actor
		// should be the first one executed before any other.
		if (coordFolder.size())
			v.push_back(fileNotFoundToNever(
			    coordinationServer(coordFolder))); // SOMEDAY: remove the fileNotFound wrapper and make DiskQueue
		...
		v.push_back(reportErrors(monitorAndWriteCCPriorityInfo(fitnessFilePath, asyncPriorityInfo),
		                         "MonitorAndWriteCCPriorityInfo"));
		v.push_back(
		    reportErrors(processClass == ProcessClass::TesterClass
		                     ? monitorLeader(connFile, cc)
		                     : clusterController(connFile, cc, asyncPriorityInfo, recoveredDiskFiles.getFuture()),
		                 "ClusterController"));
		...
		v.push_back(reportErrorsExcept(workerServer(connFile, cc, localities, asyncPriorityInfo, processClass,
		                                            ...
		                                            recoveredDiskFiles, processIDUid),
