...
class ScopedHealthyTestEndpoints {
	IFailureMonitor& monitor = IFailureMonitor::failureMonitor();
	std::map<NetworkAddress, FailureStatus> savedStatus;

public:
	void add(Endpoint const& endpoint) {
		const NetworkAddress address = endpoint.getPrimaryAddress();
		if (savedStatus.emplace(address, monitor.getState(address)).second) {
			monitor.setStatus(address, FailureStatus(false));
		}
		assertAvailable(endpoint);
	}
	void assertAvailable(Endpoint const& endpoint) const { ASSERT(monitor.getState(endpoint).isAvailable()); }

	~ScopedHealthyTestEndpoints() {
		for (const auto& [address, status] : savedStatus) {
			monitor.setStatus(address, status);
		}
	}
};

class OldLogRouterRecruitmentFixture {
	ScopedHealthyTestEndpoints healthyEndpoints;
	Reference<LogSystem> logSystem;
	Reference<LogSet> logSet;
	WorkerInterface worker;
	TLogInterface router;
	const bool forRemote;
	Future<Void> recruitment;

public:
	static constexpr double retryDelay = 0.01;

	explicit OldLogRouterRecruitmentFixture(bool forRemote)
	  : logSystem(makeReference<LogSystem>(UID(1, 1), LocalityData(), LogEpoch(1))), logSet(makeReference<LogSet>()),
	    router(UID(2, 2), UID(2, 2), LocalityData()), forRemote(forRemote) {
		worker.logRouter.getEndpoint(TaskPriority::Worker);
		router.initEndpoints();
		// These in-process RPCs have no connection that would mark their address healthy.
		healthyEndpoints.add(worker.logRouter.getEndpoint());
		healthyEndpoints.add(router.waitFailure.getEndpoint());
		logSet->locality = 1;
		...
		if (forRemote) {
			...
			logSystem->oldLogData.push_back(std::move(old));
		} else {
			logSystem->tLogs.push_back(logSet);
		}
	}

	~OldLogRouterRecruitmentFixture() { recruitment.cancel(); }

	Future<Void> start() {
		ASSERT(!recruitment.isValid());
		...
		// The eager coroutine captures its retry deadline before this synchronous scope restores the knob.
		recruitment = logSystem->recruitOldLogRouters(
		    { worker }, LogEpoch(2), 1, 100, { LocalityData() }, makeReference<PolicyOne>(), forRemote);
		return recruitment;
	}

	FutureStream<InitializeLogRouterRequest> requests() const { return worker.logRouter.getFuture(); }
	Future<InitializeLogRouterRequest> nextRequest() const {
		return timeoutError(waitOrError(requests(), recruitment), 1.0);
	}
	FutureStream<ReplyPromise<Void>> failureRequests() const { return router.waitFailure.getFuture(); }
	Future<Void> onConfigChange() { return logSystem->onLogSystemConfigChange(); }
	void reply(InitializeLogRouterRequest const& request) const { request.reply.send(router); }
	bool hasRouter() const {
		return logSet->logRouters.size() == 1 && logSet->logRouters.front()->get().id() == router.id();
	}
	bool hasNoRouters() const { return logSet->logRouters.empty(); }
	void assertPending() const {
		healthyEndpoints.assertAvailable(worker.logRouter.getEndpoint());
		healthyEndpoints.assertAvailable(router.waitFailure.getEndpoint());
		if (recruitment.isError()) {
			throw recruitment.getError();
		}
		ASSERT(!recruitment.isReady());
	}
	void cancel() { recruitment.cancel(); }
};

void assertSameLogRouterRequest(InitializeLogRouterRequest const& first, InitializeLogRouterRequest const& retry) {
	ASSERT(first.reqId == retry.reqId);
	ASSERT_EQ(first.recoveryCount, retry.recoveryCount);
	ASSERT(first.routerTag == retry.routerTag);
	ASSERT_EQ(first.startVersion, retry.startVersion);
	ASSERT(first.tLogLocalities == retry.tLogLocalities);
	ASSERT(first.tLogPolicy == retry.tLogPolicy);
	ASSERT_EQ(first.locality, retry.locality);
	ASSERT(first.recoverAt == retry.recoverAt);
	ASSERT(first.knownLockedTLogIds == retry.knownLockedTLogIds);
	ASSERT_EQ(first.allowDropInSim, retry.allowDropInSim);
	ASSERT_EQ(first.isReplacement, retry.isReplacement);
...
TEST_CASE("/LogSystem/RecruitOldLogRouters/LocalRetryPreservesRequest") {
	OldLogRouterRecruitmentFixture fixture(false);
	Future<Void> changed = fixture.onConfigChange();
	Future<Void> recruitment = fixture.start();
	InitializeLogRouterRequest request = co_await fixture.nextRequest();
	InitializeLogRouterRequest retry = co_await fixture.nextRequest();
	assertSameLogRouterRequest(request, retry);
	fixture.assertPending();
	ASSERT(fixture.hasNoRouters());
	fixture.reply(retry);
	co_await timeoutError(changed, 1.0);
	ASSERT(fixture.hasRouter());
	fixture.assertPending();
	fixture.cancel();
}

TEST_CASE("/LogSystem/RecruitOldLogRouters/LocalLateOriginalReply") {
	OldLogRouterRecruitmentFixture fixture(false);
	Future<Void> changed = fixture.onConfigChange();
	Future<Void> recruitment = fixture.start();
	InitializeLogRouterRequest request = co_await fixture.nextRequest();
...
TEST_CASE("/LogSystem/RecruitOldLogRouters/LocalReplySuccess") {
	OldLogRouterRecruitmentFixture fixture(false);
	Future<Void> changed = fixture.onConfigChange();
	Future<Void> recruitment = fixture.start();
	InitializeLogRouterRequest request = co_await fixture.nextRequest();
	...
	co_await timeoutError(changed, 1.0);
	ASSERT(fixture.hasRouter());
	ReplyPromise<Void> failureRequest = co_await fixture.failureRequests();

	co_await delay(2 * OldLogRouterRecruitmentFixture::retryDelay);
	fixture.assertPending();
	ASSERT(!fixture.requests().isReady());
	fixture.cancel();
...
TEST_CASE("/LogSystem/RecruitOldLogRouters/LocalRetryWaitAndCancellation") {
	OldLogRouterRecruitmentFixture fixture(false);
	Future<Void> recruitment = fixture.start();
	InitializeLogRouterRequest request = co_await fixture.nextRequest();
	InitializeLogRouterRequest retry = co_await fixture.nextRequest();
	...
	fixture.assertPending();
	ASSERT(!fixture.requests().isReady());
	ASSERT(request.reply.getFutureReferenceCount() > 0);
	ASSERT(retry.reply.getFutureReferenceCount() > 0);
	fixture.cancel();
	ASSERT(recruitment.isError());
	ASSERT_EQ(recruitment.getError().code(), error_code_actor_cancelled);
	ASSERT(fixture.hasNoRouters());
	ASSERT_EQ(request.reply.getFutureReferenceCount(), 0);
	ASSERT_EQ(retry.reply.getFutureReferenceCount(), 0);
}

TEST_CASE("/LogSystem/RecruitOldLogRouters/RemoteReplyWait") {
	OldLogRouterRecruitmentFixture fixture(true);
	Future<Void> recruitment = fixture.start();
	InitializeLogRouterRequest request = co_await fixture.nextRequest();
	ASSERT(!request.allowDropInSim);
	co_await delay(2 * OldLogRouterRecruitmentFixture::retryDelay);
	fixture.assertPending();
	ASSERT(!fixture.requests().isReady());
	ASSERT(fixture.hasNoRouters());
	fixture.reply(request);
	co_await timeoutError(recruitment, 1.0);
	ASSERT(fixture.hasRouter());
}
