...
Reference<LocationInfo> LocationInfo::getInterface(DatabaseContext* cx,
...
LocationInfo::~LocationInfo() {
	if (cx) {
		...
		auto it = cx->ssid_locationInfo.find(handles);
		if (it != cx->ssid_locationInfo.end()) cx->ssid_locationInfo.erase(it);
		cx = NULL;
	}
}
...
std::string printable(const VectorRef<KeyValueRef>& val) {
	std::string s;
	for (int i = 0; i < val.size(); i++) s = s + printable(val[i].key) + format(":%d ", val[i].value.size());
	return s;
}
...
void validateOptionValue(Optional<StringRef> value, bool shouldBePresent) {
	if (shouldBePresent && !value.present()) throw invalid_option_value();
	if (!shouldBePresent && value.present() && value.get().size() > 0) throw invalid_option_value();
}
...
void dumpMutations(const MutationListRef& mutations) {
	for (auto m = mutations.begin(); m; ++m) {
		...
		case MutationRef::ClearRange:
			...
			break;
		default:
			printf("  Unknown mutation %d('%s','%s')\n", m->type, printable(m->param1).c_str(),
...
ACTOR static Future<Void> delExcessClntTxnEntriesActor(Transaction* tr, int64_t clientTxInfoSizeLimit) {
	...
	loop {
		try {
			...
			if (txInfoSize < clientTxInfoSizeLimit) return Void();
			...
			for (auto& kv : txEntries) {
				...
				if (txInfoSize - numBytesToDel <= clientTxInfoSizeLimit) break;
			}
			if (numBytesToDel) {
			...
			if (txInfoSize - numBytesToDel <= clientTxInfoSizeLimit) return Void();
		} catch (Error& e) {
			wait(tr->onError(e));
		}
	}
...
ACTOR static Future<Void> clientStatusUpdateActor(DatabaseContext* cx) {
	...
	loop {
		try {
			...
			state int64_t dataSizeLimit = BUGGIFY
			                                  ? g_random->randomInt(200e3, 1.5 * CLIENT_KNOBS->TRANSACTION_SIZE_LIMIT)
...
DatabaseContext::DatabaseContext(Reference<AsyncVar<ClientDBInfo>> clientInfo, Reference<Cluster> cluster,
                                 Future<Void> clientInfoMonitor, Standalone<StringRef> dbName,
                                 Standalone<StringRef> dbId, int taskID, LocalityData clientLocality,
                                 bool enableLocalityLoadBalance, bool lockAware)
  : clientInfo(clientInfo), masterProxiesChangeTrigger(), cluster(cluster), clientInfoMonitor(clientInfoMonitor),
    dbName(dbName), dbId(dbId), transactionReadVersions(0), transactionLogicalReads(0), transactionPhysicalReads(0),
    transactionCommittedMutations(0), transactionCommittedMutationBytes(0), transactionsCommitStarted(0),
    transactionsCommitCompleted(0), transactionsTooOld(0), transactionsFutureVersions(0), transactionsNotCommitted(0),
    transactionsMaybeCommitted(0), transactionsResourceConstrained(0), taskID(taskID), outstandingWatches(0),
    maxOutstandingWatches(CLIENT_KNOBS->DEFAULT_MAX_OUTSTANDING_WATCHES), clientLocality(clientLocality),
    enableLocalityLoadBalance(enableLocalityLoadBalance), lockAware(lockAware), latencies(1000), readLatencies(1000),
...
ACTOR static Future<Void> monitorClientInfo(Reference<AsyncVar<Optional<ClusterInterface>>> clusterInterface,
                                            Standalone<StringRef> dbName, Reference<ClusterConnectionFile> ccf,
		...
		loop {
			...
			if (ccf && !ccf->fileContentsUpToDate(fileConnectionString)) {
				...
				if (ccf->canGetFilename()) {
					TraceEvent(SevWarnAlways, "IncorrectClusterFileContents")
			...
			choose {
				when(ClientDBInfo ni =
				         wait(clusterInterface->get().present()
				                  ? brokenPromiseToNever(clusterInterface->get().get().openDatabase.getReply(req))
				                  : Never())) {
...
Future<Database> DatabaseContext::createDatabase(Reference<AsyncVar<Optional<ClusterInterface>>> clusterInterface,
                                                 Reference<Cluster> cluster, Standalone<StringRef> dbName,
                                                 LocalityData const& clientLocality) {
	if (dbName != LiteralStringRef("DB")) {
		return invalid_database_name(); // we no longer offer multi-database support, so all databases *must* be named
		                                // this
	} else {
		Reference<AsyncVar<ClientDBInfo>> info(new AsyncVar<ClientDBInfo>);
		Future<Void> monitor =
		    monitorClientInfo(clusterInterface, dbName,
		                      cluster ? cluster->getConnectionFile() : Reference<ClusterConnectionFile>(), info);

		return std::move(Database(new DatabaseContext(info, cluster, monitor, dbName, LiteralStringRef(""),
		                                              TaskDefaultEndpoint, clientLocality, true, false)));
	}
}

Database DatabaseContext::create(Reference<AsyncVar<ClientDBInfo>> info, Future<Void> dependency,
                                 LocalityData clientLocality, bool enableLocalityLoadBalance, int taskID,
                                 bool lockAware) {
	return Database(new DatabaseContext(info, Reference<Cluster>(), dependency, LiteralStringRef("DB"),
...
bool DatabaseContext::getCachedLocations(const KeyRangeRef& range,
                                         vector<std::pair<KeyRange, Reference<LocationInfo>>>& result, int limit,
                                         bool reverse) {
	result.clear();
...
uint64_t extractHexOption(StringRef value) {
	...
	if (*end) throw invalid_option_value();
	return id;
}
...
void DatabaseContext::addWatch() {
	if (outstandingWatches >= maxOutstandingWatches) throw too_many_watches();
...
void setNetworkOption(FDBNetworkOptions::Option option, Optional<StringRef> value) {
	...
	case FDBNetworkOptions::TRACE_LOG_GROUP:
		if (value.present()) networkOptions.traceLogGroup = value.get().toString();
		break;
	...
	case FDBNetworkOptions::TLS_VERIFY_PEERS:
		...
		try {
			...
			TraceEvent(SevWarnAlways, "TLSValidationSetError").error(e).detail("Input", value.get().toString());
			throw invalid_option_value();
		}
		break;
	...
	case FDBNetworkOptions::SUPPORTED_CLIENT_VERSIONS: {
		...
		while (nextIndex != versionString.npos) {
			...
			networkOptions.supportedVersions.push_back_deep(
			    networkOptions.supportedVersions.arena(),
			    ClientVersionRef(versionString.substr(index, nextIndex - index)));
			index = nextIndex + 1;
		}
...
void runNetwork() {
	...
	if (networkOptions.traceDirectory.present()) systemMonitor();
}
...
void GetRangeLimits::decrement(KeyValueRef const& data) {
	...
	if (rows != CLIENT_KNOBS->ROW_LIMIT_UNLIMITED) rows--;
	if (bytes != CLIENT_KNOBS->BYTE_LIMIT_UNLIMITED) bytes = std::max(0, bytes - (int)8 - (int)data.expectedSize());
}
...
AddressExclusion AddressExclusion::parse(StringRef const& key) {
	...
	if (!s.size()) return AddressExclusion(ip);
...
Future<Standalone<RangeResultRef>> getRange(Database const& cx, Future<Version> const& fVersion,
                                            KeySelector const& begin, KeySelector const& end,
                                            GetRangeLimits const& limits, bool const& reverse,
                                            TransactionInfo const& info);

Future<Optional<Value>> getValue(Future<Version> const& version, Key const& key, Database const& cx,
                                 TransactionInfo const& info, Reference<TransactionLogInfo> const& trLogInfo);

ACTOR Future<Optional<StorageServerInterface>> fetchServerInterface(Database cx, TransactionInfo info, UID id,
	...
	loop {
		choose {
			...
			when(GetKeyServerLocationsReply rep = wait(
			         loadBalance(cx->getMasterProxies(), &MasterProxyInterface::getKeyServersLocations,
	...
	loop {
		choose {
			...
			when(GetKeyServerLocationsReply _rep =
			         wait(loadBalance(cx->getMasterProxies(), &MasterProxyInterface::getKeyServersLocations,
				...
				for (; shard < rep.results.size(); shard++) {
					...
					results.push_back(
					    make_pair(rep.results[shard].first & keys,
...
ACTOR Future<Version> waitForCommittedVersion(Database cx, Version version) {
	try {
		loop {
			choose {
				when(wait(cx->onMasterProxiesChanged())) {}
				when(GetReadVersionReply v = wait(loadBalance(
				         cx->getMasterProxies(), &MasterProxyInterface::getConsistentReadVersion,
				         GetReadVersionRequest(0, GetReadVersionRequest::PRIORITY_SYSTEM_IMMEDIATE), cx->taskID))) {
					if (v.version >= version) return v.version;
...
Future<Void> readVersionBatcher(
    DatabaseContext* const& cx,
    FutureStream<std::pair<Promise<GetReadVersionReply>, Optional<UID>>> const& versionStream, uint32_t const& flags);

ACTOR Future<Void> watchValue(Future<Version> version, Key key, Optional<Value> value, Database cx,
                              int readVersionFlags, TransactionInfo info) {
	state Version ver = wait(version);
	validateVersion(ver);
	...
	loop {
		...
		try {
			...
			if (info.debugID.present()) {
				...
				g_traceBatch.addEvent("WatchValueDebug", watchValueID.get().first(),
				                      "NativeAPI.watchValue.Before"); //.detail("TaskID", g_network->getCurrentTask());
			}
			state Version resp =
			    wait(loadBalance(ssi.second, &StorageServerInterface::watchValue,
			                     WatchValueRequest(key, value, ver, watchValueID), TaskDefaultPromiseEndpoint));
			if (info.debugID.present()) {
				g_traceBatch.addEvent("WatchValueDebug", watchValueID.get().first(),
			...
			if (v - resp < 50000000) // False if there is a master failure between getting the response and getting the
			                         // committed version, Dependent on SERVER_KNOBS->MAX_VERSIONS_IN_FLIGHT
				return Void();
			ver = v;
		} catch (Error& e) {
			if (e.code() == error_code_wrong_shard_server || e.code() == error_code_all_alternatives_failed) {
...
ACTOR Future<Standalone<RangeResultRef>> getExactRange(Database cx, Version version, KeyRange keys,
	...
	loop {
		...
		loop {
			...
			try {
				...
				++cx->transactionPhysicalReads;
				GetKeyValuesReply rep = wait(loadBalance(locations[shard].second, &StorageServerInterface::getKeyValues,
				                                         req, TaskDefaultPromiseEndpoint, false,
				                                         cx->enableLocalityLoadBalance ? &cx->queueModel : NULL));
				if (info.debugID.present())
					g_traceBatch.addEvent("TransactionDebug", info.debugID.get().first(),
...
ACTOR Future<Standalone<RangeResultRef>> getRangeFallback(Database cx, Version version, KeySelector begin,
	...
	if (b == allKeys.begin && ((reverse && !r.more) || !reverse)) r.readToBegin = true;
	if (e == allKeys.end && ((!reverse && !r.more) || reverse)) r.readThroughEnd = true;
...
void getRangeFinished(Reference<TransactionLogInfo> trLogInfo, double startTime, KeySelector begin, KeySelector end,
                      bool snapshot, Promise<std::pair<Key, Key>> conflictRange, bool reverse,
                      Standalone<RangeResultRef> result) {
	if (trLogInfo) {
		int rangeSize = 0;
		for (const KeyValueRef& kv : result.contents()) rangeSize += kv.key.size() + kv.value.size();
		trLogInfo->addLog(
		    FdbClientLogEvents::EventGetRange(startTime, now() - startTime, rangeSize, begin.getKey(), end.getKey()));
	}
	...
	try {
		...
		loop {
			if (end.getKey() == allKeys.begin && (end.offset < 1 || end.isFirstGreaterOrEqual())) {
				getRangeFinished(trLogInfo, startTime, originalBegin, originalEnd, snapshot, conflictRange, reverse,
				                 output);
				return output;
			}
			...
			try {
				...
				if (finished && !output.size()) {
					...
					if (BUGGIFY && limits.hasByteLimit() && output.size() > std::max(1, originalLimits.minRows)) {
						output.more = true;
						output.resize(output.arena(),
						              g_random->randomInt(std::max(1, originalLimits.minRows), output.size()));
...
Future<Standalone<RangeResultRef>> getRange(Database const& cx, Future<Version> const& fVersion,
                                            ...
                                            GetRangeLimits const& limits, bool const& reverse,
                                            TransactionInfo const& info) {
	return getRange(cx, Reference<TransactionLogInfo>(), fVersion, begin, end, limits, Promise<std::pair<Key, Key>>(),
	                true, reverse, info);
}

Transaction::Transaction(Database const& cx)
  : cx(cx), info(cx->taskID), backoff(CLIENT_KNOBS->DEFAULT_BACKOFF), committedVersion(invalidVersion),
    versionstampPromise(Promise<Standalone<StringRef>>()), numErrors(0),
    trLogInfo(createTrLogInfoProbabilistically(cx)) {
	setPriority(GetReadVersionRequest::PRIORITY_DEFAULT);
	if (cx->lockAware) options.lockAware = true;
}
...
void Transaction::setVersion(Version v) {
	startTime = now();
	if (readVersion.isValid()) throw read_version_already_set();
	if (v <= 0) throw version_invalid();
	readVersion = v;
}
...
Future<Optional<Value>> Transaction::get(const Key& key, bool snapshot) {
	...
	if (!snapshot) tr.transaction.read_conflict_ranges.push_back(tr.arena, singleKeyRange(key, tr.arena));
...
ACTOR Future<Standalone<VectorRef<const char*>>> getAddressesForKeyActor(Key key, Future<Version> ver, Database cx,
	...
	Future<Standalone<RangeResultRef>> futureServerUids =
	    getRange(cx, ver, lastLessOrEqual(ksKey), firstGreaterThan(ksKey), GetRangeLimits(1), false, info);
	...
	Optional<vector<StorageServerInterface>> serverInterfaces =
	    wait(transactionalGetServerInterfaces(ver, cx, info, src));
...
ACTOR Future<Key> getKeyAndConflictRange(Database cx, KeySelector k, Future<Version> version,
                                         Promise<std::pair<Key, Key>> conflictRange, TransactionInfo info) {
	try {
...
Future<Key> Transaction::getKey(const KeySelector& key, bool snapshot) {
	++cx->transactionLogicalReads;
	if (snapshot) return ::getKey(cx, key, getReadVersion(), info);
...
Future<Standalone<RangeResultRef>> Transaction::getRange(const KeySelector& begin, const KeySelector& end,
                                                         GetRangeLimits limits, bool snapshot, bool reverse) {
	++cx->transactionLogicalReads;

	if (limits.isReached()) return Standalone<RangeResultRef>();
...
Future<Standalone<RangeResultRef>> Transaction::getRange(const KeySelector& begin, const KeySelector& end, int limit,
...
void Transaction::atomicOp(const KeyRef& key, const ValueRef& operand, MutationRef::Type operationType,
	...
	if (addConflictRange) t.write_conflict_ranges.push_back(req.arena, r);
...
int Transaction::apiVersionAtLeast(int minVersion) const {
	if (cx->cluster) return cx->cluster->apiVersionAtLeast(minVersion);
...
ACTOR void checkWrites(Database cx, Future<Void> committed, Promise<Void> outCommitted, CommitTransactionRequest req,
                       Transaction* checkTr) {
				...
				if (m.cleared) {
					...
					if (shouldBeEmpty.size()) {
						TraceEvent(SevError, "CheckWritesFailed")
						    .detail("Class", "Clear")
						    .detail("KeyBegin", printable(it->range().begin).c_str())
					...
					if (!val.present() || val.get() != m.setValue) {
						TraceEvent evt = TraceEvent(SevError, "CheckWritesFailed")
						                     .detail("Class", "Set")
						                     .detail("Key", printable(it->range().begin).c_str())
...
ACTOR static Future<Void> commitDummyTransaction(Database cx, KeyRange range, TransactionInfo info,
	...
	loop {
		try {
			...
			TraceEvent("CommitDummyTransactionError")
			    .error(e, true)
			    .detail("Key", printable(range.begin))
...
void Transaction::cancelWatches(Error const& e) {
	for (int i = 0; i < watches.size(); ++i)
		if (!watches[i]->onChangeTrigger.isSet()) watches[i]->onChangeTrigger.sendError(e);
...
void Transaction::setupWatches() {
	try {
		...
		for (int i = 0; i < watches.size(); ++i)
			watches[i]->setWatch(
			    watchValue(watchVersion, watches[i]->key, watches[i]->value, cx, options.getReadVersionFlags, info));
...
ACTOR static Future<Void> tryCommit(Database cx, Reference<TransactionLogInfo> trLogInfo, CommitTransactionRequest req,
	...
	if (info.debugID.present()) TraceEvent(interval.begin()).detail("Parent", info.debugID.get());
	...
	try {
		...
		if (options.commitOnFirstProxy) {
			...
			reply = loadBalance(cx->getMasterProxies(), &MasterProxyInterface::commit, req, TaskDefaultPromiseEndpoint,
			                    true);
		}
			...
			when(CommitID ci = wait(reply)) {
				...
				if (v != invalidVersion) {
					...
					*pCommittedVersion = v;
					...
					if (trLogInfo)
						trLogInfo->addLog(
						    FdbClientLogEvents::EventCommit(startTime, latency, req.transaction.mutations.size(),
						                                    req.transaction.mutations.expectedSize(), req));
					return Void();
				} else {
					if (info.debugID.present()) TraceEvent(interval.end()).detail("Conflict", 1);
...
Future<Void> Transaction::commitMutations() {
	try {
		...
		if (options.readOnly) return transaction_read_only();
		...
		size_t transactionSize = tr.transaction.mutations.expectedSize() +
		                         tr.transaction.read_conflict_ranges.expectedSize() +
		...
		if (transactionSize > (options.customTransactionSizeLimit == 0 ? (uint64_t)CLIENT_KNOBS->TRANSACTION_SIZE_LIMIT
		                                                               : (uint64_t)options.customTransactionSizeLimit))
		...
		if (isCheckingWrites) {
			...
			tr.transaction.read_conflict_ranges.append(tr.arena, tr.transaction.write_conflict_ranges.begin(),
		...
		if (options.debugDump) {
			...
			for (auto i = tr.transaction.mutations.begin(); i != tr.transaction.mutations.end(); ++i)
				TraceEvent("TransactionMutation", u)
				    .detail("T", i->type)
				    .detail("P1", printable(i->param1))
				    .detail("P2", printable(i->param2));
		}
...
void Transaction::setPriority(uint32_t priorityFlag) {
	options.getReadVersionFlags =
	    (options.getReadVersionFlags & ~GetReadVersionRequest::FLAG_PRIORITY_MASK) | priorityFlag;
}
	...
	case FDBTransactionOptions::TRANSACTION_LOGGING_ENABLE:
		...
		if (trLogInfo) {
			if (!trLogInfo->identifier.present()) {
				...
				TraceEvent(SevWarn, "CannotChangeTransactionLoggingIdentifier")
				    .detail("PreviousIdentifier", trLogInfo->identifier.get())
				    .detail("NewIdentifier", printable(value.get()));
				throw client_invalid_operation();
			}
		} else {
...
ACTOR Future<GetReadVersionReply> getConsistentReadVersion(DatabaseContext* cx, uint32_t transactionCount,
                                                           uint32_t flags, Optional<UID> debugID) {
	try {
		if (debugID.present())
		...
		loop {
			...
			choose {
				when(wait(cx->onMasterProxiesChanged())) {}
				when(GetReadVersionReply v = wait(loadBalance(
				         cx->getMasterProxies(), &MasterProxyInterface::getConsistentReadVersion, req, cx->taskID))) {
					if (debugID.present())
						g_traceBatch.addEvent("TransactionDebug", debugID.get().first(),
		...
		if (e.code() != error_code_broken_promise) TraceEvent(SevError, "GetConsistentReadVersionError").error(e);
		throw;
	}
}
...
ACTOR Future<Void> readVersionBatcher(
	...
	loop {
		...
		if (send_batch) {
			...
			Future<Void> batch = broadcast(getConsistentReadVersion(cx, count, flags, std::move(debugID)),
...
ACTOR Future<Version> extractReadVersion(DatabaseContext* cx, Reference<TransactionLogInfo> trLogInfo,
                                         Future<GetReadVersionReply> f, bool lockAware, double startTime) {
	GetReadVersionReply rep = wait(f);
	double latency = now() - startTime;
	cx->GRVLatencies.addSample(latency);
	if (trLogInfo) trLogInfo->addLog(FdbClientLogEvents::EventGetVersion(startTime, latency));
	if (rep.locked && !lockAware) throw database_locked();
...
Future<Void> Transaction::onError(Error const& e) {
	...
	if (e.code() == error_code_not_committed || e.code() == error_code_commit_unknown_result ||
	    e.code() == error_code_database_locked || e.code() == error_code_proxy_memory_limit_exceeded) {
		if (e.code() == error_code_not_committed) cx->transactionsNotCommitted++;
		if (e.code() == error_code_commit_unknown_result) cx->transactionsMaybeCommitted++;
		if (e.code() == error_code_proxy_memory_limit_exceeded) cx->transactionsResourceConstrained++;
...
ACTOR Future<Void> trackBoundedStorageMetrics(KeyRange keys, Reference<LocationInfo> location, StorageMetrics x,
                                              StorageMetrics halfError, PromiseStream<StorageMetrics> deltaStream) {
	try {
		loop {
...
ACTOR Future<StorageMetrics> waitStorageMetricsMultipleLocations(
	...
	for (int i = 0; i < nLocs; i++)
		wx[i] = trackBoundedStorageMetrics(locations[i].first, locations[i].second, fx[i].get(), halfErrorPerMachine,
		                                   deltas);
...
ACTOR Future<StorageMetrics> waitStorageMetrics(Database cx, KeyRange keys, StorageMetrics min, StorageMetrics max,
                                                StorageMetrics permittedError, int shardLimit) {
	state int tooManyShardsCount = 0;
	loop {
		state vector<pair<KeyRange, Reference<LocationInfo>>> locations = wait(getKeyRangeLocations(
		    cx, keys, shardLimit, false, &StorageServerInterface::waitMetrics, TransactionInfo(TaskDataDistribution)));

		if (locations.size() == shardLimit) {
			TraceEvent(!g_network->isSimulated() && ++tooManyShardsCount >= 15 ? SevWarnAlways : SevWarn,
			...
			try {
				if (locations.size() > 1) {
					...
					StorageMetrics x = wait(loadBalance(locations[0].second, &StorageServerInterface::waitMetrics, req,
...
Future<StorageMetrics> Transaction::waitStorageMetrics(KeyRange const& keys, StorageMetrics const& min,
...
ACTOR Future<Standalone<VectorRef<KeyRef>>> splitStorageMetrics(Database cx, KeyRange keys, StorageMetrics limit,
                                                                StorageMetrics estimated) {
	loop {
		state vector<pair<KeyRange, Reference<LocationInfo>>> locations =
		    wait(getKeyRangeLocations(cx, keys, CLIENT_KNOBS->STORAGE_METRICS_SHARD_LIMIT, false,
		                              &StorageServerInterface::splitMetrics, TransactionInfo(TaskDataDistribution)));
		...
		if (locations.size() == CLIENT_KNOBS->STORAGE_METRICS_SHARD_LIMIT) {
			...
			try {
				...
				for (; i < locations.size(); i++) {
					...
					SplitMetricsReply res = wait(loadBalance(locations[i].second, &StorageServerInterface::splitMetrics,
					                                         req, TaskDataDistribution));
					if (res.splits.size() &&
...
Reference<TransactionLogInfo> Transaction::createTrLogInfoProbabilistically(const Database& cx) {
	...
	if (((networkOptions.logClientInfo.present() && networkOptions.logClientInfo.get()) || BUGGIFY) &&
	    g_random->random01() < clientSamplingProbability &&
