		...
		for (StatusObjectReader dc : regionArray) {
			...
			if (dc.tryGet("satellite_redundancy_mode", satelliteReplication)) {
				if (satelliteReplication == "one_satellite_single") {
					...
					info.satelliteTLogWriteAntiQuorum = 0;
					info.satelliteTLogPolicy = IRepPolicyRef(new PolicyAcross(
					    2, "dcid", IRepPolicyRef(new PolicyAcross(2, "zoneid", IRepPolicyRef(new PolicyOne())))));
...
void DatabaseConfiguration::setDefaultReplicationPolicy() {
	...
	if (remoteTLogReplicationFactor > 0 && !remoteTLogPolicy) {
		remoteTLogPolicy =
		    IRepPolicyRef(new PolicyAcross(remoteTLogReplicationFactor, "zoneid", IRepPolicyRef(new PolicyOne())));
	}
	for (auto& r : regions) {
		if (r.satelliteTLogReplicationFactor > 0 && !r.satelliteTLogPolicy) {
			r.satelliteTLogPolicy = IRepPolicyRef(
			    new PolicyAcross(r.satelliteTLogReplicationFactor, "zoneid", IRepPolicyRef(new PolicyOne())));
		}
		if (r.satelliteTLogReplicationFactorFallback > 0 && !r.satelliteTLogPolicyFallback) {
			r.satelliteTLogPolicyFallback = IRepPolicyRef(
			    new PolicyAcross(r.satelliteTLogReplicationFactorFallback, "zoneid", IRepPolicyRef(new PolicyOne())));
		}
	}
}

bool DatabaseConfiguration::isValid() const {
	if (!(initialized && tLogWriteAntiQuorum >= 0 && tLogReplicationFactor >= 1 && storageTeamSize >= 1 &&
	      getDesiredProxies() >= 1 && getDesiredLogs() >= 1 && getDesiredResolvers() >= 1 &&
	      tLogDataStoreType != KeyValueStoreType::END && storageServerStoreType != KeyValueStoreType::END &&
	      autoMasterProxyCount >= 1 && autoResolverCount >= 1 && autoDesiredTLogCount >= 1 && storagePolicy &&
...
StatusObject DatabaseConfiguration::toJSON(bool noPolicies) const {
	...
	if (initialized) {
		...
		} else if (tLogDataStoreType == KeyValueStoreType::MEMORY &&
			...
			for (auto& r : regions) {
				...
				if (r.satelliteTLogReplicationFactor == 1 && r.satelliteTLogUsableDcs == 1 &&
					...
					regionObj["satellite_redundancy_mode"] = "one_satellite_triple";
				           ...
				           r.satelliteTLogReplicationFactorFallback == 2 &&
				           r.satelliteTLogWriteAntiQuorumFallback == 0) {
