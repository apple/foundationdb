...
ACTOR Future<Optional<CoordinatorsResult>> changeQuorumChecker(Transaction* tr,
	...
	if (!desiredCoordinators.size()) {
		std::vector<NetworkAddress> _desiredCoordinators = wait(change->getDesiredCoordinators(
		    tr,
		    oldCoordinators,
		    Reference<ClusterConnectionMemoryRecord>(new ClusterConnectionMemoryRecord(oldClusterConnectionString)),
		    result));
	...
	if (newName.empty())
		newName = oldClusterConnectionString.clusterKeyName().toString();

	if (oldCoordinators == desiredCoordinators && oldClusterConnectionString.clusterKeyName() == newName)
	...
	ClientCoordinators coord(
	    Reference<ClusterConnectionMemoryRecord>(new ClusterConnectionMemoryRecord(newClusterConnectionString)));
