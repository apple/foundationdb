...
ClusterConnectionFile::ClusterConnectionFile(std::string const& filename) {
	if (!fileExists(filename)) throw no_cluster_file_found();
...
ClusterConnectionString::ClusterConnectionString(std::string const& connectionString) {
	...
	for (auto const& server : coord) {
		if (server.isTLS() != isTLS) throw connection_string_invalid();
	}
...
ACTOR Future<Void> monitorNominee(Key key, ClientLeaderRegInterface coord, AsyncTrigger* nomineeChange,
                                  Optional<LeaderInfo>* info, int generation) {
	loop {
		...
		TraceEvent("GetLeaderReply")
		    .detail("Coordinator", coord.getLeader.getEndpoint().address)
...
struct MonitorLeaderInfo {
	...
	explicit MonitorLeaderInfo(Reference<ClusterConnectionFile> intermediateConnFile)
	  : intermediateConnFile(intermediateConnFile), hasConnected(false), generation(0) {}
};
