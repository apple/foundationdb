...
class PaxosConfigTransactionImpl {
	...
	ACTOR static Future<ValueResult> get(PaxosConfigTransactionImpl* self, Key key) {
...
public:
	...
	Future<ValueResult> get(Key const& key) { return get(this, key); }
...
Future<ValueResult> PaxosConfigTransaction::get(Key const& key, Snapshot) {
	return impl->get(key);
}
