...
void ClientKnobs::initialize(Randomize randomize) {
	...
	// Timeout for waitStorageMetrics when getting persistent wrong_shard_server errors.
	// After this time, return an estimate so DD can make progress on shards with stale keyServers entries.
	// In simulation, code uses 1 hour (3600s) instead of this value.
	init( STORAGE_METRICS_WRONG_SHARD_TIMEOUT,    900.0 ); // 15 minutes
