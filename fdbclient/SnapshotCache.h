...
private:
	struct Entry {
		...
		bool operator<(StringRef const& r) const { return beginKey < r; }
...
public:
	...
	SnapshotCache& operator=(SnapshotCache&& r) noexcept(true) {
	...
	void dump() {
		for (auto it = entries.begin(); it != entries.end(); ++it) {
			TraceEvent("CacheDump")
			    .detail("Begin", printable(it->beginKey))
			    .detail("End", printable(it->endKey.toStandaloneStringRef()))
			    .detail("Values", printable(it->values));
		}
	}
