...
ACTOR Future<Void> failureMonitorClientLoop(SimpleFailureMonitor* monitor,
	...
	// The cluster controller's addresses (controller.failureMonitoring.getEndpoint().addresses) are treated specially
				...
				when(FailureMonitoringReply reply = wait(request)) {
					...
					if (reply.allOthersFailed) {
						...
						if (controller.failureMonitoring.getEndpoint().addresses.secondaryAddress.present()) {
							fmState->knownAddrs.erase(
							    controller.failureMonitoring.getEndpoint().addresses.secondaryAddress.get());
						}
