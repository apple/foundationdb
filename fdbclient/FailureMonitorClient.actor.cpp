...
ACTOR Future<Void> failureMonitorClientLoop(SimpleFailureMonitor* monitor, ClusterInterface controller,
	...
	// The cluster controller's address (controller.failureMonitoring.getEndpoint().address) is treated specially
	// because we can declare that it is down independently of the response from the cluster controller. It still needs
	// to be in knownAddrs in case the cluster controller changes, so the next cluster controller resets its state
			...
			choose {
				when(FailureMonitoringReply reply = wait(request)) {
					...
					for (int c = 0; c < reply.changes.size(); c++) {
						...
						// g_network->getLocalAddress().toString().c_str(), reply.changes[c].address.toString().c_str(),
						// reply.changes[c].status.failed ? "Failed" : "OK");
						...
						ASSERT(reply.changes[c].address != controller.failureMonitoring.getEndpoint().address ||
						       !reply.changes[c].status.failed);
					}
					before = now();
				...
				when(wait(nextRequest)) {
					...
					if (elapsed > slowThreshold && g_random->random01() < elapsed / warnAlwaysThreshold) {
						TraceEvent(elapsed > warnAlwaysThreshold ? SevWarnAlways : SevWarn, "FailureMonitorClientSlow")
...
ACTOR Future<Void> failureMonitorClient(Reference<AsyncVar<Optional<struct ClusterInterface>>> ci, bool trackMyStatus) {
	...
	state Reference<FailureMonitorClientState> fmState =
	    Reference<FailureMonitorClientState>(new FailureMonitorClientState());

	loop {
		state Future<Void> client =
		    ci->get().present() ? failureMonitorClientLoop(monitor, ci->get().get(), fmState, trackMyStatus) : Void();
		wait(ci->onChange());
	}
}
