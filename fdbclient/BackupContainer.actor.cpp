...
Reference<IBackupContainer> IBackupContainer::openContainer(const std::string& url,
	...
	//
	// Problem: IBackupContainer objects contain S3BlobStoreEndpoint objects, which contain connection pools.
	...
	//
	// Solution: Disable caching for blobstore:// URLs in simulation so each process creates its own
