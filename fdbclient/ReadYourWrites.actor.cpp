...
ReadYourWritesTransaction::ReadYourWritesTransaction(Database const& cx)
  : cache(&arena), writes(&arena), tr(cx), retries(0), creationTime(now()), commitStarted(false), options(tr) {}
...
Optional<Value> getValueFromJSON(StatusObject statusObj) {
	try {
		Value output = StringRef(
		    json_spirit::write_string(json_spirit::mValue(statusObj), json_spirit::Output_options::raw_utf8).c_str());
		return output;
...
Future<Optional<Value>> ReadYourWritesTransaction::get(const Key& key, bool snapshot) {
	...
	if (key == LiteralStringRef("\xff\xff/cluster_file_path")) {
		try {
			if (tr.getDatabase().getPtr() && tr.getDatabase()->cluster &&
			    tr.getDatabase()->cluster->getConnectionFile()) {
	...
	if (key >= getMaxReadKey()) return key_outside_legal_range();
...
void ReadYourWritesTransaction::updateConflictMap(KeyRangeRef const& keys, WriteMap::iterator& it) {
	...
	for (; it.beginKey() < keys.end; ++it) {
		if (it.is_unmodified_range() || (it.is_operation() && !it.is_independent())) {
			...
			if (!insert_range.empty()) readConflicts.insert(insert_range, true);
		}
	}
}
...
void ReadYourWritesTransaction::atomicOp(const KeyRef& key, const ValueRef& operand, uint32_t operationType) {
	...
	if (operationType == MutationRef::SetVersionstampedKey) {
		KeyRangeRef range =
		    getVersionstampKeyRange(arena, k, getMaxReadKey()); // this does validation of the key and needs to be
		                                                        // performed before the readYourWritesDisabled path
		if (!options.readYourWritesDisabled) {
...
Future<Void> ReadYourWritesTransaction::watch(const Key& key) {
	...
	if (key >= allKeys.end || (key >= getMaxReadKey() && tr.apiVersionAtLeast(300))) return key_outside_legal_range();
...
void ReadYourWritesTransaction::setOption(FDBTransactionOptions::Option option, Optional<StringRef> value) {
	...
	case FDBTransactionOptions::TIMEOUT:
		...
		timeoutActor = timebomb(options.timeoutInSeconds == 0.0 ? options.timeoutInSeconds
		                                                        : options.timeoutInSeconds + creationTime,
		                        resetPromise);
		break;
...
ReadYourWritesTransaction::ReadYourWritesTransaction(ReadYourWritesTransaction&& r) noexcept(true)
  : cache(std::move(r.cache)), writes(std::move(r.writes)), arena(std::move(r.arena)), reading(std::move(r.reading)),
    retries(r.retries), creationTime(r.creationTime), deferred_error(std::move(r.deferred_error)),
    timeoutActor(std::move(r.timeoutActor)), resetPromise(std::move(r.resetPromise)), commitStarted(r.commitStarted),
    options(r.options), transactionDebugInfo(r.transactionDebugInfo) {
	cache.arena = &arena;
	writes.arena = &arena;
	tr = std::move(r.tr);
	readConflicts = std::move(r.readConflicts);
