...
void ThreadSafeCluster::setOption(FDBClusterOptions::Option option, Optional<StringRef> value) {
	...
	onMainThreadVoid([cluster, option, passValue]() { cluster->setOption(option, passValue.contents()); },
	                 &cluster->deferred_error);
}
...
ThreadSafeTransaction::ThreadSafeTransaction(ThreadSafeDatabase* cx) {
	...
	onMainThreadVoid(
	    [tr, db]() {
		    db->addref();
		    new (tr) ReadYourWritesTransaction(Database(db));
	    },
	    NULL);
}
