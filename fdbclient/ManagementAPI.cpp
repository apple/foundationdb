...
AsyncResult<std::vector<BulkLoadJobState>> getBulkLoadJobFromHistory(Database cx, bool lockAware) {
	...
	while (true) {
		Error err;
		try {
			// The history lives in the system key space, and a caller may be reading it while the
			// database is locked -- a restore holds the lock while deciding whether its bulkload job
			// succeeded. Without these the read fails every attempt and this loop never terminates.
			tr.setOption(FDBTransactionOptions::READ_SYSTEM_KEYS);
			if (lockAware) {
				tr.setOption(FDBTransactionOptions::LOCK_AWARE);
			}
