...
namespace fileBackup {
	...
	struct BackupLogsDispatchTask : BackupTaskFuncBase {
		...
		ACTOR static Future<Void> _finish(Reference<ReadYourWritesTransaction> tr, Reference<TaskBucket> taskBucket, Reference<FutureBucket> futureBucket, Reference<Task> task) {
			...
			Key _ = wait(BackupLogsDispatchTask::addTask(tr, taskBucket, task, priority, endVersion, TaskCompletionKey::signal(onDone), logDispatchBatchFuture));
		...
		ACTOR static Future<Key> addTask(Reference<ReadYourWritesTransaction> tr, Reference<TaskBucket> taskBucket, Reference<Task> parentTask, int priority, Version beginVersion, TaskCompletionKey completionKey, Reference<TaskFuture> waitFor = Reference<TaskFuture>()) {
	...
	struct StartFullBackupTaskFunc : BackupTaskFuncBase {
		...
		ACTOR static Future<Void> _finish(Reference<ReadYourWritesTransaction> tr, Reference<TaskBucket> taskBucket, Reference<FutureBucket> futureBucket, Reference<Task> task) {
			...
			Key _ = wait(BackupLogsDispatchTask::addTask(tr, taskBucket, task, 1, beginVersion, TaskCompletionKey::joinWith(backupFinished)));
