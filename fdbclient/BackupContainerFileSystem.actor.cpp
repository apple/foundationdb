...
ACTOR static Future<Void> writeKeyspaceSnapshotFile_impl(Reference<BackupContainerFileSystem> bc,
	...
	for (i = 0; i < fileNames.size(); ++i) {
		...
		if (BackupContainerFileSystem::pathToRangeFile(rf, f, 0)) {
...
ACTOR static Future<BackupDescription> describeBackup_impl(Reference<BackupContainerFileSystem> bc, bool deepScan,
	...
	if (logStartVersionOverride != invalidVersion && logStartVersionOverride < 0) {
		...
		logStartVersionOverride = BackupContainerFileSystem::resolveRelativeVersion(
	...
	if (plogs.size() > 0) {
		...
		desc.partitioned =
		    metaLogType.present() && metaLogType.get() == BackupContainerFileSystem::PARTITIONED_MUTATION_LOG;
	}
	...
	if (!logs.empty()) {
		...
		if (desc.partitioned) {
			BackupContainerFileSystem::updatePartitionedLogsContinuousEnd(&desc, logs, scanBegin, scanEnd);
		} else {
			Version& end = desc.contiguousLogEnd.get();
			BackupContainerFileSystem::computeRestoreEndVersion(logs, nullptr, &end,
	...
	if (logStartVersionOverride == invalidVersion) {
		...
		try {
			...
			if (!metaLogType.present()) {
				updates = updates &&
				          bc->logType().set(desc.partitioned ? BackupContainerFileSystem::PARTITIONED_MUTATION_LOG
				                                             : BackupContainerFileSystem::NON_PARTITIONED_MUTATION_LOG);
			}
...
ACTOR static Future<Void> expireData_impl(Reference<BackupContainerFileSystem> bc, Version expireEndVersion, bool force,
	...
	expireEndVersion = BackupContainerFileSystem::resolveRelativeVersion(desc.maxLogEnd, expireEndVersion,
	                                                                     "ExpireEndVersion", invalid_option_value());
	restorableBeginVersion = BackupContainerFileSystem::resolveRelativeVersion(
...
ACTOR static Future<Optional<RestorableFileSet>> getRestoreSet_impl(Reference<BackupContainerFileSystem> bc,
	...
	if (logsOnly) {
		...
		if (!logFiles.empty()) {
			return BackupContainerFileSystem::getRestoreSetFromLogs(logFiles, targetVersion, restorableSet);
		}
	}
	...
	for (; i >= 0; i--) {
		...
		if (plogs.size() > 0) {
			...
			std::vector<LogFile> filtered = BackupContainerFileSystem::filterDuplicates(logs);
			...
			if (BackupContainerFileSystem::isPartitionedLogsContinuous(restorable.logs, minKeyRangeVersion,
			                                                           restorable.targetVersion)) {
		...
		if (!logs.empty() && logs.front().beginVersion <= minKeyRangeVersion) {
			return BackupContainerFileSystem::getRestoreSetFromLogs(logs, targetVersion, restorable);
		}
	}
	return Optional<RestorableFileSet>();
}

std::string BackupContainerFileSystem::versionFolderString(Version v, int smallestBucket) {
...
std::string BackupContainerFileSystem::cleanFolderString(std::string f) {
	f.erase(std::remove(f.begin(), f.end(), '/'), f.end());
	return f;
}

std::string BackupContainerFileSystem::old_rangeVersionFolderString(Version v) {
	return format("ranges/%s/", versionFolderString(v, 8).c_str());
}

std::string BackupContainerFileSystem::snapshotFolderString(Version snapshotBeginVersion) {
...
std::string BackupContainerFileSystem::logVersionFolderString(Version v, bool partitioned) {
...
bool BackupContainerFileSystem::pathToLogFile(LogFile& out, std::string path, int64_t size) {
	...
	f.fileSize = size;
	int len;
	if (sscanf(name.c_str(), "log,%" SCNd64 ",%" SCNd64 ",%*[^,],%u%n", &f.beginVersion, &f.endVersion, &f.blockSize,
	           &len) == 3 &&
	    len == name.size()) {
		out = f;
		return true;
	} else if (sscanf(name.c_str(), "log,%" SCNd64 ",%" SCNd64 ",%*[^,],%d-of-%d,%u%n", &f.beginVersion, &f.endVersion,
	                  &f.tagId, &f.totalTags, &f.blockSize, &len) == 5 &&
...
Future<Void> BackupContainerFileSystem::writeKeyspaceSnapshotFile(const std::vector<std::string>& fileNames,
                                                                  ...
                                                                  int64_t totalBytes) {
	return writeKeyspaceSnapshotFile_impl(Reference<BackupContainerFileSystem>::addRef(this), fileNames, beginEndKeys,
	                                      totalBytes);
};
...
Version BackupContainerFileSystem::resolveRelativeVersion(Optional<Version> max, Version v, const char* name, Error e) {
	if (v == invalidVersion) {
...
void BackupContainerFileSystem::computeRestoreEndVersion(const std::vector<LogFile>& logs,
                                                         std::vector<LogFile>* outLogs, Version* end,
                                                         Version targetVersion) {
	auto i = logs.begin();
...
bool BackupContainerFileSystem::isContinuous(const std::vector<LogFile>& files, const std::vector<int>& indices,
...
bool BackupContainerFileSystem::isPartitionedLogsContinuous(const std::vector<LogFile>& files, Version begin,
                                                            Version end) {
...
std::vector<LogFile> BackupContainerFileSystem::filterDuplicates(const std::vector<LogFile>& logs) {
...
void BackupContainerFileSystem::updatePartitionedLogsContinuousEnd(BackupDescription* desc,
...
Version BackupContainerFileSystem::getPartitionedLogsContinuousEndVersion(const std::vector<LogFile>& logs,
                                                                          Version begin) {
	Version end = 0;
