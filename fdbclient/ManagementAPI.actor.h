...
struct ConfigureAutoResult {
	...
	int32_t desired_read_proxies;
	int32_t desired_resolvers;
	int32_t desired_logs;

	ConfigureAutoResult()
	  : processes(-1), machines(-1), old_proxies(-1), old_read_proxies(-1), old_resolvers(-1), old_logs(-1),
	    old_processes_with_transaction(-1), old_machines_with_transaction(-1), auto_proxies(-1), auto_resolvers(-1),
	    auto_read_proxies(-1), auto_logs(-1), auto_processes_with_transaction(-1), auto_machines_with_transaction(-1),
	    desired_proxies(-1), desired_read_proxies(-1), desired_resolvers(-1), desired_logs(-1) {}
