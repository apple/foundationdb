...
class SimpleConfigTransactionImpl {
	...
	ACTOR static Future<ValueReadResult> get(SimpleConfigTransactionImpl* self, KeyRef key) {
...
Future<ValueReadResult> SimpleConfigTransaction::get(Key const& key, Snapshot snapshot) {
	return impl->get(key);
}
