				...
				if (clusterInterface->get().present()) {
					...
					if (_statusObjCluster.present()) {
						...
						if (statusObjCluster.count("fault_tolerance")) {
							...
							if (faultToleranceReader.get("max_machine_failures_without_losing_data", maxDataLoss) &&
							    faultToleranceReader.get("max_machine_failures_without_losing_availability",
							                             maxAvailLoss)) {
								// max_machine_failures_without_losing_availability <=
								// max_machine_failures_without_losing_data
								faultToleranceWriteable["max_machine_failures_without_losing_data"] =
								    std::min(maxDataLoss, coordinatorsFaultTolerance);
								faultToleranceWriteable["max_machine_failures_without_losing_availability"] =
