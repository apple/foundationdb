...
// These IDs are part of the FDB wire protocol. Keep existing values stable and
// reserve the exclusive upper bound when creating an FDB transport.
...
static_assert(static_cast<int>(WLTOKEN_CLIENTLEADERREG_GETLEADER) == WLTOKEN_FIRST_AVAILABLE);
static_assert(WLTOKEN_PROTOCOL_INFO == 10);
