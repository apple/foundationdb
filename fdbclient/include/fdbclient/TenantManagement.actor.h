...
createTenantTransaction(Transaction tr, TenantMapEntry tenantEntry, ClusterType clusterType = ClusterType::STANDALONE) {
	...
	state Future<Optional<TenantMapEntry>> existingEntryFuture = tryGetTenantTransaction(tr, tenantEntry.id);
...
// Deletes a tenant with the ID. If no matching tenant is found, this function returns
// without deleting anything. This behavior allows the function to be used idempotently: if the transaction is retried
// after having succeeded, it will see that the tenant is absent and do nothing.
ACTOR template <class Transaction>
Future<Void> deleteTenantTransaction(Transaction tr,
                                     int64_t tenantId,
                                     ClusterType clusterType = ClusterType::STANDALONE) {
	ASSERT(clusterType == ClusterType::STANDALONE);
	ASSERT(clusterType != ClusterType::METACLUSTER_MANAGEMENT);
...
Future<std::vector<std::pair<TenantName, int64_t>>> listTenantsTransaction(Transaction tr,
	...
	KeyBackedRangeResult<std::pair<TenantName, int64_t>> results =
...
Future<std::vector<std::pair<TenantName, int64_t>>> listTenants(Reference<DB> db,
	...
	loop {
		try {
			tr->setOption(FDBTransactionOptions::READ_SYSTEM_KEYS);
			tr->setOption(FDBTransactionOptions::READ_LOCK_AWARE);
			std::vector<std::pair<TenantName, int64_t>> tenants = wait(listTenantsTransaction(tr, begin, end, limit));
			return tenants;
		} catch (Error& e) {
			wait(safeThreadFutureToFuture(tr->onError(e)));
		}
...
Future<Void> renameTenantTransaction(Transaction tr,
	...
	state Future<Optional<int64_t>> newNameIdFuture = TenantMetadata::instance().tenantNameIndex.get(tr, newName);
