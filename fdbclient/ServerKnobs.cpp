...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	// clang-format off
	init( ALLOW_DANGEROUS_KNOBS,                               false ); if (isSimulated) ALLOW_DANGEROUS_KNOBS = true;
	// Versions
	...
	init( PEEK_USING_STREAMING,                                false ); if( randomize && BUGGIFY ) PEEK_USING_STREAMING = true;
	...
	*/
	...
	init( FETCH_USING_STREAMING,                               false ); if( randomize && BUGGIFY ) FETCH_USING_STREAMING = true; //Determines if fetch keys uses streaming reads
