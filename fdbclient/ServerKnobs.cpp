...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	*/
	...
	init( CC_WORKER_HEALTH_CHECKING_INTERVAL, isSimulated ? 2.0 : 60.0 );
	init( CC_DEGRADED_LINK_EXPIRATION_INTERVAL,                300.0 );
	init( CC_MIN_DEGRADATION_INTERVAL,     isSimulated ? 5.0 : 120.0 );
	...
	init( ENABLE_WORKER_HEALTH_MONITOR,                         true );
	init( WORKER_HEALTH_MONITOR_INTERVAL,   isSimulated ? 2.0 : 60.0 );
	init( PEER_LATENCY_CHECK_MIN_POPULATION, isSimulated ? 5.0 : 30.0 );
