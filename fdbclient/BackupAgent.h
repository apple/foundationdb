...
public:
	...
	Future<Version> restore(Database cx, Key tagName, Key url, bool waitForComplete = true, Version targetVersion = -1,
	                        bool verbose = true, KeyRange range = normalKeys, Key addPrefix = Key(),
	                        Key removePrefix = Key(), bool lockDB = true);
	Future<Version> atomicRestore(Database cx, Key tagName, KeyRange range = normalKeys, Key addPrefix = Key(),
...
public:
	...
	Future<Void> atomicSwitchover(Database dest, Key tagName, Standalone<VectorRef<KeyRangeRef>> backupRanges,
	...
	Future<Void> submitBackup(Reference<ReadYourWritesTransaction> tr, Key tagName,
	                          Standalone<VectorRef<KeyRangeRef>> backupRanges, bool stopWhenDone = true,
	                          Key addPrefix = StringRef(), Key removePrefix = StringRef(), bool lockDatabase = false,
	                          bool databasesInSync = false);
	Future<Void> submitBackup(Database cx, Key tagName, Standalone<VectorRef<KeyRangeRef>> backupRanges,
	                          bool stopWhenDone = true, Key addPrefix = StringRef(), Key removePrefix = StringRef(),
	                          bool lockDatabase = false, bool databasesInSync = false) {
		return runRYWTransaction(cx, [=](Reference<ReadYourWritesTransaction> tr) {
			return submitBackup(tr, tagName, backupRanges, stopWhenDone, addPrefix, removePrefix, lockDatabase,
			                    databasesInSync);
		});
	}
...
Future<Void> eraseLogData(Database cx, Key logUidValue, Key destUidValue,
                          Optional<Version> endVersion = Optional<Version>(), bool checkBackupUid = false,
                          Version backupUid = 0);
...
void decodeBackupLogValue(Arena& arena, VectorRef<MutationRef>& result, int64_t& mutationSize, StringRef value,
                          StringRef addPrefix = StringRef(), StringRef removePrefix = StringRef());
Future<Void> logErrorWorker(Reference<ReadYourWritesTransaction> const& tr, Key const& keyErrors,
                            std::string const& message);
Future<Void> logError(Database cx, Key keyErrors, const std::string& message);
Future<Void> logError(Reference<ReadYourWritesTransaction> tr, Key keyErrors, const std::string& message);
Future<Void> checkVersion(Reference<ReadYourWritesTransaction> const& tr);
Future<Void> readCommitted(Database const& cx, PromiseStream<RangeResultWithVersion> const& results,
                           Reference<FlowLock> const& lock, KeyRangeRef const& range, bool const& terminator = true,
                           bool const& systemAccess = false, bool const& lockAware = false);
Future<Void> readCommitted(Database const& cx, PromiseStream<RCGroup> const& results, Future<Void> const& active,
                           Reference<FlowLock> const& lock, KeyRangeRef const& range,
                           std::function<std::pair<uint64_t, uint32_t>(Key key)> const& groupBy,
                           bool const& terminator = true, bool const& systemAccess = false,
                           bool const& lockAware = false);
Future<Void> applyMutations(Database const& cx, Key const& uid, Key const& addPrefix, Key const& removePrefix,
                            Version const& beginVersion, Version* const& endVersion,
                            RequestStream<CommitTransactionRequest> const& commit,
                            NotifiedVersion* const& committedVersion,
                            Reference<KeyRangeMap<Version>> const& keyVersion);
...
public:
	...
	static Future<std::vector<KeyBackedTag>> getAll_impl(TagUidMap* const& tagsMap,
	                                                     Reference<ReadYourWritesTransaction> const& tr);

	Future<std::vector<KeyBackedTag>> getAll(Reference<ReadYourWritesTransaction> tr) { return getAll_impl(this, tr); }
...
public:
	...
	Future<Void> toTask(Reference<ReadYourWritesTransaction> tr, Reference<Task> task, bool setValidation = true) {
		...
		return map(tag().get(tr), [u, p, task](Optional<std::string> const& tag) -> Void {
			if (!tag.present()) throw restore_error();
			// Validation contition is that the uidPair key must be exactly {u, false}
			TaskBucket::setValidationCondition(task, KeyBackedTag(tag.get(), p).key,
			                                   Codec<UidAndAbortedFlagT>::pack({ u, false }).pack());
			return Void();
		});
	}
...
public:
	...
	Future<Void> initNewSnapshot(Reference<ReadYourWritesTransaction> tr, int64_t intervalSeconds = -1) {
		...
		if (intervalSeconds < 0) defaultInterval = copy.snapshotIntervalSeconds().getOrThrow(tr);
		...
		return map(success(beginVersion) && success(defaultInterval), [=](Void) mutable {
			...
			if (intervalSeconds < 0) intervalSeconds = defaultInterval.get();
	...
	Future<Optional<Version>> getLatestRestorableVersion(Reference<ReadYourWritesTransaction> tr) {
		...
		return map(success(lastLog) && success(firstSnapshot), [=](Void) -> Optional<Version> {
			...
			if (lastLog.get().present() && firstSnapshot.get().present() &&
			    lastLog.get().get() >= firstSnapshot.get().get()) {
