		...
		loop {
			...
			loop {
				try {
					...
					state Future<ValueResult> logVersionValue =
					    tr->get(task->params[BackupAgentBase::keyConfigLogUid].withPrefix(applyMutationsEndRange.begin),
					            Snapshot::True);
					state Future<ValueResult> rangeCountValue = tr->get(rangeCountKey, Snapshot::True);
...
struct CopyLogsTaskFunc : TaskFuncBase {
	...
	ACTOR static Future<Void> _finish(Reference<ReadYourWritesTransaction> tr,
		...
		state Future<ValueResult> fStopValue = tr->get(states.pack(DatabaseBackupAgent::keyCopyStop));
		state Future<ValueResult> fAppliedValue =
		...
		ValueResult appliedValue = wait(fAppliedValue);
		...
		ValueResult stopValue = wait(fStopValue);
...
struct CopyDiffLogsTaskFunc : TaskFuncBase {
	...
	ACTOR static Future<Void> _finish(Reference<ReadYourWritesTransaction> tr,
		...
		state Version prevBeginVersion =
		    BinaryReader::fromStringRef<Version>(task->params[DatabaseBackupAgent::keyPrevBeginVersion], Unversioned());
		state Future<ValueResult> fStopWhenDone = tr->get(conf.pack(DatabaseBackupAgent::keyConfigStopWhenDoneKey));
		...
		ValueResult stopWhenDone = wait(fStopWhenDone);
	...
	ACTOR static Future<Void> _execute(Database cx,
		...
		loop {
			try {
				...
				ValueResult tagName = wait(tr->get(tagPath));
				if (!tagName.present()) {
					return Void();
				}
	...
	ACTOR static Future<Void> _execute(Database cx,
		...
		loop {
			try {
				...
				ValueResult backupKeysPacked = wait(tr->get(config.pack(BackupAgentBase::keyConfigBackupRanges)));
		...
		loop {
			try {
				...
				state ValueResult v = wait(srcTr->get(sourceStates.pack(DatabaseBackupAgent::keyFolderId)));
				if (v.present() && BinaryReader::fromStringRef<Version>(v.get(), Unversioned()) >
				                       BinaryReader::fromStringRef<Version>(
				                           task->params[DatabaseBackupAgent::keyFolderId], Unversioned())) {
					return Void();
...
struct BackupRestorableTaskFunc : TaskFuncBase {
	...
	ACTOR static Future<Void> _execute(Database cx,
		...
		loop {
			try {
				...
				ValueResult prevBeginVersion = wait(tr.get(versionKey));
	...
	ACTOR static Future<Void> _finish(Reference<ReadYourWritesTransaction> tr,
		...
		state ValueResult stopValue = wait(tr->get(states.pack(DatabaseBackupAgent::keyStateStop)));
		state Version restoreVersion =
		    stopValue.present() ? BinaryReader::fromStringRef<Version>(stopValue.get(), Unversioned()) : -1;

		state ValueResult stopWhenDone = wait(tr->get(conf.pack(DatabaseBackupAgent::keyConfigStopWhenDoneKey)));
	...
	ACTOR static Future<Void> _execute(Database cx,
		...
		loop {
			try {
				...
				ValueResult versionRecord = wait(srcTr->get(versionKey));
		...
		loop {
			...
			try {
				...
				ValueResult backupStartVersion = wait(tr->get(config.pack(BackupAgentBase::backupStartVersion)));
		...
		loop {
			try {
				...
				state ValueResult v = wait(srcTr2->get(sourceStates.pack(DatabaseBackupAgent::keyFolderId)));
...
public:
		...
		loop {
			...
			loop {
				try {
					tr->setOption(FDBTransactionOptions::ACCESS_SYSTEM_KEYS);
					tr->setOption(FDBTransactionOptions::LOCK_AWARE);
					ValueResult drVersion = wait(tr->get(drVersionKey));
	...
	ACTOR static Future<Void> submitBackup(DatabaseBackupAgent* backupAgent,
		...
		ValueResult v = wait(tr->get(backupAgent->states.get(logUidValue).pack(DatabaseBackupAgent::keyFolderId)));
	...
	ACTOR static Future<Void> discontinueBackup(DatabaseBackupAgent* backupAgent,
		...
		state ValueResult stopWhenDoneValue =
	...
	ACTOR static Future<Void> abortBackup(DatabaseBackupAgent* backupAgent,
		...
		loop {
			...
			try {
				...
				ValueResult lastApplied = wait(tr->get(logUidValue.withPrefix(applyMutationsBeginRange.begin)));
		...
		if (!dstOnly) {
			...
			loop {
				try {
					srcTr->setOption(FDBTransactionOptions::ACCESS_SYSTEM_KEYS);
					srcTr->setOption(FDBTransactionOptions::LOCK_AWARE);
					state Future<ValueResult> backupVersionF =
					...
					state Future<ValueResult> bVersionF = srcTr->get(latestVersionKey);
	...
	ACTOR static Future<std::string> getStatus(DatabaseBackupAgent* backupAgent,
		...
		loop {
			try {
				...
				state Future<ValueResult> fPaused = tr->get(backupAgent->taskBucket->getPauseKey());
				...
				if (backupState == EBackupState::STATE_NEVERRAN) {
					...
					ValueResult tagName = wait(fTagName);
					...
					state ValueResult stopVersionKey = wait(fStopVersionKey);
					ValueResult logVersionKey = wait(flogVersionKey);
					state std::string logVersionText =
					    ...
					    (logVersionKey.present()
					         ...
					         : "unset");
					ValueResult backupKeysPacked = wait(fBackupKeysPacked);
				...
				ValueResult backupUid = wait(fBackupUid);
				if (backupUid.present()) {
					ValueResult v = wait(fBackupVerison);
					if (v.present()) {
				...
				ValueResult paused = wait(fPaused);
				if (paused.present()) {
	...
	ACTOR static Future<EBackupState> getStateValue(DatabaseBackupAgent* backupAgent,
		...
		state Key statusKey = backupAgent->states.get(BinaryWriter::toValue(logUid, Unversioned()))
		                          .pack(DatabaseBackupAgent::keyStateStatus);
		ValueResult status = wait(tr->get(statusKey, snapshot));
	...
	ACTOR static Future<UID> getDestUid(DatabaseBackupAgent* backupAgent,
		...
		ValueResult destUid = wait(tr->get(destUidKey, snapshot));
	...
	ACTOR static Future<UID> getLogUid(DatabaseBackupAgent* backupAgent,
		...
		tr->setOption(FDBTransactionOptions::ACCESS_SYSTEM_KEYS);
		tr->setOption(FDBTransactionOptions::LOCK_AWARE);
		state ValueResult logUid = wait(tr->get(backupAgent->tagNames.pack(tagName), snapshot));
