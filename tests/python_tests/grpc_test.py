...
#
# python -m grpc_tools.protoc   --proto_path=/root/src/foundationdb/fdbctl/protos   --python_out=/root/src/foundationdb/tests/python_tests/   --grpc_python_out=/root/src/foundationdb/tests/python_tests/   control_service.proto
...
def create_stub():
    ...
    stub = control_service_pb2_grpc.ControlServiceStub(channel)
    return stub
...
def test_get_workers():
    ...
    stub = create_stub()
    response = stub.GetWorkers(control_service_pb2.GetWorkersRequest())
    # print("Server replied:", response)

def test_get_coordinators():
    print("---- Testing GetCoordinators:")
    stub = create_stub()
    response = stub.GetCoordinators(control_service_pb2.GetCoordinatorsRequest())
    print("Server replied:", response)
...
def test_change_coordinators():
    ...
    req = control_service_pb2.ChangeCoordinatorsRequest()
...
def test_get_version():
    ...
    req = control_service_pb2.GetReadVersionRequest()
...
def test_get_status():
    ...
    req = control_service_pb2.GetStatusRequest()
...
def test_configure():
    ...
    req = control_service_pb2.ConfigureRequest()
    req.redundancy_mode = control_service_pb2.ConfigureRequest.TRIPLE

    try:
        response = stub.Configure(req)
        print("Configure result:", response.result)
        print("Configure message:", response.message)
        assert(response.result == control_service_pb2.ConfigureReply.SUCCESS)
    ...
    req2 = control_service_pb2.ConfigureRequest()
    req2.grv_proxies = 2
...
def test_exclude_include():
    ...
    # Exclude the worker
    exclude_req = control_service_pb2.ExcludeRequest()
    ...
    while True:
        ...
        response = stub.ExcludeStatus(control_service_pb2.ExcludeStatusRequest())
    ...
    try:
        include_req = control_service_pb2.IncludeRequest()
