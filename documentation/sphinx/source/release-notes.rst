...
* Flow: Removed ``API::createCluster`` and ``Cluster`` and added ``API::createDatabase``. The preferred way to get a ``Database`` is by using ``API::createDatabase``. `(PR #942) <https://github.com/apple/foundationdb/pull/942>`_ `(PR #1215) <https://github.com/apple/foundationdb/pull/1215>`_
...
* Flow: Converted ``Transaction`` into an interface and moved the implementation into ``TransactionImpl``. And the implementation is hidden (only available within CPP file). Transaction should be created using ``Database::createTransaction(db)`` instead of constructor. `(PR #1215) <https://github.com/apple/foundationdb/pull/1215>`_
