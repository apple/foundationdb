...
* FoundationDB transactions are optimistic: we never block on reads or writes (there are now locks), instead we abort transactions at commit time.
...
In order to assign read and write versions to transactions, a client will never talk to a master. Instead it will get both from a proxy. Getting a read version is more complex than a commit version. Let's first look at commit versions:
...
This section will only explain conceptually how transactions are resolved in FoundationDB. The implementation will use multiple servers running the *Resolver* role and the keyspace will be sharded across them. It will also only allow to resolve transactions where the commit version will be larger than ``commitVersion - 5Million`` (around 5 seconds...).
