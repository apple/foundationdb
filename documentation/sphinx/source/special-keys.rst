...
#. ``\xff\xff/management/db_locked`` Read/write. A single key that can be read and modified. Set the key with a 32-length hex string as the UID will lock the database and clear the key will unlock the database. Reading the key will return the UID hex string as the value. If the database is already locked, then the commit will fail with the ``special_keys_api_failure`` error. For more details, see help text of ``fdbcli`` command ``lock`` and ``unlock``.
