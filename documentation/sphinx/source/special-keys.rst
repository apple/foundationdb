...
#. ``\xff\xff/management/options/worker_interfaces/verify`` Read/write. Setting this key will add a verification phase in reading ``\xff\xff/worker_interfaces``. Only return worker interfaces that we can talk to in the result. This option is now only used in fdbcli command `kill`, `suspend` and `expensive_data_check` to populate the worker list.
