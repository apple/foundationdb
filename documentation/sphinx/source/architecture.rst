...
FoundationDB makes your architecture flexible and easy to operate. Your applications can send their data directly to FoundationDB or to a :doc:`layer<layer-concept>`, a user-written module that can provide a new data model, compatibility with existing systems, or even serve as an entire framework. In both cases, all data is stored in a single place via an ordered, transactional key-value API.
...
Coordinators, Storage Servers, Master). The cluster attempts to recruit
different roles as separate processes, however, it is possible that
multiple stateless roles get colocated (recruited) on a single
...
Historically, Ratekeeper and Data Distributor were coupled with the Master on
the same process. Since 6.2, both have become singletons in the
cluster. Their lifetimes are no longer tied to the Master.
...
The GRV proxies are responsible for providing read versions and communicating
with Ratekeeper to control the rate of providing read versions. To provide a
read version, a GRV proxy will ask the master for the largest committed
...
The proxies are responsible for committing transactions, reporting committed
versions to the master, and tracking the storage servers responsible for each
range of keys.
...
for that range of keys. The commit proxies provide this information to
...
The resolvers are responsible for determining conflicts between
...
servers are assigned ranges of keys and are responsible for storing all
of the data for that range. They keep 5 seconds of mutations in memory,
and an on-disk copy of the data as of 5 seconds ago. Clients must read at
...
a B-tree based on SQLite. The memory storage engine stores the data in
...
The data distributor manages the lifetime of storage servers, decides which
storage server is responsible for which data range, and ensures data is
evenly distributed across all storage servers (SS). The data distributor, as
a singleton in the cluster, is recruited and monitored by the Cluster
...
cluster, is recruited and monitored by the Cluster Controller.
...
client to communicate with FoundationDB clusters running older versions.
Currently, C, Go, Python, Java, and Ruby bindings are officially supported.
...
larger than any commit version that the client may know about (even
...
writes) to one of the commit proxies and waits for a commit or abort response
...
beginning. If the transaction commits, the commit proxy also returns
the commit version to the client and to the master so that GRV proxies can
access the latest committed version. Note that this commit version is
...
the master for the latest committed version and checks that a set of transaction
logs satisfying the replication policy is live. Then the GRV proxy returns
the maximum committed version as the read version to the client.

|image2|

The GRV proxy contacts the master for the latest committed versions because
the master is a central place for keeping the largest committed version
reported by all commit proxies.

Checking that a set of transaction logs satisfying the replication
policy is live ensures that the GRV proxy has not been replaced by a newer
...
transaction logs satisfying the replication policy is live, the GRV proxy makes
...
this approach puts more work on the master, whose role cannot be scaled.
Even though providing read versions is not very expensive, it still requires
the master to get a transaction budget from Ratekeeper, batch requests, and
potentially maintain thousands of network
...
5. The resolver responds with whether the transaction has any
   ...
   -  If there are conflicts, the commit proxy responds to the client with
...
6. Once the mutations are durable on the logs, the commit proxy responds
   with success to the user.

Note that the commit proxy sends each resolver its respective key ranges. If
...
conflict ranges, even though these future transactions can commit. In
...
performance. Additionally, each transaction has a five-second window.
...
There are a number of background tasks happening in addition to transaction
processing:

-  **Ratekeeper** collects statistical information from GRV proxies, Commit
   proxies, transaction logs, and storage servers and computes the target
   transaction rate for the cluster.

-  **Data distribution** monitors all storage servers and performs load
   balancing operations to evenly distribute data among all storage
   servers.

-  **Storage servers** pull mutations from transaction logs and write them
   into the storage engine to persist them on disk.

-  **Commit proxies** periodically send empty commits to transaction logs to
   keep commit versions increasing, in case there are no client-generated
   transactions.
...
longer recoveries (usually a few seconds) can happen. Whenever there
...
processes fails, the Master process terminates. The Cluster Controller will
...
Coordinators and locks the coordinated states to prevent another Master
...
Transaction Logs. After the previous Log Servers are stopped and a new transaction
system is recruited, the Master writes the coordinated states with the current
...
recoveries require no extra work. In contrast, Transaction Logs save the
...
commit responses, their logs are persisted in multiple Log Servers (e.g.,
...
**``commit_result_unknown`` error:** If a recovery happens while a
transaction is committing (i.e., after a commit proxy has sent mutations to
transaction logs), a client may receive
``commit_result_unknown`` and then retry the transaction. It’s
...
transactions be idempotent, so that they handle
