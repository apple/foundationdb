...
::
   ...
   user@host$ fdbbackup start [-t <TAG>] -d <BACKUP_URL> [-z] [-s <DURATION>] [-p] [-w] [-k '<BEGIN>[ <END>]']...
...
``-p`` or ``--partitioned_log``
  Specifies the backup uses the partitioned mutation logs generated by backup workers. The default is to use non-partitioned mutation logs generated by backup agents.
