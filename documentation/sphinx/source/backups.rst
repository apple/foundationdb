...
::
    ...
    blobstore://[<api_key>][:<secret>[:<security_token>]]@<hostname>[:<port>]/<name>?bucket=<bucket_name>[&prefix=<key_prefix>][&region=<region_name>][&<param>=<value>]...]
      ...
      <key_prefix>      Optional object key prefix.  When present, the backup's object key trees ('data/' and 'backups/') are placed under this prefix in the bucket instead of the bucket root.  Its path segments may contain alphanumerics, '_', '-' and '.'; leading and trailing '/' are ignored, and empty, '.' and '..' path segments are rejected.  A value containing only '/' characters is normalized to an empty prefix and uses the existing bucket-root layout.
...
The backup URL is not a security boundary.  The ``prefix`` value is used byte-for-byte after stripping leading and trailing slashes; percent-encoding is not decoded, and characters outside ``[A-Za-z0-9_-./]`` are rejected.  Within one bucket, combinations of ``<key_prefix>`` and ``<name>`` whose resulting object key trees overlap (for example a prefix whose first path segment is ``data`` or ``backups`` combined with matching backup names, or one prefix nested inside another prefix's tree) must be avoided, or the result is undefined.
