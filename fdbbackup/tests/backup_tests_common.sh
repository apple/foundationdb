...
function run_backup {
  ...
  # Add blob credentials if provided (for S3/blobstore, not file://)
  if [[ -n "${blob_credentials}" ]]; then
    cmd_args+=("--blob-credentials" "${blob_credentials}")
  fi

  # Add mode if specified
  if [[ -n "${backup_mode}" ]]; then
    cmd_args+=("--mode" "${backup_mode}")
  fi
...
function test_encryption_mismatches {
  ...
  if [[ -n "${backup_encryption_key_file}" ]]; then
    ...
    # Add blob credentials if provided
    if [[ -n "${blob_credentials}" ]]; then
      cmd_args1+=("--blob-credentials" "${blob_credentials}")
    fi
    ...
    # Add blob credentials if provided
    if [[ -n "${blob_credentials}" ]]; then
      cmd_args2+=("--blob-credentials" "${blob_credentials}")
    fi
...
# Validate restore using audit_storage validate_restore feature
# This compares restored data (with prefix) against original source data in the same cluster
# Based on design/validating_restored_data_using_one_cluster.md
# $1 build directory, $2 scratch directory, $3 backup url, $4 tag
# $5 encryption key file (optional), $6 restore mode (optional), $7 blob credentials file (optional)
function validate_restore_with_audit {
  local local_build_dir="${1}"
  local local_scratch_dir="${2}"
  local local_url="${3}"
  local local_tag="${4}"
  ...
  # The restore prefix for validation - restored data goes here
  # Uses system key space (\xff\x02/rlog/) for isolation from user data.
  # When BulkLoad has a prefix that targets system keys, the storage server
  # uses FDB client transactions to write the data to the correct location.
  local restore_prefix='\xff\x02/rlog/'
  local restore_prefix_end='\xff\x02/rlog0'
  
  local fdbcli="${local_build_dir}/bin/fdbcli"
  local cluster_file="${local_scratch_dir}/loopback_cluster/fdb.cluster"
  
  log "Starting restore validation using audit_storage validate_restore"
  
  # Step 1: Restore with add-prefix to validation keyspace (no locking for test simplicity)
  log "Restoring backup with prefix for validation..."
  local cmd_args=(
    "--dest-cluster-file" "${cluster_file}"
    "-t" "${local_tag}_validate"
    "-r" "${local_url}"
    "--add-prefix" "${restore_prefix}"
    "--log" "--logdir=${local_scratch_dir}"
  )
  ...
  # Add mode if specified (use rangefile for validation to avoid BulkLoad complications)
  # The main BulkLoad test already validated BulkLoad works; this validates the backup data itself
  if [[ -n "${restore_mode}" ]]; then
    cmd_args+=("--mode" "${restore_mode}")
  fi
  ...
  log "Validation restore completed"
  
  # Debug: Check if data was restored to the prefix (using READ_SYSTEM_KEYS option for system keyspace)
  log "Checking restored data at prefix..."
  local restored_check
  restored_check=$("${fdbcli}" -C "${cluster_file}" --exec "option on READ_SYSTEM_KEYS; getrangekeys \"${restore_prefix}\" \"${restore_prefix_end}\" 10" 2>&1) || true
  log "Restored data check: ${restored_check}"

  # Step 2: Run audit_storage validate_restore
  # Retry the audit command since it can fail if DD is restarting
  ...
  while [[ $attempt -lt $max_retries ]]; do
    ...
    # Check if this is a transient error (DD restart, movekeys conflict)
    ...
    "${fdbcli}" -C "${cluster_file}" --exec "writemode on; clearrange \"${restore_prefix}\" \"${restore_prefix_end}\"" 2>/dev/null || true
    return 1
  done
  ...
  # Step 3: Monitor audit progress until complete
  local timeout=300
  local poll_interval=5
  local elapsed=0
  ...
  while [[ $elapsed -lt $timeout ]]; do
    ...
    # Check for completion (Phase 2 = Complete)
    if echo "${status_output}" | grep -q "Phase.*2"; then
      log "Audit completed successfully after ${elapsed}s"
      break
    fi
    
    # Check for error/failed state (Phase 3 = Error, Phase 4 = Failed)
  ...
  # Step 4: Clean up
  log "Cleaning up validation data..."
  "${fdbcli}" -C "${cluster_file}" --exec "writemode on; clearrange \"${restore_prefix}\" \"${restore_prefix_end}\"" 2>/dev/null || true
...
function setup_fdb_cluster_with_backup {
  ...
  if ! start_fdb_cluster "${source_dir}" "${build_dir}" "${test_scratch_dir}" 1 "--knob_http_verbose_level=10"; then
    err "Failed start FDB cluster"
    exit 1
  fi
  log "FDB cluster is up"
  
  if ! start_backup_agent "${build_dir}" "${test_scratch_dir}" "${knobs_array[@]}"; then
