...
ACTOR static Future<Optional<VersionInfo>> getVersionInfo(Reference<IDatabase> db) {
	...
	loop {
		try {
			...
			state ThreadFuture<ValueResult> versionEpochValFuture = tr->get(versionEpochKey);
			ValueResult versionEpochVal = wait(safeThreadFutureToFuture(versionEpochValFuture));
...
ACTOR static Future<Optional<int64_t>> getVersionEpoch(Reference<ITransaction> tr) {
	loop {
		try {
			state ThreadFuture<ValueResult> versionEpochValFuture = tr->get(versionEpochSpecialKey);
			ValueResult versionEpochVal = wait(safeThreadFutureToFuture(versionEpochValFuture));
