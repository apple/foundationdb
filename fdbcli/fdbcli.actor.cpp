...
ACTOR Future<bool> fdb_cli::getWorkers(Reference<IDatabase> db, std::vector<ProcessData>* workers) {
	state Reference<ITransaction> tr = db->createTransaction();
	loop {
		try {
		...
		try {
			...
			for (iter = parsed.begin(); iter != parsed.end(); ++iter) {
				...
				if (tokencmp(tokens[0], "exclude")) {
					bool _result = wait(makeInterruptable(excludeCommandActor(db2, tokens, warn)));
					if (!_result)
						is_error = true;
					continue;
				}
