...
//		"configure [new] <single|double|triple|three_data_hall|three_datacenter|ssd|memory|memory-radixtree-beta|proxies=<PROXIES>|grv-proxies=<GRV_PROXIES>|logs=<LOGS>|resolvers=<RESOLVERS>>*",
...
//								description += format("\nNeed at least %d log servers across unique zones, %d proxies, %d GRV proxies and %d resolvers.", recoveryState["required_logs"].get_int(), recoveryState["required_proxies"].get_int(), recoveryState["required_grv_proxies"].get_int(), recoveryState["required_resolvers"].get_int());
...
//				if (statusObjConfig.get("grv-proxies", intVal))
...
//			outputString += format("| grv-proxies                 | %16d | %16d |", conf.get().old_grv_proxies, conf.get().auto_grv_proxies);
...
//	const char* opts[] = {"new", "single", "double", "triple", "three_data_hall", "three_datacenter", "ssd", "ssd-1", "ssd-2", "memory", "memory-1", "memory-2", "memory-radixtree-beta", "proxies=", "grv-proxies=", "logs=", "resolvers=", nullptr};
