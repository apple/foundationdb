...
ACTOR Future<Void> checkStatus(Future<Void> f,
	...
	if (!tr->isValid()) {
		...
		state ThreadFuture<ValueResult> statusValueF = tr->get("\xff\xff/status/json"_sr);
		ValueResult statusValue = wait(safeThreadFutureToFuture(statusValueF));
			...
			for (iter = parsed.begin(); iter != parsed.end(); ++iter) {
				...
				if (tokencmp(tokens[0], "get")) {
					if (tokens.size() != 2) {
						...
						state ThreadFuture<ValueResult> valueF =
						    getTransaction(db, tenant, tr, options, intrans)->get(tokens[1]);
						ValueResult v = wait(makeInterruptable(safeThreadFutureToFuture(valueF)));
				...
				if (tokencmp(tokens[0], "getknob")) {
					if (tokens.size() > 3 || tokens.size() < 2) {
						...
						t.append(tokens[1]);
						state ThreadFuture<ValueResult> valueF_knob =
						    getTransaction(configDb, tenant, config_tr, options, intrans)->get(t.pack());
						ValueResult v = wait(makeInterruptable(safeThreadFutureToFuture(valueF_knob)));
