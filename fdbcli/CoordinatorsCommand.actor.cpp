	...
	loop {
		...
		try {
			...
			if (automatic) {
				...
				// for (const auto& hostname : new_coordinators_hostnames) {
				...
				// 		                 }
				// 		                 new_coordinators_addresses.insert(address);
				// 		                 return Void();
				// 	                 }));
				// }
				// wait(waitForAll(fs));
				// for (const auto& address : new_coordinators_addresses) {
				// 	newCoordinatorslist.push_back(address.toString());
				// }
				...
				TraceEvent("Xxxxxxx-1").detail("NewCoordinatorsStr", new_coordinators_str).log();
			}
			wait(safeThreadFutureToFuture(tr->commit()));
			// commit should always fail here
			// If the commit succeeded, the coordinators changed and the commit will fail with commit_unknown_result() error
			ASSERT(false);
		} catch (Error& e) {
			state Error err(e);
