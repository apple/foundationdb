	...
	loop {
		...
		try {
			...
			if (e.code() == error_code_special_keys_api_failure) {
				...
				if (msg == "Too few fdbserver machines to provide coordination at the current redundancy level" &&
					...
					tr->reset();
					continue;
				} else if (msg == "No change (existing configuration satisfies request)") {
					if (retries)
