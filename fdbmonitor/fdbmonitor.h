...
uint64_t parseWithSuffix(char const* to_parse, char const* default_unit = nullptr);
...
struct EnvVarUtils {
	...
	static std::pair<std::string, std::string> extractKeyAndValue(std::string const& keyValue) {
		auto const equalIdx = keyValue.find('=');
	...
	static bool keyValueValid(std::string const& keyValue, std::string const& envvars) {
		if (keyValue.empty()) {
			log_msg(SevError, "Key-value in envvars %s can not be empty\n", envvars.c_str());
			return false;
		}
		auto const numEqualSigns = std::count(keyValue.begin(), keyValue.end(), '=');
		if (numEqualSigns != 1) {
			log_msg(SevError,
		...
		auto const [key, value] = extractKeyAndValue(keyValue);
		if (key.empty()) {
...
public:
	...
	Command(CSimpleIni const& ini, std::string _section, ProcessID id, fdb_fd_set fds, int* maxfd)
	  : fds(fds), argv(nullptr), section(_section), fork_retry_time(-1), quiet(false), envvars(), delete_envvars(),
		...
		keys.unique([](CSimpleIniA::Entry const& lhs, CSimpleIniA::Entry const& rhs) -> bool {
		...
		char const* mrd = get_value_multi(
		...
		char const* rbo = get_value_multi(
		...
		char const* rdri = get_value_multi(
		...
		char const* env = get_value_multi(ini, "envvars", ssection.c_str(), section.c_str(), "general", nullptr);
		if (env) {
			envvars = env;
		}
		...
		char const* binary = get_value_multi(ini, "command", ssection.c_str(), section.c_str(), "general", nullptr);
		if (!binary) {
			log_msg(SevError, "Unable to resolve command for %s\n", ssection.c_str());
			return;
		}

		char const* mem_rss = get_value_multi(ini, "memory", ssection.c_str(), section.c_str(), "general", nullptr);
#ifdef __linux__
		if (mem_rss) {
		...
		char const* id_s = ssection.c_str() + strlen(section.c_str()) + 1;

		std::string const pid_s = std::to_string(getpid());
	...
	void update(Command const& other) {
		quiet = other.quiet;
	...
	bool operator!=(Command const& rhs) {
