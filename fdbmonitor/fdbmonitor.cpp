...
const char* get_value_multi(const CSimpleIni& ini, const char* key, ...) {
	...
	while (!ret && (section = va_arg(ap, const char*))) ret = ini.GetValue(section, key, nullptr, nullptr);
...
public:
	...
	Command(const CSimpleIni& ini, std::string _section, uint64_t id, fdb_fd_set fds, int* maxfd)
	  : section(_section), argv(nullptr), fork_retry_time(-1), quiet(false), delete_envvars(nullptr), fds(fds),
		...
		const char* rbo = get_value_multi(ini, "restart_backoff", ssection.c_str(), section.c_str(), "general",
		                                  "fdbmonitor", nullptr);
		if(!rbo) {
...
bool argv_equal(const char** a1, const char** a2)
{
	...
	if (a1[i] != nullptr || a2[i] != nullptr) return false;
	return true;
}
...
void load_conf(const char* confpath, uid_t &uid, gid_t &gid, sigset_t* mask, fdb_fd_set rfds, int* maxfd)
{
	...
	if(loadedConf) {
		...
		const char* user = ini.GetValue("fdbmonitor", "user", nullptr, nullptr);
		const char* group = ini.GetValue("fdbmonitor", "group", nullptr, nullptr);
