...
1. Introduction
   ...
   flowserializer is that you can add a new field to a network message without
   ...
   before. Currently we are using a custom flowserializer implementation so to
...
1. Correspondence to flowserializer IDL
    - Tables
    ```
    // Flow type
    struct A {
...
1. Flatbuffers Traits
   ...
   In order to serialize a type as a flowserializer vector, struct, or union, you can implement the appropriate trait
   for your type.
    - `scalar_traits` corresponds to a flowserializer struct. See `UID` for an example.
    - `vector_like_traits` corresponds to a flowserializer vector. See `VectorRef` for an example.
    - `dynamic_size_traits` corresponds to a flowserializer vector of uint8_t. See `StringRef` for an example.
    - `union_like_traits` corresponds to a flowserializer union. See `std::variant` for an example.

1. Potential Gotchas
    - Flatbuffers 'vtables' are collected from default-constructed instances of
      each type. Consequently types serialized by flowserializer should have cheap
   ...
   - In a call to `serializer`, arenas must come after any members whose memory
     ...
     because arenas are ignored for the flowserializer schema. (Future work)
     Enforce that no fields appear after an arena at compile time.

1. File identifiers

   [File identifiers](https://google.github.io/flowserializer/md__schemas.html)
