...
template <TimeoutScenario Scenario>
Future<Void> benchTimeoutActor(benchmark::State* state) {
	...
	if constexpr (Scenario == TimeoutScenario::Ready) {
		...
		while (state->KeepRunning()) {
			Future<int> done = timeout<int>(readyFuture, 0.0, timedOutValue);
			sink += co_await done;
		...
		for (auto _ : *state) {
			...
			Future<int> done = timeout<int>(neverFuture, 1.0, timedOutValue);
			ASSERT(!done.isReady());
			benchmark::DoNotOptimize(done);
			state->PauseTiming();
			done.cancel();
...
void benchTimeout(benchmark::State& state) {
	onMainThread([&state] { return benchTimeoutActor<Scenario>(&state); }).blockUntilReady();
}

BENCHMARK_TEMPLATE(benchTimeout, TimeoutScenario::Ready)->Name("Timeout/coroutine/ready")->ReportAggregatesOnly(true);

BENCHMARK_TEMPLATE(benchTimeout, TimeoutScenario::ConstructPending)
