...
public: // introduced features
	// The 5th byte from right is dev versoin, for example, 2 in 0x0FDB00B061020000LL;
	// It was used to identify a protocol change (e.g., interface change) between release versions (say 5.2.1 and 5.2.2)
	// We stoped using the dev version at some point in the past.
	// To ensure binaries work across patch releases (e.g., 6.2.1 to 6.2.22), we ensure no protocol change.
