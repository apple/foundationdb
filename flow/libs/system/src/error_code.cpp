...
//      thread safe.
    ...
    (defined(__osf__) && !defined(_REENTRANT)) || (defined(__INTEGRITY)) || (defined(__vms)) || (defined(__QNXNTO__))
	...
	for (;;) {
		...
		if (result == 0)
			...
			sz *= 2;
			if ((bp = static_cast<char*>(std::malloc(sz))) == 0) return std::string("ENOMEM");
		}
	}
	std::string msg;
	try {
...
error_condition system_error_category::default_error_condition(int ev) const FOUNDATIONDB_NOEXCEPT {
	...
	case ERROR_DIRECTORY:
		return make_error_condition(invalid_argument); // WinError.h: "The directory name is invalid"
...
std::string system_error_category::message(int ev) const {
	...
	DWORD retval =
	    ::FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
	                     NULL, ev, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), // Default language
	                     (LPSTR)&lpMsgBuf, 0, NULL);
	detail::local_free_on_destruction lfod(lpMsgBuf);
	if (retval == 0) return std::string("Unknown error");
