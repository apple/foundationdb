...
namespace actorcompiler
{
    ...
    class TypeSwitch<R>
    {
        ...
        {
            if (!ok)
                throw new Exception("Typeswitch didn't match.");
            return result;
        }
    }
    ...
    class Context
    {
        ...
        public Context WithTarget(Function newTarget)
        {
            return new Context
            {
                target = newTarget,
        ...
        )
        {
            return new Context
            {
                ...
                next = null,
                catchFErr = catchFErr,
                tryLoopDepth = tryLoopDepth + deltaLoopDepth
            };
        }

        public Context WithCatch(Function newCatchFErr)
        {
            return new Context
            {
                target = target,
    ...
    class Function
    {
        ...
        public void Indent(int change)
        {
            for (int i = 0; i < change; i++)
                indentation += '\t';
            if (change < 0)
                indentation = indentation.Substring(-change);
            if (overload != null)
            {
    ...
    class LiteralBreak : Function
    {
        ...
        public override string call(params string[] parameters)
        {
            ...
            if (parameters.Length != 0)
                throw new Exception("LiteralBreak called with parameters!");
            return "break";
        }
    };
    ...
    class LiteralContinue : Function
    {
        ...
        public override string call(params string[] parameters)
        {
            wasCalled = true;
            if (parameters.Length != 0)
                throw new Exception("LiteralContinue called with parameters!");
            return "continue";
        }
    };
    ...
    class DescrCompiler
    {
        ...
        public void Write(TextWriter writer, out int lines)
        {
            ...
            writer.WriteLine(
                memberIndentStr + "template<> struct Descriptor<struct {0}> {{",
                descr.name
            );
            writer.WriteLine(
                memberIndentStr + "\tstatic StringRef typeName() {{ return \"{0}\"_sr; }}",
                descr.name
            );
            ...
            foreach (var dec in descr.body)
            {
                ...
                writer.WriteLine(
                    memberIndentStr + "\t\tstatic StringRef name() {{ return \"{0}\"_sr; }}",
                    dec.name
                );
                writer.WriteLine(
                    memberIndentStr + "\t\tstatic StringRef typeName() {{ return \"{0}\"_sr; }}",
                ...
                writer.WriteLine(
                    memberIndentStr + "\t\tstatic StringRef comment() {{ return \"{0}\"_sr; }}",
                    dec.comment
                );
                writer.WriteLine(memberIndentStr + "\t\ttypedef {0} type;", dec.type);
                writer.WriteLine(
                    memberIndentStr + "\t\tstatic inline type get({0}& from);",
                    descr.name
                );
            ...
            if (descr.superClassList != null)
                writer.WriteLine(
                    memberIndentStr + "struct {0} : {1} {{",
                    descr.name,
                    descr.superClassList
                );
            else
            ...
            foreach (var dec in descr.body)
            {
                writer.WriteLine(
                    memberIndentStr
                        + "{0} Descriptor<{1}>::{2}Descriptor::get({1}& from) {{ return from.{2}; }}",
                    dec.type,
                    descr.name,
                    dec.name
                );
    ...
    class ActorCompiler
    {
        ...
        private void WriteActorFunction(TextWriter writer, string fullReturnType)
        {
            ...
            writer.WriteLine(
                ...
                actor.nameSpace == null ? "" : actor.nameSpace + "::"
            );
            LineNumber(writer, actor.SourceLine);

            string newActor = string.Format(
                "new {0}({1})",
                fullClassName,
        ...
        private void WriteActorClass(TextWriter writer, string fullStateClassName, Function body)
        {
            ...
            string callback_base_classes = string.Join(
                ", ",
                callbacks.Select(c => string.Format("public {0}", c.type))
            );
        ...
        public void Write(TextWriter writer)
        {
            string fullReturnType =
                actor.returnType != null ? string.Format("Future<{0}>", actor.returnType) : "void";
            for (int i = 0; ; i++)
            {
                className = string.Format(
                    ...
                    i != 0 ? i.ToString() : "",
                    actor.enclosingClass != null && actor.isForwardDeclaration
                        ? actor.enclosingClass.Replace("::", "_") + "_"
                        : actor.nameSpace != null
                            ? actor.nameSpace.Replace("::", "_") + "_"
                            : ""
                );
            ...
            var fullStateClassName =
                stateClassName
                + GetTemplateActuals(new VarDeclaration { type = "class", name = fullClassName });
            ...
            if (endContext.target != null)
            {
                if (actor.returnType == null)
                    CompileStatement(
                        new ReturnStatement { FirstSourceLine = actor.SourceLine, expression = "" },
                        endContext
                    );
                else
                    throw new Error(
            ...
            if (isTopLevel && actor.nameSpace == null)
        ...
        )
        {
            ...
            if (catchFErr != null)
            {
                ...
                if (useLoopDepth)
                    cx.target.WriteLine(
                        "\tloopDepth = {0};",
                        catchFErr.call("error", AdjustLoopDepth(catchLoopDepth))
                    );
                else
                ...
                if (useLoopDepth)
                    cx.target.WriteLine(
                        "\tloopDepth = {0};",
                        catchFErr.call("unknown_error()", AdjustLoopDepth(catchLoopDepth))
                    );
                else
        ...
        {
            var formals = (
                actor.templateFormals != null
                    ? actor.templateFormals.AsEnumerable()
                    : Enumerable.Empty<VarDeclaration>()
            )
            ...
            writer.WriteLine(
                ...
                string.Join(
                    ", ",
                    formals.Select(p => string.Format("{0} {1}", p.type, p.name)).ToArray()
                )
            );
        }
        ...
        string GetTemplateActuals(params VarDeclaration[] extraParameters)
        {
            ...
            if (formals.Length == 0)
                ...
                return "<" + string.Join(", ", formals.Select(p => p.name).ToArray()) + ">";
        }

        bool WillContinue(Statement stmt)
        {
            return Flatten(stmt)
                .Any(st => (st is ChooseStatement || st is WaitStatement || st is TryStatement));
        }
        ...
        void CompileStatement(StateDeclarationStatement stmt, Context cx)
        {
            ...
            if (
                actor
                    .body.statements.Select(x => x as StateDeclarationStatement)
                    .TakeWhile(x => x != null)
                    .Any(x => x == stmt)
            )
            {
        ...
        void CompileStatement(ForStatement stmt, Context cx)
        {
            ...
            bool noCondition =
                stmt.condExpression == ""
                || stmt.condExpression == "true"
                || stmt.condExpression == "1";
            ...
            else
            {
                ...
                Statement fullBody = noCondition
                    ? stmt.body
                    : new CodeBlock
                    {
                        statements = new Statement[]
                        {
                            ...
                            .Concat(AsCodeBlock(stmt.body).statements)
                            .ToArray(),
                        FirstSourceLine = stmt.FirstSourceLine
                    };
                ...
                Function continueF =
                    stmt.nextExpression == ""
                        ? loopF
                        : getFunction(cx.target.name, "continue", loopDepth);
                ...
                Function endLoop = Compile(
                    AsCodeBlock(fullBody),
                ...
                if (endLoop != null && endLoop != loopBody)
                {
                    if (stmt.nextExpression != "")
                        CompileStatement(
                            new PlainOldCodeStatement
                            {
                                code = stmt.nextExpression + ";",
                                FirstSourceLine = stmt.FirstSourceLine
                            },
                            cx.WithTarget(endLoop)
                        );
        ...
        void CompileStatement(RangeForStatement stmt, Context cx)
        {
            ...
            if (WillContinue(stmt.body))
            {
                ...
                state.Add(
                    new StateVar
                    {
                        SourceLine = stmt.FirstSourceLine,
                        name = iter,
                        type = "decltype(std::begin(std::declval<" + container.type + ">()))",
                        initializer = null
                    }
                );
            ...
            else
            {
                EmitNativeLoop(
                    stmt.FirstSourceLine,
                    "for( " + stmt.rangeDecl + " : " + stmt.rangeExpression + " )",
                    stmt.body,
                    cx
                );
            }
        ...
        private bool EmitNativeLoop(int sourceLine, string head, Statement body, Context cx)
        {
            ...
            Compile(
                AsCodeBlock(body),
                cx.LoopContext(cx.target, literalBreak, new LiteralContinue(), 0),
                true
            );
            cx.target.Indent(-1);
            cx.target.WriteLine("}");
        ...
        void CompileStatement(ChooseStatement stmt, Context cx)
        {
            ...
            var choices = codeblock
                .statements.OfType<WhenStatement>()
                .Select(
                    (ch, i) =>
                        new
                        {
                            ...
                            CallbackType = string.Format(
                                "{3}< {0}, {1}, {2} >",
                                fullClassName,
                                this.whenCount + i,
                                ch.wait.result.type,
                                ch.wait.isWaitNext ? "ActorSingleCallback" : "ActorCallback"
                            ),
                            CallbackTypeInStateClass = string.Format(
                                "{3}< {0}, {1}, {2} >",
                                className,
                                this.whenCount + i,
                                ch.wait.result.type,
            ...
            if (choices.Length != codeblock.statements.Length)
                throw new Error(
                    codeblock.statements.First(x => !(x is WhenStatement)).FirstSourceLine,
                    ...
                    else
                    {
                        ...
                        if (overload != null)
                        {
                            overload.WriteLine(
                                "loopDepth = {0};",
                                cx.next.call(
                                    string.Format("std::move({0})", ch.Stmt.wait.result.name),
                                    "loopDepth"
                                )
                            );
                            r.setOverload(overload);
            ...
            foreach (var ch in choices)
            {
                ...
                cx.target.WriteLine(
                    ...
                    ch.Stmt.wait.isWaitNext ? "FutureStream" : "StrictFuture",
                ...
                if (firstChoice)
                {
                    ...
                    if (actor.IsCancellable())
                        cx.target.WriteLine(
                            ...
                            cx.catchFErr.call(
                                "actor_cancelled()",
                                AdjustLoopDepth(cx.tryLoopDepth)
                            ),
                            This
                        );
                }
                ...
                cx.target.WriteLine(
                    ...
                    ch.Future,
                    ch.Body.call(ch.Future + "." + getFunc + "()", "loopDepth"),
        ...
        void CompileStatement(BreakStatement stmt, Context cx)
        {
            ...
            else
            {
                cx.target.WriteLine(
                    "return {0}; // break",
                    cx.breakF.call("loopDepth==0?0:loopDepth-1")
                );
            }
            cx.unreachable();
        }
        ...
        void CompileStatement(TryStatement stmt, Context cx)
        {
            ...
            var catchFErr = getFunction(
                cx.target.name,
                "Catch",
                "const Error& " + catchErrorParameterName,
                loopDepth0
            );
        ...
        void CompileStatement(ThrowStatement stmt, Context cx)
        {
            ...
            if (stmt.expression == "")
            {
                ...
                cx.target.WriteLine(
                    "return {0};",
                    cx.catchFErr.call(stmt.expression, AdjustLoopDepth(cx.tryLoopDepth))
                );
            cx.unreachable();
        }
        ...
        void CompileStatement(Statement stmt, Context cx)
        {
            ...
            var method = typeof(ActorCompiler).GetMethod(
                "CompileStatement",
                System.Reflection.BindingFlags.NonPublic
                    | System.Reflection.BindingFlags.Instance
                    | System.Reflection.BindingFlags.ExactBinding,
                null,
                new Type[] { stmt.GetType(), typeof(Context) },
                null
            );
            if (method == null)
                throw new Error(
            ...
            catch (System.Reflection.TargetInvocationException e)
            {
                if (!(e.InnerException is Error))
                    Console.Error.WriteLine(
                        "\tHit error <{0}> for statement type {1} at line {2}\n\tStack Trace:\n{3}",
        ...
        Context Compile(CodeBlock block, Context context, bool okToContinue = true)
        {
            ...
            foreach (var stmt in block.statements)
            {
                ...
                {
                    if (cx.target == null)
                        throw new Exception("Unreachable continuation called?");
                    if (!okToContinue)
                        throw new Exception("Unexpected continuation");
                    cx.target = cx.next;
                    cx.next = null;
                }
            }
        ...
        Function getFunction(
            ...
            string[] overloadFormalParameters
        )
        {
            string proposedName;
            if (
                addName == "cont"
                && baseName.Length >= 5
                && baseName.Substring(baseName.Length - 5, 4) == "cont"
            )
        ...
        IEnumerable<Statement> Flatten(Statement stmt)
        {
            ...
            var fl = new TypeSwitch<IEnumerable<Statement>>(stmt)
                ...
                .Case<TryStatement>(s =>
                    Flatten(s.tryBody).Concat(s.catches.SelectMany(c => Flatten(c.body)))
                )
        ...
        {
            state = actor
                .parameters.Select(p => new StateVar
                {
                    SourceLine = actor.SourceLine,
                    name = p.name,
                    type = p.type,
                    initializer = p.name,
                    initializerConstructorSyntax = false
