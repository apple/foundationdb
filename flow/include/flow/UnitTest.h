...
public:
	...
	void set(std::string const& name, int64_t value);
...
struct UnitTest {
	typedef Future<Void> (*TestFunction)(UnitTestParameters const& params);
	...
	UnitTest(char const* name, char const* file, int line, TestFunction func);
};
...
#define TEST_CASE(name) static Future<Void> FILE_UNIQUE_NAME(disabled_testcase_func)(UnitTestParameters const& params)
#define ACTOR_TEST_CASE(actorname, name)

#else

#define TEST_CASE(name)                                                                                                \
	static Future<Void> FILE_UNIQUE_NAME(testcase_func)(UnitTestParameters const& params);                             \
	namespace {                                                                                                        \
