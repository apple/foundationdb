...
public:
	...
	struct BarrierList : ThreadSafeReferenceCounted<BarrierList> {
		...
		void triggerAll() {
			MutexHolder h(mutex);
			for (int i = ntriggered; i < barriers.size(); i++) unsafeTrigger(i);
	...
	TraceLog()
	  : bufferLength(0), loggedLength(0), opened(false), preopenOverflowCount(0), barriers(new BarrierList),
	    logTraceEventMetrics(false), formatter(createLogFormatter()) {}
	...
	void open(std::string const& directory, std::string const& processName, std::string logGroup,
		...
		basename = format("%s/%s.%s.%s", directory.c_str(), processName.c_str(), timestamp.c_str(),
		                  g_random->randomAlphaNumeric(6).c_str());
	...
	void annotateEvent(TraceEventFields& fields) {
		if (localAddress.present()) {
			fields.addField("Machine", format("%d.%d.%d.%d:%d", (localAddress.get().ip >> 24) & 0xff,
			                                  (localAddress.get().ip >> 16) & 0xff, (localAddress.get().ip >> 8) & 0xff,
			                                  localAddress.get().ip & 0xff, localAddress.get().port));
		}
			...
			for (int idx = 0; idx < events.size(); idx++) {
				if (events[idx].size() > 0) {
					...
					for (auto itr = events[idx].begin(); itr != events[idx].end(); ++itr) {
						if (itr->first == "Time") {
							rolledFields.addField("Time",
							                      format("%.6f", (g_trace_clock == TRACE_CLOCK_NOW) ? now() : timer()));
...
void openTraceFile(const NetworkAddress& na, uint64_t rollsize, uint64_t maxLogsSize, std::string directory,
	...
	std::string baseName = format("%s.%03d.%03d.%03d.%03d.%d", baseOfBase.c_str(), (na.ip >> 24) & 0xff,
	                              (na.ip >> 16) & 0xff, (na.ip >> 8) & 0xff, na.ip & 0xff, na.port);
...
TraceEvent::TraceEvent(const char* type, UID id)
  : id(id), type(type), severity(SevInfo), initialized(false), enabled(true) {}
TraceEvent::TraceEvent(Severity severity, const char* type, UID id)
  : id(id), type(type), severity(severity), initialized(false), enabled(true) {}
TraceEvent::TraceEvent(TraceInterval& interval, UID id)
  : id(id), type(interval.type), severity(interval.severity), initialized(false), enabled(true) {
	init(interval);
}
TraceEvent::TraceEvent(Severity severity, TraceInterval& interval, UID id)
  : id(id), type(interval.type), severity(severity), initialized(false), enabled(true) {
	init(interval);
}
...
bool TraceEvent::init() {
	...
	if (enabled) {
		...
		if (g_network && g_network->isSimulated()) {
			...
			detailf("Machine", "%d.%d.%d.%d:%d", (local.ip >> 24) & 0xff, (local.ip >> 16) & 0xff,
			        (local.ip >> 8) & 0xff, local.ip & 0xff, local.port);
		}
		detail("ID", id);
		if (err.isValid()) {
...
TraceEvent& TraceEvent::detail(std::string key, double value) {
	init();
	if (enabled) tmpEventMetric->setField(key.c_str(), value);
	return detailfNoMetric(std::move(key), "%g", value);
}
TraceEvent& TraceEvent::detail(std::string key, int value) {
	init();
	if (enabled) tmpEventMetric->setField(key.c_str(), (int64_t)value);
...
TraceEvent& TraceEvent::detail(std::string key, const UID& value) {
	return detailf(std::move(key), "%016llx", value.first()); // SOMEDAY: Log entire value?  We also do this explicitly
	                                                          // in some "lists" in various individual TraceEvent calls
}
		...
		if (enabled) {
			...
			if (g_traceLog.isOpen()) {
				...
				if (g_traceLog.logTraceEventMetrics && isNetworkThread()) {
					...
					// accumulated in *this to it and then log() it. Note that if the event metric is disabled it won't
					// actually be logged BUT any new fields added to it will be registered. If the event IS logged, a
					// timestamp will be returned, if not then 0.  Either way, pass it through to be used if possible in
					// the Sev* event metrics.
...
void TraceBatch::dump() {
	...
	if (g_network->isSimulated()) {
		NetworkAddress local = g_network->getLocalAddress();
		machine = format("%d.%d.%d.%d:%d", (local.ip >> 24) & 0xff, (local.ip >> 16) & 0xff, (local.ip >> 8) & 0xff,
		                 local.ip & 0xff, local.port);
	}
