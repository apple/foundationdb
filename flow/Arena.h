	...
	static ArenaBlock* create(int dataSize, Reference<ArenaBlock>& next) {
		...
		if (dataSize <= SMALL - TINY_HEADER && !next) {
			...
			if (reqSize < LARGE) {
				...
				// Worst-case allocation pattern: 1 +10 +17 +42 +67 +170 +323 +681 +1348 +2728 +2210 +2211 (+1K +3K+1
				// +4K)* Overhead: 4X for small arenas, 3X intermediate, 1.33X for large arenas
...
inline Arena::Arena(size_t reservedSize) : impl(0) {
	...
	if (reservedSize) ArenaBlock::create((int)reservedSize, impl);
}
...
inline void Arena::dependsOn(const Arena& p) {
	if (p.impl) ArenaBlock::dependOn(impl, p.impl.getPtr());
}
inline size_t Arena::getSize() const {
	return impl ? impl->totalSize() : 0;
}
inline bool Arena::hasFree(size_t size, const void* address) {
	return impl && impl->unused() >= size && impl->getNextData() == address;
}
...
public:
	...
	StringRef eat(const char* sep) { return eat(StringRef((const uint8_t*)sep, strlen(sep))); }
...
public:
	...
	VectorRef(Arena& p, const VectorRef<T>& toCopy, typename std::enable_if<memcpy_able<T2>::value, int>::type = 0)
	  : data((T*)new (p) uint8_t[sizeof(T) * toCopy.size()]), m_size(toCopy.size()), m_capacity(toCopy.size()) {
	...
	VectorRef(Arena& p, const VectorRef<T>& toCopy, typename std::enable_if<!memcpy_able<T2>::value, int>::type = 0)
	  : data((T*)new (p) uint8_t[sizeof(T) * toCopy.size()]), m_size(toCopy.size()), m_capacity(toCopy.size()) {
		for (int i = 0; i < m_size; i++) new (&data[i]) T(p, toCopy[i]);
	}
	...
	void append_deep(Arena& p, It begin, int count) {
		if (m_size + count > m_capacity) reallocate(p, m_size + count);
		for (int i = 0; i < count; i++) new (&data[m_size + i]) T(p, *begin++);
		m_size += count;
	}
	...
	void resize(Arena& p, int size) {
		...
		for (int i = m_size; i < size; i++) new (&data[i]) T();
		m_size = size;
	}
