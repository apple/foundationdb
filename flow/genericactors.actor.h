...
std::vector<Future<decltype(actorFunc(T()).getValue())>> mapAsync(std::vector<Future<T>> const& what,
                                                                  F const& actorFunc) {
...
struct FlowLock : NonCopyable, public ReferenceCounted<FlowLock> {
	...
	Future<Void> takeUpTo(int& amount) { return takeMoreActor(this, &amount); }
...
private:
	...
	ACTOR static Future<Void> takeActor(FlowLock* lock, int taskID, int amount) {
		state std::list<std::pair<Promise<Void>, int>>::iterator it =
		...
		try {
			double duration =
			    BUGGIFY_WITH_PROB(.001) ? g_random->random01() * FLOW_KNOBS->BUGGIFY_FLOW_LOCK_RELEASE_DELAY : 0.0;
			choose {
...
public:
	...
	AndFuture(AndFuture&& f) noexcept(true) { futures = std::move(f.futures); }
	...
	void operator=(AndFuture&& f) noexcept(true) { futures = std::move(f.futures); }
