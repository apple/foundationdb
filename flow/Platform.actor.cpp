...
void profileHandler(int sig) {
	...
	// We can't get the time from a timer() call because it's not signal safe.
	 ps->timestamp = 0; // FIXME(swift): ps->timestamp = checkThreadTime.is_lock_free() ? checkThreadTime.load() : 0; // FIXME: problem with Swift build: lib/libflow.a(Platform.actor.g.cpp.o):Platform.actor.g.cpp:function profileHandler(int): error: undefined reference to '__atomic_is_lock_free'
