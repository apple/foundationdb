...
public:
	AbortableSingleAssignmentVar(ThreadFuture<T> future, ThreadFuture<Void> abortSignal, Error abortError)
	...
	void fire(const Void& unused, int& userParam) override {
		...
		if (!hasBeenSet) {
			...
			if (future.isReady() && !future.isError()) {
				...
				ThreadSingleAssignmentVar<T>::sendError(abortError);
			} else {
				ASSERT(false);
			}
		} else {
...
template <class T>
ThreadFuture<T> abortableFuture(ThreadFuture<T> f, ThreadFuture<Void> abortSignal, Error abortError) {
