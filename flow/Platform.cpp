...
#if defined(__linux__)
void getNetworkTraffic(uint32_t ip, uint64_t& bytesSent, uint64_t& bytesReceived, uint64_t& outSegs,
...
void atomicReplace(std::string const& path, std::string const& content, bool textmode) {
	...
	try {
		...
		std::string tempfilename =
		    parentDirectory(path) + CANONICAL_PATH_SEPARATOR + g_random->randomUniqueID().toString() + ".tmp";
...
std::string parentDirectory(std::string const& filename) {
	...
	size_t sep = abs.find_last_of(CANONICAL_PATH_SEPARATOR);
	if (sep == std::string::npos) {
		TraceEvent(SevError, "GetParentDirectoryOfFile").detail("File", filename).GetLastError();
		throw platform_error();
	}
	...
	while (true) {
		...
		if (lastValue == net2liveness) {
			...
			if (lastSignal == 0 || t - lastSignal >= logInterval) {
				if (lastSignal > 0) {
					logInterval = std::min(FLOW_KNOBS->SLOWTASK_PROFILING_MAX_LOG_INTERVAL,
					                       FLOW_KNOBS->SLOWTASK_PROFILING_LOG_BACKOFF * logInterval);
				}
