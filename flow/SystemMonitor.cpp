...
#define TRACEALLOCATOR(size)                                                                                           \
	TraceEvent("MemSample")                                                                                            \
	    .detail("Count", FastAllocator<size>::getMemoryUnused() / size)                                                \
	    .detail("TotalSize", FastAllocator<size>::getMemoryUnused())                                                   \
	    .detail("SampleCount", 1)                                                                                      \
	    .detail("Hash", "FastAllocatedUnused" #size)                                                                   \
	    .detail("Bt", "na")
#define DETAILALLOCATORMEMUSAGE(size)                                                                                  \
	detail("AllocatedMemory" #size, FastAllocator<size>::getMemoryUsed())                                              \
	    .detail("ApproximateUnusedMemory" #size, FastAllocator<size>::getMemoryUnused())
...
SystemStatistics customSystemMonitor(std::string eventName, StatisticsState* statState, bool machineMetrics) {
	SystemStatistics currentStats =
	    getSystemStatistics(machineState.folder.present() ? machineState.folder.get() : "",
	                        machineState.ip.present() ? machineState.ip.get() : 0, &statState->systemState);
	NetworkData netData;
	netData.init();
	if (!DEBUG_DETERMINISM && currentStats.initialized) {
		{
			TraceEvent e(eventName.c_str());
			e.detail("Elapsed", currentStats.elapsed)
			    ...
			    .detail("AIOSubmitLag",
			            (g_network->networkMetrics.secSquaredSubmit - statState->networkMetricsState.secSquaredSubmit) /
			                currentStats.elapsed)
			    .detail("AIODiskStall", (g_network->networkMetrics.secSquaredDiskStall -
			...
			n.detail("CantSleep", netData.countCantSleep - statState->networkState.countCantSleep)
			...
			for (int i = 0; i < NetworkMetrics::SLOW_EVENT_BINS; i++)
				if (int c = g_network->networkMetrics.countSlowEvents[i] -
			...
			for (int i = 0; i < NetworkMetrics::PRIORITY_BINS; i++)
				if (double x = g_network->networkMetrics.secSquaredPriorityBlocked[i] -
				               statState->networkMetricsState.secSquaredPriorityBlocked[i])
