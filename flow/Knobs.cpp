...
FlowKnobs::FlowKnobs(bool randomize, bool isSimulated) {
	...
	// GenericActors
	init(MAX_DELIVER_DUPLICATE_DELAY, 1.0);
	if (randomize && BUGGIFY) MAX_DELIVER_DUPLICATE_DELAY = 10.0;
	...
	if (randomize && BUGGIFY) MAX_CLOGGING_LATENCY = 0.1 * g_random->random01();
	init(MAX_BUGGIFIED_DELAY, 0);
	if (randomize && BUGGIFY) MAX_BUGGIFIED_DELAY = 0.2 * g_random->random01();
...
bool Knobs::setKnob(std::string const& knob, std::string const& value) {
	...
	if (int64_knobs.count(knob) || int_knobs.count(knob)) {
		...
		if (StringRef(value).startsWith(LiteralStringRef("0x"))) {
			if (sscanf(value.c_str(), "0x%llx%n", &v, &n) != 1 || n != value.size()) throw invalid_option_value();
		} else {
			if (sscanf(value.c_str(), "%lld%n", &v, &n) != 1 || n != value.size()) throw invalid_option_value();
		}
