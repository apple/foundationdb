...
pkgbuild --root $CLIENTSDIR --identifier FoundationDB-clients --version $VERSION.$RELEASE --scripts $SRCDIR/packaging/osx/scripts-clients FoundationDB-clients.pkg
...
pkgbuild --root $SERVERDIR --identifier FoundationDB-server --version $VERSION.$RELEASE --scripts $SRCDIR/packaging/osx/scripts-server FoundationDB-server.pkg
