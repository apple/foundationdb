...
extern "C" fdb_error_t fdb_setup_network_impl() {
	CATCH_AND_RETURN(API->setupNetwork(););
}

extern "C" fdb_error_t fdb_setup_network_v13(const char* localAddress) {
...
extern "C" DLLEXPORT FDBFuture* fdb_cluster_configure_database(FDBCluster* c, int config_type, int config_mode,
                                                               uint8_t const* db_name, int db_name_length) {
...
extern "C" DLLEXPORT fdb_bool_t fdb_future_is_error_v22(FDBFuture* f) {
...
extern "C" DLLEXPORT fdb_error_t fdb_future_get_error_impl(FDBFuture* f) {
	return TSAVB(f)->getErrorCode();
}

extern "C" DLLEXPORT fdb_error_t fdb_future_get_error_v22(FDBFuture* f, const char** description) {
...
extern "C" DLLEXPORT fdb_error_t fdb_future_get_version(FDBFuture* f, int64_t* out_version) {
...
extern "C" DLLEXPORT fdb_error_t fdb_future_get_cluster(FDBFuture* f, FDBCluster** out_cluster) {
	CATCH_AND_RETURN(*out_cluster = (FDBCluster*)((TSAV(Reference<ICluster>, f)->get()).extractPtr()););
}

extern "C" DLLEXPORT fdb_error_t fdb_future_get_database(FDBFuture* f, FDBDatabase** out_database) {
...
extern "C" fdb_error_t fdb_future_get_keyvalue_array_impl(FDBFuture* f, FDBKeyValue const** out_kv, int* out_count,
                                                          fdb_bool_t* out_more) {
...
extern "C" fdb_error_t fdb_future_get_keyvalue_array_v13(FDBFuture* f, FDBKeyValue const** out_kv, int* out_count) {
	CATCH_AND_RETURN(Standalone<RangeResultRef> rrr = TSAV(Standalone<RangeResultRef>, f)->get();
	                 *out_kv = (FDBKeyValue*)rrr.begin(); *out_count = rrr.size(););
}

extern "C" fdb_error_t fdb_future_get_string_array(FDBFuture* f, const char*** out_strings, int* out_count) {
...
extern "C" DLLEXPORT FDBFuture* fdb_create_cluster(const char* cluster_file_path) {
	return (FDBFuture*)API->createCluster(cluster_file_path ? cluster_file_path : "" /*, g_api_version*/).extractPtr();
}

extern "C" DLLEXPORT fdb_error_t fdb_cluster_set_option(FDBCluster* c, FDBClusterOption option, uint8_t const* value,
                                                        int value_length) {
	CATCH_AND_RETURN(CLUSTER(c)->setOption((FDBClusterOptions::Option)option,
	                                       value ? StringRef(value, value_length) : Optional<StringRef>()););
}

extern "C" DLLEXPORT void fdb_cluster_destroy(FDBCluster* c) {
	CATCH_AND_DIE(CLUSTER(c)->delref(););
}

extern "C" DLLEXPORT FDBFuture* fdb_cluster_create_database(FDBCluster* c, uint8_t const* db_name, int db_name_length) {
	return (FDBFuture*)(CLUSTER(c)->createDatabase(StringRef(db_name, db_name_length)).extractPtr());
}
...
extern "C" FDBFuture* fdb_transaction_get_impl(FDBTransaction* tr, uint8_t const* key_name, int key_name_length,
                                               fdb_bool_t snapshot) {
	return (FDBFuture*)(TXN(tr)->get(KeyRef(key_name, key_name_length), snapshot).extractPtr());
}

extern "C" FDBFuture* fdb_transaction_get_v13(FDBTransaction* tr, uint8_t const* key_name, int key_name_length) {
	return fdb_transaction_get_impl(tr, key_name, key_name_length, 0);
}

extern "C" FDBFuture* fdb_transaction_get_key_impl(FDBTransaction* tr, uint8_t const* key_name, int key_name_length,
...
extern "C" FDBFuture* fdb_transaction_get_key_v13(FDBTransaction* tr, uint8_t const* key_name, int key_name_length,
                                                  fdb_bool_t or_equal, int offset) {
	return fdb_transaction_get_key_impl(tr, key_name, key_name_length, or_equal, offset, false);
}

extern "C" FDBFuture* fdb_transaction_get_addresses_for_key(FDBTransaction* tr, uint8_t const* key_name,
                                                            int key_name_length) {
	return (FDBFuture*)(TXN(tr)->getAddressesForKey(KeyRef(key_name, key_name_length)).extractPtr());
}

extern "C" FDBFuture* fdb_transaction_get_range_impl(FDBTransaction* tr, uint8_t const* begin_key_name,
...
extern "C" FDBFuture* fdb_transaction_get_range_selector_v13(FDBTransaction* tr, uint8_t const* begin_key_name,
                                                             int begin_key_name_length, fdb_bool_t begin_or_equal,
                                                             int begin_offset, uint8_t const* end_key_name,
...
extern "C" FDBFuture* fdb_transaction_get_range_v13(FDBTransaction* tr, uint8_t const* begin_key_name,
...
extern "C" fdb_error_t fdb_transaction_set_option_impl(FDBTransaction* tr, FDBTransactionOption option,
                                                       uint8_t const* value, int value_length) {
	CATCH_AND_RETURN(TXN(tr)->setOption((FDBTransactionOptions::Option)option,
	                                    value ? StringRef(value, value_length) : Optional<StringRef>()););
}

extern "C" void fdb_transaction_set_option_v13(FDBTransaction* tr, FDBTransactionOption option) {
