...
std::string ConsumerClientFDB6::repStateKey =
    "\xff/snowCannonProxy/ReplicatorState";

ConsumerClientFDB6::ConsumerClientFDB6(std::string clusterFile)
  ...
  if (err) {
    log.trace("ConsumerClientFDB6ErrorCheckingAPIVersion",
...
int ConsumerClientFDB6::stopNetwork() {
  ...
  if (err) {
    log.trace("ConsumerClientFDB6StopNetworkError",
              {{"error", STR(fdb_get_error(err))}});
  }
...
void *fdb_network_thread(void *args) {
  ...
  if (err) {
    log.trace(LogLevel::Error, "ConsumerClientFDB6StartNetworkError",
              {{"error", STR(fdb_get_error(err))}});
  }
  return NULL;
}
...
int ConsumerClientFDB6::startNetwork() {
  ...
  if (doNetworkTrace) {
    ...
    err = fdb_network_set_option(FDB_NET_OPTION_TRACE_ENABLE,
                                 (uint8_t *)networkTracePath.c_str(),
                                 networkTracePath.size());
    if (err) {
      log.trace(LogLevel::Error, "Client6EnableNetworkTracingError",
                {{"error", STR(fdb_get_error(err))}});
    }
    err = fdb_network_set_option(FDB_NET_OPTION_KNOB,
                                 (uint8_t *)networkKnobJson.c_str(),
                                 networkKnobJson.size());
    if (err) {
      log.trace(LogLevel::Error, "Client6EnableJsonError",
                {{"error", STR(fdb_get_error(err))}});
    }
  }
  ...
  if (err) {
    log.trace(LogLevel::Error, "Client6SetupNetworkError",
              {{"error", STR(fdb_get_error(err))}});
  ...
  if (err != 0) {
    log.trace(LogLevel::Error, "Client6StartNetworkThreadError",
              {{"error", STR(fdb_get_error(err))}});
  ...
  if (err) {
    log.trace(LogLevel::Error, "Client6CreateDatabaseError",
              {{"error", STR(fdb_get_error(err))}});
    return err;
  }
  return err;
  ...
  {                                                                            \
    ...
    if (ferr) {                                                                \
      log.trace(fmt::format("Client6_{}_TxnRetryOnError", tag),                \
                {{"err", STR(fdb_get_error(ferr))},                            \
                 {"buffer", reqBuffer->toStr()}});                             \
      FDBFuture *futErr = fdb_transaction_on_error(txn, ferr);                 \
      fdb_error_t err2 = fdb_future_set_callback(                              \
          futErr, &ConsumerClientFDB6::retryTxnCB, reqBuffer);                 \
      if (err2) {                                                              \
        log.trace(LogLevel::Error,                                             \
                  fmt::format("Client6_{}_TxnRetryFAIL", tag),                 \
...
FDBTransaction *ConsumerClientFDB6::createTransaction(MessageBuffer *buffer,
                                                      fdb_error_t &err) {
  std::unique_lock lock(txnMutex);
  log.trace("Client6CreateTxn", {{"buffer", buffer->toStr()}});
  if (txnMap.find(buffer->id) != txnMap.end()) {
    log.trace(LogLevel::Error, "Client6CreateTxnAlreadyExistsError",
              {{"id", STR(buffer->id)}});
    assert(0);
  }
  FDBTransaction *txn;
...
FDBTransaction *ConsumerClientFDB6::getTransaction(MessageBuffer *buffer) {
  ...
  if (txnMap.find(buffer->id) == txnMap.end()) {
    log.trace(LogLevel::Error, "Client6NoTxnAssociatedWithBuffer",
              {{"id", STR(buffer->id)}});
    assert(0);
  }
  return txnMap[buffer->id];
...
void ConsumerClientFDB6::cleanTransaction(MessageBuffer *buffer) {
  ...
  if (txnMap.find(buffer->id) != txnMap.end()) {
    ...
    log.trace(LogLevel::Error, "Client6CleanNoTxnAssociatedWithBuffer",
              {{"id", STR(buffer->id)}});
    assert(0);
  }
}
...
void ConsumerClientFDB6::retryTxnCB(FDBFuture *fut, void *arg) {
  ...
  if (err) {
    log.trace(LogLevel::Error, "Client6RetryTxnError",
              {{"error", STR(fdb_get_error(err))}});
    g_FDB6Client->sendResponse(reqBuffer, true);
    return;
  }
...
int ConsumerClientFDB6::beginTxn(MessageBuffer *reqBuffer) {
  ...
  log.trace("Client6BeginTxnGetRepState",
            {{"arg", statePassed.toStr()}, {"buffer", reqBuffer->toStr()}});

  err = fdb_transaction_set_option(txn, FDB_TR_OPTION_ACCESS_SYSTEM_KEYS, NULL,
                                   0);
  CHECK(err, "TxnSetOptionAccessSystemKeys");

  // Get operation
  FDBFuture *f = fdb_transaction_get(
      txn, (uint8_t *)ConsumerClientFDB6::repStateKey.c_str(),
      ConsumerClientFDB6::repStateKey.size(), false);
  err = fdb_future_set_callback(f, &ConsumerClientFDB6::checkReplicatorStateCB,
                                reqBuffer);
...
void ConsumerClientFDB6::checkReplicatorStateCB(FDBFuture *fut, void *arg) {
  ...
  log.trace("Client6GetRepStateCBVersion", {{"version", STR(rv)},
                                            {"present", STR(out_present)},
                                            {"err", STR(fdb_get_error(err))}});

  err =
      fdb_future_get_value(fut, &out_present, (const uint8_t **)&val, &valLen);
  CHECK(err, "GetValue");
  auto repState =
  ...
  if (out_present) {
    ...
    log.trace("Client6GetRepStateCBState",
              {{"repState", reqBuffer->replyRepState.toStr()}});
  } else {
    log.trace("Client6GetRepStateCBStateNotFound");
  }

  fdb_future_destroy(fut);
  if (reqBuffer->type != MessageBufferType::T_SetReplicatorStateReq &&
      !reqBuffer->checkReplicatorIDRegistry(log)) {
    reqBuffer->error = -1;
...
void ConsumerClientFDB6::setReplicatorState(MessageBuffer *reqBuffer) {
  ...
  fdb_transaction_set(txn, (uint8_t *)ConsumerClientFDB6::repStateKey.c_str(),
                      ConsumerClientFDB6::repStateKey.size(),
                      (uint8_t *)&reqBuffer->setStateObj,
                      sizeof(ReplicatorState));
  ...
  err =
      fdb_future_set_callback(fut, &ConsumerClientFDB6::commitTxnCB, reqBuffer);
...
void ConsumerClientFDB6::pushBatch(MessageBuffer *reqBuffer) {
  ...
  for (int i = 0; i < reqBuffer->getMutations()->Length(); i++) {
    ...
    log.trace(LogLevel::Debug, "Client6PushBatchTraceMutation",
              {{"mut", printObj(*m)}});
    if (m->type() == 0) { // FDBMutationType == SetValue
      log.trace(LogLevel::Debug, "Client6PushBatchTraceMutKey",
                {{"key", m->param1()->str()}});
      fdb_transaction_set(
          txn, (uint8_t *)m->param1()->c_str(), m->param1()->str().size(),
      ...
      log.trace(LogLevel::Error, "Client6PushBatchMutationTypeNotSupported",
                {{"type", STR(m->type())}});
    }
  }
...
void ConsumerClientFDB6::verifyRange(MessageBuffer *reqBuffer) {
  ...
  fut = fdb_transaction_get_range(
      txn,
      FDB_KEYSEL_FIRST_GREATER_OR_EQUAL((uint8_t *)range->begin()->c_str(),
                                        range->begin()->str().size()),
      FDB_KEYSEL_FIRST_GREATER_THAN((uint8_t *)range->end()->c_str(),
                                    range->end()->str().size()),
      0 /* limit */, 0 /* target_bytes */,
      FDB_STREAMING_MODE_WANT_ALL /* FDBStreamingMode */, 0 /* iteration */,
      0 /* snapshot */, 0 /* reverse */
  );

  err = fdb_future_set_callback(fut, &ConsumerClientFDB6::verifyRangeCB,
                                reqBuffer);
...
void ConsumerClientFDB6::verifyRangeCB(FDBFuture *fut, void *arg) {
  ...
  string endValue;
  reqBuffer->respType =
      MessageResponseType::T_FinishResp; // change response type

  err = fdb_future_get_keyvalue_array(fut, &out_kv, &out_count, &out_more);
  log.trace("Client6VerifyRangeCB",
            {{"buffer", reqBuffer->toStr()}, {"outCount", STR(out_count)}});
  ...
  for (int i = 0; i < out_count; i++) {
    ...
    string value =
        string((const char *)out_kv[i].value, out_kv[i].value_length);
    if (i == 0) {
      beginKey = key;
      beginValue = value;
    }
    ...
    log.trace(LogLevel::Debug, "Client6VerifyRangeCheckKV",
              {{"key", key}, {"value", value}});
  ...
  if (checksum != argChecksum) {
    ...
    log.trace("Client6VerifyRangeSUCCESSTrace",
              {{"bKey", beginKey},
               {"eKey", endValue},
               {"calculated", STR(checksum)},
               {"expected", STR(argChecksum)},
