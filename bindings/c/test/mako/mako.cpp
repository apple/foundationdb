...
int populate(TX tx,
    ...
    const auto key_begin = insert_begin(args.rows, worker_id, thread_id,
                                        args.num_processes, args.num_threads);
    const auto key_end = insert_end(args.rows, worker_id, thread_id,
                                    args.num_processes, args.num_threads);
    auto xacts = 0;
    ...
    for (auto i = key_begin; i <= key_end; i++) {
        ...
        if (args.txntrace) {
            ...
            if (to_integer_seconds(time_now - time_last_traced) >= args.txntrace) {
                ...
                auto err = Error{};
                err = tx.set_option_nothrow(
                    FDB_TR_OPTION_DEBUG_TRANSACTION_IDENTIFIER, keystr);
                if (err) {
                    fmt::print(stderr,
                               "ERROR: set_option(TR_OPTION_DEBUG_TRANSACTION_IDENTIFIER): {}\n",
                                err.what());
                }
                err = tx.set_option_nothrow(FDB_TR_OPTION_LOG_TRANSACTION, BytesRef());
                if (err) {
                    fmt::print(stderr,
                               "ERROR: set_option(TR_OPTION_LOG_TRANSACTION): {}\n",
                               err.what());
                }
            }
        }
        ...
        if (i == key_end || (i - key_begin + 1) % num_commit_every == 0) {
            ...
            if (is_sample_target) time_commit_start = steady_clock::now();
            auto future_commit = tx.commit();
            const auto rc = wait_and_handle_error(tx, future_commit,
                                                  "COMMIT_POPULATE_INSERT");
            if (rc == FutureRC::OK) {
            ...
            if (is_sample_target) {
                ...
                const auto commit_latency_us =
                    to_integer_microseconds(time_commit_end - time_commit_start);
                const auto tx_duration_us =
                    to_integer_microseconds(time_commit_end - time_tx_start);
    ...
    fmt::print(debugme,
            ...
            key_end - key_begin + 1, key_begin, key_end,
            to_double_seconds(time_end - time_start));
...
inline int next_key(mako_args_t const& args) {
    if (args.zipf) return zipfian_next();
    return urand(0, args.rows - 1);
}

const std::array<OpDesc, MAX_OP> op_desc{{
    {"GRV",             {StepKind::READ},                  false },
...
const std::map<std::pair<int/*op*/, int/*sub-op step*/>,
               Future(*)(TX,
                         ...
                         ByteString&/*key1*/,
                         ByteString&/*key2*/,
                         ByteString&/*value*/)> operation_fn_table{
    {
        {OP_GETREADVERSION, 0},
    ...
    },
    {
        ...
        [](TX tx, mako_args_t const& args, ByteString& begin, ByteString& end, ByteString&) {
            ...
            return tx.get_range<key_select::inclusive, key_select::inclusive>(
                    begin, end, 0/*limit*/, 0/*target_bytes*/, args.streaming_mode,
                    0/*iteration*/, false/*snapshot*/,
                    args.txnspec.ops[OP_GETRANGE][OP_REVERSE]).erase_type();
        }
    },
    {
        {OP_SGET, 0},
    ...
    },
    {
        ...
        [](TX tx, mako_args_t const& args, ByteString& begin, ByteString& end, ByteString&) {
            ...
            return tx.get_range<key_select::inclusive, key_select::inclusive>(
                    begin, end, 0/*limit*/, 0/*target_bytes*/, args.streaming_mode,
                    0/*iteration*/, true/*snapshot*/,
                    args.txnspec.ops[OP_SGETRANGE][OP_REVERSE]).erase_type();
        }
    },
    {
        {OP_UPDATE, 0},
    ...
    },
    {
        ...
        [](TX tx, mako_args_t const& args, ByteString& key, ByteString&, ByteString& value) {
            ...
            randstr<false/*clear-before-append*/>(
                    key, args.key_length - static_cast<int>(key.size()));
            randstr(value, args.value_length);
            tx.set(key, value);
            return Future();
        }
    ...
    },
    {
        ...
        [](TX tx, mako_args_t const& args, ByteString& key_begin, ByteString& key, ByteString& value) {
            ...
            for (auto i = 0; i <= range; i++) {
                fmt::format_to(std::back_inserter(key), "{0:0{1}d}", i, range_digits);
                if (i == range) break; // preserve "exclusive last"
                // preserve first key for step 1
                if (i == 0) key_begin = key;
                tx.set(key, value);
    ...
    },
    {
        ...
        [](TX tx, mako_args_t const& args, ByteString& begin, ByteString& end, ByteString&) {
            ...
            auto r = tx.read_blob_granules(
                    begin, end, 0/*begin_version*/, -1/*end_version, use txn's*/, granuleContext);
            ...
            if (!err || err.is(2037/*blob_granule_not_materialized*/))
                return Future();
            const auto fp = (err.is(1020/*not_committed*/) ||
                             err.is(1021/*commit_unknown_result*/) ||
                             err.is(1213/*tag_throttled*/)) ? annoyme : stderr;
...
OpIterator get_op_begin(mako_args_t const& args) noexcept {
    for (auto op = 0; op < MAX_OP; op++) {
        if (op == OP_COMMIT || op == OP_TRANSACTION ||
...
OpIterator get_op_next(mako_args_t const& args, OpIterator current) noexcept {
    ...
    for ( ; op < MAX_OP; op++, count=0) {
        if (op == OP_COMMIT || op == OP_TRANSACTION ||
            args.txnspec.ops[op][OP_COUNT] <= count)
            continue;
    ...
    while (op_iter != OpEnd) {
        ...
        if (step + 1 == op_desc[op].steps() /* last step */) {
            if (op_desc[op].needs_commit())
                needs_commit = true;
            const auto op_usec =
                to_integer_microseconds(time_step_end - time_per_op_start[op]);
            stats.add_latency(op, op_usec);
            if (do_sample) sample_bins[op].put(op_usec);
        }
        // move to next op
...
int run_workload(TX tx,
    ...
    if (dotrace) traceid.reserve(32);

    if (dotagging) tagstr.reserve(16);
    ...
    while (1) {
        ...
        if (dotrace) {
            ...
            if (to_integer_seconds(time_now - time_last_trace) >= 1) {
                ...
                traceid.clear();
                fmt::format_to(std::back_inserter(traceid),
                               "makotrace{:0>19d}", total_xacts);
                fmt::print(debugme, "DEBUG: txn tracing {}\n", traceid);
                auto err = Error{};
                err = tx.set_option_nothrow(
                                    FDB_TR_OPTION_DEBUG_TRANSACTION_IDENTIFIER,
                                    to_bytes_ref(traceid));
                if (err) {
                    fmt::print(
                        stderr, "ERROR: TR_OPTION_DEBUG_TRANSACTION_IDENTIFIER: {}\n", err.what());
                }
                err = tx.set_option_nothrow(FDB_TR_OPTION_LOG_TRANSACTION,
                                            BytesRef());
                if (err) {
        ...
        if (dotagging > 0) {
            tagstr.clear();
            fmt::format_to(std::back_inserter(tagstr), "{}{}{:0>3d}",
                           KEY_PREFIX,
                           args.txntagging_prefix,
                           urand(0, args.txntagging - 1));
            auto err = tx.set_option_nothrow(FDB_TR_OPTION_AUTO_THROTTLE_TAG,
                                                      to_bytes_ref(tagstr));
            if (err) {
...
std::string get_stats_file_name(std::string_view dirname,
                                int worker_id, int thread_id, int op) {

    return fmt::format("{}/{}_{}_{}",
                       dirname, worker_id + 1, thread_id + 1, op_desc[op].name());
}
...
void worker_thread(thread_args_t& thread_args) {
    ...
    static_assert(std::is_same_v<
            decltype(std::decay_t<decltype(*thread_args.process)>::shm), mako_shmhdr_t*>);
    ...
    fmt::print(debugme,
            ...
            worker_id, args.num_processes, thread_id, args.num_threads,
            database_index, fmt::ptr(pthread_self()));

    const auto thread_tps =
        args.tpsmax == 0 ? 0 :
                           compute_thread_tps(args.tpsmax, worker_id, thread_id,
                                              args.num_processes, args.num_threads);

    const auto thread_iters =
        args.iteration == 0 ? 0 :
                              compute_thread_iters(args.iteration, worker_id, thread_id,
                                                   args.num_processes, args.num_threads);
    ...
    if (args.mode == MODE_CLEAN) {
        ...
        auto rc =
            populate(tx, args, worker_id, thread_id, thread_tps, stats, sample_bins);
        if (rc < 0) {
            fmt::print(stderr, "ERROR: populate failed\n");
        }
    } else if (args.mode == MODE_RUN) {
        auto rc = run_workload(tx,
                               args,
                               thread_tps,
                               throttle_factor,
                               thread_iters,
    ...
    if (args.mode == MODE_BUILD || args.mode == MODE_RUN) {
        ...
        for (auto op = 0; op < MAX_OP; op++) {
            if (args.txnspec.ops[op][OP_COUNT] > 0 ||
                op == OP_COMMIT || op == OP_TRANSACTION) {
                const auto filename = get_stats_file_name(dirname, worker_id,
                                                          thread_id, op);
                ...
                thread_args.sample_bins[op].for_each_block(
                        [fp](auto ptr, auto count) {
                            fwrite(ptr, sizeof(*ptr) * count, 1, fp);
                        });
            }
        }
    }
...
int worker_process_main(mako_args_t& args, int worker_id, mako_shmhdr_t* shm, pid_t pid_main) {
    ...
    if (args.flatbuffers) {
        ...
        err = network::set_option_nothrow(
                    FDB_NET_OPTION_USE_FLATBUFFERS,
    ...
    if (args.log_group[0] != '\0') {
        err = network::set_option_nothrow(FDB_NET_OPTION_TRACE_LOG_GROUP,
                                          BytesRef(to_byte_ptr(args.log_group)));
        if (err) {
    ...
    if (args.trace) {
        ...
        err = network::set_option_nothrow(FDB_NET_OPTION_TRACE_ENABLE,
                                          BytesRef(to_byte_ptr(args.tracepath)));
        if (err) {
            fmt::print(stderr, "ERROR: network_set_option(TRACE_ENABLE): {}\n", err.what());
        }
        if (args.traceformat == 1) {
            err = network::set_option_nothrow(FDB_NET_OPTION_TRACE_FORMAT,
                                              BytesRef(to_byte_ptr("json")));
            if (err) {
                fmt::print(stderr,
                           "ERROR: fdb_network_set_option(FDB_NET_OPTION_TRACE_FORMAT): {}\n", err.what());
            }
        }
    }
    ...
    if (args.knobs[0] != '\0') {
        ...
        while (knob != NULL) {
            ...
            err = network::set_option_nothrow(FDB_NET_OPTION_KNOB,
                                              BytesRef(to_byte_ptr(knob)));
            if (err) {
...
void print_stats(mako_args_t const& args, mako_stats_t const* stats,
                 double const duration_sec, FILE* fp) {
    ...
    /* Conflicts */
    const auto conflicts_diff =
        (current.get_conflict_count() - prev.get_conflict_count()) / duration_sec;
...
void print_stats_header(mako_args_t const& args, bool show_commit, bool is_first_header_empty, bool show_op_stats) {
    /* header */
    if (is_first_header_empty) put_title("");
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0) {
    ...
    if (show_commit) put_field("COMMIT");
    if (show_op_stats) {
...
void print_report(mako_args_t const& args,
    ...
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0 && op != OP_TRANSACTION) {
            ...
            if (fp) {
                ...
                fmt::fprintf(fp, "\"%s\": %lu",
                             get_ops_name(op), final_stats.get_error_count(op));
            }
        }
    }
    if (fp) {
    ...
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0 || op == OP_TRANSACTION || op == OP_COMMIT) {
            ...
            if (fp) {
                ...
                fmt::fprintf(fp, "\"%s\": %lu",
                             get_ops_name(op),
                             final_stats.get_latency_sample_count(op));
            }
        }
    }
    fmt::print("\n");
        ...
        if (args.txnspec.ops[op][OP_COUNT] > 0 || op == OP_TRANSACTION || op == OP_COMMIT) {
            ...
            if (lat_max == 0) {
                ...
                if (fp) {
                    ...
                    fmt::fprintf(fp, "\"%s\": %lu",
                                 get_ops_name(op), final_stats.get_latency_us_max(op));
                }
            }
        }
    }
    ...
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0 || op == OP_TRANSACTION || op == OP_COMMIT) {
            ...
            const auto num_points = data_points[op].size();
            const auto point_95pct =
                static_cast<size_t>(std::max(0., (num_points * 0.95) - 1));
            ...
            if (fp) {
                ...
                fmt::fprintf(fp, "\"%s\": %lu",
                             get_ops_name(op), data_points[op][point_95pct]);
            }
        }
    }
    fmt::printf("\n");
    ...
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0 || op == OP_TRANSACTION || op == OP_COMMIT) {
            ...
            const auto num_points = data_points[op].size();
            const auto point_99pct =
                static_cast<size_t>(std::max(0., (num_points * 0.99) - 1));
            ...
            if (fp) {
                ...
                fmt::fprintf(fp, "\"%s\": %lu",
                             get_ops_name(op), data_points[op][point_99pct]);
            }
        }
    }
    fmt::print("\n");
    ...
    for (auto op = 0; op < MAX_OP; op++) {
        if (args.txnspec.ops[op][OP_COUNT] > 0 || op == OP_TRANSACTION || op == OP_COMMIT) {
            ...
            const auto num_points = data_points[op].size();
            const auto point_99_9pct =
                static_cast<size_t>(std::max(0., (num_points * 0.999) - 1));
            ...
            if (fp) {
                ...
                fmt::fprintf(fp, "\"%s\": %lu",
                             get_ops_name(op), data_points[op][point_99_9pct]);
            }
        }
    }
    fmt::print("\n");
