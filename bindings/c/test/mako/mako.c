...
#define fdb_wait_and_handle_error(_func, _f, _t)			\
  do {									\
	...
	return FDB_CONFLICT;						\
      ...
      fdb_future_destroy(_f);						\
      if (err2) {							\
        if (err2 != 2017 /* used_during_commit */) {			\
	  /* not retryable, but do not abort */				\
	  return FDB_ERROR;						\
        }								\
	fprintf(stderr,							\
...
int cleanup(FDBTransaction *transaction, mako_args_t *args) {
  ...
  fprintf(printme, "INFO: Clear range: %6.3f sec\n",
	  ((timer_end.tv_sec - timer_start.tv_sec) * 1000000000.0 +
...
/* populate database */
int populate(FDBTransaction *transaction, mako_args_t *args, int worker_id,
             int thread_id, int thread_tps, mako_stats_t *stats) {
  int i;
  struct timespec timer_start, timer_end;
...
int run_op_getrange(FDBTransaction *transaction, char *keystr, char *keystr2,
...
int run_op_clearrange(FDBTransaction *transaction, char *keystr,
                      char *keystr2) {
...
/* run one transaction */
int run_transaction(FDBTransaction *transaction, mako_args_t *args,
                    mako_stats_t *stats, char *keystr, char *keystr2,
                    char *valstr) {
  int i;
  int count;
  int rc;
  ...
  do {
	...
	for (count = 0; count < args->txnspec.ops[i][OP_COUNT]; count++) {
	  ...
	  case OP_GET:
	    ...
	    break;
	  case OP_GETRANGE:
	    rc = run_op_getrange(transaction, keystr, keystr2, valstr, 0,
	  ...
	  case OP_SGET:
	    ...
	    break;
	  case OP_SGETRANGE:
	    rc = run_op_getrange(transaction, keystr, keystr2, valstr, 1,
				 args->txnspec.ops[i][OP_REVERSE]);
	    break;
	  case OP_UPDATE:
	    randstr(valstr, args->value_length + 1);
	    ...
	    break;
	  case OP_INSERT:
	    randstr(keystr + KEYPREFIXLEN, args->key_length - KEYPREFIXLEN +
		    1); /* make it (almost) unique */
	    randstr(valstr, args->value_length + 1);
	  ...
	  case OP_INSERTRANGE:
	    ...
	    randstr(keystr + KEYPREFIXLEN,
		    randstrlen + 1); /* make it (almost) unique */
	    randstr(valstr, args->value_length + 1);
	  ...
	  case OP_SETCLEARRANGE:
		...
		keystr2[strlen(keystr)] = '\0';
	      ...
	      rc = run_op_insert(transaction, keystr, valstr);
	      if (rc != FDB_SUCCESS) {
		/* rollback not necessary, just commit what's been done */
		break;
	      }
	    }
	    /* commit insert so mutation goes to storage */
	  ...
	  if (rc != FDB_SUCCESS) {
	    stats->errors[i]++;
	    /* we don't need to complete the entire transaction, immediately retry */
	    if (rc == FDB_FAILURE) {
	      retry++;
	    } else if (rc == FDB_ABORT) {
	      retry = DEFAULT_RETRY_COUNT+1; /* will not retry */
	    }
	    break; /* out of each-op loop */
	  } else {
	    /* success */
	    stats->ops[i]++;
	  }
    ...
    if (rc == FDB_SUCCESS && (docommit | args->commit_get)) {
      rc = commit_transaction(transaction);
      if (rc == FDB_SUCCESS) {
	/* success */
	stats->ops[OP_COMMIT]++;
...
void *worker_thread(void *thread_args) {
  ...
  mako_stats_t *stats =
      (void *)((thread_args_t *)thread_args)->process->shm +
      sizeof(mako_shmhdr_t) /* skip header */
  ...
  fprintf(debugme, "DEBUG: worker_id:%d (%d) thread_id:%d (%d) (tid:%d)\n", worker_id,
	  args->num_processes, thread_id, args->num_threads,
  ...
  if (args->iteration) {
    thread_iters = compute_thread_iters(args->iteration, worker_id, thread_id,
                                        args->num_processes, args->num_threads);
  }
...
int worker_process_main(mako_args_t *args, int worker_id, mako_shmhdr_t *shm) {
  ...
  if (args->flatbuffers) {
    ...
    err =
        fdb_network_set_option(FDB_NET_OPTION_USE_FLATBUFFERS,
                               (uint8_t *)&args->flatbuffers, sizeof(uint8_t));
    if (err) {
      fprintf(
          stderr,
          "ERROR: fdb_network_set_option(FDB_NET_OPTION_USE_FLATBUFFERS): %s\n",
          fdb_get_error(err));
    }
#else
  ...
  thread_args =
      (thread_args_t *)calloc(sizeof(thread_args_t), args->num_threads);
  if (!thread_args) {
  ...
  for (i = 0; i < args->num_threads; i++) {
    ...
    rc = pthread_create(&worker_threads[i], NULL, worker_thread,
                        (void *)&thread_args[i]);
    if (rc != 0) {
...
void print_stats(mako_args_t *args, mako_stats_t *stats, struct timespec *now,
  ...
  double durationns = (now->tv_sec - prev->tv_sec) * 1000000000.0 +
  ...
  for (op = 0; op < MAX_OP; op++) {
    if (args->txnspec.ops[op][OP_COUNT] > 0) {
      printf("%" STR(STATS_FIELD_WIDTH) "lld ",
             ops_total[op] - ops_total_prev[op]);
  ...
  printf("%" STR(STATS_FIELD_WIDTH) ".2f ",
         (totalxacts - totalxacts_prev) * 1000000000.0 / durationns);
...
void print_report(mako_args_t *args, mako_stats_t *stats,
                  struct timespec *timer_now, struct timespec *timer_start) {
  int i, j, op;
  uint64_t totalxacts = 0;
  uint64_t conflicts = 0;
  ...
  uint64_t durationns = (timer_now->tv_sec - timer_start->tv_sec) * 1000000000 +
  ...
  /* TPS */
  printf("%" STR(STATS_FIELD_WIDTH) ".2f ",
         totalxacts * 1000000000.0 / durationns);
  /* Conflicts */
  printf("%" STR(STATS_FIELD_WIDTH) ".2f\n",
...
int main(int argc, char *argv[]) {
  ...
  /* map it */
  shm = (mako_shmhdr_t *)mmap(NULL, shmsize, PROT_READ | PROT_WRITE, MAP_SHARED,
                              shmfd, 0);
  if (shm == MAP_FAILED) {
