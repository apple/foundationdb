...
int run_op_getrange(FDBTransaction* transaction, char* keystr, char* keystr2, char* valstr, int snapshot, int reverse) {
	...
	do {
		f = fdb_transaction_get_range(
		    transaction, FDB_KEYSEL_FIRST_GREATER_OR_EQUAL((uint8_t*)keystr, strlen(keystr)),
		    FDB_KEYSEL_LAST_LESS_OR_EQUAL((uint8_t*)keystr2, strlen(keystr2)) + 1, 0 /* limit */, 0 /* target_bytes */,
		    FDB_STREAMING_MODE_WANT_ALL /* FDBStreamingMode */, 0 /* iteration */, snapshot, reverse /* reverse */);
		err = wait_future(f);
...
/* run one transaction */
int run_transaction(FDBTransaction* transaction, mako_args_t* args, mako_stats_t* stats, char* keystr, char* keystr2,
                    char* valstr) {
	int i;
	int count;
	int rc;
...
int run_workload(FDBTransaction* transaction, mako_args_t* args, int thread_tps, int thread_iters, volatile int* signal,
                 mako_stats_t* stats) {
	int xacts = 0;
	int rc = 0;
