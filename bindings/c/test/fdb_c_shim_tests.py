...
class FdbCShimTests:
    def __init__(
        ...
        self.api_tester_bin = self.build_dir.joinpath(
            "bin", "fdb_c_shim_api_tester")
        assert self.api_tester_bin.exists(), "{} does not exist".format(self.api_tester_bin)
        self.unit_tests_bin = self.build_dir.joinpath(
            "bin", "fdb_c_shim_unit_tests")
        assert self.unit_tests_bin.exists(), "{} does not exist".format(self.unit_tests_bin)
        self.shim_lib_tester_bin = self.build_dir.joinpath(
            "bin", "fdb_c_shim_lib_tester")
        assert self.shim_lib_tester_bin.exists(
        ), "{} does not exist".format(self.shim_lib_tester_bin)
        self.api_test_dir = self.source_dir.joinpath(
            "bindings", "c", "test", "apitester", "tests")
