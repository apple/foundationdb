...
public:
  static std::shared_ptr<ConsumerAdapter>
  create(boost::asio::io_context &io_context, unsigned p,
         std::shared_ptr<ConsumerClientIF> ec) {
    return std::shared_ptr<ConsumerAdapter>(
        new ConsumerAdapter(io_context, p, ec));
  }
...
private:
  ConsumerAdapter(boost::asio::io_context &io_context, unsigned p,
                   std::shared_ptr<ConsumerClientIF> ec);

  void waitForRequests();
  void handleConnect(const boost::system::error_code &error);
  void handleHeader(std::shared_ptr<MessageBuffer> reqBuffer,
                    const boost::system::error_code &error,
                    std::size_t bytesRead);
  void handleRequest(std::shared_ptr<MessageBuffer> reqBuffer,
                     const boost::system::error_code &error,
                     std::size_t bytesRead);
  ...
  int createAndQueueResponse(MessageBuffer *reqBuffer, bool freeBuffer,
                             MessageResponseType respType);
  int queueResponse(std::shared_ptr<MessageBuffer> respBuffer);
  void sendResponse();
  void handleFinishSendResponse(const boost::system::error_code &error,
