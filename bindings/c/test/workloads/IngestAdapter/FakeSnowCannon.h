...
class FakeSnowCannon : public std::enable_shared_from_this<FakeSnowCannon> {
private:
  std::mutex writeLock;
...
public:
  ...
  static std::shared_ptr<FakeSnowCannon>
  create(boost::asio::io_context &io_context, unsigned p) {
    return std::shared_ptr<FakeSnowCannon>(new FakeSnowCannon(io_context, p));
  }
  ~FakeSnowCannon();
...
private:
  FakeSnowCannon(boost::asio::io_context &io_context, unsigned p);
  int queueRequest(std::shared_ptr<MessageBuffer> reqBuffer, int endpoint);
  void sendRequest();
  void waitForResponses();
  void handleAccept(const boost::system::error_code &error);
  void handleFinishWrite(int endpoint, const boost::system::error_code &error,
  ...
  static void handle_sig(const boost::system::error_code &error,
                         int signal_number);
  ...
  void updateStatsOnResponse(
      const ConsAdapter::serialization::ConsumerAdapterResponse *resp,
      MessageStats epStats);
};

static FakeSnowCannon *g_sc = NULL;
