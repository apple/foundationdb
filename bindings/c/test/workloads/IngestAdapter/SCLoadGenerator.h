...
/*
  The SCLoadGenerator provides an API to generate requests with randomized data
  ...
  update SCLoadGenerator with the responses.
  Finally, it
 */

class SCLoadGenerator {
  struct VerifyRangeInfo {
    std::pair<int64_t, int64_t> keyRange;
    int checksum;
    VerifyRangeInfo() {}
    VerifyRangeInfo(int64_t b, int64_t e, int cs)
        : keyRange(b, e), checksum(cs){};
    std::string toStr() const {
      return fmt::format("keyStart:{} keyEnd:{} checksum:{}", keyRange.first,
                         keyRange.second, checksum);
    }
  ...
  int rangeCountMax = 3;
  int maxOutstandingVerifyRanges =
      100; // max key range verifies waiting for send and response
  ...
  std::set<VerifyRangeInfo> verifyRangesWaitingForResponse; // remove on verify
                                                            // finish response
  std::map<int, std::vector<VerifyRangeInfo>>
      epToVerifyRangesWaitingForResponse; // add on verify, remove on
                                          // verify finish response
  std::map<int, VerifyRangeInfo>
      epToVerifyRangeWaitingForPush; // add on push, remove on push
                                     // response
  std::map<int, MessageStats> epToStats;

  void printMutVector(const flatbuffers::Vector<flatbuffers::Offset<
                          ConsAdapter::serialization::Mutation>> *mutations);
  void printRangesVector(const flatbuffers::Vector<flatbuffers::Offset<
                             ConsAdapter::serialization::KeyRange>> *keyRanges);
...
public:
  ...
  int endpointsWaitingForVerifyFinish() {
    return epToVerifyRangesWaitingForResponse.size();
  }
  int verifyReqsWaitingToSend() { return verifyRangesWaitingToSend.size(); }

  void init(int kRange, int vRange, int mCountMax, int mKRSize, int rCountMax,
            int maxOSVR);
};
