...
enum class MessageResponseType : uint8_t {
  T_UNINIT = 0,
  T_ReplyResp,
  T_FinishResp
};
...
struct MessageHeader {
  ...
  std::string toStr() const {
    return fmt::format("(size:{} sum:{})", size, checksum);
  }
  MessageHeader(){};
...
struct UIDExt {
  ...
  UIDExt(ConsAdapter::serialization::UID serialObj)
      : part1(serialObj.part1()), part2(serialObj.part2()) {}
  ConsAdapter::serialization::UID serialize() {
    return ConsAdapter::serialization::UID(part1, part2);
  }
  bool operator==(UIDExt const &other) {
...
struct GlobalVersionExt {
  ...
  int32_t switchCount;
  std::string toStr() const {
    return fmt::format("(sw:{}, v:{}, prev:{})", switchCount, version,
                       previousVersion);
  }
  GlobalVersionExt() {}
  GlobalVersionExt(const GlobalVersionExt &other)
      : version(other.version), previousVersion(other.previousVersion),
  ...
  GlobalVersionExt(ConsAdapter::serialization::GlobalVersion serialObj)
      : version(serialObj.version()),
        previousVersion(serialObj.previousVersion()),
        switchCount(serialObj.switchCount()) {}
  ConsAdapter::serialization::GlobalVersion serialize() {
    return ConsAdapter::serialization::GlobalVersion(version, previousVersion,
                                                     switchCount);
  }
};
...
struct ReplicatorStateExt {
  ...
  std::string toStr() const {
    return fmt::format("(id:{}, lastRepVersion:{})", id.toStr(),
                       lastRepVersion.toStr());
  }

  ReplicatorStateExt() {}
  ReplicatorStateExt(ConsAdapter::serialization::ReplicatorState serialObj)
      : lastRepVersion(GlobalVersionExt(serialObj.lastRepVersion())),
        id(UIDExt(serialObj.id())) {}
  ConsAdapter::serialization::ReplicatorState serialize() {
    return ConsAdapter::serialization::ReplicatorState(
        lastRepVersion.serialize(), id.serialize());
  }
};
...
struct MessageBuffer {
  ...
  const ConsAdapter::serialization::GetReplicatorStateReq *
  getGetReplicatorStateReq();
  const ConsAdapter::serialization::SetReplicatorStateReq *
  getSetReplicatorStateReq();
  ...
  const flatbuffers::Vector<
      flatbuffers::Offset<ConsAdapter::serialization::Mutation>> *
  getMutations();
  const flatbuffers::Vector<
      flatbuffers::Offset<ConsAdapter::serialization::KeyRange>> *
  getRanges();
  ...
  ~MessageBuffer() {
    std::cout << "delete message buffer endpoint:" << endpoint
              << " header:" << printObj(header) << std::endl;
  }
};
