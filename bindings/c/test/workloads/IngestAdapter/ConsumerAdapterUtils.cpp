...
const flatbuffers::Vector<flatbuffers::Offset<Mutation>> *
MessageBuffer::getMutations() {
  auto pushReq = getPushBatchReq();
  return pushReq->mutations();
}

const flatbuffers::Vector<flatbuffers::Offset<KeyRange>> *
MessageBuffer::getRanges() {
...
std::string MessageBuffer::toStr() {
  return fmt::format("message id:{} endpoint:{} type:{} replyType:{} curErr:{} "
                     " curVerifyRange:{}",
                     id, endpoint, printRequestType(type),
                     printResponseType(respType), error, curVerifyRange);
}
...
bool MessageBuffer::checkReplicatorIDRegistry(Log &log) {
  ...
  if (replyRepState.id == reqState.id) {
    ...
    log.trace("ReplicatorIDDoesntMatchError",
              {{"cur", replyRepState.toStr()}, {"req", reqState.toStr()}});
    return false;
  }
}
...
void MessageBuffer::prepareWrite() {
  Crc32 crc;
  header =
      MessageHeader(serializer.GetSize(), crc.sum(serializer.GetBufferPointer(),
                                                  serializer.GetSize()));
  // TODO: make vector of buffer references...
  writeBuffers.clear();
  writeBuffers.push_back(boost::asio::buffer((void *)&header, sizeof(header)));
  writeBuffers.push_back(
      boost::asio::buffer(serializer.GetBufferPointer(), serializer.GetSize()));
  readyForWrite = true;
}
...
std::string printObj(const GlobalVersion &v) {
  return fmt::format("(sw:{}, v:{}, prev:{})", v.switchCount(), v.version(),
                     v.previousVersion());
};

std::string printObj(const ReplicatorState &s) {
  return fmt::format("(id:{}, lastRepVersion:{})", printObj(s.id()),
                     printObj(s.lastRepVersion()));
};

// need to do something special for serialized strings
std::string printObj(const Mutation &m) {
  return fmt::format("(type:{}, key:{}, val:{})", m.type(), m.param1()->str(),
                     m.param2()->str());
};
...
std::string printObj(const UIDExt &id) { return id.toStr(); };

std::string printObj(const GlobalVersionExt &v) { return v.toStr(); };

std::string printObj(const ReplicatorStateExt &s) { return s.toStr(); };
