  ...
  std::shared_ptr<ConsumerClientIF> consumerClient;
	...
	void start(FDBDatabase* db, GenericPromise<bool> done) override {
      ...
      consumerClient.reset(new ConsumerClientFDB6(db, log));
		 consumerClient->registerTxnResponseCallback(boost::bind(&IngestAdapterWorkload::txnResponseCB, this, _1,
		 _2));
	...
	void sendRequests() {
	  ...
	  { auto chooseReq = random(); if (chooseReq < 10) { sendRequest(MessageBufferType::T_GetReplicatorStateReq); } else
	  if (chooseReq < 90 || requestGen.verifyReqsWaitingToSend() == 0) { sendRequest(MessageBufferType::T_PushBatchReq);
		    } else {
	...
	void handleResponse(MessageBuffer* buf) {
		        ...
		        if (buf->respType == MessageResponseType::T_FinishResp ||
		            (buf->respType == MessageResponseType::T_ReplyResp && buf->type !=
		   MessageBufferType::T_VerifyRangeReq)) { if (!err) {
		                // success
