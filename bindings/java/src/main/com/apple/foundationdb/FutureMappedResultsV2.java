...
class FutureMappedResultsV2 extends NativeFuture<MappedRangeResultInfoV2> {
	...
	public MappedRangeResultV2 getResults() {
		...
		try {
			...
			System.out.println("Hfu5 buffer is " + (buffer == null));
			if (buffer != null) {
				...
				System.out.println("Hfu5 AAAAAAAA ");
				MappedRangeResultV2 res;
				try {
					long p = getPtr();
					System.out.println("Hfu5 BBBBBBB ");
					res = Result_get(p);
					return res;
				} catch (Error e) {
					System.out.printf("Hfu5 Error %s", e);
				}
				System.out.println("Hfu5 ZZZZZZZZ %s");
				return new MappedRangeResultV2(new MappedKeyValueV2[2], false);
			}
		} finally {
