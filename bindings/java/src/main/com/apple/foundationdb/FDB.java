...
public class FDB {
	 ...
	 *  <a href="/foundationdb/administration.html#default-cluster-file" target="_blank">default fdb.cluster file</a>
	 *  is to be used.
	 * @param timer the Transaction timer to use for instrumentation calls, or {@code null} if no instrumentation is desired.
	 *
	 * @return a {@code CompletableFuture} that will be set to a FoundationDB {@link Database}
	 */
	public Database open(String clusterFilePath,TransactionTimer timer) throws FDBException {
		return open(clusterFilePath, DEFAULT_EXECUTOR,timer);
	}
	 ...
	 *  is to be used.
	 * @param e the {@link Executor} to use to execute asynchronous callbacks
	 * @param timer the {@link TransactionTimer} to use to record instrumentation metrics, or {@code null} if no 
	 *  instrumentation is desired.
	 *
	 * @return a {@code CompletableFuture} that will be set to a FoundationDB {@link Database}
	 */
	public Database open(String clusterFilePath, Executor e,TransactionTimer timer) throws FDBException {
		synchronized(this) {
			if(!isConnected()) {
				startNetwork();
			}
		...
		return new FDBDatabase(Database_create(clusterFilePath), e,timer);
	}
