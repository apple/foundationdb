...
public class DirectorySubspace extends Subspace implements Directory {
	...
	DirectorySubspace(List<String> path, byte[] prefix, DirectoryLayer directoryLayer) {
		this(path, prefix, directoryLayer, DirectoryLayer.EMPTY_BYTES);
	}
	...
	public String toString() {
		return getClass().getSimpleName() + '(' + DirectoryUtil.pathStr(path) + ", " + ByteArrayUtil.printable(getKey()) + ')';
	}
	...
	public CompletableFuture<DirectorySubspace> moveTo(TransactionContext tcx, List<String> newAbsolutePath) {
		DirectoryLayer dir = getLayerForPath(DirectoryLayer.EMPTY_PATH);
		...
		return dir.move(tcx,
						getPartitionSubpath(DirectoryLayer.EMPTY_PATH, dir),
