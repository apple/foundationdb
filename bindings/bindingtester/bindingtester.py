...
class ResultSet(object):
    ...
    def check_for_errors(self):
        ...
        indices = [0 for _ in range(len(self.tester_results))]
        ...
        while True:
            ...
            # Attempt to 'align' the results. If two results have matching sequence numbers, then they should be
            # compared. Only those testers which have a result matching the minimum current sequence number will be
            # included. All others are considered to have not produced a result and will be evaluated in a future
            # iteration.
            sequence_nums = [
            ...
            # If these results aren't using sequence numbers, then we match two results based on whether they share
            # the same key
            else:
                min_key = reduce(
            ...
            # Fill in 'None' values for testers that didn't produce a result and generate an output string describing
            # the results
            all_results = {
...
class TestRunner(object):
    ...
    def _run_tester(self, test):
        ...
        timer = Timer(self.args.timeout, kill_proc)
        try:
            timer.start()
            ret_code = proc.wait()
    ...
    def _get_results(self, subspace):
        util.get_logger().info(
