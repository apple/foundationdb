...
def api_version(ver):
    ...
    # Query the loaded libfdb_c for its actual max supported API version.
    # This allows the bundled binding to work with any FDB library version.
    header_version = fdb.impl._capi.fdb_get_max_api_version()

    if ver > header_version:
        raise RuntimeError(
            "API version %d is not supported by the installed FoundationDB C library "
            "(max supported: %d)." % (ver, header_version)
        )
    ...
    if err == 2203:
        ...
        raise RuntimeError("FoundationDB API error (%d)" % err)
