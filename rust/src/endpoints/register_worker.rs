...
fn serialize_request(
    ...
    let (master_token, master) = create_request_stream(builder, &public_address)?;
    let (_, commit_proxy) = create_request_stream(builder, &public_address)?;
    let (_, grv_proxy) = create_request_stream(builder, &public_address)?;
    let (_, data_distributor) = create_request_stream(builder, &public_address)?;
    let (_, ratekeeper) = create_request_stream(builder, &public_address)?;
    let (_, blob_manager) = create_request_stream(builder, &public_address)?;
    let (_, blob_worker) = create_request_stream(builder, &public_address)?;
    let (_, resolver) = create_request_stream(builder, &public_address)?;
    let (_, storage) = create_request_stream(builder, &public_address)?;
    let (_, log_router) = create_request_stream(builder, &public_address)?;
    let (_, debug_ping) = create_request_stream(builder, &public_address)?;
    let (_, coordination_ping) = create_request_stream(builder, &public_address)?;
    let (_, wait_failure) = create_request_stream(builder, &public_address)?;
    let (_, set_metrics_rate) = create_request_stream(builder, &public_address)?;
    let (_, event_log_request) = create_request_stream(builder, &public_address)?;
    let (_, trace_batch_dump_request) = create_request_stream(builder, &public_address)?;
    let (_, disk_store_request) = create_request_stream(builder, &public_address)?;
    let (_, exec_req) = create_request_stream(builder, &public_address)?;
    let (_, worker_snap_req) = create_request_stream(builder, &public_address)?;
    ...
    let config_broadcast_interface = {
        ...
        let id = Some(crate::flow::uid::UID::default().into());
