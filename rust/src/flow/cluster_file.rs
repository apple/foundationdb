...
impl ClusterFile {
    fn parse_string(input: &str) -> Result<Self> {
        ...
        if desc_id_tokens.len() != 2 {
            return Err(format!("Expected key:desc before @ sign.  Got {:?} from {}", desc_id_tokens, input).into());
        }
        let description = desc_id_tokens[0].to_string();
        let id = desc_id_tokens[1].to_string();

        if !description.chars().all(|x| char::is_alphanumeric(x) || x == '_') {
            return Err(format!("Description should only contain a-z, A-Z, 0-9, _  Got: {}", description).into());
        }
        ...
        for host_port in host_port_tokens {
            ...
            let (tls_mode, host_port) = match tokens.len() {
                ...
                _ => { return Err(format!("Expected host:port[:tls]  Got: {}", host_port).into()); }
            };
                

            let host = match host_port.parse() {
                Ok(sockaddr) => {
                    ClusterHost::IPAddr(sockaddr, tls_mode)
                },
                Err(_) => {
...
fn test_cluster_file() -> Result<()> {
    ...
    ClusterFile::parse_string("test:test@127.0.0.1:4501:tls,127.0.0.1:4502:tls,127.0.0.1:4503:tls")?;
    ClusterFile::parse_string("test:test@127.0.0.1:4501:tls,127.0.0.1:4502:tls,127.0.0.1:4502:tls").expect_err("Expected error!");
