...
local int gz_avail(state) gz_statep state;
{
	...
	if (state->eof == 0) {
		...
		if (gz_load(state, state->in + strm->avail_in, state->size - strm->avail_in, &got) == -1) return -1;
		strm->avail_in += got;
...
local int gz_fetch(state) gz_statep state;
{
	...
	do {
		...
		case LOOK: /* -> LOOK, COPY (only if never GZIP), or GZIP */
			...
			break;
		case COPY: /* -> COPY */
			if (gz_load(state, state->out, state->size << 1, &(state->x.have)) == -1) return -1;
			state->x.next = state->out;
			return 0;
...
int len;
{
	...
	/* check that we're reading and that there's no (serious) error */
	if (state->mode != GZ_READ || (state->err != Z_OK && state->err != Z_BUF_ERROR)) return NULL;
	...
	if (left) do {
			...
			if (state->x.have == 0 && gz_fetch(state) == -1) return NULL; /* error */
...
int ZEXPORT gzdirect(file) gzFile file;
{
	...
	if (state->mode == GZ_READ && state->how == LOOK && state->x.have == 0) (void)gz_look(state);
