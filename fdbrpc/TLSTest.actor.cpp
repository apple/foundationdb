...
template <>
struct fmt::formatter<StringRef> {
	constexpr auto parse(format_parse_context& ctx) -> decltype(ctx.begin()) { return ctx.begin(); }

	template <class FormatContext>
	auto format(const StringRef& sr, FormatContext& ctx) -> decltype(ctx.out()) {
		return fmt::format_to(ctx.out(), "{}", std::string_view(reinterpret_cast<const char*>(sr.begin()), sr.size()));
	}
};
...
ACTOR Future<Void> waitAndPrintResponse(Future<SessionInfo> response, Result* rc) {
	try {
		...
		logc("Probe response: trusted={} peerAddress={} authorizedTenants=[{}]",
		     info.isPeerTrusted,
		     info.peerAddress.toString(),
		     fmt::join(info.authorizedTenants, ", "));
...
int runHost(TLSCreds creds, int addrPipe, int completionPipe, Result expect) {
	...
	openTraceFile(
	    NetworkAddress(), 10 << 20, 10 << 20, ".", IsServer ? "mtls_unittest_server" : "mtls_unittest_client");
