...
Future<Void> sendErrorOnProcess(ISimulator::ProcessInfo* const& process, Promise<Void> const& promise, Error const& e,
                                int const& taskID);
		...
		for (int writeOffset = 0; writeOffset < length; writeOffset += pageLength) {
			...
			for (int pageOffset = 0; pageOffset < pageLength; pageOffset += sectorLength) {
				...
				if (saveDurable || pageKillMode == NO_CORRUPTION ||
				    (pageKillMode == FULL_CORRUPTION && g_random->random01() < 0.25)) {
