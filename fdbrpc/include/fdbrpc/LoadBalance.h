...
template <class Interface, class Request, class Multi, bool P, class Model>
Future<REPLY_TYPE(Request)> loadBalanceImpl(Reference<MultiInterface<Multi>> alternativesInput,
	...
	RequestData<Request, Interface, Multi, Model, P> firstRequestData(compareReplicas);
	...
	int bestAlt = deterministicRandom()->randomInt(0, alternatives->countBest());
	int nextAlt = deterministicRandom()->randomInt(0, std::max(alternatives->size() - 1, 1));
	if (nextAlt >= bestAlt)
		nextAlt++;
	...
	int startAlt = nextAlt;
	int startDistance = (bestAlt + alternatives->size() - startAlt) % alternatives->size();
	...
	while (true) {
		...
		RequestStream<Request, P> const* stream = nullptr;
		...
		if (!stream && !firstRequestData.isValid()) {
			...
			if (!alternatives->alwaysFresh()) {
				...
				co_await allAlternativesFailedDelay(okFuture);
			} else {
				co_await okFuture;
			}
			...
			if (firstRequestData.checkAndProcessResult(atMostOnce)) {
				...
				co_await firstRequestData.maybeDoPostRequestComparison(
				    request, model, alternatives, channel, requiredReplicas);

				ASSERT(firstRequestData.response.isReady());
				co_return firstRequestData.response.get().get();
			}
			...
			bool firstRequestSuccessful = false;
			...
			if (firstRequestSuccessful || secondRequestSuccessful) {
				...
				RequestData<Request, Interface, Multi, Model, P>* requestData =
				    firstRequestSuccessful ? &firstRequestData : &secondRequestData;
				co_await requestData->maybeDoPostRequestComparison(
				    request, model, alternatives, channel, requiredReplicas);

				ASSERT(requestData->response.isReady());
				co_return requestData->response.get().get();
			}
			...
			while (true) {
				...
				if (res.index() == 0) {
					if (model) {
						...
						model->secondBudget = std::min(model->secondBudget + FLOW_KNOBS->SECOND_REQUEST_BUDGET_GROWTH,
...
Future<REPLY_TYPE(Request)> loadBalance(Reference<MultiInterface<Multi>> alternatives,
	...
	return loadBalanceImpl<Interface, Request, Multi, P, Model>(std::move(alternatives),
	                                                            channel,
	                                                            std::move(request),
	                                                            taskID,
	                                                            atMostOnce,
...
Future<REPLY_TYPE(Request)> basicLoadBalanceImpl(Reference<ModelInterface<Multi>> alternativesInput,
	...
	while (true) {
		...
		if (!stream) {
			...
			ErrorOr<REPLY_TYPE(Request)> result = co_await stream->tryGetReply(request);
