	...
	while (true) {
		...
		if (!stream && !firstRequestData.isValid()) {
			...
			if (firstRequestSuccessful || secondRequestSuccessful) {
				...
				co_await requestData->maybeDoReplicaComparison(request, model, alternatives, channel, requiredReplicas);
...
Future<REPLY_TYPE(Request)> loadBalance(Reference<MultiInterface<Multi>> alternatives,
	...
	return loadBalanceImpl<Interface, Request, Multi, P>(std::move(alternatives),
	                                                     channel,
	                                                     std::move(request),
	                                                     taskID,
	                                                     atMostOnce,
