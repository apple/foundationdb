...
public:
	LatencySample(std::string name, UID id, double loggingInterval, int sampleSize)
	  : IMetric(name, knobToMetricModel(FLOW_KNOBS->METRICS_DATA_MODEL)), id(id), sampleStart(now()),
	    sample(sampleSize), latencySampleEventHolder(makeReference<EventCacheHolder>(id.toString() + "/" + name)) {
