...
protected:
	...
	FlowReceiver(Endpoint const& remoteEndpoint, bool stream)
	  : endpoint(remoteEndpoint), m_isLocalEndpoint(false), m_stream(stream), m_flowReceiverId(-1) {
		...
		if (m_stream && endpoint.getPrimaryAddress().isValid() && endpoint.getPrimaryAddress().isPublic()) {
...
public:
	...
	explicit RequestStream(const Endpoint& endpoint) : queue(new NetNotifiedQueue<T, IsPublic>(0, 1, endpoint)), m_promiseRefTrackingId(-1) {
		if (queue->isRemoteEndpoint() && g_network && g_network->global(INetwork::enFlowTransport)) {
	...
	void operator=(const RequestStream& rhs) {
		...
		if (rhs.queue->isRemoteEndpoint() && g_network && g_network->global(INetwork::enFlowTransport)) {
		...
		if (queue) {
			queue->delPromiseRef();
			if (m_promiseRefTrackingId >= 0 && g_network && g_network->global(INetwork::enFlowTransport)) {
