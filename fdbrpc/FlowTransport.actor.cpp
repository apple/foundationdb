				...
				when(wait(delay(FLOW_KNOBS->CONNECTION_MONITOR_TIMEOUT))) {
					...
					if (startingBytes == peer->bytesReceived) {
						if (peer->destination.isPublic()) {
							...
							peer->pingLatenciesForChecking.addSample(now() - startTime);
						}
...
Peer::Peer(TransportData* transport, NetworkAddress const& destination)
  : transport(transport), destination(destination), outgoingConnectionIdle(true), lastConnectTime(0.0),
    ...
    pingLatenciesForChecking(destination.isPublic() ? FLOW_KNOBS->PING_SAMPLE_AMOUNT : 1), timeoutCount(0),
    lastForCheckingTime(0.0), lastLoggedTime(0.0), connectOutgoingCount(0), connectIncomingCount(0),
