...
Future<REPLY_TYPE(Request)> loadBalance(
    Reference<MultiInterface<Interface>> alternatives, RequestStream<Request> Interface::*channel,
    Request request = Request(), int taskID = TaskDefaultPromiseEndpoint,
	...
	loop {
		...
		if (!stream && !firstRequest.isValid()) {
			...
			if (now() - g_network->networkMetrics.newestAlternativesFailure >
			...
			double minDelay =
			    std::min(FLOW_KNOBS->CACHE_REFRESH_INTERVAL_WHEN_ALL_ALTERNATIVES_FAILED - (now() - serversValidTime),
			             FLOW_KNOBS->ALTERNATIVES_FAILURE_MIN_DELAY);
			double delay = std::max(std::min((now() - g_network->networkMetrics.oldestAlternativesFailure) *
			                                     FLOW_KNOBS->ALTERNATIVES_FAILURE_DELAY_RATIO,
			                                 FLOW_KNOBS->ALTERNATIVES_FAILURE_MAX_DELAY),
			                        minDelay);

			if (serversValidTime == ALWAYS_FRESH) delay = ALWAYS_FRESH;
...
inline Future<REPLY_TYPE(Request)> loadBalance(Reference<Multi> alternatives,
                                               RequestStream<Request> Interface::*channel, Request request = Request(),
                                               int taskID = TaskDefaultPromiseEndpoint, bool atMostOnce = false,
                                               QueueModel* model = NULL) {
