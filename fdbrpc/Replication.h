...
public:
	...
	bool selectReplicas(IRepPolicyRef const& policy, std::vector<LocalityEntry> const& alsoServers,
	                    std::vector<LocalityEntry>& results) {
		LocalitySetRef fromServers = LocalitySetRef::addRef(this);
		return policy->selectReplicas(fromServers, alsoServers, results);
	}

	bool selectReplicas(IRepPolicyRef const& policy, std::vector<LocalityEntry>& results) {
		return selectReplicas(policy, std::vector<LocalityEntry>(), results);
	}

	bool validate(IRepPolicyRef const& policy) const {
	...
	std::vector<LocalityEntry> const& getMutableEntries() const { return _mutableEntryArray; }
	...
	static void staticDisplayEntries(LocalitySetRef const& fromServers, std::vector<LocalityEntry> const& entryArray,
	                                 const char* name = "zone") {
	...
	LocalityEntry const& random() const { return _entryArray[g_random->randomInt(0, _entryArray.size())]; }
...
protected:
	...
	struct LocalityCacheRecord {
		...
		LocalityCacheRecord(AttribRecord const& attribute, LocalitySetRef resultset)
...
struct LocalityMap : public LocalityGroup {
	...
	bool selectReplicas(IRepPolicyRef const& policy, std::vector<V*>& results) {
		return selectReplicas(policy, std::vector<LocalityEntry>(), results);
	}
