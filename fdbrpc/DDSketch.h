...
public:
	explicit DDSketchBase(double errorGuarantee)
	  : errorGuarantee(errorGuarantee), populationSize(0), 
	    zeroPopulationSize(0), minValue(T()), maxValue(T()),
	    sum(T()) {}
...
public:
	DDSketch(double errorGuarantee = 0.1)
	  : DDSketchBase<DDSketch<T>, T>(errorGuarantee), 
	    gamma((1.0 + errorGuarantee) / (1.0 - errorGuarantee)),
	...
	int getIndex(T sample) {
		static_assert(__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__, 
			"Do not support non-little-endian systems");
...
public:
	DDSketchSlow(double errorGuarantee = 0.1)
	  : DDSketchBase<DDSketchSlow<T>, T>(errorGuarantee), 
	    gamma((1.0 + errorGuarantee) / (1.0 - errorGuarantee)),
	    logGamma(log(gamma)) {
