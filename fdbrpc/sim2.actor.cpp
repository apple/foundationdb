...
private:
	...
	ACTOR static Future<Void> trackLeakedConnection(Sim2Conn* self) {
		...
		if (self->process->address.isPublic()) {
			wait(
			    delay(FLOW_KNOBS->CONNECTION_MONITOR_IDLE_TIMEOUT * FLOW_KNOBS->CONNECTION_MONITOR_IDLE_TIMEOUT * 1.5));
		} else {
...
public:
	...
	virtual ProcessInfo* newProcess(const char* name, IPAddress ip, uint16_t port, bool sslEnabled,
	                                ...
	                                const char* dataFolder, const char* coordinationFolder) {
	...
	virtual bool killMachine(Optional<Standalone<StringRef>> machineId, KillType kt, bool forceKill,
	                         KillType* ktFinal) {
		auto ktOrig = kt;
