...
bool simulator_should_inject_fault(const char* context, const char* file, int line, int error_code) {
	...
	if (p->fault_injection_p2 && deterministicRandom()->random01() < p->fault_injection_p2 &&
		...
		if (h1 < p->fault_injection_p1 * std::numeric_limits<uint32_t>::max()) {
			CODE_PROBE(true, "A fault was injected");
			CODE_PROBE(error_code == error_code_io_timeout, "An io timeout was injected");
			CODE_PROBE(error_code == error_code_io_error, "An io error was injected");
			CODE_PROBE(error_code == error_code_platform_error, "A platform error was injected.");
...
private:
	...
	void rollRandomClose() {
		...
		if (!stableConnection &&
			...
			CODE_PROBE(true, "Simulated connection failure");
...
public:
		...
		if (mustBeDurable || deterministicRandom()->random01() < 0.5) {
			...
			try {
				...
				if (!currentProcess->rebooting) {
					...
					ASSERT(f.isReady());
					wait(::delay(0.05 * deterministicRandom()->random01()));
					CODE_PROBE(true, "Simulated durable delete");
				}
				wait(g_simulator.onProcess(currentProcess, currentTaskID));
				return Void();
			} catch (Error& e) {
			...
			TraceEvent(SevDebug, "Sim2DeleteFileImplNonDurable")
			    .detail("Filename", filename)
			    .detail("Durable", mustBeDurable);
			CODE_PROBE(true, "Simulated non-durable delete");
			return Void();
		}
	}
	...
	void killProcess_internal(ProcessInfo* machine, KillType kt) {
		CODE_PROBE(true, "Simulated machine was killed with any kill type");
		CODE_PROBE(kt == KillInstantly, "Simulated machine was killed instantly");
		CODE_PROBE(kt == InjectFaults, "Simulated machine was killed with faults");
		CODE_PROBE(kt == FailDisk, "Simulated machine was killed with a failed disk");
	...
	bool killMachine(Optional<Standalone<StringRef>> machineId,
		...
		CODE_PROBE(true, "Trying to killing a machine");
		CODE_PROBE(kt == KillInstantly, "Trying to kill instantly");
		CODE_PROBE(kt == InjectFaults, "Trying to kill by injecting faults");
		...
		if (processesOnMachine != processesPerMachine && kt >= RebootAndDelete) {
			CODE_PROBE(true, "Attempted reboot, but the target did not have all of its processes running");
			TraceEvent(SevWarn, "AbortedKill")
			    .detail("KillType", kt)
			    .detail("MachineId", machineId)
		...
		if (processesOnMachine != processesPerMachine) {
			CODE_PROBE(true, "Attempted reboot and kill, but the target did not have all of its processes running");
			TraceEvent(SevWarn, "AbortedKill")
			    .detail("KillType", kt)
			    .detail("MachineId", machineId)
			    .detail("Reason", "Machine processes does not match number of processes per machine")
		...
		CODE_PROBE(kt == RebootAndDelete, "Resulted in a reboot and delete");
		CODE_PROBE(kt == Reboot, "Resulted in a reboot");
		CODE_PROBE(kt == KillInstantly, "Resulted in an instant kill");
		CODE_PROBE(kt == InjectFaults, "Resulted in a kill by injecting faults");
	...
	bool killDataCenter(Optional<Standalone<StringRef>> dcId, KillType kt, bool forceKill, KillType* ktFinal) override {
		...
		CODE_PROBE(kt != ktMin, "DataCenter kill was rejected by killMachine");
		CODE_PROBE((kt == ktMin) && (kt == RebootAndDelete), "Datacenter kill Resulted in a reboot and delete");
		CODE_PROBE((kt == ktMin) && (kt == Reboot), "Datacenter kill Resulted in a reboot");
		CODE_PROBE((kt == ktMin) && (kt == KillInstantly), "Datacenter kill Resulted in an instant kill");
		CODE_PROBE((kt == ktMin) && (kt == InjectFaults), "Datacenter kill Resulted in a kill by injecting faults");
		CODE_PROBE((kt == ktMin) && (kt != ktOrig), "Datacenter Kill request was downgraded");
		CODE_PROBE((kt == ktMin) && (kt == ktOrig), "Datacenter kill - Requested kill was done");
...
class UDPSimSocket : public IUDPSocket, ReferenceCounted<UDPSimSocket> {
	...
	bool randomDropPacket() {
		...
		CODE_PROBE(res, "UDP packet drop");
		return res;
	}
...
ACTOR void doReboot(ISimulator::ProcessInfo* p, ISimulator::KillType kt) {
	...
	try {
		...
		CODE_PROBE(kt == ISimulator::RebootProcess, "Simulated process rebooted");
		CODE_PROBE(kt == ISimulator::Reboot, "Simulated machine rebooted");
