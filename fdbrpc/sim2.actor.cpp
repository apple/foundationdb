...
bool simulator_should_inject_fault(const char* context, const char* file, int line, int error_code) {
	...
	if (p->fault_injection_p2 && g_random->random01() < p->fault_injection_p2 && !g_simulator.speedUpSimulation) {
		uint32_t h1 = line + (p->fault_injection_r >> 32), h2 = p->fault_injection_r;
...
void ISimulator::displayWorkers() const {
	...
	for (auto processInfo : getAllProcesses()) {
		std::string dataHall = processInfo->locality.dataHallId().present()
		                           ...
		                           : "[unset]";
		std::string zoneId =
		    processInfo->locality.zoneId().present() ? processInfo->locality.zoneId().get().printable() : "[unset]";
...
class hash<Endpoint> {
public:
	size_t operator()(const Endpoint& s) const { return hashlittle(&s, sizeof(s), 0); }
};
} // namespace std
...
struct SimClogging {
	double getSendDelay(NetworkAddress from, NetworkAddress to) {
		...
		if (!g_simulator.speedUpSimulation && clogSendUntil.count(to.ip)) t = std::max(t, clogSendUntil[to.ip]);
	...
	double setPairLatencyIfNotSet(uint32_t from, uint32_t to, double t) {
		...
		if (i == clogPairLatency.end()) i = clogPairLatency.insert(make_pair(make_pair(from, to), t)).first;
		return i->second;
	}
...
struct Sim2Conn : IConnection, ReferenceCounted<Sim2Conn> {
	...
	void connect(Reference<Sim2Conn> peer, NetworkAddress peerEndpoint) {
		...
		// connection
		auto latency = g_clogging.setPairLatencyIfNotSet(peerProcess->address.ip, process->address.ip,
		                                                 FLOW_KNOBS->MAX_CLOGGING_LATENCY * g_random->random01());
	...
	void peerClosed() { leakedConnectionTracker = trackLeakedConnection(this); }
...
private:
	...
	ACTOR static Future<Void> receiver(Sim2Conn* self) {
		loop {
			...
			ASSERT(g_simulator.getCurrentProcess() == self->peerProcess);
			state int64_t pos = g_random->random01() < .5
			                        ? self->sentBytes.get()
			                        : g_random->randomInt64(self->receivedBytes.get(), self->sentBytes.get() + 1);
	...
	ACTOR static Future<Reference<IAsyncFile>> open(
		...
		try {
			wait(delay(FLOW_KNOBS->MIN_OPEN_TIME +
			           g_random->random01() * (FLOW_KNOBS->MAX_OPEN_TIME - FLOW_KNOBS->MIN_OPEN_TIME)));
...
private:
	...
	SimpleFile(int h, Reference<DiskParameters> diskParameters, bool delayOnWrite, const std::string& filename,
	  ...
	  : h(h), diskParameters(diskParameters), delayOnWrite(delayOnWrite), filename(filename),
	    actualFilename(actualFilename), dbgId(g_random->randomUniqueID()), flags(flags) {}
	...
	ACTOR static Future<int> read_impl(SimpleFile* self, void* data, int length, int64_t offset) {
		...
		if (randLog)
			fprintf(randLog, "SFR1 %s %s %s %d %lld\n", self->dbgId.shortString().c_str(), self->filename.c_str(),
			        opId.shortString().c_str(), length, offset);
	...
	ACTOR static Future<Void> write_impl(SimpleFile* self, StringRef data, int64_t offset) {
		...
		if (randLog) {
			...
			fprintf(randLog, "SFW1 %s %s %s %d %d %lld\n", self->dbgId.shortString().c_str(), self->filename.c_str(),
			        opId.shortString().c_str(), a, data.size(), offset);
		}
...
private:
	...
	ACTOR static void incoming(Reference<Sim2Listener> self, double seconds, Reference<IConnection> conn) {
		wait(g_simulator.onProcess(self->process));
		wait(delay(seconds));
		if (((Sim2Conn*)conn.getPtr())->isPeerGone() && g_random->random01() < 0.5) return;
...
public:
	...
	Future<class Void> delay(double seconds, int taskID, ProcessInfo* machine) {
		...
		if (!currentProcess->rebooting && machine == currentProcess && !currentProcess->shutdownSignal.isSet() &&
		    ...
		    g_random->random01() < 0.25) { // FIXME: why doesnt this work when we are changing machines?
	...
	virtual bool check_yield(int taskID) {
		...
		if (--yield_limit <= 0) {
			yield_limit = g_random->randomInt(
	...
	virtual void setCurrentTask(int taskID) { currentTaskID = taskID; }
	...
	virtual Future<Reference<IConnection>> connect(NetworkAddress toAddr, std::string host) {
		...
		((Sim2Listener*)peerp->listener.getPtr())
		    ->incomingConnection(0.5 * g_random->random01(), Reference<IConnection>(peerc));
	...
	virtual void getDiskBytes(std::string const& directory, int64_t& free, int64_t& total) {
		...
		if (diskSpace.totalSpace == 0) {
			...
			diskSpace.baseFreeSpace = std::min<int64_t>(
			    diskSpace.totalSpace, std::max(5e9, (g_random->random01() * (1 - .075) + .075) * diskSpace.totalSpace) +
	...
	virtual ProcessInfo* newProcess(const char* name, uint32_t ip, uint16_t port, LocalityData locality,
		...
		if (!machine.zoneId.present()) machine.zoneId = locality.zoneId();
		for (int i = 0; i < machine.processes.size(); i++) {
			if (machine.processes[i]->locality.zoneId() !=
			    locality.zoneId()) { // SOMEDAY: compute ip from locality to avoid this check
				TraceEvent("Sim2Mismatch")
				    .detail("IP", format("%x", ip))
				    .detailext("ZoneId", locality.zoneId())
				    .detail("NewName", name)
				    .detailext("ExistingMachineId", machine.processes[i]->locality.zoneId())
		...
		TraceEvent("NewMachine")
		    ...
		    .detailext("ZoneId", m->locality.zoneId())
	...
	virtual bool canKillProcesses(std::vector<ProcessInfo*> const& availableProcesses,
		...
		KillType newKt = kt;
		if ((kt == KillInstantly) || (kt == InjectFaults) || (kt == RebootAndDelete) ||
	...
	virtual void destroyProcess(ISimulator::ProcessInfo* p) {
		TraceEvent("ProcessDestroyed")
		    ...
		    .detailext("ZoneId", p->locality.zoneId());
	...
	void killProcess_internal(ProcessInfo* machine, KillType kt) {
		...
		if (kt == KillInstantly) {
			TraceEvent(SevWarn, "FailMachine")
			    ...
			    .detailext("ZoneId", machine->locality.zoneId())
			    .detail("Process", describe(*machine))
	...
	virtual void rebootProcess(Optional<Standalone<StringRef>> zoneId, bool allProcesses) {
		if (allProcesses) {
			...
			if (processes.size()) doReboot(g_random->randomChoice(processes), RebootProcess);
		}
	}
			...
			if (!canKillProcesses(processesLeft, processesDead, kt, &kt)) {
				...
				for (auto process : processesLeft) {
					TraceEvent("DeadMachineSurvivors")
					    ...
					    .detail("ProcessesLeft", processesLeft.size())
					    .detail("ProcessesDead", processesDead.size())
					    .detail("SurvivingProcess", describe(*process));
				}
				for (auto process : processesDead) {
					TraceEvent("DeadMachineVictims")
					    ...
					    .detail("ProcessesLeft", processesLeft.size())
					    .detail("ProcessesDead", processesDead.size())
					    .detail("VictimProcess", describe(*process));
				}
			} else {
				...
				if (processInfo->isAvailableClass()) {
					if (processInfo->isExcluded() || processInfo->isCleared() || !processInfo->isAvailable()) {
						processesDead.push_back(processInfo);
					           ...
					           datacenterZones.find(processInfo->locality.zoneId()) == datacenterZones.end()) {
						processesLeft.push_back(processInfo);
					} else {
						processesDead.push_back(processInfo);
					}
	...
	virtual void clogPair(uint32_t from, uint32_t to, double seconds) { g_clogging.clogPairFor(from, to, seconds); }
	...
	virtual MachineInfo* getMachineById(Optional<Standalone<StringRef>> const& zoneId) { return &machines[zoneId]; }
	...
	struct Task {
		...
		ProcessInfo* machine;
		Promise<Void> action;
		Task(double time, int taskID, uint64_t stable, ProcessInfo* machine, Promise<Void>&& action)
		  : time(time), taskID(taskID), stable(stable), machine(machine), action(std::move(action)) {}
		Task(double time, int taskID, uint64_t stable, ProcessInfo* machine, Future<Void>& future)
		  : time(time), taskID(taskID), stable(stable), machine(machine) {
			future = action.getFuture();
		}
		Task(Task&& rhs) noexcept(true)
		...
		void operator=(Task&& rhs) noexcept(true) {
			time = rhs.time;
			taskID = rhs.taskID;
			stable = rhs.stable;
			machine = rhs.machine;
	...
	void execTask(struct Task& t) {
		if (t.machine->failed) {
			...
			if (randLog)
				fprintf(randLog, "T %f %d %s %lld\n", this->time, int(g_random->peek() % 10000),
	...
	virtual Future<Void> onProcess(ISimulator::ProcessInfo* process, int taskID) { return delay(0, taskID, process); }
...
ACTOR void doReboot(ISimulator::ProcessInfo* p, ISimulator::KillType kt) {
	...
	try {
		...
		if (p->rebooting) return;
		TraceEvent("RebootingProcess")
		    .detail("KillType", kt)
		    .detail("Address", p->address)
		    .detailext("ZoneId", p->locality.zoneId())
		    .detailext("DataHall", p->locality.dataHallId())
