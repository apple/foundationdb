...
public:
	...
	struct Task {
		...
		Task(Task&& rhs) noexcept : time(rhs.time), taskID(rhs.taskID), stable(rhs.stable), machine(rhs.machine), action(std::move(rhs.action)) {}
		...
		void operator= (Task&& rhs) noexcept { time = rhs.time; taskID = rhs.taskID; stable = rhs.stable; machine = rhs.machine; action = std::move(rhs.action); }
