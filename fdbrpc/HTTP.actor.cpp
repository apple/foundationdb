...
ACTOR Future<Reference<HTTP::Response>> doRequest(Reference<IConnection> conn,
	...
	std::cerr << "HTTPRequest " << "RemoteAddress " << conn->getPeerAddress().toString() << " Verb " << verb
	          << " Resource " << resource << " RequestContentLen " << contentLen << std::endl;
	...
	try {
		...
		std::cerr << "ResponseCode " << r->code 
	          << " ResponseContentLen " << r->contentLen << " Elapsed " << elapsed << std::endl;
...
ACTOR Future<Void> sendProxyConnectRequest(Reference<IConnection> conn, std::string remoteHost, std::string remoteService) { // NetworkAddress toAddr) {
	state Headers headers;
	// headers["Host"] = toAddr.ip.toString() + ":" + std::to_string(toAddr.port);
	...
	loop {
		...
		try {
			Reference<Response> _r = wait(timeoutError(
			    doRequest(
			        conn, "CONNECT", remoteHost + ":" + remoteService, headers, nullptr, 0, sendReceiveRate, &bytes_sent, sendReceiveRate),
			    requestTimeout));
			r = _r;
		} catch (Error& e) {
...
ACTOR Future<Reference<IConnection>> proxyConnectImpl(std::string remoteHost, std::string remoteService, std::string proxyHost, std::string proxyService) {
	state NetworkAddress remoteEndpoint =
	    wait(map(INetworkConnections::net()->resolveTCPEndpoint(remoteHost, remoteService), [=](std::vector<NetworkAddress> const& addresses) -> NetworkAddress {
	...
	// state NetworkAddress proxyEndpoint =
	//     wait(map(INetworkConnections::net()->resolveTCPEndpoint(proxyHost, proxyService), [=](std::vector<NetworkAddress> const& addresses) -> NetworkAddress {
	// 	    NetworkAddress addr = addresses[deterministicRandom()->randomInt(0, addresses.size())];
	// 	    addr.fromHostname = true;
	// 		// addr.flags = NetworkAddress::FLAG_TLS;
	// 	    return addr;
	...
	// state Reference<IConnection> connection = wait(INetworkConnections::net()->connect(proxyEndpoint));
...
Future<Reference<IConnection>> proxyConnect(const std::string& remoteHost, const std::string& remoteService, const std::string& proxyHost, const std::string& proxyService) {
