...
The recovery process is like a state machine, changing from one state to the next state. `ClusterRecovery.cpp` implements the cluster recovery. We will describe in the rest of this document what each phase does to drive the recovery to the next state.
...
The logic of locking the tLogs is implemented in the `epochEnd()` function in [LogSystem.cpp](https://github.com/apple/foundationdb/blob/main/fdbserver/logsystem/LogSystem.cpp).
...
Later, the CC will use the recruited tLogs and `recoveryVersion` to create a new `LogSystem` for the new generation.
